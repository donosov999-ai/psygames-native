...
## 2б. ЛИНИЯ ОДНА — «льём везде на Flutter, никаких двух веток»

Решение Дениса 24.09.2026, отменяет более раннее решение того же дня о двух линиях.

**Было (несколько часов):** основная линия Tauri/Expo вела номера 2.54.x и уезжала в
Play, App Store и на десктоп; гибрид на Flutter вёл свои 2.55.x и уезжал только в
TestFlight. Номер гибрида задавался ПОЛЕМ ЗАПУСКА рабочего процесса — то есть руками.

**Стало:** линия одна. `flutter-testflight.yml` берёт версию из `frontend/app.json` —
того же файла, из которого её берут Play, App Store и десктоп. Поле запуска осталось
запасным ходом и больше не обязательно; если оно расходится с `app.json`, прогон пишет
предупреждение.

Сторожит `frontend/src/__tests__/version-sources-agree.test.ts` → «гибрид берёт версию
из app.json, а не из поля запуска».

⚠️ **Наследство разделения, которое ещё живо.** В TestFlight уже лежат сборки 2.55.9…
2.55.16, выпущенные, пока линии были разведены. Значит следующая общая версия обязана
быть ВЫШЕ 2.55.16 — App Store не принимает версию, равную или меньшую уже загруженной.
Практически: следующий общий выпуск — **2.56.0**.
