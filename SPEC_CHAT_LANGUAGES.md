# ТЗ чату «Билингво» — раздел «Языки», две развилки, ДВЕНАДЦАТЬ игр, зарядка и ОБЩИЙ СЛОЙ ЗВУКА
...
| Ритм и высота | `app/games/rhythm-pitch.tsx` |

⚠️ **ИСПРАВЛЕНО 16.09.2026: В «СЛУХЕ» ПЯТЬ ЭКРАНОВ, А НЕ ЧЕТЫРЕ.** До этого дня
и здесь, и в карточке реестра, и в ответах Денису стояло «11 игр». Пятый —
`rhythm-pitch` («Ритм и высота»), невербальный слух: ритм и высота тона, опора
для тоновых языков. Ошибка механическая: состав снимался `awk` по диапазону
блока `hubContents.ts`, диапазон обрывался на четвёртой строке, пятая в вывод не
попадала. Нашлось сверкой со `STRUCTURE.md`, где стоит «Слух — 5».
