...
// ⚠️ Четвёртый промах словаря (10.09.2026), теперь В ДРУГУЮ СТОРОНУ: «отзыв» стоял
// голым куском строки и поймал слово «отзывается» в фразе «каждая из сорока
// ОТЗЫВАЕТСЯ на жест». Гейт потребовал разметить репорты у релиза, в котором про
// репорты нет ни слова, — а естественная «починка» такого требования это переписать
// нормальный текст под словарь, то есть ровно наоборот. Хвост слова теперь закрыт:
// «отзыв», «отзывы», «отзывов» ловятся, «отзывается» и «отзывчивый» — нет.
const TESTER_MARKERS = /репорт|тестировщик|тестер|испытател|отзыв(ы|ов|ам|ах|а|е)?(?![а-яё])|из чата тестеров|жалоб|отчёт(ы|ов|ам)?\s+(испытател|тестир|\d)/i;
