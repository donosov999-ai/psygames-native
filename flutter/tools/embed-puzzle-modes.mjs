...
for (const f of readdirSync(SECTIONS).sort()) {
  ...
  for (const [engineName, m] of Object.entries(table)) {
    ...
    modes[engineName] = {
      ...
      input: m['ввод'] ?? null,
      /*
       * 🔴 КЛЮЧ ПРАВИЛА — ПО ПРАВИЛУ ИМЕНОВАНИЯ, А НЕ СПИСКОМ.
       *
       * В развилках описание карточки лежит под `<ключ названия>Desc`
       * (`puzzlesBridges` → `puzzlesBridgesDesc`), и так же оно названо в общем
       * словаре. Значит вести второй список незачем — достаточно правила, а
       * существование ключа проверит проба словаря.
       *
       * Ради этого ключа всё и затевалось: нативный экран головоломок показывал
       * доску без единого слова о правилах, и человек оставался с ней один на один.
       */
      descKey: `${m['имя']}Desc`,
