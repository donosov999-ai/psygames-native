...
void main() {
  ...
  /// 🔴 ЭТО НЕ ПРОБА РЕШАТЕЛЯ, А ЗАМЕР РАЗДАЧИ — И ОН КРАСНЫЙ НАРОЧНО НЕ БУДЕТ.
  /// Числа зафиксированы, чтобы дефект не растворился: пока он не починен, проба
  /// стережёт, что его не стало БОЛЬШЕ, а когда починят — упадёт и потребует
  /// переписать себя на ноль.
  test('📍 сколько уровней нельзя выиграть в принципе (замер 24.09.2026)', () {
    ...
    // ignore: avoid_print
    print('НЕВЫИГРЫВАЕМЫЕ УРОВНИ (узкая сетка): ${broken.length} из $full · '
        '${broken.entries.map((e) => 'L${e.key} (${e.value})').join(' · ')}');
    expect(broken.length, lessThanOrEqualTo(9),
        reason: 'невыигрываемых уровней стало больше, чем было 24.09.2026: $broken');
