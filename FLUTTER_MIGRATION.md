...
|---|---|---|---|---|
| ✅ | `anagrams` | psygames-words-claude-mac  | 24.09  | нативно, перехват в гибриде. ЧЕТЫРЕ игры за одним адресом: классика, «Все слова», кроссворд, слово-квадрат — каждая своим ключом `?mode=`; словари и кольца 10+8 языков данными |
...
🔴 **Перехват включать только когда готовы ВСЕ режимы.** Включить перехват на одном
готовом режиме — значит отнять у человека три остальных, и никакая проба этого не
заметит: маршрут-то открывается, экран за ним чужой.

✅ **Развязка (24.09, «Слова»): ключ на КАЖДЫЙ режим, а не один на адрес.**
`HybridApp.routeOf` сперва ищет адрес ВМЕСТЕ с хвостом и только потом без него, поэтому
карта `native` держит четыре ключа — `?mode=classic|all|cross|square` — плюс голый
`/games/anagrams` на режим по умолчанию. Готовые режимы можно включать по одному:
невключённый просто не назван в карте и уходит в веб, как любая неперенесённая игра.
Проба обязана сверять НЕ «узнаётся ли адрес», а КАКОЙ ЭКРАН за ним стоит — иначе она
зелёная при всех четырёх режимах, ведущих на классику
(`flutter/test/hybrid_routes_test.dart`, мутация «кроссворд → классика» её краснит).
...
| Цена экрана С РЕЖИМАМИ | — | 2330 строк на четыре игры «Анаграмм» (правила 877 + экраны 834 + поля 619), **0 правок каркаса**, плюс 3,7 МБ словарей и колец ДАННЫМИ. Планируя судоку с семью сетками или «Головоломки» с 42 режимами, считайте по этой строке, а не по предыдущей |
