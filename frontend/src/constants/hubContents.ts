...
export const HUB_CONTENTS: Record<string, HubSubGame[]> = {
   ...
   */
  /*
   * ГОЛОВОЛОМКИ ТЭТХЭМА: один экран `/games/puzzles`, режим в параметре `?mode=`.
 ...
 */
/**
 * 🔴 СОСТАВ ХАБОВ ИЗ ФАЙЛА НАСТРОЕК — «чтобы можно было фасовать без пересборки»
 * (решение Дениса 13.09.2026).
 *
 * Файл задаёт для хаба СПИСОК МАРШРУТОВ в нужном порядке; сами карточки — их
 * иконки, ключи имени и описания, признак набора — по-прежнему берутся отсюда,
 * из заводского реестра. То есть файлом нельзя ПРИДУМАТЬ карточку, можно только
 * убрать существующую или переставить. Это нарочно: карточка без словарных
 * ключей показалась бы игроку пустым местом, а причину искали бы в другом месте.
 *
 * Наложение живёт модульной переменной, потому что `visibleHubCards` зовут из
 * шести мест, и менять им всем подпись ради конфигурации было бы дороже самой
 * задачи. Ставит его `ProfileContext` — там же, где читается файл.
 */
let переопределениеХабов: Record<string, ЭлементХаба[]> | null = null;

export function установитьХабыИзФайла(x: Record<string, ЭлементХаба[]> | null): void {
  переопределениеХабов = x;
}

/**
 * 🔴 ФАЙЛ УМЕЕТ И ОПИСАТЬ КАРТОЧКУ, А НЕ ТОЛЬКО ВЫБРАТЬ ИЗ ЗАВОДСКИХ.
 *
 * Вопрос Дениса 13.09.2026: «почему добавка в хабы — это код? это по сути фильтр
 * отображения, такой же, как отображение в профиле; почему нельзя, чтобы всё
 * правилось в редакторе?» Он прав, и прежнее ограничение было не принципом, а
 * следствием формата: файл передавал ОДИН МАРШРУТ, а карточке нужны ещё значок и
 * словарные ключи имени и описания — поэтому «придумать» её было нечем.
 *
 * Теперь элемент списка — либо маршрут строкой (заводская карточка, как раньше),
 * либо описание целиком. Ключи имени и описания — словарные, те же, что у
 * заводских карточек: подпись остаётся на двенадцати языках, а не на одном.
 * Так сорок две головоломки Тэтхэма раскладываются по развилкам БЕЗ правки кода.
 */
export type ЭлементХаба = string | {
  маршрут: string;
  значок?: string;
  /** Ключ словаря, например `puzzlesSolo`. Не текст — иначе подпись была бы одноязычной. */
  имя: string;
  описание?: string;
};

/** Заводской состав хаба — нужен редактору и разбору файла, чтобы сверять маршруты. */
export function заводскиеКарточкиХаба(hubRoute: string): HubSubGame[] {
...
export function visibleHubCards(
  ...
  const карточки = списокИзФайла
    ? списокИзФайла
        .map((э): HubSubGame | undefined => {
          if (typeof э === 'string') return заводские.find((c) => c.route === э);
          /* Описанная файлом карточка: значок по умолчанию — общий, чтобы забытое
             поле не делало пустое место на экране. */
          return { route: э.маршрут, icon: (э.значок ?? 'extension-puzzle') as HubSubGame['icon'],
