 ...
 */
let переопределениеХабов: Record<string, string[]> | null = null;

export function установитьХабыИзФайла(x: Record<string, string[]> | null): void {
...
export function visibleHubCards(
  ...
  const карточки = списокИзФайла
    ? списокИзФайла
        .map((маршрут) => заводские.find((c) => c.route === маршрут))
