...
export const WHATS_NEW: WhatsNewEntry[] = [
  {
    ...
    date: '2026-08-19',
    ru: [
      "Семь новых упражнений: «Соедини точки», «Одна линия», «Лица и имена», «Трекер объектов», «Навигатор», «Ритм и высота», «Дворец памяти». Всего в каталоге стало 71",
      "«Дворец памяти» — метод мест: раскладываете предметы по постоянному маршруту и вспоминаете их вперёд и назад",
      "«Ритм и высота» — первое упражнение на слух: повторить ритм и восстановить путь высот. Слов в задании нет, микрофон не нужен",
      "На тропинке уровней подписана ступень сложности словом — «Начинающий», «Лёгкий», «Средний» и дальше. Раньше там стоял голый номер, и по нему нельзя было понять, куда ты идёшь",
      "Служебные кнопки (отмена, подсказка, перемешать) собраны в один ряд в шапке и больше не стоят вперемешку со счётчиками и с полем для ответа",
      "«Судоку-самурай» спрашивает перед выходом: партия на час больше не исчезает от промаха по «назад»",
      "В «Числовой шкале» подтверждение теперь одно, а не два подряд",
      "Дневной перерыв теперь считается с вашим набором упражнений: раньше он выдавал те, которых в наборе нет",
    ...
    en: [
      "Seven new exercises: Connect the Dots, One Line, Faces and Names, Object Tracker, Navigator, Rhythm and Pitch, Memory Palace. The catalogue now holds 71",
      "Memory Palace — the method of loci: place items along a fixed route, then recall them forward and in reverse",
      "Rhythm and Pitch — the first exercise for the ear: echo a rhythm and rebuild a path of pitches. No words in the task, no microphone needed",
      "The level path now names the difficulty tier in words — Beginner, Easy, Medium and on. It used to show a bare number, which told you nothing about where you were heading",
      "Service buttons (undo, hint, shuffle) are gathered into one row in the header and no longer sit mixed in with counters or with the answer area",
      "Samurai Sudoku asks before you leave: an hour-long board no longer vanishes from a stray tap on “back”",
      "Number Line now takes one confirmation instead of two in a row",
      "The midday break now respects your own set of exercises: it used to hand out ones your set does not include",
