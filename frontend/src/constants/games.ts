...
export const GAMES: GameConfig[] = [
  ...
  },
  {
    ...
    category: 'logic',
    // С 06.09.2026 живёт в развилке «Сортировка» (просьба Дениса), а не отдельной
    // карточкой в каталоге из 91 строки.
    hideFromMenu: true,
  ...
  {
    /**
     * Развилка «Сортировка» (06.09.2026, просьба Дениса: «нам нужен хаб
     * СОРТИРОВКА, добавим в него эти шарики и тортики»).
     *
     * ⚠️ ЧЕМ ОНА ОТЛИЧАЕТСЯ ОТ «БАШЕН», а то по названию они путаются. В башнях
     * ход ограничен ПОРЯДКОМ (диск на диск меньше нельзя), и место не кончается.
     * Здесь ограничивает ВМЕСТИМОСТЬ ёмкости: колба и ниша конечны, и главный
     * дефицит — куда положить, а не в каком порядке.
     *
     * ⚠️ ПЕРЕЛИВАЛКА СТОИТ И ТАМ, И ТУТ — намеренно. Реестр это разрешает: у игры
     * один РОДИТЕЛЬ (`mergedInto` решает, какая развилка открывает её профилю), а
     * показываться она вправе в нескольких развилках. Довод автора «Башен» верен
     * — там тоже считают наперёд; вырывать её оттуда значило бы чинить то, что
     * не сломано.
     */
    id: 'sorting_group',
