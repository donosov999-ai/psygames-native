...
const CHESS: ProfileDef = {
  ...
  // (v1.105.0) + четыре игры лаборатории 19.08.2026 + «Детский мат» (05.09.2026).
  // Формула описывает ЯДРО, а не длину списка: набор рос трижды, и сверять его с
  // «девяткой» больше нельзя.
  allowed_games: [
    // Base (1 per category)
    ...
    'chess_blind',
    /**
     * 05.09.2026, требование Дениса дословно: «в профиле шахматисты он должен
     * быть доступен обязательно». Речь о «Детском мате» — этюдах на скорость
     * узнавания узора. Для шахматиста это ближе к предмету профиля, чем любая
     * общая проба в списке: Chase & Simon (1973) показали, что мастера от
     * новичка отличает не глубина перебора, а узнавание знакомых кусков позиции.
     *
     * ⚠️ Развилку «Шахматы» (`chess_group`) в список НЕ дописываем: правило
     * реестра — развилка открыта, если открыта хоть одна игра за ней, и
     * `hub-membership` это стережёт. Дописать её руками значило бы завести
    ...
    'anagrams',  // 06.09.2026 — развилка «Языки»: буквенные пазлы — вход в словарь без чтения текстов
    'trail_making',  // 06.09.2026 — развилка «Маршруты»: соединять по порядку, не отрывая взгляда (не песочница, в отличие от dots_connect),
    /**
     * 🔴 РАЗВИЛКА, ОТКРЫТАЯ ОДНОЙ ИГРОЙ, — ПУСТАЯ РАЗВИЛКА (правка 06.09.2026).
     *
     * Развилки «Языки», «Маршруты» и «Шахматы» открывались этому профилю ОДНИМ
     * упражнением: карточка появлялась, человек заходил и видел внутри одну
     * игру вместо семи. Замер до правки: «Слова» 1 из 7, «Слух» 0 из 4,
     * «Маршруты» 1 из 3, «Шахматы» 1 из 2. Требование Дениса: открывать
     * развилку ЦЕЛИКОМ, а не по одному упражнению.
     *
     * ⚠️ «Языки» — оболочка над «Словами» и «Слухом»: открыть её и не открыть
     * их значит открыть пустую дверь во вторую пустую дверь.
     */
    'vocab_srs',
    'semantic_sort',
    'cloze',
...
const KIDS: ProfileDef = {
  ...
  allowed_games: [
    ...
    // «Дворец памяти» — решение Дениса 09.09.2026 по заявке раздела «Дворец/Лица»: метод мест —
    // учебная техника (списки, порядок), правила картинкой; лесенка вперёд не пускает без прохождения.
    'memory_palace',
    ...
    'scholars_mate',  // 06.09.2026 — развилка «Шахматы»: узнавание матового узора — считать не нужно, нужно УВИДЕТЬ
    'trail_making',  // 06.09.2026 — развилка «Маршруты»: соединять по порядку, не отрывая взгляда (не песочница, в отличие от dots_connect),
    /**
     * 🔴 РАЗВИЛКА, ОТКРЫТАЯ ОДНОЙ ИГРОЙ, — ПУСТАЯ РАЗВИЛКА (правка 06.09.2026).
     *
     ...
     */
    /**
     * `chess_blind` детям ОТКРЫТ (слово Дениса 06.09.2026: «шахматы это детская
     * игра, вполне, открываем туда полностью»). Развилка «Шахматы» у детей
     * целиком — обе игры.
     */
    'chess_blind',
    'vocab_srs',
...
const VASILYEVA: ProfileDef = {
  ...
  allowed_games: [
    ...
    'trail_making',  // 06.09.2026 — развилка «Маршруты»: соединять по порядку, не отрывая взгляда
    // ⚠️ Не `dots_connect` и не `one_line`: обе помечены `sandbox`, и профилю без
    // песочницы развилка открылась бы ПУСТОЙ — ровно третий случай из шапки
    // `filterAllowedGames`. Открывает развилку та игра, что реально доедет.
    'scholars_mate',  // 06.09.2026 — развилка «Шахматы»: узнавание матового узора — считать не нужно, нужно УВИДЕТЬ
  ],
  morning_playlist: [
...
const FREE: ProfileDef = {
  ...
  emoji: '🎁',
  color: '#f59e0b',
  description: '12 тренажёров · подобранный набор для старта',
  long_description: 'Набор для старта, без кода. По одному тренажёру из каждой категории: Шульте (внимание), Парные картинки (память), Мишени (реакция), Математический спринт + Считалка, Поиск отличий, Анаграммы, Ханойская башня, Сортировка товаров, N-back (рабочая память — облегчённая), Детский мат и Прокладывание пути. Двенадцать — это осознанный размер: полный каталог в первый день не помогает, а отпугивает. Последние две добавлены 06.09.2026, чтобы стартовому набору были видны развилки «Шахматы» и «Маршруты» — просьба Дениса «добавить во все профили». Остальные профили — такие же подборки под конкретную задачу, открываются кодом.',
  long_description_en: 'The starter set — no code needed. One trainer from each category: Schulte tables (attention), Picture Pairs (memory), Targets (reaction), Math Sprint + Counter, Spot the Difference, Anagrams, Tower of Hanoi, Goods Sort, N-back (working memory — light version), the Scholar\'s Mate drill and Trail Making. Themed profiles unlock with an access code.',
...
const WOMEN: ProfileDef = {
  ...
  allowed_games: [
    ...
    // Семья сортировок целиком, а не один «Товары» (решение Дениса 09.09.2026: пицца должна
    // быть видна). Замер хаба /games/sorting-hub: из семи упражнений видны были два (товары +
    // Ханой) — остальные пять не стояли ни в одном whitelist, только у профилей с
    // allowed_games: 'all'. Правило игры у всех одно (ёмкость), разная только еда.
    // ⚠️ В FREE намеренно НЕ добавлены: «двенадцать — осознанный размер» (long_description),
    // гейт game-standard сторожит совпадение числа в подписи с набором.
    'water_sort', 'ball_sort', 'nut_sort', 'cake_sort', 'pizza_sort',
...
const POLYGLOT: ProfileDef = {
  ...
  allowed_games: [
    ...
    'scholars_mate',  // 06.09.2026 — развилка «Шахматы»: узнавание матового узора — считать не нужно, нужно УВИДЕТЬ
    /**
     * 🔴 ТРИ ЯЗЫКОВЫХ УПРАЖНЕНИЯ, КОТОРЫЕ ЯЗЫКОВОЙ ПРОФИЛЬ НЕ ПОЛУЧИЛ. 11.09.2026.
     *
     * 📍 ЗАМЕР (origin/main 84d8b13a, состав по `filterAllowedGames` для всех 13
     * профилей): «Слух» открыт 4 упражнениями из 4 у `chess`, `kids`, `women` и у
     * двух профилей с `allowed_games: 'all'` — а у `polyglot` ДВА из четырёх. По
     * «Словам» — 7 из 7 против 6 из 7. Профиль, собранный ПОД ЯЗЫКИ, беднее
     * шахматного ровно в языковом разделе.
     *
     * Причина не в замысле, а в порядке появления: «Тоны» приехали 04.09.2026
     * (c965bab1) и были вписаны в три списка, до этого — «Диктант». Полиглота при
     * этом не открывали. Ровно случай «проверял под тем профилем, куда вписал».
     *
     * ⚠️ Что «Тоны» здесь уместны, сказано строкой выше моей правки: комментарий
     * к `rhythm_pitch` называет причиной «просодия, тоновые языки» — упражнения
     * про сами тоны тогда ещё не существовало.
     *
     * Сторожит `languages-profile-has-all-language-games.test.ts`.
     */
    'chinese_tones',  // слух — тоновые языки: тот самый различительный слух, ради которого взят rhythm_pitch
    'dictation',      // слух — письмо под диктовку: слух → орфография, самое языковое из четырёх
    'story_recall',   // память — понимание и пересказ связного текста на языке
  ],
...
/**
 * ВСЕГДА ОТКРЫТО — вне зависимости от профиля.
 *
 * 🔴 «Ментальная ротация» здесь по прямому решению Дениса 09.09.2026: «это
 * градообразующий хаб, нужен во всех профилях, в полном составе». До этого обе
 * игры развилки стояли только у «Шахматиста», «Кибермозга» и NZT — у остальных
 * десяти профилей развилка открывалась пустой: заголовок «Выбери упражнение» и
 * ни одной карточки под ним (замер 09.09: 10 профилей из 13 — ноль карточек).
 *
...
const ALWAYS_ALLOWED = new Set<string>([
   ...
   *
   * ⚠️ ПОЧЕМУ Я СПРАШИВАЛ. Забег идёт 8:24 без остановок, и я открыл его сперва
   * только взрослым профилям, а `kids` оставил Денису — длинный непрерывный забег
   * не похож на детский формат. Он решил иначе, и это его право: ограничение по
   * возрасту тут было бы моим домыслом, а не замером.
   *
   * 📌 В расписание зарядки игра ВСЁ РАВНО не входит — это другое решение и другой
   * механизм (`allow` в warmup): 8:24 не короткая проба.
   */
  'number_run',
  ...
  'sudoku',
  /**
   * 🔴 ГОЛОВОЛОМКИ ТЭТХЭМА — во всех профилях, решение Дениса 10.09.2026 «берём все».
   * Раздел новый и градообразующий: семнадцать головоломок на одном экране, у каждой
   * гарантия единственного решения. Без этой строки развилка встречала бы пустотой —
   * ровно то, что ловит гейт `hub-not-empty-in-every-profile`, и что я уже проверил
   * глазами на симуляторе: заголовок есть, карточек ноль.
   */
  'puzzles',
  /**
   * 🔴 «ПАУЗА» — ВО ВСЕХ ПРОФИЛЯХ. Решение Дениса 13.09.2026: «открой всем», после
   * того как я назвал замер: восстановительных в каталоге всего два — дыхание и
   * «Пауза», — и вторая была открыта лишь двум профилям из тринадцати.
   *
   * Довод за: «Пауза» — телесные практики, тут нечего измерять и не за что платить
   * прогрессом; держать их платными значило продавать отдых. Дыхание по той же
   * причине открыто всем с v1.124.0.
   */
  'pause',
]);
...
export function filterAllowedGames(profile: ProfileDef) {
   ...
   */
  /**
   * ⚠️ ТРИ ДЫРЫ, КОТОРЫЕ ПРЯТАЛ ПРЕДПРОСМОТР. До 04.09.2026 новые развилки были
   * видны только профилю `nzt48`, и гейт для остальных профилей их пропускал.
   * Как только развилки включили всем, он показал сразу три расхождения — и все
   * три реальные:
   *
   *  1. ВСЕГДА РАЗРЕШЁННОЕ не открывало свою развилку. `picture_pairs` открыт
   *     всем через ALWAYS_ALLOWED, но в `allowed` его нет — и «Зрительная память»
   *     оставалась закрытой при открытом упражнении внутри.
   *  2. ВЛОЖЕННАЯ РАЗВИЛКА не открывала родительскую. «Слова» открывались
   *     упражнением внутри, а зонтик «Языки», в котором они лежат, — нет.
   *     Поэтому подъём идёт до неподвижной точки, а не в один шаг.
   *  3. ПЕСОЧНИЦА открывала развилку, в которой потом ничего не оставалось:
   *     ребёнок отфильтровывался как сырой, а развилка висела пустой. Считаем
   *     только по тем играм, что реально доедут до человека.
   */
  const wantsSandboxEarly = profile.allow_sandbox === true;
  const реально = new Set(
    GAMES.filter((g) => (!g.sandbox || wantsSandboxEarly) && (allowed.has(g.id) || ALWAYS_ALLOWED.has(g.id)))
      .map((g) => g.id),
  );
  const openHubs = new Set<string>();
  for (let шаг = 0; шаг < 5; шаг += 1) {
    const было = openHubs.size;
    for (const g of GAMES) {
      if (!g.mergedInto) continue;
      if (реально.has(g.id) || openHubs.has(g.id)) openHubs.add(g.mergedInto);
    }
    if (openHubs.size === было) break;              // неподвижная точка
  }
