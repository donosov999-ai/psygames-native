...
const WOMEN: ProfileDef = {
  ...
  evening_playlist: [
    ...
    // История шага в двух репортах Вали.
    // v1.170: заявлял 90 с, отрабатывал за 21 («обман») — easy без настроек даёт
    // 8 тривиальных раундов. Я поднял до 20, чтобы дотянуть до заявленной минуты.
    // v1.171, голосом: «первая игра три раза, а вторая двадцать — странно». Длину
    // я починил ценой перекоса: человек сравнивает не секунды, а РАУНДЫ, и 3 против
    // 20 читается как несправедливость, даже когда по времени всё ровно.
    // Итог: 10 раундов и честные 45 с в заявке. Отличия рядом идут 51 с по факту
    // (замер по 32 сессиям), так что шаги сопоставимы и по времени, и на глаз.
    { game_id: 'visual_search',    game_route: '/games/visual-search',    difficulty: 'medium', settings: { trials: 10 }, est_duration_sec: 45 },
