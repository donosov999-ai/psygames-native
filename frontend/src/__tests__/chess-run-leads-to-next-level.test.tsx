...
describe('«Детский мат»: конец подхода ведёт дальше', () => {
  it('🔴 подход пройден целиком → экран «уровень взят» и следующая ступень', () => {
    ...
    for (const задача of колода) {
       ...
       *
       * ⚠️ ИГРОВЫЕ ЧАСЫ ДВИГАЮТСЯ ВМЕСТЕ С НАСТЕННЫМИ, и это не формальность.
       * С 09.09.2026 срок показа вердикта считается по `now()`, чтобы пауза его
       * держала. Крутить здесь только `jest.advanceTimersByTime` значило бы
       * изображать игрока на вечной паузе: позиция не сменится никогда, и проба
       * покраснеет на исправном коде.
       */
      TestRenderer.act(() => { mockЧасы.t += 700; jest.advanceTimersByTime(700); });
    }
   ...
   */
  /**
   * 🔴 ПАУЗА ОБЯЗАНА ДЕРЖАТЬ И ПЕРЕХОД К СЛЕДУЮЩЕЙ ПОЗИЦИИ.
   *
   * Соседний дефект того же рода, что найден 09.09 в «Доске в уме»: показ вердикта
   * уводил на следующую позицию обычным `setTimeout` по НАСТЕННЫМ часам. Ответил,
   * нажал паузу в те же полсекунды — и позиция сменилась за спиной у меню.
   *
   * Пауза здесь моделируется честно и просто: игровые часы (`mockЧасы.t`) СТОЯТ,
   * а настоящие таймеры jest идут. Ровно это и делает `holdGame()` в приложении.
   * Окно верное: 550 мс на верный ответ, 1400 на неверный — крутим три секунды,
   * то есть заведомо больше обоих.
   */
  it('🔴 игровые часы стоят — следующая позиция НЕ приходит', () => {
    let tree: any;
    TestRenderer.act(() => {
      tree = TestRenderer.create(React.createElement(ScholarsMateScreen as any));
      mounted.push(tree);
    ...
    // Отвечаем на первую позицию верно — появляется вердикт.
    const uci = колода[0]!.solutions[0]!;
    нажать(tree, uci.slice(0, 2));
    нажать(tree, uci.slice(2, 4));
    expect(`на счётчике первая позиция: ${/\b1\/8\b/.test(текст(tree))}`)
      .toBe('на счётчике первая позиция: true');

    // ПАУЗА: игровые часы стоят, настенные идут три секунды.
    TestRenderer.act(() => { jest.advanceTimersByTime(3000); });

    expect(`позиция сменилась на паузе: ${/\b2\/8\b/.test(текст(tree))}`)
      .toBe('позиция сменилась на паузе: false');

    // Часы пошли — переход обязан состояться, иначе это не пауза, а зависание.
    TestRenderer.act(() => { mockЧасы.t += 3000; jest.advanceTimersByTime(300); });
    expect(`после снятия паузы позиция сменилась: ${/\b2\/8\b/.test(текст(tree))}`)
      .toBe('после снятия паузы позиция сменилась: true');
  });
  ...
  it('🔴 на 19-м уровне два промаха из десяти НЕ берут уровень', () => {
    ...
    колода.forEach((задача: any, i: number) => {
      ...
      // Неверный ответ показывается 1,4 с — и часы, и таймеры идут вместе (см. выше).
      TestRenderer.act(() => { mockЧасы.t += 1600; jest.advanceTimersByTime(1600); });
    });
