...
describe('слот шапки не под глобальной справкой', () => {
  ...
  it('🔴 в слоте есть питомец — иначе отступ снова станет лишним', () => {
    // Отступ оправдан только тем, что слот непустой. Уберут питомца — вернётся
    // сдвинутый заголовок на 63 экранах, и об этом надо будет вспомнить здесь.
    expect(SHELL).toContain('<GamePet');
  });
