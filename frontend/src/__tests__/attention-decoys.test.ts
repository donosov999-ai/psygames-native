...
describe('помехи вокруг стимула: общий набор знаков', () => {
  ...
  /**
   * 🔴 ПОМЕХА — ТОЛЬКО ГЕОМЕТРИЧЕСКАЯ ФИГУРА ИЗ ЗАКРЫТОГО СПИСКА.
   *
   * Два разных требования в одном, и оба важны:
   * 1. Помеха не должна «афишировать» задачу: буква рядом со словом Струпа
   *    читается вместе с ним, цифра рядом со стимулом переключения — судится
   *    вместе с ним. Проба стала бы смысловой вместо перцептивной.
   * 2. Помеха должна выглядеть НАМЕРЕННОЙ, а не сбоем отрисовки. Первая
   *    редакция брала `#`, `§`, `%`, `&` — условие 1 они выполняли, а читались
   *    как кракозябра вместо текста. 📌 Владелец 10.09.2026: «выглядят как
   *    ошибки, а не как части приложения». Список закрыт ФИГУРАМИ из того же
   *    набора иконок, которым нарисован весь интерфейс.
   *
   * ⚠️ Звезды, сердца и «искр» в списке нет намеренно: у WCST форма — это САМА
   * задача, и звезда там один из четырёх эталонов.
   */
  it('🔴 каждая помеха — фигура из закрытого списка, без букв, цифр и «задачных» форм', () => {
    const РАЗРЕШЕНО = ['ellipse-outline', 'square-outline', 'triangle-outline',
                       'diamond-outline', 'cube-outline', 'prism-outline'];
    const чужие = DECOY_GLYPHS.filter((g) => !РАЗРЕШЕНО.includes(g));
    expect(`не из списка фигур: ${чужие.join(', ') || '—'}`).toBe('не из списка фигур: —');
    // отдельно: ни одна помеха не текстовый знак — имя иконки всегда через дефис
    const текстовые = DECOY_GLYPHS.filter((g) => !g.includes('-outline'));
    expect(`текстовые знаки среди помех: ${текстовые.join(', ') || '—'}`)
      .toBe('текстовые знаки среди помех: —');
    // и ни одна не совпадает с формой-эталоном WCST (звезда, круг задачи и т.п.)
    const задачные = DECOY_GLYPHS.filter((g) => /star|heart|sparkles/.test(g));
    expect(`формы, занятые задачей: ${задачные.join(', ') || '—'}`).toBe('формы, занятые задачей: —');
  });
