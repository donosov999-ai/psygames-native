 ...
 * него. Языков в филвордах 5 → 7.
 *
 * ⚠️ САМО ЧИСЛО НЕ ПРИШПИЛИВАТЬ. Здесь стояло `toBe(4)` для fr/it как факт о данных
 * («у них ноль трёхбуквенных»). 09.09.2026 словарь переводов (`translationVocab.ts`,
 * коммит 78990ebe раздела языков) дорос до всех языков, и в пуле появились живые
 * трёхбуквенные: fr — 25 (AMI BON CLÉ DIX EAU FEU …), it — ровно 8 (BLU DUE ORA QUI
 * SEI TRE UNO VIA), то есть впритык к MIN_PER_CORE_LENGTH. Пол сместился на 3, и
 * проба покраснела не потому, что игра сломалась, а потому что пришпилила вывод из
 * данных. Проверяем свойство, а не число: пол лежит в окне лестницы, и поле
 * собирается на всей лестнице.
 */
it('fr и it поддержаны и собирают поле на всей лестнице', () => {
  for (const l of ['fr', 'it']) {
    expect(FILLWORDS_LOCALES).toContain(l);
    expect([3, 4]).toContain(полДлиныЯзыка(l));
    for (const L of [1, 5, 20, 50, 94, 150, 202]) {
      ...
      // Слов короче пола языка на поле нет — пол берём из данных, не число 4 (см. шапку).
      for (const w of p.words) expect([...w.word].length).toBeGreaterThanOrEqual(полДлиныЯзыка(l));
    }
  }
});
