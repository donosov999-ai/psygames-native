...
describe('блок «Сегодня» на главном экране', () => {
  ...
  it('рисуется БЕЗУСЛОВНО — за комментарием сразу разметка, а не условие', () => {
    ...
    /**
     * 🔴 13.09.2026: ДОПУЩЕНА РОВНО ОДНА ОБЁРТКА — ВЫБОР ВЛАДЕЛЬЦА, И БОЛЬШЕ НИЧЕГО.
     *
     * Проба ловит беду «блок исчез САМ»: `{today.rows.length > 0 && …}` в пустой день
     * стирает строку, и узнать, что она бывает, неоткуда. Эта беда никуда не делась
     * и по-прежнему запрещена.
     *
     * Но появился редактор состава (просьба Дениса «какие блоки показывать»), и там
     * владелец может убрать блок ОСОЗНАННО, файлом настроек. Это не стечение данных,
     * а решение человека, и запрещать его проба не должна.
     *
     * Поэтому допуск сужен до одной точной формы: `показыватьБлок('сегодня', …)`.
     * Любое другое условие перед корнем — снова красное. Если бы здесь стояло просто
     * «разрешаем любое условие», проба перестала бы ловить то, ради чего написана.
     */
    const ВЫБОР_ВЛАДЕЛЬЦА = "{показыватьБлок('сегодня', блокиГлавной) && (";
    if (after.startsWith(ВЫБОР_ВЛАДЕЛЬЦА)) {
      after = after.slice(ВЫБОР_ВЛАДЕЛЬЦА.length).trimStart();
    }
