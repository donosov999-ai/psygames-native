  ...
  it('развилка видна ровно тогда, когда за ней открыта хоть одна игра', () => {
    ...
    for (const p of PROFILES) {
      ...
      for (const хаб of GAMES.filter((g) => isHubGame(g.id))) {
         ...
         */
        /**
         * ⚠️ Развилка в ПРЕДПРОСМОТРЕ (`previewIn`) видна только своим профилям —
         * заведено 04.09.2026, чтобы девять новых развилок сперва проверил владелец,
         * а остальным каталог остался прежним. Для чужих профилей её отсутствие —
         * норма, а не расхождение.
         */
        if (хаб.previewIn && !хаб.previewIn.includes(p.id)) continue;
