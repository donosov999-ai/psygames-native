...
describe('игры за развилками', () => {
  ...
  it('развилку без единой открытой игры не показываем — пустой вход хуже отсутствия', () => {
    const women = new Set(filterAllowedGames(PROFILES.find((p) => p.id === 'women')!).map((g) => g.id));
    // «Микро-релакс» не открывает ни одной игры конфликта внимания
    expect(women.has('attention_conflict')).toBe(false);
  });
});
