...
const RAISES_IN_WARMUP: Record<string, string> = {
  'cake-sort.tsx': 'стол раздаётся из deal(lvl.level), плейлист параметров не передаёт — доска шага зарядки и личной партии одна и та же, значит и подъём значит то же',
  'water-sort.tsx': 'уровень поднимается только по СОБРАННОМУ раскладу (`завершить` зовётся из ветки isSolved), а собрать его в зарядке — то же достижение, что и в личной партии: расклад один и тот же, лестница одна',
