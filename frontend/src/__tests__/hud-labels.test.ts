 ...
 */
/**
 * ⚠️ ШАПКА ЖИВЁТ В ДВУХ ПРОПАХ: `stats` (вёрстка от игры) и `hud` (данные).
 *
 * 02.09.2026 игры начали переезжать со строк текста на `hud`, и гейт «где
 * штрафуют за время — там виден остаток» покраснел на number-bonds: остаток
 * никуда не делся, он просто переехал в другой проп. Гейт, знающий только одну
 * форму шапки, сообщает о поломке, которой нет.
 */
function statsBlocks(src: string): string[] {
  const out: string[] = [];
  const re = /\b(?:stats|hud)=\{/g;
 ...
 */
/**
 * Экраны, где числа пока без подписей, — с потолком и причиной. Список ТОЛЬКО
 * сокращается: 02.09.2026 из него вышел `sudoku-samurai` (шапка переехала на `hud`,
 * и подписи пришли вместе с бейджами); 02.09 из него вышел и `sudoku-fractal` — его
 * «✗12» и породило вопрос «что за красные цифры».
 */
const BUSY: Record<string, { max: number; why: string }> = {
  ...
  // 'mahjong.tsx' убран 07.09.2026: экран переведён на hud из трёх подписанных
  // счётчиков (уровень, найдено, доступно), безымянных пилюль не осталось —
  // и сам гейт этого потребовал строкой «подписан целиком, убрать из BUSY».
...
describe('подписи чисел в шапке игры', () => {
  /**
   * 🔴 ПРИБОР ПРОВЕРЯЕТСЯ КОНТРОЛЕМ С ИЗВЕСТНЫМ ОТВЕТОМ, А НЕ СЧЁТОМ ЭКРАНОВ.
   *
   * Было: `parsed.length > 30` — «хотя бы у половины экранов шапка разбирается».
   * Порог оказался ТУХНУЩИМ ПО ПОСТРОЕНИЮ: разбираются только РУКОПИСНЫЕ JSX-шапки,
   * а проект переводит экраны на шапку-данные (`hud={[{ key, label, value }]}`), и
   * каждый такой переезд уменьшает число на единицу. 12.09.2026 счётчик дошёл до 30
   * и прогон покраснел — но покраснел он на УЛУЧШЕНИИ: коммит `64f8e1c6` снял
   * рукописную шапку у `phonemic-fluency`. Проба требовала не чинить, а откатить
   * канон, то есть мерила ровно наоборот.
   *
   * Стало: контроль на литерале. Парсер обязан найти подписи в заведомо известном
   * куске — это и есть «прибор жив», и оно не зависит от того, сколько экранов ещё
   * не переехало. Счётчик оставлен, но с честным полом: он говорит «материал ещё
   * есть», а не «перееду быть не должно».
   */
  it('есть что проверять — прибор проверен контролем, а не счётом', () => {
    expect(FILES.length).toBeGreaterThan(50);
    expect(existsSync(join(ROOT, 'src/components/GameShell.tsx'))).toBe(true);

    /**
     * ⚠️ ЗДЕСЬ БЫЛ ЖЁСТКИЙ ПОЛ, И main ЕГО СНЯЛ — правильно, эта сторона взята
     * целиком. Сохраняю только факт, который иначе потеряется: два экрана из
     * счёта ушли НЕ переездом на шапку-данные, а уборкой. У WCST в слоте `stats`
     * лежал пустой `{null}` с комментарием, у CPT бейдж правил переехал в один
     * ряд со «СТОП» — над полем стояло три полосы вместо двух, и коробка стимула
     * из-за этого садилась ниже соседской (раздел «Конфликт внимания», 11.09.2026).
     * То есть убывание счёта имеет две разные причины, и обе — норма.
     */

    // КОНТРОЛЬ: известный ответ — две подписи. Сломается парсер — покраснеет здесь,
    // и покраснеет всегда, сколько бы экранов ни переехало на шапку-данные.
    const контроль = `
      <GameShell stats={
        <View>
          <Text style={styles.cap}>{t('hud_letter')}</Text>
          <Text style={styles.val}>{счёт}</Text>
        </View>
      } />`;
    expect(leaves(statsBlocks(контроль).join('\n')).length).toBeGreaterThanOrEqual(2);

    // ПОЛ: материал для проверки ещё есть. Число УБЫВАЕТ по мере переезда экранов
    // на шапку-данные — это норма, а не регресс. Дошло до пола — не поднимать
    // обратно рукописные шапки, а снимать этот счётчик: контроль выше самодостаточен.
    const parsed = FILES.filter((f) => leaves(statsBlocks(readFileSync(join(GAMES, f), 'utf8')).join('\n')).length > 0);
    expect(`экранов с рукописной шапкой: ${parsed.length} (было 31 на метке v2.53.9)`)
      .toMatch(/^экранов с рукописной шапкой: (1[0-9]|[2-9][0-9])/);
  });
  ...
  it('🔴 у каждого числа в шапке есть слово рядом', () => {
    ...
    for (const f of FILES) {
      ...
      for (const leaf of all) {
        ...
        if (sibling) continue;
        /**
         * 🔴 ЗНАЧОК С ПОДПИСЬЮ ДЛЯ СКРИНРИДЕРА — ТОЖЕ ПОДПИСЬ.
         *
         * Решение Дениса 03.09.2026: «сократить слова или заменить на иконки, если
         * речь про верхний тулбар». Причина не в красоте: замер показал, что в
         * судоку шесть счётчиков со словами занимают вдвое больше ширины, чем даёт
         * экран 360, и лишнее либо вылезает за край, либо (хуже) прячется.
         *
         * Правило гейта при этом не смягчается, а уточняется. Оно всегда звучало
         * «человек должен видеть, по чему его судят» — и значок это условие
         * выполняет, если рядом стоит слово для скринридера. Голое число без
         * значка и без подписи по-прежнему нарушение.
         *
         * Признак: в той же коробке есть `Ionicons` И есть `accessibilityLabel`
         * (или подпись живёт на кнопке-обёртке, как у бейджей каркаса).
         */
        const наЗначке = all.some((o) => sameBox(o.path, leaf.path) && /Ionicons/.test(o.text))
          || /Ionicons/.test(leaf.text);
        const естьПодписьДляЧтения = /accessibilityLabel/.test(
          statsBlocks(src).join('\n'),
        );
        if (наЗначке && естьПодписьДляЧтения) continue;
