...
describe('подписи чисел в шапке игры', () => {
  it('есть что проверять — иначе тест зелен вслепую', () => {
    ...
    /**
     * Порог — защита от ПОЛОМКИ ПАРСЕРА, а не мера качества: если разбор
     * сломается, он вернёт ноль и весь набор станет зелёным вслепую.
     *
     * ⚠️ 11.09.2026 опущен 30 → 28. Раздел «Конфликт внимания» убрал слот `stats`
     * у двух экранов, и оба раза по делу: у WCST там лежал пустой `{null}` с
     * комментарием (место занимал, содержимого не было), у CPT бейдж правил
     * переехал в один ряд со «СТОП» — над полем стояло ТРИ полосы вместо двух, и
     * коробка стимула из-за этого садилась ниже соседской.
     * То есть счёт упал не от поломки разбора, а от уборки: было 32, стало 30.
     * Число здесь всегда было счётом на момент записи, а не «половиной экранов»
     * из комментария выше: файлов в папке под сотню.
     */
    const parsed = FILES.filter((f) => leaves(statsBlocks(readFileSync(join(GAMES, f), 'utf8')).join('\n')).length > 0);
    expect(parsed.length).toBeGreaterThan(28);
  });
