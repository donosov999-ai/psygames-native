...
describe('сортировка: звёзды и лимит честные', () => {
  it('порог звёзд считается отдельной функцией, а не формулой с нулевым лимитом', () => {
    ...
    /**
     * ⚠️ Множитель эталона больше не тройка: замер поиском A* 02.09.2026 показал,
     * что прикидка `types × 3` завышала минимум на треть, и три звезды были
     * недостижимы на 95 % досок. Проверяем СВЯЗЬ — что эталон считается от лимита
     * или от числа видов, — а само число живёт в игре (`REF_PER_TYPE`).
     */
    expect(game).toMatch(/cfg\.moveLimit > 0 \? cfg\.moveLimit : Math\.round\(cfg\.types \* REF_PER_TYPE\)/);
    /**
