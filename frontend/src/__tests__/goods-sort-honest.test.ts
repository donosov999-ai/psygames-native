...
describe('сортировка: обещание совпадает с игрой', () => {
  ...
  /**
   * 🔴 СИГНАЛЬНАЯ НИТЬ СРАБОТАЛА — И ЗАМЕНЕНА НАСТОЯЩЕЙ ПРОВЕРКОЙ.
   *
   * Здесь стояло `expect(game).not.toMatch(/hiddenLayer|backRow|скрытый ряд/)`
   * с оговоркой «если скрытые слои когда-нибудь появятся — напомнит обновить
   * текст». 07.09.2026 они появились: с L52 у части ниш есть второй ряд, он
   * выходит вперёд, когда ниша опустеет.
   *
   * ⚠️ Заменять нить переименованием стиля было бы подгонкой кода под пробу:
   * нить сделала ровно то, ради чего заведена. Поэтому она превращается в
   * проверку того, что и требовалось напомнить, — что справка про механику
   * ЕСТЬ, а не что механики нет.
   *
   * ⚠️ И проверяем ИСПОЛНЕНИЕМ: спрашиваем у словаря готовый текст правила, а не
  ...
  it('🔴 задние ряды появились — и справка о них есть на обоих родных языках', () => {
    ...
    for (const язык of ['ru', 'en']) {
      const т = levelRuleText(язык, 'goods_sort', правило);
      expect(т.title.length).toBeGreaterThan(3);
      expect(т.rule.length).toBeGreaterThan(20);
    }
  });
});
