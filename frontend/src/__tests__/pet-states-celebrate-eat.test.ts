...
describe('состояния питомца: celebrate и eat', () => {
  ...
  it('🔴 экраны зовут состояния по СМЫСЛУ, а не подставляют замену сами', () => {
    ...
    /**
     * ⚠️ ВЫРАЖЕНИЕ ИЗМЕНИЛОСЬ 11.09.2026, И ЭТО ПОЧИНКА, А НЕ ДРЕЙФ. Раньше здесь
     * стоял `feastAnim ? 'eat' : 'idle'` — ОДИН флаг на три кнопки заботы, и кот
     * принимался жевать, когда его гладили или мыли (Денис: «из кнопок только кадры
     * еды есть»). Теперь у каждого действия своё состояние.
     * Смысл пробы прежний: экран зовёт состояние ПО СМЫСЛУ и не подставляет замену
     * сам — подстановка живёт в `ЗАМЕНА` внутри PetSprite.
     */
    expect(pet).toMatch(/показатьДействие\('eat'/);
    expect(pet).toMatch(/показатьДействие\('groom'/);
    expect(pet).toMatch(/показатьДействие\('wiggle'/);
    // Замену экран не подставляет: 'wave' вместо еды и 'jump' вместо праздника — не его дело.
    expect(pet).not.toMatch(/показатьДействие\('(wave|jump)'/);
  });
});
