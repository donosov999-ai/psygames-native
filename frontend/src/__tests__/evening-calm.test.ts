...
describe('вечерний набор', () => {
  ...
  /**
   * 🔴 ПРЕМИСА ЭТОЙ ПРОВЕРКИ ИСЧЕЗЛА 07.09.2026 — И ЭТО НЕ ПОБЛАЖКА, А УСИЛЕНИЕ.
   *
   * Требовалось: `{!isCalm && (…)}` — маджонг ПРЯЧЕТ секундомер в спокойном
   * режиме. Раздел сортировки убрал таймер из маджонга целиком («раньше он
   * спорил с таймером зарядки», mahjong.tsx). Прятать больше нечего, и старое
   * утверждение требовало бы вернуть то, чего быть не должно.
   *
   * Новое утверждение строже прежнего: секундомера нет НИ В КАКОМ режиме.
   * Появится обратно — покраснеет, и тогда вернётся и правило про `isCalm`.
   */
  it('🔴 у маджонга секундомера нет вовсе — ни в спокойном режиме, ни в обычном', () => {
    const часы = /\btimeLeft\b|\bsetTimeLeft\b|\bstopwatch\b/.exec(mahjong);
    expect(`секундомер в маджонге: ${часы ? часы[0] : 'нет'}`).toBe('секундомер в маджонге: нет');
  });
