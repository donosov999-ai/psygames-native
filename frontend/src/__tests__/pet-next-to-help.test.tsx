...
describe('питомец в углу справки', () => {
  ...
  it('🔴 по нажатию на питомца открывается его экран — на КАЖДОМ экране игры', () => {
    /* Просьба Дениса 03.09.2026: «по клику в тулбаре надо разворачивать окно маскота,
       всегда». Медальон был картинкой без обработчика: жали — ничего не происходило. */
    const нажатия: number[] = [];
    const root = нарисовать({ onPetPress: () => нажатия.push(1) });
    const кнопка = root.findAll((у) => (у.props as Record<string, unknown>).testID === 'pet-open')[0];
    expect(кнопка).toBeTruthy();
    expect(typeof (кнопка.props as Record<string, unknown>).onPress).toBe('function');
    ((кнопка.props as { onPress: () => void }).onPress)();
