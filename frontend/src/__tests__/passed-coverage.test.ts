 ...
 */
/**
 * ⚠️ МАДЖОНГ ВЫВЕДЕН ОТСЮДА 06.09.2026. У него проигрыш НЕ надо было вводить —
 * он существовал: вставшая доска при исчерпанных перетасовках и отменах, где
 * кнопка «начать заново» появляется как ЕДИНСТВЕННЫЙ честный выход. Замер по
 * случайной игре, 30 партий на уровень: L5 — 0%, L15 — 3%, L28 — 60%, L40 —
 * 53%. Событие просто не доходило до статистики.
 *
 * Теперь `restartLevel` пишет `passed: false`. Понижение лестницы (`lvl.fail`)
 * НЕ сделано намеренно: оно меняет продвижение игрока и ждёт решения Дениса
 * (задача 8543237a). Поэтому маджонг остаётся в списке односторонних лестниц
 * ниже, но уже не в списке «исхода нет вовсе».
 */
const SOLVABLE_ALWAYS_TRUE = new Set(['hanoi', 'picture-pairs']);
