...
describe('перетаскивание не отменяет тапы и не заводит своих правил', () => {
   ...
   */
  /*
   * 🔴 ОБА УТВЕРЖДЕНИЯ ПРО ТАП ПЕРЕЕХАЛИ НА ИСПОЛНЕНИЕ — `goods-sort-taps-and-fly`.
   *
   * Здесь сверялся текст: `onPress={() => handleItemTap(i, s)}` и
   * `accessibilityLabel={cellLabel(i, cell)}`. 07.09.2026 обе покраснели от
   * переноса стопки товаров в запоминающий компонент при неизменном поведении.
   * Обратная сторона хуже: сними кто `onPress` с товара, оставив эту строку
   * где-нибудь ещё в файле, — проверка осталась бы зелёной, а игра со
   * скринридером стала бы непроходимой.
   *
   * Теперь экран поднимается, и спрашивается ДЕРЕВО: у ниши и у каждого товара
   * есть кнопка с обработчиком и подписью, и тап по товару его действительно
   * выделяет (иначе проверка зелена и у кнопки, которая ничего не делает).
   */
