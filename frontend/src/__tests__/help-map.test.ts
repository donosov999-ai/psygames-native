...
describe('карта справки', () => {
  ...
  it('в карте нет маршрутов, которых нет в каталоге', () => {
    /**
     * ⚠️ КЛЮЧ С РЕЖИМОМ — ЗАКОННЫЙ, И ЭТО НЕ ПОСЛАБЛЕНИЕ. Сорок две головоломки
     * Тэтхэма живут на одном маршруте `/games/puzzles` и различаются `?mode=`.
     * С 16.09.2026 карта раскрывает их поимённо: иначе справка всем сорока двум
     * показывала текст «Чёт-нечета» (жалоба 2b774e8b — «Как играть? Где справка?»
     * на четырёх РАЗНЫХ головоломках за 90 секунд). Сверяется всё равно маршрут:
     * часть до «?» обязана быть в каталоге, выдумать её режимом нельзя.
     */
    const stale = Object.keys(HELP_MAP).filter((r) => !routes.includes(r.split('?')[0]));
