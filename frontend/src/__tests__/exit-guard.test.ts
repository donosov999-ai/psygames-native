 ...
 *
 * ⚠️ ШЕСТЬ ЭКРАНОВ-ОБЁРТОК, ПЕРЕСЕЛЁННЫХ НА КАРКАС 20.08.2026, СЮДА НЕ ПОПАЛИ —
 * И ЭТО РЕШЕНИЕ, А НЕ НЕДОСМОТР. `dots-connect`, `one-line`, `faces-names`,
 * `object-tracker`, `navigator`, `rhythm-pitch` спотыкаются на ТРЕТЬЕМ пункте
 * правила: зерно партии у всех шести — `<игра>-${level}`, то есть расклад
 * ВОСПРОИЗВОДИМ по номеру уровня и сделано это нарочно («не вышло — крутани ещё
 * раз» должно быть второй попыткой, а не лотереей). Продолжать там нечего:
 * повторный вход выдаёт ту же сетку, тот же граф, тех же людей, ту же
 * траекторию, то же задание. Слой `services/resume` им поэтому не нужен, и
 * `resumable` они не обещают — врать про сохранение хуже, чем честно потерять.
 *
 * ⚠️ НО ВОПРОС ПРИ ВЫХОДЕ ИМ НУЖЕН, И ЭТО НЕ ПРОТИВОРЕЧИЕ. Воспроизводима
 * ЗАДАЧА, а не вложенная в неё работа: проложенные пути, пройденные рёбра,
 * заученные лица, минута слежения глазами, набитая поправка задержки. Их не
 * возвращает ничто, и стирать их молча по промаху нельзя. Именно эту половину
 * (спросить, ничего не сохраняя) и стережёт `module-games-guard.test.ts` —
 * реестр здесь про ХРАНЕНИЕ партии, и место им не в нём.
 */
...
describe('вопрос о выходе не стоит игроку партии', () => {
  ...
  /**
   * 🔴 ВОПРОС ОБЯЗАН БЫТЬ САМОЙ ВЕРХНЕЙ НАКЛАДКОЙ. Иначе на него нельзя ответить:
   * накладки в каркасе растянуты во весь экран и ловят нажатия, поэтому та, что
   * выше, съедает кнопки «остаться» и «выйти» целиком — экран запирается до
   * перезагрузки приложения.
   *
   * Ровно это и случилось 19.08.2026: пауза (90) оказалась выше вопроса (85),
   * и все девять игр с вопросом при выходе полчаса были заперты. Нашёл
   * параллельный заход живой проверкой `elementFromPoint` по центру кнопок —
   * ни один статический гейт этого не видел.
   *
   * Проверяем ПОРЯДОК, а не отдельный случай с паузой: следующая накладка
   * повторила бы беду молча.
   */
  it('🔴 вопрос о выходе лежит выше всех накладок каркаса', () => {
    const layers = [...SHELL.matchAll(/(\w*[Oo]verlay):\s*\{[^}]*zIndex:\s*(\d+)/g)]
      .map((m) => ({ name: m[1], z: Number(m[2]) }));
    expect(layers.length).toBeGreaterThanOrEqual(3);   // иначе проверка слепа
    const exit = layers.find((l) => l.name === 'exitOverlay');
    expect(exit).toBeDefined();
    const above = layers.filter((l) => l.name !== 'exitOverlay' && l.z >= exit!.z)
      .map((l) => `${l.name} (${l.z}) не ниже вопроса (${exit!.z}) — съест кнопки`);
    expect(above).toEqual([]);
  });

  /**
   * И вторым слоем: плашку паузы поверх вопроса не рисуем вовсе. Порядок уже
   * спасает от перехвата нажатий, но серая пелена с надписью «Пауза» позади
   * вопроса объясняет происходящее хуже самого вопроса.
   */
  it('плашка паузы не показывается, пока висит вопрос', () => {
    const flat = SHELL.replace(/\s+/g, ' ');
    // 17.09.2026: то же для вопроса «Начать заново?» из ряда значков под полем (`спросЗаново`).
