...
describe('«Соедини точки» — ядро делает то, на чём держится стыковка', () => {
  ...
  it('🔴 сложность растёт полем и парами, а не сокращением времени', () => {
    ...
    // ⚠️ ПОТОЛОК ПОЛЯ 10 → 11 (06.09.2026): поле выросло, и это ЗАМЕР, а не
    // подгонка числа под правку. Число пар упёрто в палитру (14 цветов),
    // поэтому на большем поле путь удлиняется сам; лишнюю площадь забирают
    // стены (`ЦЕЛЬ_НА_ПУТЬ` в генераторе), и на путь остаётся те же 6,4 клетки.
    // 11×11 при 31 стене даёт 90 свободных клеток — ровно столько же, сколько
    // 10×10 при десяти стенах, то есть выросло ПОЛЕ, а не длина пути.
    expect(Math.max(...Array.from({ length: LEVELS }, (_, i) => size(i + 1)))).toBe(11);
