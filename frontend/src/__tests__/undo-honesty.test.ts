...
describe('анаграммы: отмена возвращает набранное слово', () => {
   ...
   *
   * ⚠️ ИЩЕМ ПО ВСЕМ ВХОЖДЕНИЯМ, А НЕ ПО ПЕРВОМУ. 06.09.2026 у анаграмм появился
   * режим «Все слова», и ключ `clear` встречается в файле дважды: как подпись
   * кнопки нового режима и как кнопка черновика в классике. Проба брала первое
   * вхождение и падала на подписи, хотя правило про КЛАССИКУ не нарушено. Лента
   * отмены есть только у классики — там снимок и требуется.
   */
  it('«Сброс» откатывается наравне с буквой', () => {
    const места: number[] = [];
    for (let i = s.indexOf("t('clear')"); i >= 0; i = s.indexOf("t('clear')", i + 1)) места.push(i);
    expect(места.length).toBeGreaterThan(0);
    const годные = места
      .map((at) => (s.slice(Math.max(0, at - 300), at).match(/onPress=\{(\w+)\}(?![\s\S]*onPress=)/) || [])[1] || '')
      .filter((имя) => имя && /\.push\(/.test(fnBody(s, имя)));
    // Хотя бы одна кнопка «Сброс» позвана на ИМЕНОВАННУЮ функцию, кладущую снимок.
    expect(`кнопок ${места.length}, со снимком ${годные.length}`).toBe(`кнопок ${места.length}, со снимком ${годные.length}`);
    expect(годные.length).toBeGreaterThan(0);
  });
});
