 ...
 *
 * Фрактальная судоку — самая длинная партия в приложении («событие на несколько
 * часов», см. шапку экрана): десять сеток 9×9, решаемых снизу вверх. Ход здесь
 * обратим ровно как в обычной судоку, доска открыта целиком, нового знания
 * возврат не даёт — то есть все доводы за отмену действуют, и с двойной силой.
 *
 * Почему не сделано здесь: экран числился «уже сделанным» в задании и был помечен
 * как чужой (⛔️ не трогать). Замер 19.08.2026 показал обратное — `useMoveHistory`
 * в нём не упоминается ни разу. Правку должен внести тот, за кем файл закреплён.
 *
 * Эта строчка не даёт долгу потеряться: как только отмена появится, проверка
 * «долг протух» покраснеет и попросит перенести файл в WITH_UNDO.
 */
const DEBT: Record<string, string> = {
  'sudoku-fractal.tsx': 'ход обратим и партия длиннее всех; файл закреплён за другим агентом',
};
...
describe('цена отмены назначена по смыслу игры', () => {
  ...
  it('потраченный бюджет переживает выход из партии', () => {
    expect(m).toMatch(/undosUsed,?\s*\}\)/);           // уезжает в снимок партии
