...
// ⚠️ ЧЕРЕЗ `исходникЭкрана`: «шарики» и «гайки» — тонкие маршруты к экрану
// переливалки, и общий хук отмены лежит там. Читать сам маршрут значило бы
// требовать копию экрана ради зелени гейта.
const src = (f: string) => исходникЭкрана(f.replace(/\.tsx$/, ''));
...
const WITH_UNDO: Record<string, string> = {
  ...
  'cake-sort.tsx': 'стол на виду целиком, скрытого нет — перебором «сделал, посмотрел, откатил» ничего не разведаешь; отмена возвращает снимок и СНИМАЕТ ход со счётчика, поэтому перебор не бесплатен',
  // Тот же экран, что у тортов (тонкий маршрут `CakeSortScreen`), значит и отмена та же.
  'pizza-sort.tsx': 'тот же экран и та же отмена, что у тортов: стол на виду целиком, отмена возвращает снимок и снимает ход со счётчика',
  ...
  // Тот же экран, что у переливалки (тонкий маршрут), значит и отмена та же.
  'ball-sort.tsx': 'тот же экран и та же отмена, что у переливалки: перелив обратим, счётчик ходов при отмене не уменьшается',
  'nut-sort.tsx': 'тот же экран и та же отмена, что у переливалки: перелив обратим, счётчик ходов при отмене не уменьшается',
...
const WITHOUT_UNDO: Record<string, string> = {
  'puzzles.tsx': 'already: отмена ЕСТЬ, но общий хук ей не подходит — историю ходов держит движок Тэтхэма, а не наше состояние — кнопка в меню паузы зовёт psy_undo → midend_process_key(u). Свести к общему хуку значило бы вести вторую историю рядом с чужой',
  ...
  'number-run.tsx': 'timing: забег непрерывный, 12 этапов подряд без остановок; подобранный блок остаётся позади в ту же секунду, и отменять на ходу нечего — возврат только перезапуском всего забега',
  ...
  'spatial-lab.tsx': 'already: отмена, возврат и повтор хода есть — но своей лентой внутри пространственного ядра (`undo`/`redo`/`replay` в `spatial-core/core.mjs`), а не общим `useMoveHistory`. Причина в том, что ход здесь — поворот плитки или циклический сдвиг, и лента хранит не снимок доски, а последовательность поворотов: по ней же работает «Повтор», проигрывающий партию с начала. Общий хук снимков этого не умеет',
