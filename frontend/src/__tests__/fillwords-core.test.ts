...
describe('филворды: где нет слов — там нет режима', () => {
  ...
  it('🔴 пригодность языка считается по словарю, а не объявлена списком', () => {
    ...
    const причины = (l: string): string[] => {
      ...
      /*
        ⚠️ ОПОРНЫЕ ДЛИНЫ СЧИТАЮТСЯ ОТ ПОЛА ЯЗЫКА, А НЕ ОТ ТРОЙКИ ДЛЯ ВСЕХ.
        Здесь стояло жёсткое [3,4,5], и после того как пол стал свойством языка,
        проба обвинила fr и it в том, что у них «длины 3 меньше восьми» — а у них
        трёхбуквенных нет ПО ЗАМЕРУ СБОРЩИКА (МИН_ДЛИНА = 4: при пороге 3
        девятнадцать процентов слов оказываются мусором вроде ana, sin, abc).
        Проба обязана спрашивать то же, что и предикат: три длины подряд начиная
        с самой короткой, которая у языка есть.
      */
      const пол = полДлиныЯзыка(l);
      for (let i = 0; i < 3; i++) {
        const len = пол + i;
