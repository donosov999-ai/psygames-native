...
describe('🔴 прогон настоящей партии: предложенное выполнимо, невыполнимое не предложено', () => {
      ...
      while (tiles.length > 0 && страж < 400) {
        ...
        for (const e of выходы as MahjongExit[]) {
          if (e === 'shuffle') {
            ...
            if (dealSolvable(positionsOf(tiles), SYMBOLS.length, 200, r).tiles.length === 0) {
   ...
   * этот файл и получил репутацию шаткого — и до 06.09.2026 зерно у него было
   * фиксировано ЛИШЬ НАПОЛОВИНУ.
   *
   * В `beforeAll` тот же прогон накрыт потолком и падает с адресом. Потолок
   * поднят ниже — по замеру, а не на глаз.
   *
   * 🔴 И ВТОРАЯ ПОЛОВИНА ТОЙ ЖЕ ШАТКОСТИ, найдена 06.09.2026. Ходы партии шли
   * от `rng(s * 7919 + L)`, а ПЕРЕТАСОВКА — от глобального `Math.random`:
   * четвёртый параметр `dealSolvable` по умолчанию именно он
   * (`vendor/solvable.ts:136`). То есть файл называл себя воспроизводимым,
   * будучи воспроизводимым наполовину. Замер: шесть повторов ОДНОГО прогона
   * дали L40 43/33/20/23/30/23 % вставших насмерть; с сеятелем — 23 % во всех
   * шести. Пороги здесь стоят «≥ 1», поэтому красным это не всплывало —
   * шаткость была, а сигнала не было. Теперь `r` подаётся во все три вызова.
   */
  let итоги: Итог[] = [];
...
// ═════════════════════════════════════════════════════════════════════════════
/**
 * 🔴 ВСТАВШАЯ ДОСКА — ЭТО ПРОИГРЫШ, И ОН ЗАПИСЫВАЕТСЯ.
 *
 * До 06.09.2026 партия писалась ровно одним способом — `passed: true` при
 * сборе уровня. Проигрыш существовал (кнопка «начать заново» появляется ТОЛЬКО
 * когда доска встала, а перетасовать и отменить нечем), но не доходил ни до
 * статистики, ни до карты исходов.
 *
 * Проба читает ИСХОДНИК экрана — и это тот редкий случай, когда так и надо:
 * проверяется не поведение отрисовки, а то, что в единственной ветке выхода из
 * тупика стоит запись исхода. Отрисовать её нельзя: `saveSession` уходит в
 * сеть, а ветка достигается только на мёртвой доске с исчерпанными бюджетами.
 */
describe('исход вставшей доски', () => {
  const экран = (require('fs').readFileSync(
    require('path').join(__dirname, '../../app/games/mahjong.tsx'), 'utf8',
  ) as string);

  /** Тело `restartLevel` — единственной ветки выхода из тупика. */
  const пересдача = (): string => {
    const i = экран.indexOf('const restartLevel = () => {');
    expect(`ветка выхода найдена: ${i >= 0}`).toBe('ветка выхода найдена: true');
    /**
     * ⚠️ ГРАНИЦА ТЕЛА, А НЕ «ДВЕ С ПОЛОВИНОЙ ТЫСЯЧИ ЗНАКОВ». Первая редакция
     * брала срез фиксированной длины и захватывала СОСЕДНЮЮ функцию — проба
     * покраснела на `lvl.fail()`, которого в выходе из тупика нет. Меряем
     * ровно тело: от объявления до строки `  };` на нулевом отступе.
     */
    const конец = экран.indexOf('\n  };', i);
    expect(`тело ограничено: ${конец > i}`).toBe('тело ограничено: true');
    /**
     * 🔴 КОММЕНТАРИИ ВЫРЕЗАЮТСЯ ДО ПОИСКА. Без этого проба покраснела на моём же
     * пояснении «`lvl.fail()` здесь НЕ зовётся» — то есть обвинила текст,
     * объясняющий её собственное правило. Это ВТОРОЙ такой случай за 06.09.2026
     * (первый — гейт ширины экрана): признак класса, а не случайность.
     */
    return экран.slice(i, конец)
      .replace(/\/\*[\s\S]*?\*\//g, ' ')
      .replace(/(^|[^:])\/\/[^\n]*/g, '$1');
  };

  it('🔴 выход из тупика пишет партию с passed: false', () => {
    const тело = пересдача();
    expect(`saveSession в выходе: ${/saveSession\(/.test(тело)}`).toBe('saveSession в выходе: true');
    expect(`исход записан проигрышем: ${/passed:\s*false/.test(тело)}`)
      .toBe('исход записан проигрышем: true');
  });

  it('🔴 в записи виден ПРИЗНАК тупика — иначе проигрыш неотличим от обычного', () => {
    expect(`пометка тупика: ${/stuck:\s*true/.test(пересдача())}`).toBe('пометка тупика: true');
  });

  /**
   * 🔴 РЕШЕНИЕ ПЕРЕМЕНИЛОСЬ 11.09.2026 — И ПРОБА СКАЗАЛА ОБ ЭТОМ ПЕРВОЙ.
   *
   * Прежняя редакция требовала, чтобы `lvl.fail()` здесь НЕ вызывался, и писала:
   * «если однажды он появится, проба обязана об этом сказать, а не промолчать».
   * Он появился — по решению Дениса лестница маджонга теперь опускается, — и
   * проба честно покраснела. Требование перевёрнуто, а не снято: теперь красным
   * будет ОТСУТСТВИЕ понижения.
   *
   * Почему понижать вообще: доска встаёт насмерть у 60 % раздач на 28-м уровне и
   * у 53 % на 40-м (замер 06.09.2026, 30 партий на уровень). Без понижения
   * лестница односторонняя — дошедший до 28-го остаётся там навсегда.
   * Почему с пятого, а не с третьего: на такой доле тупиков три подряд — обычное
   * невезение. Порог задан третьим доводом `usePersistentLevel('mahjong', 1, 5)`.
   *
   * ⚠️ И понижение обязано быть ОГОВОРЕНО зарядкой: шаг плейлиста уровень не
   * двигает ни вверх, ни вниз — это общее правило, его сторожит `warmup-level-drift`.
   */
  it('🔴 лестница ПОНИЖАЕТСЯ — но только в личной партии, не в зарядке', () => {
    const код = пересдача().replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/.*$/gm, '');
    expect(`понижение есть: ${/lvl\.fail\(\)/.test(код)} · оговорено зарядкой: ${/if \(!isPreset\) lvl\.fail\(\)/.test(код)}`)
      .toBe('понижение есть: true · оговорено зарядкой: true');
  });
});
