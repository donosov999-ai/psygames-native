...
describe('исход вставшей доски', () => {
  ...
  /**
   * ⚠️ ОБРАТНАЯ СТОРОНА: лестницу пересдача НЕ понижает. Это решение Дениса
   * (задача 8543237a), а не следствие замера — и если однажды `lvl.fail()`
   * здесь появится, проба обязана об этом сказать, а не промолчать.
   */
  it('⚠️ лестница НЕ понижается — пока так решено', () => {
    expect(`понижение в выходе: ${/lvl\.fail\(\)/.test(пересдача())}`)
      .toBe('понижение в выходе: false');
  });
});
