...
describe('тихий вечер у каждой игры вечернего набора', () => {
  ...
  it('🔴 судоку прячет секундомер на спокойном шаге', () => {
    ...
    /**
     * ⚠️ ИЩЕМ СЧЁТЧИК ВРЕМЕНИ, А НЕ СПОСОБ ЕГО ФОРМАТИРОВАНИЯ. Здесь стояло
     * `flat.indexOf('elapsedTime.toFixed(1)')`. 03.09.2026 время в шапке перевели
     * на общий `hudTime` (шесть знаков «295.3с» не помещались среди шести
     * счётчиков), и гейт покраснел — хотя правило «на спокойном шаге секундомера
     * нет» соблюдено ровно как было.
     *
     * Признак — сама запись счётчика в шапке (`key: 'time'`), она от способа
     * форматирования не зависит.
     */
    const at = flat.indexOf("key: 'time'");
