...
import { медианаМс, starsFor, итогПодхода } from '@/src/games/scholars-mate/core/run';
...
describe('«Детский мат»: отработка одного узора', () => {
  ...
  /**
   * 🔴 ГЕЙТ ПРОВЕРЯЕТ ОБА ПУТИ НАБОРА, А НЕ ОДИН.
   *
   * 📍 Правило «весь подход за одну сторону» стояло в лестнице и отсутствовало
   * в отработке узора. Отчёт Дениса 05.09.2026 со скриншотом «Мата Бодена»:
   * доска белыми вниз, а ходят чёрные — «пытаешься ходить за белых, теряешь
   * время». Проба на лестнице была зелёной: свойство проверялось там, где я его
   * написал, и только там.
   */
  it('🔴 в отработке узора подход тоже идёт за ОДНУ сторону', () => {
    const смешанные: string[] = [];
    for (const имя of NAMED_MOTIFS) {
      for (const seed of [1, 2, 3, 4]) {
        const стороны = new Set(buildNamedDeck(имя, 12, seed).map((p) => sideToMove(p)));
        if (стороны.size > 1) смешанные.push(`${имя}/${seed}`);
      }
    }
    expect(смешанные.slice(0, 5)).toEqual([]);
  });

  it('🔴 и обе стороны в отработке встречаются — иначе за чёрных не поиграть', () => {
    const стороны = new Set(
      NAMED_MOTIFS.flatMap((имя) => [1, 2, 3, 4, 5].map((s) => buildNamedDeck(имя, 12, s)[0]))
        .filter(Boolean).map((p) => sideToMove(p!)),
    );
    expect([...стороны].sort()).toEqual(['b', 'w']);
  });
...
/**
 * 🔴 КОНЕЦ ПОДХОДА. Отчёт Дениса 05.09.2026: «партию прошёл, завершилось всё и
 * всё висит, где следующий уровень» — подход в режиме упирался в карточку без
 * ступени и без продолжения.
 */
describe('итог подхода', () => {
  const о = { касались: true, взял: true, поток: false, предустановка: false, цепочка: true };

  test('подход без касаний не считается: назад в настройку, ступень стоит', () => {
    expect(итогПодхода({ ...о, касались: false, взял: false })).toEqual({ фаза: 'config', ступень: 'стоит' });
  });

  test('обычный подход взят — ступень вверх и экран уровня', () => {
    expect(итогПодхода(о)).toEqual({ фаза: 'cleared', ступень: 'вверх' });
  });

  test('обычный подход не взят — ступень вниз, но экран уровня всё равно есть', () => {
    expect(итогПодхода({ ...о, взял: false })).toEqual({ фаза: 'cleared', ступень: 'вниз' });
  });

  test('🔴 отработка узора и жертвы идёт ПО ЛЕСТНИЦЕ, а не в тупик', () => {
    // Отработка отличается от лестницы только пулом, поэтому доводов о ней тут
    // и нет: с точки зрения итога это ровно тот же подход.
    expect(итогПодхода(о).фаза).toBe('cleared');
    expect(итогПодхода(о).ступень).toBe('вверх');
  });

  test('поток — единственный без ступеней', () => {
    expect(итогПодхода({ ...о, поток: true })).toEqual({ фаза: 'result', ступень: 'стоит' });
    expect(итогПодхода({ ...о, поток: true, взял: false })).toEqual({ фаза: 'result', ступень: 'стоит' });
  });

  test('заданный извне уровень лестницу не двигает', () => {
    expect(итогПодхода({ ...о, предустановка: true })).toEqual({ фаза: 'result', ступень: 'стоит' });
  });

  test('игра без цепочки уровней вверх не идёт, но экран уровня показывает', () => {
    expect(итогПодхода({ ...о, цепочка: false })).toEqual({ фаза: 'cleared', ступень: 'стоит' });
  });
});
