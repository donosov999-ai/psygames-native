...
import { медианаМс, starsFor } from '@/src/games/scholars-mate/core/run';
...
/**
 * 🔴 ЗВЁЗДЫ РАЗЛИЧАЮТ ПОДХОДЫ, А НЕ ХВАЛЯТ ВСЕХ.
 *
 * 📍 ЗАМЕР НА ЖИВЫХ ПОДХОДАХ 05.09.2026 (15 сыгранных, `cognitive_sessions`):
 * прежнее правило «медиана ≤ 33% отведённого времени» давало ТРИ звезды в 14
 * случаях из 15. Причина арифметическая: доля считалась от окна ответа
 * (20…4 с), а медиана меряет время до первого касания — 1,3…2,9 с. На нижних
 * уровнях 33% от двадцати секунд это 6,6 с, туда попадает кто угодно.
 *
 * Проба гоняет правило по ТЕМ ЖЕ пятнадцати подходам.
 */
describe('«Детский мат»: звёзды', () => {
  /** Живые подходы: [уровень, медиана мс]. Взяты из базы 05.09.2026. */
  const ЖИВЫЕ: [number, number][] = [
    [4, 1532], [7, 1490], [3, 2239], [8, 1442], [10, 2045], [1, 2741], [12, 2855],
    [13, 2441], [1, 5144], [2, 3253], [1, 12538], [11, 1922], [6, 1290], [5, 1393], [9, 1851],
  ];

  it('🔴 на живых подходах звёзды РАЗНЫЕ, а не три у всех', () => {
    const счёт = { 1: 0, 2: 0, 3: 0 } as Record<number, number>;
    for (const [L, м] of ЖИВЫЕ) счёт[starsFor(м, L)]! += 1;
    // Ни одна оценка не забирает больше половины подходов.
    for (const з of [1, 2, 3]) {
      expect(`${з}★: ${счёт[з]! <= ЖИВЫЕ.length / 2}`).toBe(`${з}★: true`);
    }
    // И все три встречаются.
    expect(Object.values(счёт).every((n) => n > 0)).toBe(true);
  });

  it('🔴 порог растёт с уровнем — наверху позиции труднее', () => {
    // Одна и та же медиана наверху заслуживает большего.
    expect(starsFor(2100, 1)).toBeLessThan(starsFor(2100, 40));
    // Но не до бесконечности: очень медленный ответ — одна звезда на любом уровне.
    expect(starsFor(9000, 40)).toBe(1);
  });

  it('🔴 быстрый ответ даёт три звезды на любом уровне', () => {
    for (const L of [1, 20, 40]) expect(starsFor(900, L)).toBe(3);
  });

  it('нет медианы — нет звёзд сверх одной', () => {
