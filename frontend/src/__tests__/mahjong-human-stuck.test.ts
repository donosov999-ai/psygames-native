 ...
 *
 * 🚫 ЧЕТВЁРТОГО, «ИДЕАЛЬНОГО», ЗДЕСЬ НЕТ — И ЭТО ОСОЗНАННО, А НЕ ЗАБЫТО.
 * Сначала он был написан (полный перебор с памятью) и должен был отвечать на
 * вопрос «а доска вообще была разбираема отсюда». Убран по замеру: на полном
 * наборе L40 это 144 плитки, ветвление под сорок ходов, и бюджет узлов
 * упирается в потолок раньше, чем даёт ответ, — то есть вернул бы `null`
 * («не знаю») там, где он и нужен. Отвечать «не знаю» дорогим перебором смысла
 * нет. Что ПЕРЕНОСИТСЯ из отклонённого: сам вопрос остаётся открытым, и мерить
 * его надо не перебором, а встречно — от конца партии назад.
 */
