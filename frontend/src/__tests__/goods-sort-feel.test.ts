...
describe('ответ на действие', () => {
   ...
   * есть ветка «мгновенно».
   *
   * 🔴 ЧТО ЗДЕСЬ БЫЛО И ПОЧЕМУ УБРАНО (09.09.2026). Проверка требовала в
   * `shakeNiche` БУКВАЛЬНОЙ строки `if (reduced) return;` — то есть не «есть ли
   * ветка», а «написана ли она этими знаками». Когда у отказа в щадящем режиме
   * появился свой НЕПОДВИЖНЫЙ сигнал (алая рамка вместо дрожания), проверка
   * покраснела на ПРАВИЛЬНОЙ правке и потребовала вернуть молчание. Ровно то, от
   * чего предостерегает шапка этого файла двумя абзацами выше.
   *
   * Требование «в щадящем нет движения, но сообщение остаётся» держит теперь
   * `goods-sort-refusal-is-visible`: он монтирует экран, делает запрещённый ход и
   * смотрит на нишу по метке `niche-refused` — в обычном режиме сдвиг, в щадящем
   * рамка, и наоборот ни в коем случае. Мутации в обе стороны красные.
   *
   * Здесь остаётся то, что чтением исходника проверяется честно: щадящий режим
   * вообще спрашивается, и у вспышки ветка есть.
   */
  it('щадящий режим спрашивается, и у вспышки есть своя ветка', () => {
...
describe('полёт товара', () => {
  ...
  /*
   * 🔴 «НАСТОЯЩИЙ ТОВАР СПРЯТАН, ПОКА ЛЕТИТ КОПИЯ» ПЕРЕЕХАЛО НА ИСПОЛНЕНИЕ —
   * `goods-sort-taps-and-fly`.
   *
   * Здесь сверялся текст: `const arriving = fly?.toCell === i && s === cell.length - 1`.
   * 07.09.2026 проба покраснела от переноса стопки товаров в запоминающий
   * компонент при полностью неизменном поведении — второй такой случай за сутки.
   * Теперь ход делается по-настоящему, и меряется ПРОЗРАЧНОСТЬ прилетевшего
   * товара: то, что видит человек, а не то, как оно сегодня записано.
   */
