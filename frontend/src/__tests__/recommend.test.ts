...
describe('словарь и разметка', () => {
   ...
   */
  it('🔴 условие показа блока зависит только от данных и от выбора владельца', () => {
    /**
     * 🔴 13.09.2026: К «есть ли что показывать» ДОБАВЛЕН ТОЛЬКО ВЫБОР ВЛАДЕЛЬЦА.
     *
     * Смысл пробы прежний: блок рекомендаций не имеет права зависеть от посторонних
     * условий — ни от часа, ни от профиля, ни от случайности. Единственное, что
     * добавилось, — редактор состава главной, где владелец убирает блок осознанно
     * (просьба Дениса 13.09.2026). Это решение человека, а не стечение данных.
     *
     * Допуск поэтому не «любое условие», а ДВЕ точные строки. Третья форма снова
     * красная — иначе проба перестала бы стеречь то, ради чего написана.
     */
    const home = read('app/index.tsx');
    const m = /\{([^{}]*?)&&\s*\(\s*\n\s*<View style=\{styles\.recoBlock\}/.exec(home);
    const условие = m ? m[1].trim() : 'блок с styles.recoBlock не найден в разметке';
    const ДОПУСТИМЫЕ = [
      'reco.length > 0',
      "reco.length > 0 && показыватьБлок('рекомендации', блокиГлавной)",
    ];
    expect(ДОПУСТИМЫЕ).toContain(условие);
  });
