...
describe('наборы «найди все слова»', () => {
  it('есть все десять языков, и ни один не пустой', () => {
    /**
     ...
     */
    expect(allWordsLocales()).toEqual(['ar', 'de', 'en', 'es', 'fr', 'it', 'ja', 'ko', 'pt', 'ru']);
  ...
  it('🔴 язык без своего набора получает английский, а не пустоту', () => {
    // ko, ar и ja с 06.09.2026 имеют свои наборы, поэтому здесь их больше нет
    for (const loc of ['zh', 'hi']) {
  ...
  it('база длиной 6–8, целей 6…22, дублей нет, цель ≠ база', () => {
    const плохо: string[] = [];
    for (const loc of allWordsLocales()) {
      for (let i = 1; i <= allWordsCount(loc); i++) {
        const p = allWordsPack(loc, i)!;
        const n = [...p.base].length;
        // ⚠️ НИЖНЯЯ ГРАНИЦА ШЕСТЬ, А НЕ СЕМЬ. У японского плитка — знак каны,
        // то есть слог: шесть знаков там по «объёму слова» примерно как восемь
        // латинских букв. Требовать семи значило бы выкинуть язык целиком.
        if (n < 6 || n > 8) плохо.push(`${loc} «${p.base}»: база ${n} знаков`);
