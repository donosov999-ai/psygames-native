...
it('классика показывает латинские языки и не показывает ko/ja/ar', async () => {
  ...
  // ⚠️ Снять обязательно, см. пояснение у второй пробы.
...
it('в партии на португальском плитки складываются в португальское слово', async () => {
  ...
  for (let i = 0; i < ПАРТИЙ; i++) {
    ...
    /*
     * 🔴 СНЯТЬ ЭКРАН ОБЯЗАТЕЛЬНО. Без этого десять смонтированных партий
     * остаются живыми до конца файла, и после сноса окружения jest их
     * отрисовка продолжается: `useWindowDimensions` бьёт по мёртвому `window`,
     * падает «window.dispatchEvent is not a function» — УЖЕ ПОСЛЕ того, как
     * все семь проб позеленели. 07.09.2026 это остановило выпуск 2.49.0, а под
     * нагрузкой (несколько прогонов разом) выглядело вовсе как тайм-аут 20 с,
