...
const ОБЪЯСНЕНО: Record<string, Ступень> = {
  ...
  'cpt.target': { на: [9, 12, 15], зачем: 'правило `newtarget` с L9: мишень перестаёт быть X и меняется блоками по три уровня (K → T → H). Карточка называет саму смену, а текущую букву — подсказка над полем' },
  'cpt.colorRule': { на: [13], зачем: 'правило `colorrule` с L13: засчитывается только КРАСНАЯ мишень, та же буква другого цвета — ловушка' },
};
...
const ДОЛГ: Record<string, Ступень> = {
  ...
  // Добавлено 07.09.2026 координатором: раздел «Объём памяти» ввёл ось 9, но
  // правила не завёл, и main встал красным на выпуске 2.49.0.
  'digit-span.surpriseDir': { на: [15], зачем: 'с L15 направление ответа (прямое/обратное) объявляется ПОСЛЕ показа ряда — игрок держит ряд, ещё не зная, как его спросят. В DS_RULES только reverse с L11, про эту механику не сказано нигде. Чинит владелец: `{ key: "surprise_dir", fromLevel: DS_VOLUME_TOP + 1 }` в DS_RULES (app/games/digit-span.tsx:41) плюс ключи lr_digit_span_surprise_dir_{title,rule,example} в 13 языках src/contexts/translations/' },
};
