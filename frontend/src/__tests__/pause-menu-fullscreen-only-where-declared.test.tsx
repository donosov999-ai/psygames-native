/**
 * ПУНКТ «ПОЛНОЭКРАННЫЙ РЕЖИМ» В МЕНЮ ПАУЗЫ — ТОЛЬКО У ИГРЫ, КОТОРАЯ ЕГО ОБЪЯВИЛА.
 *
 * Меню паузы общее, его собирает каркас для всех игр. Переключатель полос телефона
 * имеет смысл только там, где партия идёт на весь экран (`useImmersive`, сейчас —
 * «Числовой забег»). Остальным он показывал бы кнопку, от которой ничего не меняется,
 * — а кнопка, которая делает вид, хуже отсутствующей.
 *
...
function открытьМеню(): TestRenderer.ReactTestRenderer {
  let tr!: TestRenderer.ReactTestRenderer;
  act(() => {
    tr = TestRenderer.create(
      <GameShell title="Числовой забег" onBack={() => {}}>
        <Text>поле</Text>
      </GameShell>,
    );
  });
...
describe('меню паузы: полноэкранный режим', () => {
  it('🔴 игра без полноэкранного режима — пункта нет', () => {
    const tr = открытьМеню();
    expect(`пункт: ${подписьПункта(tr)}`).toBe('пункт: —');
  });

  it('🔴 игра объявила режим — пункт есть и выключает его', () => {
    const снять = declareImmersiveCapable();
    try {
      const tr = открытьМеню();
  ...
  it('игра объявила режим уже при открытом меню — пункт появляется без переоткрытия', () => {
    const tr = открытьМеню();
    let снять!: () => void;
