...
it('🔴 ряд служебных — один, ПРИБИТ сразу под полем (не в конце содержимого); над полем и в полосе счётчиков пусто', async () => {
  for (const пропсы of [{}, { auxInHud: true }, { bottom: 'actions' }, { auxInHud: true, bottom: 'actions' }]) {
    ...
    /*
     * ⚠️ ПОЛОСЫ СЧЁТЧИКОВ МОЖЕТ НЕ БЫТЬ ВОВСЕ (23.09.2026): каркас перестал её
     * рисовать, когда показывать нечего — она съедала 79 px у «Паузы». Прежняя
     * запись брала `[0]!` и падала на восклицательном знаке, а не на смысле.
     * Смысл же прежний и от этого только крепче: ряда служебных в полосе нет.
     */
    const полоса = якоря(r.root, 'game-hud')[0];
    expect(полоса ? якоря(полоса, 'game-aux').length : 0).toBe(0);
