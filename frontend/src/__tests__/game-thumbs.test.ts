...
const WITHOUT_THUMB: Record<string, string> = {
  ...
  ball_sort: 'шкурка переливалки: тот же экран, шарики вместо жидкости — превью повторяло бы её',
  nut_sort: 'шкурка переливалки: тот же экран, гайки на стержне — превью повторяло бы её',

  spatial_group: 'развилка: меню трёх проб на вращение в уме — фигуры, числа, трубы',
  sorting_group: 'развилка: меню проб на раскладывание по ёмкостям, картинка обещала бы конкретную игру',
  ...
  inhibition_group: 'развилка: меню двух проб на торможение готового движения',
  chess_group: 'развилка: меню двух шахматных упражнений — доска в уме и детский мат',
  routes_group: 'развилка: меню двух головоломок на покрытие пути',
  puzzles_group: 'развилка: меню СОРОКА головоломок Тэтхэма, картинка обещала бы одну из них',
  puzzles:
    'экран-носитель сорока режимов: что рисовать, зависит от `?mode=` — одно превью на сорок разных досок обмануло бы в тридцати девяти случаях. В каталоге карточки нет вовсе (hideFromMenu), вход только через развилку',
  flexibility_group: 'развилка: меню трёх проб на переключение признака',
  pause:
  ...
  water_sort:
    'заведена 05.09.2026, превью ещё не рисовали; карточка живёт внутри развилки «Башни», а не в общей сетке',
  'sudoku-samurai':
