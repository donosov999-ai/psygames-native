...
describe('публичный репозиторий: инфраструктура наружу не течёт', () => {
  ...
  /**
   * 🔴 СИМЛИНК, ЗАКОММИЧЕННЫЙ В ПУБЛИЧНЫЙ РЕПОЗИТОРИЙ, — ЭТО УТЕЧКА ПУТИ.
   *
   * 📍 Случилось 07.09.2026, и виноват я, координатор. При слиянии подмёл
   * `git add -A -- frontend`, и в main уехала ссылка `frontend/node_modules` на
   * моё ВРЕМЕННОЕ дерево (`/private/tmp/claude-501/…/scratchpad/wt-v2/…`).
   * Содержимое симлинка — это его цель, то есть абсолютный путь на моей машине,
   * видимый всем. И тому, кто склонирует репозиторий, ссылка указывает в никуда:
   * `npm ci` ставит зависимости через висячую ссылку.
   *
   * ⚠️ ПОЧЕМУ .gitignore НЕ СПАС. Там стояло `node_modules/` — СО СЛЕШЕМ, а слеш
   * означает «каталог». Симлинк с тем же именем каталогом не является и под
   * правило не попадал. Починено строкой без слеша; этот гейт сторожит, что
   * дыра не откроется снова другим именем.
   *
   * Проверяется ИНДЕКС git, а не рабочее дерево: в дереве ссылка нужна и живёт
  ...
  it('🔴 в индексе нет ни одного симлинка — путь на чужой машине наружу не течёт', () => {
    ...
    expect(`симлинков в индексе: ${ссылки.length} → ${ссылки.join(', ')}`)
      .toBe('симлинков в индексе: 0 → ');
  });
  ...
  it('🔴 node_modules игнорируется И как каталог, И как ссылка', () => {
    ...
    /*
     * ⚠️ ПУТЬ БЕРЁТСЯ ТАМ, ГДЕ СИМЛИНКА НЕТ. Первая редакция спрашивала про
     * `frontend/node_modules/foo` и получала «не игнорируется» — не потому, что
     * правило не работает, а потому что `check-ignore` не заходит ВНУТРЬ
     * симлинка, который там лежит. Мерили помеху, а не правило.
     */
    expect(`каталог: ${проверить('frontend/src/node_modules/foo')}`).toBe('каталог: true');
    expect(`ссылка: ${проверить('frontend/node_modules')}`).toBe('ссылка: true');
  });
});
