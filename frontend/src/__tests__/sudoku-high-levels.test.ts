...
describe('доводка доски до потолка', () => {
  ...
  /**
   * 🔴 ЗЕРНО ПРИШИТО НА ВЕСЬ НАБОР. Доски здесь идут через `Math.random`, и до
   * 03.09.2026 набор был жребием: та же правка то зелёная, то красная. Красный по
   * жребию стоил выпуска v2.37.1. Генератор при этом не подменён — подменён только
   * источник случайности, то есть проверяется настоящий код на воспроизводимом входе.
   */
  const БЫЛ_RANDOM = Math.random;
  beforeAll(() => {
    let s = 123456789;
    Math.random = () => { s = (s * 1664525 + 1013904223) >>> 0; return s / 4294967296; };
  });
  afterAll(() => { Math.random = БЫЛ_RANDOM; });
   ...
   */
  /**
   * 🔴 ПОРОГ СТОИТ ПО ЗАМЕРУ, А НЕ ПО ДОГАДКЕ, И ВЫБОРКА БОЛЬШЕ ОДНОЙ ДОСКИ.
   *
   * Прежняя редакция брала три случайные доски и требовала «долито ≤ 15» на
   * КАЖДОЙ. 03.09.2026 она уронила выпуск v2.37.1: попалась доска с шестнадцатью.
   * Дефекта не было — был слишком тугой порог на случайном входе, то есть гейт,
   * краснеющий по жребию. Такой хуже отсутствия: он тратит выпуск и приучает
   * смотреть на красное как на шум.
   *
   * Замер 03.09.2026, 120 досок на пришитом зерне (`generatePuzzle(52,…)`,
   * настоящий путь приложения): долито min 1 · медиана 3 · 90% 7 · 99% 12 · max 17.
   * Отсюда и проверяемое утверждение: типичная доливка — ЕДИНИЦЫ клеток
   * (медиана ≤ 8), и ни одна доска не «заливается» (≤ 25 из 52 дырок, то есть
   * меньше половины). Оба числа с запасом к замеру и оба про смысл, а не про
   * конкретную доску.
   */
  it('🔴 на настоящем входе доливает единицы клеток, а не заливает доску', () => {
    const долито: number[] = [];
    for (let attempt = 0; attempt < 60 && долито.length < 12; attempt++) {
      const base = generatePuzzle(52, d.N, d.BR, d.BC, 'none');
      const g = gradePuzzle(base.puzzle, { N: 9, BR: 3, BC: 3, variant: 'none' });
      ...
      долито.push(before - now);
      expect(`доска осталась задачей: ${now >= 20}`).toBe('доска осталась задачей: true');
    }
    expect(`подходящих досок нашлось: ${долито.length > 0}`).toBe('подходящих досок нашлось: true');
    const по = [...долито].sort((a, b) => a - b);
    const медиана = по[Math.floor(по.length / 2)];
    const худшая = по[по.length - 1];
    expect(`на ${по.length} досках медиана ${медиана} ≤ 8 → ${медиана <= 8}`)
      .toBe(`на ${по.length} досках медиана ${медиана} ≤ 8 → true`);
    expect(`худшая доливка ${худшая} ≤ 25 → ${худшая <= 25}`)
      .toBe(`худшая доливка ${худшая} ≤ 25 → true`);
  });
});
...
describe('рубеж стоит на пути', () => {
  ...
  it('🔴 запасной путь пропускает доску через доводку', () => {
     ...
     */
    /**
     * ⚠️ С 27.08 `gen` объявлен как `let`: после доводки его может ПЕРЕ-
     * присвоить снятие подсказок варианта (`liftByClueRemoval`, приём Тэтхэма
     * №2) — и это законно, потому что lift сам держит потолок `max` и работает
     * ПОСЛЕ `easeToCeiling`. Гейт по-прежнему требует: первичное присвоение —
     * только из доводки (`eased`/`поднято`), никогда из сырого `fb`.
     */
    const присвоение = /(?:const|let) gen = ([^;]+);/.exec(tail);
