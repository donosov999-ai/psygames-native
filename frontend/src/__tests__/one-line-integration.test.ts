...
describe('«Одна линия»: сложность растёт содержанием, а не таймером', () => {
  /**
   * ⚠️ ЧИСЛО УРОВНЕЙ ЗДЕСЬ НЕ ПРИБИВАЕТСЯ. Стояло `toBe(48)` — и когда рисованных
   * фигур стало сорок, проверка потребовала бы оставить генератору восемь уровней,
   * то есть запретила бы игре расти. Прибивать надо ОТНОШЕНИЕ: сгенерированных
   * уровней должно быть больше, чем рисованных, иначе лестница генератора
   * бессмысленна — точки прибавляются раз в три уровня.
   */
  it('уровней хватает и рисованным, и генератору, и число берётся из ядра', () => {
    expect(LEVELS).toBeGreaterThan(AUTHORED_LEVEL_COUNT);
    expect(`сгенерированных ${LEVELS - AUTHORED_LEVEL_COUNT} ≥ рисованных ${AUTHORED_LEVEL_COUNT}`)
      .toBe(`сгенерированных ${LEVELS - AUTHORED_LEVEL_COUNT} ≥ рисованных ${Math.min(AUTHORED_LEVEL_COUNT, LEVELS - AUTHORED_LEVEL_COUNT)}`);
  ...
  it('🔴 граф генератора растёт по уровням: вершины, рёбра, обманки-пересечения', () => {
    ...
    // ⚠️ ВСЕ ТРИ — ИЗ ГЕНЕРАТОРА. Раньше средний брался с 20-го уровня, а после
    // расширения набора рисованных фигур 20-й стал рисованным: сравнивались
    // генератор с рисунком, и «рост» мерил не то. Отступаем от конца набора.
    const small = at(AUTHORED_LEVEL_COUNT + 1);
    const mid = at(AUTHORED_LEVEL_COUNT + 16);
    const big = at(AUTHORED_LEVEL_COUNT + 36);
    /**
