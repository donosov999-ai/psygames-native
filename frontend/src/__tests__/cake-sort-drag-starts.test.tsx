 ...
 */
/**
 * 🔴 СТОЛБЦЫ СЧИТАЮТСЯ ОТ НАРИСОВАННОГО, А НЕ ПИШУТСЯ ТРОЙКОЙ.
 *
 * 📍 Здесь стояло `cols = 3` по умолчанию, и 11.09.2026 раскладка стола
 * перестала быть трёхстолбцовой: пол столбцов опущен до двух, и маленький стол
 * из пяти тарелок теперь 2×3 (тарелка 168 вместо 109, см. `tableFit`). Две
 * пробы этого набора покраснели на ИСПРАВНОМ коде — они целились в координаты
 * несуществующей сетки. Столбцы выводятся из ширины стола и диаметра тарелки,
 * то есть из того же, что видит игрок; тогда любая будущая раскладка не
 * потребует править эти пробы вовсе.
 */
function столбцов(d: number, boardW: number): number {
  if (d <= 0 || boardW <= 0) return 1;
  return Math.max(1, Math.round((boardW - PLATE_GAP) / (d + PLATE_GAP)));
}

function центрТарелки(i: number, d: number, boardW: number, всего: number, cols = столбцов(d, boardW)) {
