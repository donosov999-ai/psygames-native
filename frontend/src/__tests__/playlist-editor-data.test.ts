  ...
  const снимок = {
    ...
    хабы: Object.fromEntries(
      Object.entries(HUB_CONTENTS).map(([маршрут, карточки]) => [
        маршрут,
        /* Имя по-русски, а не только ключ словаря: в редакторе строки развилки
           читались как `goodsSort` и `suitePositions` — понять по ним, что это
           «Сортировка товаров» и «Матрица памяти», нельзя. Ключ остаётся рядом:
           им карточка описывается в файле. */
