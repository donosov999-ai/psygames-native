...
describe('гайки на болте', () => {
  ...
  /**
   * 🔴 ПОЛНЫЙ СТЕРЖЕНЬ ОБЯЗАН ВЫГЛЯДЕТЬ ПОЛНЫМ (задача 67f58742).
   *
   * 📍 Отчёт 17.09.2026 (кадр q61zy46d71.jpg, 430×932): «не даёт перетащить
   * нижнюю левую гайку на верхний ряд… и в другую сторону гайка не двигается».
   * Игра была ПРАВА — оба стержня полные, — но выглядели свободными: замер по
   * кадру дал над полным стержнем 81 px пустой резьбы, это 1,9 высоты гайки.
   * Причина: болт рисовался на всю высоту гнезда (`height: в * 0.94`), а не на
   * высоту ВМЕСТИМОСТИ, и обещал место, которого нет.
   *
   * ⚠️ МЕРЯЕМ НАРИСОВАННОЕ, а не формулу: высоту болта против высоты столба
   * гаек. Порог — треть гайки сверху: столько нужно, чтобы резьба читалась как
   * резьба, и мало, чтобы пообещать ещё одну гайку.
   */
  it('🔴 болт кончается над последней гайкой, а не тянется во всё гнездо', async () => {
    const r = await открыть('@/app/games/nut-sort');
    const ширинаГайки = плоско(гайки(r)[0].props.style).width as number;
    const высотаГайки = ширинаГайки / АСПЕКТ_ГАЙКИ;
    const НАХЛЁСТ = 0.42;
    const плохо: string[] = [];
    болты(r).forEach((b: any, i: number) => {
      const с = плоско(b.props.style);
      /* Вместимость на этом уровне одна на все сосуды: столб = гайка + (cap−1) шагов. */
      const шаг = высотаГайки * (1 - НАХЛЁСТ);
      for (const cap of [4]) {
        const столб = высотаГайки + (cap - 1) * шаг;
        const лишнее = (с.height as number) - столб;
        if (лишнее > высотаГайки * 0.5) {
          плохо.push(`болт ${i}: высота ${(с.height as number).toFixed(1)} при столбе ${столб.toFixed(1)} — `
            + `лишних ${лишнее.toFixed(1)} px, это ${(лишнее / высотаГайки).toFixed(1)} гайки`);
        }
      }
    });
    expect(плохо).toEqual([]);
   ...
   *
   * 🔴 ПОВОРОТ ИМЕННО ВОКРУГ ОСИ БОЛТА (`rotateY` с перспективой), А НЕ В
   * ПЛОСКОСТИ ЭКРАНА. Денис 09.09.2026: «анимация тоже неправильно, она
   * вращаться должна в горизонтальной плоскости… завинчиваются». Плоский
   * `rotate` крутит гайку как стрелку часов — это не навинчивание, а вертушка.
   * Без `perspective` тот же `rotateY` выглядит горизонтальным сжатием, поэтому
   * проверяются ОБА: и ось, и перспектива.
   *
    ...
    for (let a = 0; a < все.length && !сделан; a += 1) {
      for (let b = 0; b < все.length && !сделан; b += 1) {
        ...
        const крутятся = гайки(r).filter((n: any) => {
          ...
          return Array.isArray(t) && t.some((x: any) => x.rotateY !== undefined);
        });
        if (крутятся.length > 0) {
          сделан = true;
          // Ось есть — теперь перспектива: без неё поворот читается сплющиванием.
          for (const n of крутятся) {
            const t = плоско(n.parent?.props?.style)?.transform as any[];
            expect(t.some((x: any) => x.perspective !== undefined)).toBe(true);
            // И ни в коем случае не плоский `rotate` — это вертушка, а не резьба.
            expect(t.some((x: any) => x.rotate !== undefined)).toBe(false);
          }
