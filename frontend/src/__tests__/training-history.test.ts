...
describe('чей это раунд', () => {
  ...
  it('раунды без метки владельца показываем всем — это месяцы чужих тренировок не стереть', () => {
    expect(belongsToProfile({}, 'odv999')).toBe(true);
    expect(belongsToProfile({}, 'kids')).toBe(true);
  });
});
