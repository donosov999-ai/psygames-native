 ...
 *
 * 📍 07.09.2026: счётчики уехали из готовой вёрстки в данные (`hud`), и подпись
 * со значением стали РАЗНЫМИ узлами — жёсткий пробел в «Errors 1» перестал
 * совпадать, проба читала ноль там, где на экране единица. Соседний
...
describe('серия: подсказка и читаемый счётчик', () => {
  ...
  /**
   * 🔴 ПРОВЕРЯЕМ ЭКРАН, А НЕ ИСХОДНИК.
   *
   * Прежняя редакция искала в тексте файла `foundPair` и `marginLeft: 0` —
   * имена той вёрстки, которой чинили репорт Дениса 23.08.2026 («непонятно,
   * сколько слов ждёт система»: подпись уезжала к одному краю, число к другому).
   * 07.09.2026 счётчик переехал в `hud`, вёрстку рисует каркас — и проба
   * покраснела, хотя на экране всё правильно: она стерегла ИМЕНА, а не смысл.
   *
   * Смысл же прежний и проверяется прямо: подпись и число стоят рядом, между
   * ними нет других чисел. Так требование переживает и следующий переезд.
   */
  it('🔴 подпись и число держатся вместе — число не уносит к другому краю', async () => {
    const r = await mountProof();
    try {
      pressLabel(r, EN.entry);
      const текст = joined(r.root);
      const m = /Found[^\d]*(\d+)\/(\d+)/.exec(текст);
      expect(m).not.toBeNull();
      // Между подписью и числом — не больше короткого разделителя, иначе это
      // снова «подпись слева, число справа».
      const зазор = m![0].length - `Found`.length - m![1].length - 1 - m![2].length;
      expect(зазор).toBeLessThanOrEqual(3);
    } finally {
      r.unmount();
    }
  });
  ...
  /**
   * 🔴 ЗАДАНИЕ СТОИТ НАД ПОЛЕМ, А НЕ В ШАПКЕ (отчёт `a269f970`, 08.09.2026).
   *
   * Плитки «А Б» жили в слоте `stats` — в общей плашке счётчиков наверху экрана, —
   * а искать эти буквы надо в сетке ниже. Глаз ходил через весь экран между тем,
   * ЧТО искать, и тем, ГДЕ искать, и делал это на каждой клетке.
   *
   * ⚠️ Проверяем СОСЕДСТВО В ДЕРЕВЕ, а не пиксели и не имена стилей: имена уже
   * подводили этот же набор 07.09, когда счётчик переехал в каркас и проба
   * покраснела при правильном экране. Соседство переживает и переезд, и
   * переименование: задание и поле — дети одного родителя, задание идёт первым.
   *
   * ⚠️ ПОКРЫТА СЕРИЯ, ОБЫЧНАЯ ПАРТИЯ — НЕТ. Первая контрпроба не покраснела
   * именно поэтому: я вернул задание в шапку у обычной партии, а проба меряла
   * серию. Задание перенесено в ОБОИХ режимах, но пробой держится только серия:
   * запуск обычной партии в этом наборе роняет узел по памяти (у неё свой
   * таймер, а фейковых часов здесь нет). Долг записан, чтобы не выглядело
   * покрытием того, чего нет.
   */
  it('🔴 задание — сосед поля сверху, а не житель шапки', async () => {
    const r = await mountProof();
    try {
      pressLabel(r, EN.entry);
      const цель = r.root.findAll((n: any) => n.props?.testID === 'proof-target', { deep: true });
      const поле = r.root.findAll((n: any) => n.props?.testID === 'proof-grid', { deep: true });
      expect(`задание на экране: ${цель.length > 0} · поле: ${поле.length > 0}`)
        .toBe('задание на экране: true · поле: true');

      // Оба — дети ОДНОГО родителя, и задание стоит раньше поля.
      const дети = (цель[0].parent?.children ?? []) as any[];
      const iЦель = дети.findIndex((n: any) => n?.props?.testID === 'proof-target');
      const iПоле = дети.findIndex((n: any) => n?.props?.testID === 'proof-grid');
      expect(`общий родитель: ${iЦель >= 0 && iПоле >= 0}`).toBe('общий родитель: true');
      expect(`задание раньше поля: ${iЦель < iПоле}`).toBe('задание раньше поля: true');

      // И между ними ничего не вклинилось — иначе это снова «через весь экран».
      expect(`шагов между заданием и полем: ${iПоле - iЦель}`).toBe('шагов между заданием и полем: 1');
    } finally {
      r.unmount();
    }
  });
  ...
  /**
   * 🔴 ЗАГОЛОВОК СЕРИИ ВЛЕЗАЕТ В ШАПКУ.
   *
   * 📍 ЗАМЕР 08.09.2026 по скриншотам тестировщиков (`a269f970`, `18be48ff`): в
   * шапке стояла подпись входа — «Серия: три правила на одном поле букв», 52 знака
   * в самом длинном из двенадцати переводов, — и на телефоне от неё оставалось
   * «Серия: три прав…». Обрезано на ДВУХ скриншотах из четырёх: беда общая для
   * серий, а не про эту игру.
   *
   * ⚠️ Планка 24 знака — литералом. Она снята с той же шапки: между кнопкой
   * «назад», питомцем и справкой заголовку остаётся около половины ширины
   * телефона, а это 15–18 знаков в кегле 18. Двадцать четыре — потолок с запасом
   * на языки подлиннее; всё, что больше, гарантированно обрежется.
   */
  it('🔴 заголовок серии влезает в шапку, а не обрезается многоточием', async () => {
    const r = await mountProof();
    try {
      pressLabel(r, EN.entry);
      const шапки = r.root.findAll(
        (n: any) => n.props?.accessibilityRole === 'header' && typeof n.props?.numberOfLines === 'number',
        { deep: true },
      );
      expect(`заголовок найден: ${шапки.length > 0}`).toBe('заголовок найден: true');
      const текст = joined(шапки[0]).trim();
      expect(`заголовок «${текст}» длиной ${текст.length}, планка 24`)
        .toBe(`заголовок «${текст}» длиной ${текст.length}, планка 24`);
      expect(текст.length).toBeLessThanOrEqual(24);
      // и это НЕ длинная подпись входа
      expect(`заголовок равен подписи входа: ${текст === EN.entry}`)
        .toBe('заголовок равен подписи входа: false');
    } finally {
      r.unmount();
    }
  });
