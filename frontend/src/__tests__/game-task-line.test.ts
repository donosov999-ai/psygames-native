...
const ADDED: Record<string, string> = {
  ...
  // `SortGameScreen` переливалки с другой шкуркой. Читатель ниже идёт по
  // делегированию, иначе гейт требовал бы копию экрана ради своей же зелени.
  //
  // 🔴 НО СТРОКА У КАЖДОЙ ШКУРКИ СВОЯ, И ЗДЕСЬ ЭТО ЗАКРЕПЛЕНО (16.09.2026).
  // Прежняя запись требовала у «гаек» и «шариков» ключ `waterSortHint` — то есть
  // закрепляла ровно дефект из отчёта e0e027fc (задача 44e6cd21): на поле гайки,
  // а строка внизу «Нажми пробирку, потом вторую — верхний столбик перельётся».
  // Ключ теперь свой у каждой шкурки, и проверка перевода ниже стережёт все три.
  'ball-sort.tsx': 'ballSortHint',
  'nut-sort.tsx': 'nutSortHint',
};

/**
 * Где строка берётся не литералом `t('<ключ>')`, а разрешителем ключа по шкурке.
 * ⚠️ Разрешитель — не лазейка: что каждая шкурка на ЖИВОМ экране говорит своими
 * словами, стережёт `sort-skins-speak-their-own-words.test.tsx` рендером, а не
 * буквами. Здесь проверяется только то, что строка рисуется в игровой фазе.
 */
const ЧЕРЕЗ_ШКУРКУ: Record<string, string> = {
  'water-sort.tsx': "тс('Hint')",
  'ball-sort.tsx': "тс('Hint')",
  'nut-sort.tsx': "тс('Hint')",
};
...
const ALREADY: Record<string, string> = {
  ...
  // Головоломки Тэтхэма: правила у двадцати движков РАЗНЫЕ и ни одни не наши, поэтому
  // задание висит над доской всю партию — `описание` = t(КЛЮЧ_ОПИСАНИЯ[режим]).
  'puzzles.tsx': 'КЛЮЧ_ОПИСАНИЯ',
...
describe('строка «что делать» во время партии', () => {
  ...
  it('🔴 заведённая строка рисуется в игровой фазе, а не лежит в конфиге', () => {
    ...
    for (const [f, key] of Object.entries(ADDED)) {
      ...
      const ждём = ЧЕРЕЗ_ШКУРКУ[f] ?? `t('${key}')`;
      if (!play.includes(ждём)) bad.push(`${f}: ${ждём} не в игровом рендере`);
    }
    expect(bad).toEqual([]);
  });
  ...
  it('хаб остаётся хабом — иначе его надо переводить в игры', () => {
    for (const [f, why] of Object.entries(NOT_A_GAME)) {
       ...
       */
      /**
       * ⚠️ МОДУЛЬ НЕ ОБЯЗАН ЗВАТЬСЯ `*Game`. «Пауза» с 27.08.2026 рисует партию
       * встроенной страницей `<WarmupPage>` — это тот же модуль внутри каркаса,
       * просто имя у него не по шаблону. Смысл проверки — «каркас не пустой», и
       * ради него имя расширено списком, а не регуляркой «любой компонент»:
       * иначе гейт зазеленел бы на любом `<View>`.
       */
      const МОДУЛИ_НЕ_ПО_ШАБЛОНУ = ['WarmupPage'];
      const wrapsModule = /<[A-Z]\w*Game\b/.test(playRender(src))
        || МОДУЛИ_НЕ_ПО_ШАБЛОНУ.some((имя) => new RegExp(`<${имя}\\b`).test(playRender(src)));
