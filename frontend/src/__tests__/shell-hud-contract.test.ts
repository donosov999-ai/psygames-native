...
describe('договор шапки каркаса', () => {
  ...
  /**
   * ⚠️ ПРОВЕРЯЕМ ПРИСУТСТВИЕ `stats` В РАЗМЕТКЕ, А НЕ ЕГО МЕСТО В ТЕРНАРНИКЕ.
   *
   * Прежняя редакция искала дословное `) : stats}` — то есть требовала, чтобы
   * `stats` стоял ВЕТКОЙ ELSE у бейджей. 02.09.2026 это перестало быть верным и
   * оказалось вредным: игра, перешедшая на `hud`, разом теряла всё, что осталось
   * в `stats` (бейдж правила уровня, плашку стимула). Теперь рисуется и то, и
   * другое, а гейт стережёт главное — что `stats` вообще попадает в разметку.
   */
  it('🔴 старый способ (stats) продолжает работать — иначе 72 игры ослепнут разом', () => {
    expect(/\{stats\}/.test(shellCode)).toBe(true);
    // И рядом с бейджами, а не вместо них: обе ветки живут одновременно.
    expect(/hud\.slice\(0, HUD_MAX\)/.test(shellCode)).toBe(true);
  });
