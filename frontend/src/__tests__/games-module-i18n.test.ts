...
const SCREEN_NOT_REBUILT: Record<string, string> = {
  // ⚠️ ПУСТО, И ЭТО РЕЗУЛЬТАТ. Последняя запись — `chess-blind` — снята 23.08.2026:
  // экран пересобран серией из трёх блоков и читает словарь модуля напрямую
  // (`getChessBlindStrings(language)`), то есть утверждение «словарь читать
  // некому» стало ложным и покраснело, как и было задумано. Механизм оставлен:
  // следующее ядро, приехавшее раньше своего экрана, впишется сюда с причиной.
};
