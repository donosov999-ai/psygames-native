...
describe('подъём партии — только через общий хук', () => {
  ...
  // ⚠️ 10 → 11 (06.09.2026): подъём партии появился у «Сортировки тортов».
  // Счёт держим руками намеренно — он ловит обратное: если хук случайно
  // выпилят разом у половины экранов, гейт покраснеет, а не опустеет молча.
  it('и таких экранов одиннадцать — счёт держим, чтобы гейт не опустел молча', () => {
    const withBoot = games.filter((f: string) => /\buseResumeBoot\s*[<(]/.test(code(f)));
    expect(withBoot.length).toBe(11);
  });
});
