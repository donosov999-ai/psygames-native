...
describe('попадание по тарелке', () => {
  ...
  it('🔴 край круга ловится, а точка сразу за ним — уже нет', () => {
    /**
     * 🔴 ПРЕМИСА УТОЧНЕНА 08.09.2026 (отчёт `54748daf` «плохо хватался с тарелок»).
     * Радиус попадания был ровно `plate / 2` — по нарисованному краю, — и палец на
     * самой кромке читался как промах: засчитывалось лишь 61,5…73,5 % площади
     * клетки. Теперь радиус берётся с половиной зазора, то есть до половины шага
     * сетки; дальше расширять нельзя — там начинается соседняя тарелка.
     * Смысл пробы прежний: за краем — мимо. Изменилось, где этот край.
     */
    const c = центр(5);
    const r = L.plate / 2 + PLATE_GAP / 2;
    expect(plateAtPoint(c.x + r - 1, c.y, COLS, L.plate, 12)).toBe(5);
    expect(plateAtPoint(c.x + r + 1, c.y, COLS, L.plate, 12)).toBeNull();
    // и по-прежнему НЕ достаёт до середины между тарелками
    expect(plateAtPoint(c.x + (L.plate + PLATE_GAP) / 2 + 1, c.y, COLS, L.plate, 12)).not.toBe(5);
  });
