...
describe('развилка и профиль', () => {
  ...
  it('🔴 отрисованная развилка не показывает закрытое профилю', () => {
    ...
    /**
     * ⚠️ ПРИМЕР ЗАКРЫТОГО СМЕНЁН 06.09.2026: было `chess-blind`, но развилка
     * «Шахматы» открыта детям ЦЕЛИКОМ (слово Дениса: «шахматы это детская игра,
     * вполне»). Проба про то, что фильтр РАБОТАЕТ, а не про конкретную игру, —
     * поэтому берём другую, действительно закрытую детям. Их 51.
     */
    expect(`memory-palace разрешён детям: ${можно.has('/games/memory-palace')}`)
      .toBe('memory-palace разрешён детям: false');
    ...
    /**
     * ⚠️ ШАХМАТЫ У ДЕТЕЙ ТЕПЕРЬ ЦЕЛИКОМ, включая «Доску в уме». Проверяем
     * обратное тому, что было: развилка не пустая и показывает ОБЕ игры.
     * Отрицательный случай фильтра ловится ниже, на мнемонике.
     */
    const шахматы = нарисовать('/games/chess-hub');
    expect(`«Доска в уме» на экране шахмат: ${/Доска в уме|Board in Mind/.test(шахматы.текст)}`)
      .toBe('«Доска в уме» на экране шахмат: true');

    // 🔴 И отрицательный случай — иначе проба перестала бы ловить сломанный фильтр.
    const мнемоника = нарисовать('/games/mnemonics-hub');
    expect(`«Дворец памяти» на экране мнемоники: ${/Дворец памяти|Memory Palace/i.test(мнемоника.текст)}`)
      .toBe('«Дворец памяти» на экране мнемоники: false');
