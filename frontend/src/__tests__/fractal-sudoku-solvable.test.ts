...
describe('сверка чужими реализациями — чтобы движок не судил сам себя', () => {
  ...
  it('быстрый решатель совпадает с ядром и на заведомо неоднозначных досках', () => {
    // Специально ломаем задание: снимаем ЧЕТЫРЕ настоящие подсказки (не пустые клетки —
    // их в задании и так большинство). Решений станет несколько, и оба решателя обязаны
    // сказать это одинаково. Без такой пары сверка была бы односторонней: совпадение
    // только на единственных решениях ничего не доказывает про «много решений».
    const bad: string[] = [];
    const f = generateFractal(10);
    for (const [i, ch] of f.children.entries()) {
      const holed = ch.puzzle.map((r) => [...r]);
      let removed = 0;
      for (let r = 0; r < N && removed < 4; r++) {
        for (let c = 0; c < N && removed < 4; c++) if (holed[r][c] !== 0) { holed[r][c] = 0; removed++; }
      }
      const mine = countSolutionsFast(flatten(holed), 2);
      const house = houseSolutions(holed);
      if (mine !== house) bad.push(`дочерняя ${i}: свой решатель ${mine}, ядро ${house}`);
      if (mine < 2) bad.push(`дочерняя ${i}: снятие четырёх подсказок не размножило решения — проверять нечего`);
    }
    expect(bad).toEqual([]);
  }, 300000);
});
