/* psygames-puzzle-where-error · VER 2 · 17.09.2026 */
/**
 * 🔴 «ГДЕ ОШИБКА?»: ЛИШНИЕ ЛИНИИ ОБВЕДЕНЫ, СТУПЕНЬ НЕ СДАНА, РАМКИ ГАСНУТ С ПЕРВЫМ ХОДОМ.
 *
 * 📍 Отзывы Дениса 16.09.2026 (28a9d55c, 0b70eff2, d6dd7d66) и 17.09 (dd4cda8a, «Мосты»; задача 9022b6ff): «решил —
 * не засчитано, а почему, не видно». У «Галактик» своей подсветки ошибок нет, «Мосты» красят только перегруженный
 * остров. Проба меряет:
 *   · снимок моста (`scripts/capture-where-error.mjs`, привязан к md5): цвета поставленной линии входят в `ЦВЕТ_ЛИНИИ`,
 *     решить → отменить возвращает рисунок и позицию — у обоих режимов;
 *   · `лишниеЛинии` на НАСТОЯЩИХ рисунках: неверная линия — одна рамка точно на ней, верная — ни одной;
...
const РЕЖИМЫ = ['Galaxies', 'Bridges'] as const;
...
const мосты = (снимок as any).режимы.Bridges;
...
jest.mock('@/src/games/tatham-bridge', () => ({
  движки: () => Promise.resolve(['Galaxies', 'Singles', 'Palisade', 'Bridges'].map((имя, индекс) => ({ индекс, имя, умеетТекстом: false, решаем: true, ступени: [{ индекс: 0, имя: 'Easy', параметры: ({ Galaxies: '7x7dn', Palisade: '5x5n5', Bridges: '7x7i30e10m2d0' } as Record<string, string>)[имя] ?? '5x5de' }] }))),
  доскаСтрок: () => [],
}));
...
const вКрестовине = (узел: any) => { for (let p = узел?.parent; p; p = p.parent) if (p.props?.testID === 'arrow-pad-with-solution') return true; return false; };
...
describe('«Где ошибка?» — снимок моста', () => {
  ...
  it.each(РЕЖИМЫ)('🔴 %s: цвета поставленной линии — из ЦВЕТ_ЛИНИИ, а решить → отменить возвращает доску и позицию', (режим) => {
    const р = (снимок as any).режимы[режим];
    expect(р.верная && р.неверная).toBeTruthy();
    for (const проба of [р.верная, р.неверная]) {
      expect(проба.цветаНовых.length).toBeGreaterThan(0);
      expect(проба.цветаНовых.filter((ц: number) => !ЦВЕТ_ЛИНИИ[режим]!.includes(ц))).toEqual([]);
      expect({ рисунок: проба.вернулосьРисунком, позиция: проба.вернулосьПозицией }).toEqual({ рисунок: true, позиция: true });
    }
  });
});

describe('«Где ошибка?» — лишниеЛинии на настоящих рисунках «Галактик» и «Мостов»', () => {
  it.each(РЕЖИМЫ)('🔴 %s: неверная линия — одна рамка, и она накрывает середину поставленной линии', (режим) => {
    const р = (снимок as any).режимы[режим];
    const рамки = лишниеЛинии(режим, разобрать(р.неверная.доска), разобрать(р.неверная.решение))!;
    expect(рамки).toHaveLength(1);
    const { x, y } = р.неверная.точка;
    const [рамка] = рамки;
    expect(рамка!.x <= x && x <= рамка!.x + рамка!.ш && рамка!.y <= y && y <= рамка!.y + рамка!.в).toBe(true);
    // Рамка — вокруг линии, а не вокруг клетки: поперёк линии она не шире 24 единиц (клетка у обоих 24–32).
    expect(Math.min(рамка!.ш, рамка!.в)).toBeLessThanOrEqual(24);
  });

  it.each(РЕЖИМЫ)('🔴 %s: верная линия и сама решённая доска — ни одной рамки (ложной тревоги нет)', (режим) => {
    const р = (снимок as any).режимы[режим];
    expect(лишниеЛинии(режим, разобрать(р.верная.доска), разобрать(р.верная.решение))).toEqual([]);
    expect(лишниеЛинии(режим, разобрать(р.неверная.решение), разобрать(р.неверная.решение))).toEqual([]);
  });

  it('🔴 «Мосты», перегруженный остров: мосты красные — сверка их видит, а не говорит «лишних нет»', () => {
    const п = мосты.перегруз;
    expect(п.красных).toBeGreaterThan(0);
    expect(п.вернулосьРисунком).toBe(true);
    const доска = разобрать(п.доска);
    expect(доска.some((д: any) => д.вид === 'прямоугольник' && д.цвет === 8)).toBe(true);
    const рамки = лишниеЛинии('Bridges', доска, разобрать(п.решение))!;
    const { x, y } = п.точка;
    expect(рамки.some((р) => р.x <= x && x <= р.x + р.ш && р.y <= y && y <= р.y + р.в)).toBe(true);
  });
...
describe('«Где ошибка?» — экран', () => {
  it('🔴 «Галактики»: нажал — лишняя граница обведена красным, строка говорит сколько, ступень НЕ сдана', async () => {
    ...
    expect(вКрестовине(кнопка(дерево))).toBe(false);
  ...
  it('🔴 «Мосты» (dd4cda8a): кнопка есть; итог сверки в слоте вытесняет «красным отмечено», пока доска та же', async () => {
    const доска = [...разобрать(мосты.неверная.доска), { вид: 'прямоугольник', x: 1, y: 1, ш: 3, в: 3, цвет: 8 }];
    const дерево = await экран('Bridges', доска, разобрать(мосты.неверная.решение));
    expect(кнопка(дерево)).toBeTruthy();
    // У режима с крестовиной лупа — в её пустой клетке, а не четвёртой в ряду (ряд шире → кнопка отзыва на «Отменить»).
    expect(вКрестовине(кнопка(дерево))).toBe(true);
    expect(слот(дерево)).toBe('puzzleErrorShown');
    await нажать(дерево);
    expect(слот(дерево)).toBe('puzzleWrongLines');
    expect(mockCanvas.подсветка.length).toBeGreaterThan(0);
  });
  ...
  it('🔴 пять команд — одной строкой значков: у «Галактик» переключатель без подписи, у «Частокола» подпись на месте', async () => {
    ...
    // «Мосты»: лампочка в пустой клетке крестовины, в ряду четыре команды — подпись переключателя остаётся.
    const экранМостов = await экран('Bridges', разобрать(мосты.неверная.доска), разобрать(мосты.неверная.решение));
    expect(кнопка(экранМостов)).toBeTruthy();
    expect(тексты(переключатель(экранМостов))).toEqual(['puzzleSecondNoBridge']);
  });
