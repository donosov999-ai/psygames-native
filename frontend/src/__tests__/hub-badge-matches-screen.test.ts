...
describe('значок развилки не врёт', () => {
  /**
   * ⚠️ ПОРОГ ОПУЩЕН 16 → 12 РУКАМИ, 12.09.2026, и это не подгонка под красную пробу.
   * Развилок стало 13 вместо 18: решением Дениса расформированы пять —
   * «Торможение», «Риск», «Маршруты», «Гибкость», «Зрительная память» (задачи
   * 80eb49c9, 4dce5eb9, 81486a4a, 8f0b0428, 43bc1ca2). Порог остаётся ЛИТЕРАЛОМ
   * и стоит НИЖЕ нынешнего числа: он ловит обвал реестра, а не считает развилки.
   */
  it('есть что проверять: развилок ≥ 12, профилей ≥ 10, и они различаются доступом', () => {
    expect(ХАБЫ.length).toBeGreaterThanOrEqual(12);
  ...
  it('🔴 главный экран не считает состав развилки сам — он его спрашивает', () => {
    /*
     * ⚠️ ФАЙЛ СМЕНИЛСЯ 07.09.2026, И ЭТО НЕ ПОДГОНКА ПОД ЗЕЛЁНОЕ. Разделы
     * каталога уехали из `app/index.tsx` в общий `CategorySections`: их теперь
     * рисуют ДВА экрана — вкладка «Игры» (все девять) и главная (три любимых).
     * Спрашивать состав развилки обязано то место, где рисуется значок, и оно
     * ровно одно. Проверка та же, носитель другой — иначе гейт сторожил бы
     * файл, в котором значка больше нет, и зеленел бы навсегда.
     */
    const src = безКомментариев(fs.readFileSync(path.join(КОРЕНЬ, 'src/components/CategorySections.tsx'), 'utf8'));
    expect(`зовёт hubBadgeCount: ${src.includes('hubBadgeCount(')}`).toBe('зовёт hubBadgeCount: true');
    // И на главной своего подсчёта не завелось обратно.
    const главная = безКомментариев(fs.readFileSync(path.join(КОРЕНЬ, 'app/index.tsx'), 'utf8'));
    expect(`главная считает сама: ${главная.includes('hubBadgeCount(')}`).toBe('главная считает сама: false');
    /**
  ...
  it('🔴 экраны развилок не хранят своих списков карточек', () => {
    ...
    for (const х of ХАБЫ) {
      ...
      /**
       * ⚠️ ГРАНИЦА СЛОВА ОБЯЗАТЕЛЬНА. Без неё выражение ловит ЛЮБОЕ поле,
       * кончающееся на `route`, — замер 06.09.2026: состав зарядки
       * (`game_route: '/games/anagrams'`) сошёл за три карточки в «Словах» и
       * две в «Шахматах», и гейт покраснел на коде, где списка нет. Проверяется
       * ровно `route:`, а не `game_route:`.
       */
      const своих = [...src.matchAll(/(?<![A-Za-z0-9_$\u0400-\u04FF])route:\s*'\/games\//g)].length;
