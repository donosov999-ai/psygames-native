...
describe('питомец живёт у справки', () => {
  it('🔴 питомец ОДИН и стоит в правом углу шапки, а не в плашке счётчиков', () => {
     ...
     */
    /**
     * ⚠️ 03.09.2026 ДОМ СМЕНИЛСЯ: питомец переехал ИЗ шапки каркаса В сквозной слой
     * справки. Причина в кадре Дениса: на экране настройки «Доски в уме» шапку рисует
     * сама игра, шапки каркаса там нет — и питомца не было. Слой справки монтируется
     * в корневом макете и виден на каждом экране с правилами. Правило прежнее —
     * питомец ОДИН и живёт в одном месте, — только место другое.
     */
    const слой = читать('src/components/GameHelpOverlay.tsx');
    expect(слой).toContain('<GamePet');
    expect((слой.match(/<GamePet/g) || []).length).toBe(1);     // один, не два
    expect(каркас).not.toContain('<GamePet');
  ...
  it('🔴 ответ на ход НЕ потерян: настроение доходит до питомца', () => {
    const каркас = читать('src/components/GameShell.tsx');
    expect(каркас).toContain('setGameMood(');                    // канал жив в каркасе
    /**
     ...
     */
    // Настроение теперь читает сам слой справки — там же, где рисуется питомец.
