...
describe('питомец живёт у справки', () => {
  ...
  it('🔴 ответ на ход НЕ потерян: настроение доходит до питомца', () => {
    ...
    /**
     * ⚠️ ПРОВЕРЯЕМ СМЫСЛ, А НЕ ДОСЛОВНОЕ ВЫРАЖЕНИЕ. Прежняя строка требовала
     * `<GamePet mood={pet ?? autoMood}` буквально и покраснела 03.09.2026 на
     * правке, которая ничего не сломала: правый угол вынесен в HeaderRightSlot,
     * настроение идёт туда пропом и дальше в питомца. Требуется одно — чтобы
     * СЛИТОЕ настроение (своё от игры, иначе автоматическое) уходило в угол;
     * что оно доходит до питомца, проверяет рисованием pet-next-to-help.
     */
    expect(каркас).toMatch(/mood=\{pet \?\? autoMood\}/);
  });
