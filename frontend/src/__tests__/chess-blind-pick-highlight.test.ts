...
describe('«Доска в уме»: вопрос про подсветку — значит подсветка видна', () => {
  ...
  it('🔴 вопрос НАЗЫВАЕТ клетку, а не только ссылается на подсветку', async () => {
    ...
    expect(весь).toMatch(/(на клетке|on square)/i);
    expect(весь).toMatch(/[a-h][1-8]/);
    await TestRenderer.act(async () => { r.unmount(); });
  });

  /**
   * 🔴 И КЛЕТКА НАЗВАНА КРУПНО. Отчёт a4cc1a7d (04.09.2026) — ВТОРОЙ про то же:
   * «подсветка, не видно нихуя, какую фигуру выделять». Замер в браузере показал,
   * что рамки доезжают (3 штуки, 45×45, 3 px), то есть предыдущий обход —
   * назвать клетку словами — сработал ровно наполовину: назвали, но кеглем 14
   * серым по светлому. Поэтому здесь меряется РАЗМЕР: имя клетки обязано быть
   * набрано крупно, иначе «назвали» снова не равно «видно».
   */
  it('🔴 имя клетки набрано КРУПНО, а не подписью в 14 пунктов', async () => {
    const r = await монтировать();
    expect(await доОпроса(r)).toBe(true);
    const крупные = r.root.findAll((n: any) => {
      const s = n.props?.style;
      const плоский = Array.isArray(s) ? s.reduce((a: any, x: any) => ({ ...a, ...(x || {}) }), {}) : (s || {});
      const текстУзла = String(n.props?.children ?? '');
      return Number(плоский.fontSize || 0) >= 24 && /^[a-h][1-8]$/.test(текстУзла.trim());
    });
    expect(`клеток, названных крупно: ${крупные.length >= 1}`).toBe('клеток, названных крупно: true');
    await TestRenderer.act(async () => { r.unmount(); });
  });
});
