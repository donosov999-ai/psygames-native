...
const МЕХАНИКИ: Механика[] = [
  ...
  // ⚠️ `lookalike` помечен порогом, а не отрезком, хотя у правила стоит toLevel 8:
  // механика (двойники) с L3 и до конца, а КАРТОЧКА обрывается на L8 по текстовой
  // причине — её текст на двенадцати языках называет X поимённо, а с L9 мишень уже
  // не X. Отрезком её пометить нельзя: гейт справедливо потребовал бы, чтобы и
  // механика кончалась на L8.
  { игра: 'cpt', ключ: 'newtarget', вид: 'порог', есть: (L) => cpt(L).target !== 'X' },
  { игра: 'cpt', ключ: 'colorrule', вид: 'порог', есть: (L) => cpt(L).colorRule },
  ...
  // Ниша ровно на один товар: та же доска из девяти ниш, что и у соседней механики.
  { игра: 'goods-sort', ключ: 'singlecap', вид: 'порог', есть: (L) => capsFor(L, 9).includes(1) },
  /*
...
const РЕДАКЦИОННЫЕ: Record<string, string> = {
  ...
  'chess-blind:moves': 'ход вслепую стоит на ПЕРВОЙ ступени с 12.09.2026 — порога не существует, правило вводное. До этого механика начиналась с шестой, но дальше 4-го уровня не заходил никто, и её не видел ни один игравший',
