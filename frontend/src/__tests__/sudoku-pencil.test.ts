...
/**
 * РАЗМЕТКА КНОПКИ КАРАНДАША — по ОБЪЯВЛЕНИЮ, а не по значку и не по подписи.
 *
 * ⚠️ Здесь уже дважды ломался якорь, и оба раза по одной причине: за него брали то, что
 * видит игрок, а оно меняется. 09.09.2026 с подписи ушло число (отчёт 779c482d) — якорь
 * по тексту подписи увёл бы в пустоту. 16.09.2026 значок стал зависеть от состояния
 * (`icon={pencil ? 'pencil' : 'pencil-outline'}`, отчёт «Релакс» 33f21fc6) — и литерал
 * `icon="pencil-outline"` перестал находиться.
 *
 * 🔴 И вот чем это дорого. Из трёх мест, где стоял литеральный якорь, покраснело ОДНО.
 * Два других `indexOf` вернули −1, `slice(-1, 299)` дал ПУСТУЮ строку, и проверки вида
...
describe('обе игры дотягиваются до карандаша, а не просто импортируют его', () => {
  ...
  it('🔴 слой пометок доезжает до отрисовки в обеих играх', () => {
    ...
    //
    // 🔴 ПРЕМИСА СМЕНИЛАСЬ 08.09.2026. Разметка пометок была скопирована в четыре
    // экрана, и во всех четырёх слот считался как треть КЛЕТКИ — при том, что слой
    // лежит ВНУТРИ рамки и трети клетки там нет: три слота не влезали, девять цифр
    // раскладывались в пять рядов и лезли на соседние клетки. Теперь разметка одна на
    // все судоку — `PencilMarksLayer`, и геометрию сторожит `pencil-marks-fit`
    // РЕНДЕРОМ. Здесь остаётся своё: экран считает кандидатов и отдаёт их слою.
    expect(/\{renderMarks\(r, c, v, bg\)\}/.test(SUDOKU)).toBe(true);   // и фон клетки — слою, ради контраста
    const rm = fnBody(SUDOKU, 'renderMarks');
    const digitsAt = rm.indexOf('sudokuVisibleMarks(');
    const layerAt = rm.indexOf('<PencilMarksLayer');
    expect(`судоку: пометки считаются и рисуются: ${digitsAt >= 0 && layerAt > digitsAt}`)
      .toBe('судоку: пометки считаются и рисуются: true');
    // и посчитанное уходит В СЛОЙ, а не теряется по дороге
    expect(`судоку: кандидаты отданы слою: ${/digits=\{digits\}/.test(rm)}`)
      .toBe('судоку: кандидаты отданы слою: true');
    ...
    expect(/<PencilMarksLayer[\s\S]{0,120}?digits=\{penciled\}/.test(SAMURAI)).toBe(true);
  });

  it('🔴 разметка пометок ОДНА на все четыре судоку, а не четыре копии', () => {
    /**
     * 🔴 ЗАЧЕМ ОТДЕЛЬНАЯ ПРОВЕРКА. Копий было четыре, дефект геометрии — во всех
     * четырёх, а залечена вслепую одна: в глубоком фрактале кто-то подобрал
     * `cell / 3.2`, и симптом там стал тише. Ровно так починка одного носителя
     * оставляет остальные сломанными. Пока копия одна — правка доезжает до всех.
     */
    const SCREENS = [
      ['sudoku', SUDOKU],
      ['samurai', SAMURAI],
      ['fractal', code('sudoku-fractal.tsx')],
      ['fractal-deep', code('sudoku-fractal-deep.tsx')],
    ] as const;
    for (const [name, src] of SCREENS) {
      expect(`${name}: зовёт общий слой: ${/<PencilMarksLayer/.test(src)}`)
        .toBe(`${name}: зовёт общий слой: true`);
      // и НЕ считает слот сам — ни третью клетки, ни подобранной вслепую долей
      const own = src.match(/width: [a-zA-Z]+ ?\/ ?3(\.\d+)?[,\s]/g) ?? [];
      expect(`${name}: своей геометрии слота нет: ${own.length === 0}`)
        .toBe(`${name}: своей геометрии слота нет: true`);
    }
  });
  ...
  it('🔴 на кнопке-РЕЖИМЕ не стоит число: его путают с остатком', () => {
    ...
    const btn = разметкаПера(300);
    expect(`разметка кнопки карандаша найдена: ${btn.length > 0}`).toBe('разметка кнопки карандаша найдена: true');
