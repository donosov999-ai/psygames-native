...
describe('дисциплина движения во всём приложении', () => {
  ...
  /**
   * ИСКЛЮЧЕНИЯ — ДВИЖЕНИЕ, КОТОРОЕ ЯВЛЯЕТСЯ СОДЕРЖАНИЕМ, А НЕ УКРАШЕНИЕМ.
   *
   * 🔴 Правило Дениса 09.09.2026: «гейты не имеют права ломать функционал; чинить надо
   * код или сам гейт, а не упрощать то, что работало». Повод — этот самый гейт: 09.09 я
   * подчинил ему поворот фигуры в «Мысленном вращении» и поворот блока/трубы в
   * «Повороте чисел» и «Сети труб», и на устройстве с включённой настройкой упражнение
   * перестало показывать поворот вовсе — разбор ошибки замирал на той картинке, из-за
   * которой человек ошибся, а блок перескакивал без движения.
   *
   * Настройка «меньше движения» гасит ДЕКОР: пролёты экранов, подпрыгивания, петли,
   * блёстки. Она не отменяет ДЕЙСТВИЕ, за которым человек и пришёл: здесь поворот —
   * это само упражнение и его объяснение. Экран может форсировать мгновенный показ
   * пропом `reduceMotion`, но системная настройка сюда не дотягивается.
   *
   * Первое исключение — сам доводчик: он и есть реализация правила, настройку ему
   * передают аргументом.
   */
  const MOTION_OK = new Set([
    'src/components/juice/motion.ts',
    'src/components/RotationShape.tsx',      // 3D-поворот фигуры из кубиков — суть упражнения
    'src/components/RotationWorkbench.tsx',  // ручное вращение в разборе ошибки
    'src/components/SpatialLab.tsx',         // поворот блока 2×2 и трубы — ход игрока
  ]);
