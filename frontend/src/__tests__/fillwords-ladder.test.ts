...
describe('лестница филвордов', () => {
   ...
   */
  it('🔴 при показанном списке слов поле не выше десяти строк', () => {
    const беды: string[] = [];
    for (let L = 1; L <= 300; L += 1) {
      const c = fillwordsLevel(L, true);
      if (c.rows > 10) беды.push(`L${L}: ${c.rows} строк со списком`);
    }
    expect(беды.slice(0, 3)).toEqual([]);
  });

  it('🔴 и влезает по высоте на самом узком экране вместе со списком', () => {
    const H = 568; const W = 320;
    const резерв = 210; const списокПрибл = 126;
    const доступно = Math.max(200, H - резерв) - списокПрибл;
    const беды: string[] = [];
    for (let L = 1; L <= 300; L += 1) {
      const c = fillwordsLevel(L, true);
      const клетка = Math.max(22, Math.min(Math.floor((W - 24) / c.cols), Math.floor(доступно / c.rows), 72));
      if (клетка * c.rows > доступно) беды.push(`L${L}: ${c.rows}×${c.cols} требует ${клетка * c.rows} при ${доступно}`);
    }
    expect(беды.slice(0, 3)).toEqual([]);
  });

  it('без списка потолок прежний — шестнадцать строк', () => {
    expect(fillwordsLevel(300).rows).toBe(16);
    expect(fillwordsLevel(300, false).rows).toBe(16);
  });
});
