...
describe('memory-matrix: правило уровня и фаза показа', () => {
   ...
   */
  /**
   * ⚠️ БЫЛА `it.failing` — БУДИЛЬНИКОМ, пока дефект жив. Починка внесена
   * координатором 06.09.2026 в `useLevelRules` (перепроверка `enabled` ПОСЛЕ
   * чтения хранилища), проба начала проходить, и `.failing` сама покраснела
   * словами «Failing test passed even though it was supposed to fail» — как и
   * было задумано автором пробы. Снято, теперь это обычный гейт.
   */
  it('🔴 нажал «Начать» во время чтения флага — правило НЕ всплывает поверх сетки', async () => {
