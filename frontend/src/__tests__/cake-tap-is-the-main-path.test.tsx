...
/** Куски в развёрнутой тарелке — по метке, а не по форме: форм на экране много. */
const куски = (r: any) => r.root.findAll((n: any) => typeof n.type !== 'string'
...
describe('тап — главный путь', () => {
  ...
  /**
   * 🔴 ХОД СТАЛ ТРЁХШАГОВЫМ — РЕШЕНИЕ ДЕНИСА 16.09.2026, А НЕ ПОЛОМКА.
   *
   * Было: тап по тарелке-источнику, тап по цели, едет ВЕРХНИЙ кусок. Стало:
   * тап раскрывает тарелку в увеличенный круг, тап по куску берёт ИМЕННО ЕГО,
   * тап по цели кладёт. Причина смены — кусок «сверху» на круге не видно
   * ниоткуда, и игра брала не тот, по которому целился человек (отчёт c72e57cb).
   *
   * ⚠️ Проба переписана целиком, а не подпёрта: старая утверждала бы, что
   * двух тапов хватает, и краснела бы на ИСПРАВНОЙ игре.
   */
  it('🔴 тап раскрывает тарелку в увеличенный круг', async () => {
    const r = await открыть();
    expect(куски(r).length).toBe(0);            // премиса: до тапа круга нет
    const откуда = подписи(r).findIndex((л: string) => /: [1-9]\d*\/6$/.test(л));
    await тап(r, откуда);
    expect(куски(r).length).toBeGreaterThan(0);
  }, 120_000);

  it('🔴 три шага делают ход: раскрыть, взять кусок, положить', async () => {
    const r = await открыть();
    ...
    await тап(r, откуда);
    const к = куски(r);
    expect(к.length).toBeGreaterThan(0);        // премиса: круг раскрылся
    await TestRenderer.act(async () => { к[0].props.onPress(); });
    expect(куски(r).length).toBe(0);            // премиса: круг закрылся, кусок в руке
    await тап(r, куда);
  ...
  it('🔴 запрещённый ход тапом ВИДЕН, а не только слышен', async () => {
    ...
    /*
     * ⚠️ ШАГОВ ТЕПЕРЬ ТРИ: раскрыть тарелку, взять кусок, ткнуть в цель. Без
     * среднего шага рука пуста, и тап по второй тарелке просто раскрыл бы ЕЁ —
     * отказа не случилось бы вовсе, а проба зеленела бы, «не дойдя до места».
     */
