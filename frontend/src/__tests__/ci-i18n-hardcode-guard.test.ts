 ...
 *
 * ПОЧЕМУ ХРАПОВИК, А НЕ СТЕНА. Долг был 14 экранов, 106 мест; 19.08 одиннадцать экранов
 * закрыты целиком (83 места ушли в словарь), а следом — оставшиеся три (picture-pairs 13,
 * set-game 6, sudoku-samurai 4). ДОЛГ ПОГАШЕН ЦЕЛИКОМ: список ниже пуст, и требование
 * теперь одно и жёсткое — ноль во ВСЕХ экранах. Храповик оставлен как механизм: если
 * длинный текст придётся заводить поэтапно, файл записывают сюда с потолком и причиной,
 * и это число может только УМЕНЬШАТЬСЯ.
 *
...
/**
 * ДОЛГ: экран → сколько мест зашито мимо словаря и почему это ещё не перенесено.
 * Число — ПОТОЛОК: вырасти нельзя, уменьшить можно и нужно.
 *
 * ⚠️ СЕЙЧАС ПУСТ, И ЭТО РЕЗУЛЬТАТ, А НЕ ЗАБЫВЧИВОСТЬ. Последние три записи —
 * picture-pairs (13), set-game (6), sudoku-samurai (4) — погашены 19.08.2026: подписи
 * режимов и параметров уровня, разбор примера «что такое SET» и экран исчерпанного
 * лимита ошибок самурая уехали в словарь на все двенадцать языков. Пустой список
 * означает, что проверка ниже требует НОЛЯ ОТ ВСЕХ экранов без исключений.
 *
 * Запись сюда добавляют только тогда, когда перенос физически разбит на заходы
 * (длинный методический текст, занятый параллельной правкой файл), — с потолком и
 * причиной. Проверка «исключение всё ещё нужно» не даст записи протухнуть.
 */
