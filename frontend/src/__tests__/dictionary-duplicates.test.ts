...
const ALLOWED: Record<string, string> = {
  'a11yErase + puzzleSecondErase':
    '[СМЫСЛ] «Стереть». `a11yErase` — подпись для чтения с экрана у кнопки забоя, `puzzleSecondErase` — ВИДИМАЯ подпись второго действия у головоломок Тэтхэма (заведена 16.09.2026 вместе с честным органом подсветки чисел). Свести нельзя: подпись второго действия порежимна и завтра может стать «Убрать костяшку», а подпись для чтения с экрана обязана остаться общей. Совпадение слова — совпадение русского языка, а не смысла.',
  'a11yBack + back':
  ...
  'cakeSortDesc + pizzaSortDesc':
    '[СМЫСЛ] «Собирайте круг из шести одинаковых кусков». Торты и пицца — один движок и одно правило игры, разная только еда; текст совпадает НАРОЧНО (решение раздела сортировки, src/games/cake-sort/INTEGRATION.md). Развести описания значило бы соврать про правило ради непохожести.',
  ...
  'label_words + wordsGroup':
    '[СМЫСЛ] «Слова». Подпись количества в упражнениях против имени развилки «Слова» — разные места и разные роли, совпало только слово.',
  'catVocab_time + time':
 ...
 */
const KNOWN_GROUPS = 30;   // +1 04.09: label_words + wordsGroup · +1 07.09: cakeSortDesc + pizzaSortDesc (одно правило, разная еда) · +1 16.09: a11yErase + puzzleSecondErase (подпись для чтения с экрана против видимой подписи второго действия)
