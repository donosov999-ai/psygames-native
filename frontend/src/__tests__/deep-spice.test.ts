...
describe('приправа Бездны: её видно на экране', () => {
  ...
  /**
   * ⚠️ ЗДЕСЬ СТОЯЛ ЛИТЕРАЛ, И ОН УРОНИЛ MAIN 11.09.2026, ЗАДЕРЖАВ ВЫПУСК.
   * Сторожилась строка снимка ЦЕЛИКОМ:
   *   `'({ preset, spice, seed, path, grids, marks, errors, elapsed'`
   * В снимок добавились два поля (`rating`, `band` — ступень трудности, коммит
   * 20252e09). Приправа осталась на месте, дефекта не возникло, продолжение партии
   * работало — а проба покраснела, потому что сравнивала ПОРЯДОК СЛОВ в исходнике.
   *
   * 🔴 УРОК В ОДНУ СТРОКУ: греп по исходнику отвечает на вопрос «как написано», а
   * сторожить надо «что величина переживает сохранение и подъём». Первое ломается от
   * любой соседней правки, второе — только от настоящего дефекта.
   *
   * Теперь здесь разбирается СПИСОК полей (порядок не важен, новые поля не мешают), а
   * сам круг «сохранил → поднял → собралась та же доска» проверяется поведением, на
   * настоящем экране с настоящими нажатиями:
   *   __tests__/fractal-deep-band-reaches-board.test.tsx,
  ...
  it('снимок партии помнит приправу — иначе продолжение соберёт другую доску', () => {
    ...
    const тело = screen.match(/const snapshot = \(\): DeepResume => \(\{([^}]*)\}\)/)?.[1];
    expect(`строка снимка найдена: ${тело !== undefined}`).toBe('строка снимка найдена: true');
    const поля = (тело ?? '').split(',').map((x) => (x.split(':')[0] ?? '').trim()).filter(Boolean);

    // Всё, что ЗАДАЁТ ДОСКУ, обязано лежать в снимке: полоса и приправа входят в
    // выбор доски узла (pickBoard, spiceLeaf), пресет — в размер дерева.
    for (const поле of ['preset', 'spice', 'rating', 'seed', 'grids']) {
      expect(`${поле} в снимке: ${поля.includes(поле)}`).toBe(`${поле} в снимке: true`);
    }
  });
});
