...
describe('маршруты игр', () => {
  ...
  const AWAITING_CATALOG: Record<string, string> = {
    /*
     * 09.09.2026 — пространственный пакет из лаборатории: «Сеть труб» и «Поворот
     * чисел» на общем ядре поворотов (`src/games/spatial-core`), по 50 ступеней
     * каждая. Экран, ядро и графика приехали этим коммитом; строка в `games.ts`,
     * ключи словаря на двенадцать языков, иконка, раздел и место в профилях —
     * следующим заходом-интегратором, ровно как было с семью играми 19.08.
     *
     * ⚠️ Пока записи нет, экран доступен по прямому маршруту, но в каталоге не
     * значится и в подборки зарядки не попадает.
     */
    ...
    //
    // ⚠️ И он понадобился снова 07.09.2026: «Пицца» — тот же экран, что у
    // «Тортов» (тонкий маршрут `CakeSortScreen` со своей шкуркой и своим
    // `gameId`), поэтому маршрут и картинки едут своим коммитом, а строка в
    // `games.ts`, ключи словаря и место в профилях — заходом-интегратором.
  };
  ...
  it('каждый экран игры есть в реестре — иначе игра написана, но недоступна', () => {
    ...
    // К ним же приравнены расформированные развилки: карточки в каталоге нет,
    // файл оставлен по прямому указанию Дениса «удалять только по отдельному
    // слову» (12.09.2026). Список и сторож при нём — в _helpers/hubScreens.ts,
    // проба `расформированные развилки` следит, чтобы он не протух.
    const NOT_GAMES = new Set(['_layout', ...[...РАСФОРМИРОВАННЫЕ].map((f) => f.replace(/\.tsx$/, ''))]);
