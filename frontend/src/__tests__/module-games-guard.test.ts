...
describe('экраны-обёртки лабораторных модулей', () => {
  ...
  it('🔴 каркас действительно рисует плашку паузы', () => {
    ...
    // 17.09.2026: и не поверх вопроса «Начать заново?» из ряда значков под полем (`спросЗаново`).
    expect(flat).toContain('{paused && !exitGuard.asking && !спросЗаново && (');
  });
...
describe('часы партии идут по игровым, а не по настенным', () => {
  it('🔴 модулю подан gameNow, а не Date.now', () => {
    ...
    for (const f of Object.keys(LIVE_PREDICATES)) {
      ...
      /**
       * 🔴 ВСТРОЕННАЯ СТРАНИЦА ЧАСЫ ПРОПОМ НЕ ПРИНИМАЕТ — И ЭТО НЕ ЛАЗЕЙКА.
       *
       * «Пауза» с 27.08.2026 показывает страницу «Зарядки» целиком, готовой
       * сборкой в `<iframe>`. Часы там свои, внутри страницы, и подать туда
       * `now` нечем: проп не пересекает границу окна.
       *
       * Требование гейта при этом остаётся В СИЛЕ — партия обязана вставать,
       * когда приложение на паузе. Только выполняется оно не пропом, а
       * передачей САМОЙ ПАУЗЫ внутрь: экран подписан на `onGameHold` и шлёт
       * странице `warmup:hold`, а её `embed.js` жмёт собственную кнопку паузы.
       * Поэтому здесь проверяется то же самое по сути: подписка на общую паузу
       * есть, отправка внутрь есть, и всё это из общего места.
       *
       * ⚠️ Послабление ИМЕННОЕ и узкое: только для экрана, который рисует
       * партию встроенной страницей. Любой другой экран без `now` падает, как
       * падал.
       */
      const встроеннаяСтраница = /<WarmupPage\b/.test(play) || /<WarmupPage\b/.test(src);
      if (встроеннаяСтраница) {
        const модуль = code(read('src/games/pause/ui/WarmupPage.tsx'));
        /**
         * ⚠️ ПРОВЕРЯЕТСЯ ВЫЗОВ, А НЕ УПОМИНАНИЕ. Первая редакция искала слово
         * `onGameHold` — и осталась зелёной, когда вызов убрали: слово никуда
         * не делось, оно стоит в строке импорта. Гейт, который удовлетворяется
         * наличием текста, проверяет текст, а не поведение. Поэтому здесь ищем
         * `onGameHold(` со скобкой — то есть подписку, — и отдельно `return`
         * перед ней: подписка без снятия течёт при уходе с экрана.
         */
        if (!/onGameHold\s*\(/.test(модуль)) bad.push(`${f}: страница не подписана на общую паузу`);
        if (!/return\s+onGameHold\s*\(/.test(модуль)) bad.push(`${f}: подписка на паузу не снимается`);
        if (!/warmup:hold/.test(модуль)) bad.push(`${f}: пауза не передаётся внутрь страницы`);
        if (!/from '@\/src\/services\/gamePause'/.test(модуль)) bad.push(`${f}: пауза не из общего места`);
        continue;
      }
