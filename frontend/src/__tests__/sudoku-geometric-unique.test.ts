 ...
 * Термометр (42–45), стрелка (46–49), термоклетка (50–53) и джигсо (62–65) не
...
// 23.09: джигсо переехал на 62–65 (выше пояса ALS) — вершина его полосы теперь L65.
const CASES: [number, string][] = [[45, 'thermo'], [49, 'arrow'], [53, 'thermocage'], [65, 'jigsaw']];
...
describe('геометрические варианты: у выданной доски ровно одно решение', () => {
  it('есть что проверять — уровни те самые, и геометрия на доске есть', () => {
    ...
    const fx = buildAt(65);   // 27.08 джигсо стал вершиной вариантов, 23.09 переехал выше пояса ALS
    if (!fx) { console.log('бюджет: фикстура L65 не добрана — пропуск'); return; }
    const { cfg, gen } = fx;
  ...
  it('🔴 свой перебор действительно умеет находить ВТОРОЕ решение', () => {
    ...
    //
    /**
     * 🔴 ПРОБА ПРО ПЕРЕБОР — ЗНАЧИТ И ДОСКА ДОЛЖНА БЫТЬ ГАРАНТИРОВАННО НЕОДНОЗНАЧНОЙ.
     *
     * Здесь проверка шла по НАСТОЯЩЕЙ доске уровня 53: снимали по одной подсказке и
     * ждали, что хоть раз появится второе решение. Это не гарантировано ничем.
     * Термометры и клетки-суммы доопределяют снятую цифру, и доска остаётся
     * однозначной — комментарий прошлой редакции честно отмечал флак четыре раза за
     * 29.08, а контрольный выстрел «оставить 8 подсказок» лишь понизил вероятность,
     * не убрав причину: восьми подсказок вместе с термометрами и клетками тоже может
     * хватать на однозначность. 02.09 упало снова, уже в сборке.
     *
     * Утверждение проверки — «наш перебор УМЕЕТ найти второе решение». Для него
     * нужна доска, у которой решений заведомо больше одного, и такая берётся не из
     * генератора, а из определения: ПУСТАЯ обычная сетка 9×9 (вариант «none»). У неё решений
     * порядка 6·10²¹, и любой исправный перебор обязан дойти до второго. Никакой
     * зависимости от удачи генератора здесь больше нет.
     *
     * ⚠️ Вариантные ограничения сюда НЕ передаём намеренно: они и были источником
     * недоказуемости. Умение считать варианты проверяют пробы ниже — там, где доска
     * приходит от генератора вместе со своими ограничениями.
     */
    const пусто: Cell[][] = Array.from({ length: 9 }, () => Array(9).fill(0) as Cell[]);
    const решений = countOwn(пусто, 9, 3, 3, 'none', undefined, undefined, undefined, undefined, 2);
    expect(`перебор нашёл второе решение: ${решений >= 2}`).toBe('перебор нашёл второе решение: true');
  }, 300000);
