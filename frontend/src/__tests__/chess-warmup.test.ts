...
// Замер живой базы — та же величина, которой сборщик набирает бюджет.
...
describe('шахматная зарядка', () => {
  ...
  /**
   * ⚠️ 09.09.2026 У ЭТОЙ ПРОБЫ ИЗМЕНИЛАСЬ ПРЕМИСА, а не результат — правка в
   * общем сборщике `chessWarmup.ts::темаШаги` (раздел «Слова»).
   *
   * Бюджет набирается ЗАМЕРЕННЫМ временем (медианы живой базы + переход), а
   * `est_duration_sec` осталось ЗАПАСОМ на игры, по которым замера ещё нет.
   * Складывать здесь объявленные числа значит мерить не ту величину: у шахмат
   * они завышены (90 объявлено против ~30 замеренных), поэтому после починки в
   * тот же бюджет помещается больше шагов, и сумма ОБЪЯВЛЕННЫХ вылезает за край,
   * хотя человек укладывается.
   *
   * Само требование не изменилось и проверяется по-прежнему: набор обязан
   * укладываться в выбранное время. Только время теперь настоящее.
   */
  it('🔴 «задавать время»: набор укладывается в выбранную длительность', () => {
    ...
    for (const m of ДЛИТЕЛЬНОСТИ) {
      const total = chessWarmupSteps({ minutes: m, blindLevel: 3, mateLevel: 7 })
        .reduce((a, x) => a + estimateStepSec(x), 0);
