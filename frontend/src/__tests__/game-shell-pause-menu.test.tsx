...
import { isGameHeld, holdGame, __resetGameClock } from '@/src/services/gamePause';
...
describe('меню паузы GameShell — стрелка «назад» открывает меню', () => {
  ...
  /**
   * 🔴 «ПРОДОЛЖИТЬ ИГРУ» ЗАКРЫВАЕТ МЕНЮ САМ, А НЕ ЖДЁТ ОБЩЕГО СЧЁТЧИКА.
   *
   * ПОВОД — ЖИВОЙ ОТЧЁТ. «Релакс», 15.09.2026, версия 2.54.11, судоку, уровень 60,
   * партия шла 12:39: «Кнопка продолжить игру не работает, не могу вернуться в игру».
   * Кадр показывает исправно нарисованное меню — значит дефект не в вёрстке.
   *
   * ПРИЧИНА. Меню видно, пока `paused`, а `paused` — это отражение ОБЩЕГО счётчика
   * пауз (`gamePause`, подписка `onGameHold`). Пункт «Продолжить» снимал только СВОЮ
   * задержку и надеялся, что счётчик дойдёт до нуля. Держателей в приложении четыре:
   * FeedbackWidget, GameHelpOverlay и две в самом каркасе. Держит хоть один — счётчик
   * не ноль, `paused` не гаснет, меню не закрывается, и кнопка выглядит мёртвой.
   * У неё это воспроизводится постоянно: она открывает «Сообщить о проблеме» ИЗ ЭТОГО
   * ЖЕ меню, а окно отзыва берёт свою задержку.
   *
   * ⚠️ Проба берёт ЧУЖУЮ задержку настоящим `holdGame()`, а не подменой: подменённый
   * счётчик доказывал бы поведение подмены, а не каркаса.
   */
  it('🔴 1б. «Продолжить» закрывает меню, даже когда игру держит кто-то ещё', () => {
    const tr = смонтировать({ pauseActions: меню() });
    нажать(tr, 'game-back');
    expect(`меню открыто: ${есть(tr, 'game-pause-menu')}`).toBe('меню открыто: true');

    // Кто-то ещё держит игру — так делает окно отзыва, открытое из этого же меню.
    const чужаяЗадержка = holdGame();
    нажать(tr, 'pause-action:resume');
    /**
     * ⚠️ СНИМАЕМ ЧУЖУЮ ЗАДЕРЖКУ ДО ПРОВЕРОК, А НЕ ПОСЛЕ. Упавший `expect` бросает, и
     * строка после него не выполнится — задержка утечёт в следующие пробы и уронит
     * их все (проверено: при первом заходе так и вышло, красными стали ещё три).
     * Счётчик пауз живёт в модуле, `__resetGameClock` его не трогает.
     */
    const менюПосле = есть(tr, 'game-pause-menu');
    const держалосьЧужим = isGameHeld();
    чужаяЗадержка();

    expect(`меню после «Продолжить»: ${менюПосле}`).toBe('меню после «Продолжить»: false');
    // Чужая задержка при этом НЕ снимается — она не наша, часы остаются стоять.
    expect(`чужая задержка цела: ${держалосьЧужим}`).toBe('чужая задержка цела: true');
  });
