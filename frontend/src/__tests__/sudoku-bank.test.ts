...
describe('банк: варианты не тронуты', () => {
  it('в банке только классика — все уровни с правилом варианта идут мимо него', () => {
    ...
    // Лестница 23.09: классика 9×9 — уровни 5–8 (вход), 54–61 (пояс ALS) и 66–80 (цепи,
    // легенда). Кривые блоки переехали выше пояса, на 62–65 (задача 3d4d4578), поэтому
    // банковский кусок теперь идёт двумя отрезками, а не одним.
    expect(classic).toEqual([
      5, 6, 7, 8,
      ...Array.from({ length: 8 }, (_, i) => 54 + i),
      ...Array.from({ length: 15 }, (_, i) => 66 + i),
    ]);
