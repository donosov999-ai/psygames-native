...
describe('раунд играется по-настоящему', () => {
  /**
   * ⚠️ Таймаут задан явно: тест рисует настоящее дерево экрана и идёт около
   * четырёх секунд при стандартном пороге в пять. В одиночку проходит, в полном
   * параллельном прогоне не успевает — и краснеет не на поломке, а на нехватке
   * ядер. См. тот же разбор в `level-times.test.ts`.
   */
