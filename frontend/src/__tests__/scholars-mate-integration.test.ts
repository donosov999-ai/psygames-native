...
/**
 * 🔴 МАТ С ЖЕРТВОЙ ДОИГРЫВАЕТСЯ ДО МАТА.
 *
 * 📍 Было: верность решалась строкой `line[0] === uci` — человек отдавал
 * фигуру, получал «✓» и ехал дальше, а мат не ставил ни разу на всех 371
 * позиции. Жертва без мата — это просто отданная фигура.
 */
describe('«Детский мат»: жертва играется целиком', () => {
  /** Уровень с жертвами и зерно, где первая позиция — линия в 2–3 хода. */
  function сЖертвой(): { L: number; seed: number } | null {
    for (let L = 29; L <= 40; L++) {
      for (let seed = 1; seed <= 20; seed++) {
        const первая = buildDeck(L, seed)[0];
        if (первая?.kind === 'sacrifice' && (первая.line?.length ?? 0) > 1) return { L, seed };
      }
    }
    return null;
  }

  it('есть что проверять: позиция с линией нашлась', () => {
    expect(сЖертвой()).toBeTruthy();
  });

  it('🔴 первый ход НЕ засчитывается как решение — партия продолжается', () => {
    const где = сЖертвой()!;
    const с = стол(где.L, где.seed);
    const задача = buildDeck(где.L, где.seed)[0]!;
    const первый = задача.line![0]!;
    с.тап(первый.slice(0, 2));
    с.тап(первый.slice(2, 4));
    // Ни «✓», ни «✕»: позиция та же, ждут следующий ход.
    expect(с.текст()).not.toContain('✓');
    expect(с.текст()).toContain(`1 / ${levelParams(где.L).count}`);
  });

  it('🔴 вся линия доигрывается, и только тогда приходит «верно»', () => {
    const где = сЖертвой()!;
    const с = стол(где.L, где.seed);
    const задача = buildDeck(где.L, где.seed)[0]!;
    const наши = задача.line!.filter((_, i) => i % 2 === 0);
    for (const ход of наши) {
      с.тап(ход.slice(0, 2));
      с.тап(ход.slice(2, 4));
    }
    expect(с.текст()).toContain('✓');
  ...
  it('🔴 неверный ход в середине линии закрывает попытку', () => {
    const где = сЖертвой()!;
    const с = стол(где.L, где.seed);
    const задача = buildDeck(где.L, где.seed)[0]!;
    const g = new (require('chess.js').Chess)(shownFen(задача));
