...
describe('«Детский мат»: партия на живом дереве', () => {
  ...
  it('🔴 вопрос на экране соответствует виду ПЕРВОЙ позиции колоды', () => {
    ...
    // ⚠️ Только уровни, где вид ОБЪЯВЛЯЕТСЯ. С 29-го он намеренно скрыт — ось
    // «непредсказуемость», рубеж и довод в `объявлятьВид`.
    for (const L of [1, 15, 25, 27]) {
  ...
  /**
   * 🔴 НА ВЕРХНИХ СТУПЕНЯХ ВИД ЗАДАНИЯ НЕ ОБЪЯВЛЯЕТСЯ, А ПОСЛЕ ОТВЕТА НАЗЫВАЕТСЯ.
   *
   * Ось «непредсказуемость» (правило «потолков нет нигде», 06.09.2026): с 29-го
   * уровня в колоде `sacrifice` и `fromGames`, оба — «сделай ход», и отличить их
   * можно только по позиции. Половину задачи раньше выдавали в готовом виде.
   *
   * ⚠️ Скрытый вопрос — задача, а не загадка без разгадки: постфактум вид обязан
   * быть назван, иначе человек лишён обратной связи.
   */
  it('🔴 с 29-го уровня вид задания скрыт, а после ответа назван', () => {
    const ожидаемый: Record<string, string> = {
      mate: LABELS.mate, fromGames: LABELS.mate,
      defend: LABELS.defend, threat: LABELS.threat, sacrifice: LABELS.sacrifice,
    };
    const L = 33;
    const первая = buildDeck(L, 1)[0]!;
    expect(`первая позиция ур.${L} — ход, а не «да/нет»: ${первая.kind !== 'threat'}`)
      .toBe(`первая позиция ур.${L} — ход, а не «да/нет»: true`);

    const с = стол(L, 1);
    const свой = ожидаемый[первая.kind]!;
    expect(`до ответа вид назван: ${с.текст().includes(свой)}`).toBe('до ответа вид назван: false');
    expect(`знак вопроса на месте: ${с.текст().includes('?')}`).toBe('знак вопроса на месте: true');

    // Отвечаем — верно или нет, неважно: вид обязан появиться в обоих случаях.
    const uci = первая.solutions[0]!;
    с.тап(uci.slice(0, 2));
    с.тап(uci.slice(2, 4));
    expect(`после ответа вид назван: ${с.текст().includes(свой)}`).toBe('после ответа вид назван: true');
  });
...
/**
 * 🔴 МЕРЯЕТСЯ УЗНАВАНИЕ, А НЕ СКОРОСТЬ ПАЛЬЦА.
 *
 * 📍 Ход делается двумя тапами. По Фиттсу два тапа по доске такого размера
 * стоят ≈1,21 с плюс саккады — у полного времени есть пол в районе 1,7–2,2 с,
 * ниже которого оно не опустится, сколько ни узнавай. Само узнавание знакомого
 * куска позиции — порядка 50 мс. Поэтому в медиану идёт время до ПЕРВОГО
 * касания: палец идёт к фигуре, которую глаз уже нашёл.
 */
describe('«Детский мат»: в медиану идёт узнавание', () => {
  it('🔴 время до первого касания меньше полного времени позиции', () => {
    const с = стол(1);
    const задача = buildDeck(1, 1)[0]!;
    const uci = задача.solutions[0]!;
    с.тап(uci.slice(0, 2));      // первое касание
    с.тик(2000);                  // «думает» между тапами
    с.тап(uci.slice(2, 4));
    с.тик(700);
    // Доигрываем подход, чтобы получить итог.
    for (const z of buildDeck(1, 1).slice(1)) {
      const u = z.solutions[0]!;
      с.тап(u.slice(0, 2));
      с.тап(u.slice(2, 4));
      с.тик(700);
    }
    const r = с.итоги[0]!;
    expect(r.medianMs).toBeGreaterThan(0);
    expect(r.medianFullMs).toBeGreaterThan(0);
    // Пауза между тапами вошла в полное время и НЕ вошла в узнавание.
    expect(r.medianFullMs).toBeGreaterThan(r.medianMs);
    // И первая попытка это показывает поимённо.
    const первая = r.attempts[0]!;
    expect(`${первая.msFirst} < ${первая.ms}`).toBe(`${первая.msFirst} < ${первая.ms}`);
    expect(первая.msFirst).toBeLessThan(первая.ms - 1500);
  });

  it('🔴 прозеванная позиция не даёт нулевого «узнавания»', () => {
    const с = стол(1);
    с.тик(levelParams(1).seconds * 1000 + 300);
    с.тик(1600);
    const r = с.итоги[0] ?? null;
    // Итога ещё нет — смотрим попытку через следующую позицию: главное, что
    // прозеванная не приносит msFirst = 0, иначе медиана «улучшалась» бы зевками.
    expect(r).toBeNull();
    expect(с.вехи).toContain(true);
  });
});
...
/**
 * 🔴 ПОДХОД, КОТОРОГО НЕ БЫЛО, НЕ ДОЛЖЕН НИЧЕГО СТОИТЬ.
 *
 * 📍 НАЙДЕНО В ЖИВЫХ ДАННЫХ 05.09.2026, а не придумано: три подхода подряд с
 * 0 решённых, 10–11 «ошибками» и нулевой медианой. Прогон тех же колод через
 * компонент даёт 10 из 10 — механика цела, экран просто оставили открытым, и
 * секундомер добил позиции таймаутами. Цена: доля верных 0 меньше порога,
 * значит уровень ПОНИЖАЛСЯ за игру, которой не было.
 */
describe('«Детский мат»: открытый экран — это не подход', () => {
  it('🔴 подход без единого касания помечен как несыгранный', () => {
    const с = стол(1);
    const п = levelParams(1);
    // Просто ждём: ни одного тапа за всю колоду.
    for (let i = 0; i < п.count; i++) {
      с.тик(п.seconds * 1000 + 300);
      с.тик(1600);
    }
    expect(с.итоги.length).toBe(1);
    const r = с.итоги[0]!;
    expect(`решено ${r.solved}, касались ${r.touched}`).toBe('решено 0, касались false');
  });

  it('🔴 подход хотя бы с одним ответом — сыгранный, даже если ответ неверный', () => {
    const с = стол(1);
    const задача = buildDeck(1, 1)[0]!;
    const g = new (require('chess.js').Chess)(shownFen(задача));
    const мимо = (g.moves({ verbose: true }) as { from: string; to: string }[])
    ...
    с.тап(мимо.slice(2, 4));
    с.тик(1600);
    // Остальные позиции досиживаем таймаутами.
    for (let i = 1; i < levelParams(1).count; i++) {
      с.тик(levelParams(1).seconds * 1000 + 300);
      с.тик(1600);
    }
    const r = с.итоги[0]!;
