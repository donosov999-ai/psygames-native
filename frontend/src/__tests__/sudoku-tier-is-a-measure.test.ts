...
describe('ступень — величина, а не отметка о срабатывании', () => {
  ...
  it('🔴 длина и цена вывода считаются у КАЖДОЙ доски, включая нерешённую', () => {
     ...
     * и на нерешаемой `steps` законно остаётся нулём.
     *
     * 🔴 НО «НА ВОСЬМИ ДЫРКАХ РЕШАТЕЛЬ РАБОТАЕТ ВСЕГДА» — ЭТО БЫЛО НЕВЕРНО, И ЦЕНА
     * ОШИБКИ ВЫШЛА В КРАСНЫЙ CI. 📍 Замер 11.09.2026, 600 досок на восьми дырках:
     * нерешаемых 6 (1,00 %), `steps === 0` у двух. Слово «всегда» стояло без замера,
     * и проба падала примерно раз на сотню — сегодня дважды подряд в CI.
     *
     * Одна доска здесь и не нужна: проверяется, что у РЕШЁННОЙ доски `steps` и `cost`
     * — числа, а не что генератор не промахивается. Поэтому берём первую решаемую из
     * сорока попыток. При доле промаха 1 % сорок мимо подряд — это 10⁻⁸⁰, то есть
     * детерминированно; а если однажды не наберётся, сообщение скажет ровно это, и
     * читатель не будет искать дефект в `steps`.
     */
    let g = generatePuzzle(8, N, BR, BC, 'towers');
    let towers = (g as unknown as { towers?: TowersMap }).towers;
    let r = gradePuzzle(g.puzzle, { N, BR, BC, variant: 'towers', towers });
    let попыток = 1;
    while (!r.solved && попыток < 40) {
      g = generatePuzzle(8, N, BR, BC, 'towers');
      towers = (g as unknown as { towers?: TowersMap }).towers;
      r = gradePuzzle(g.puzzle, { N, BR, BC, variant: 'towers', towers });
      попыток += 1;
    }
    expect(`решаемая доска нашлась за ${попыток} попыток из 40: ${r.solved}`)
      .toBe(`решаемая доска нашлась за ${попыток} попыток из 40: true`);
...
describe('небоскрёбы: вариант обязан заслуживать своё имя на ВСЕЙ лестнице', () => {
  ...
  /**
   * 🔴 ПОРОГ СТОЯЛ ВНУТРИ СОБСТВЕННОГО ШУМА — ПРОБА МИГАЛА И ДВАЖДЫ ВАЛИЛА ТЕГ 2.53.7.
   *
   * Прежняя редакция брала 30 досок и требовала «не меньше пятой части», то есть 6.
   * 📍 ЗАМЕР 11.09.2026, пятнадцать прогонов подряд, по 30 досок каждый:
   *   5 · 7 · 9 · 10 · 11 · 11 · 12 · 12 · 13 · 13 · 13 · 14 · 15 · 15 · 16
   * Среднее 12,4 из 30 (41 %), разброс 5…16. Порог 6 отстоит от среднего меньше
   * чем на два с половиной разброса — и один прогон из пятнадцати (тот, что дал 5)
   * покраснел. Это не дефект генератора, это бросок монеты: ровно та же природа,
   * что у `sudoku-selection-axis`, где пришлось мерить распределение, а не верить
   * одному прогону.
   *
   * ЧИНИМ НЕ ОПУСКАНИЕМ ПОРОГА, А РАЗМЕРОМ ВЫБОРКИ. Девяносто досок вместо
   * тридцати стоят полсекунды, зато разброс доли падает. Доля при этом ОСТАЛАСЬ
   * ТА ЖЕ, что задумал автор, — «не меньше пятой части», то есть 18 из 90.
   * 📍 Перезамер, те же пятнадцать прогонов по 90 досок: 24 · 25 · 28 · 29 · 32 ·
   * 33 · 33 · 36 · 36 · 37 · 37 · 39 · 39 · 39 · 41. Минимум 24 — на шесть выше
   * порога, среднее 34 — на три с половиной разброса. Прежние «6 из 30»
   * выполнялись шумом, нынешние «18 из 90» — нет.
   *
   * 📍 ПРОБА КУСАЕТСЯ, И ВОТ ЧЕМ ЭТО ПОМЕРЕНО. Углубление верхней ступени на шесть
   * дырок роняет долю решаемых с 20/60 до 8/60 (33 % → 13 %) — это ниже порога, и
   * гейт краснеет. ⚠️ Попутно вскрылось, что дальше углублять НЕЧЕМ: при запросе
   * +12 и +20 генератор отдаёт те же 22,6 и 23,6 дырки вместо 34 и 42, потому что
   * больше не снять, не потеряв единственность. Верхняя ступень башен УЖЕ стоит
   * у потолка генератора — это отдельная находка, и мутацию «сделать ещё глубже»
   * сильнее уже не поставить.
   */
  it('🔴 верхняя ступень остаётся РЕШАЕМОЙ — глубина не может расти бесконечно', () => {
    ...
    expect(`решаемых на верхней ступени ${решено}/${ДОСОК} — не меньше ${ПОЛ}: ${решено >= ПОЛ}`)
      .toBe(`решаемых на верхней ступени ${решено}/${ДОСОК} — не меньше ${ПОЛ}: true`);
  });
});
