 ...
 */
/**
 * 🔴 ОТКРЫТИЕ «БЕЗДНЫ» НЕ МАТЕРИАЛИЗУЕТ ВСЁ ДЕРЕВО.
 *
 * ЗАМЕР 11.09.2026: нажатие «Начать» на пресете abyss вызывало 2548 материализаций —
 * то есть всю партию целиком. Весь смысл services/fractal-deep — ленивая
 * материализация: «полная партия глубины 3 — это тысячи досок; породить их разом
 * нельзя ни по времени, ни по памяти» (шапка файла). Экран это сводил на нет, и
 * человек с телефона получал многосекундное замирание на открытии.
 *
 * ПРИЧИНА. Отрисовка корня зовёт `deepValueAt` на каждую кормимую клетку, та —
 * `deepNodeDone(ребёнок)`, а он `deepNodeProgress`, который спрашивает СВОИХ детей.
 * Рекурсия доходит до дна.
 *
 * ⚠️ ПОТОЛОК — ХРАПОВИК ВНИЗ. Число может только уменьшаться: выросло — красная,
 * упало — красная с просьбой опустить потолок, чтобы достижение закрепилось.
 */
describe('открытие Бездны остаётся ленивым', () => {
  /**
   * Известный потолок на 16.09.2026. ЗАМЕРЕНО 49 — корень плюс дешёвые «призраки»
   * его кормимых клеток; было 2548. Потолок 60 оставляет запас на другую доску из
   * полосы и при этом не спрячет возврат рекурсии. Менять ТОЛЬКО в меньшую сторону.
   */
  const ПОТОЛОК = 60;

  it(`🔴 открытие abyss материализует не больше ${ПОТОЛОК} узлов, а не всё дерево`, () => {
    const tr = поднять();
    жать(tr, 'deep-preset-abyss');
    mockПолосы.length = 0;
    жать(tr, 'deep-start');
    const было = mockПолосы.length;
    expect(`материализаций ${было} при потолке ${ПОТОЛОК}`)
      .toBe(`материализаций ${Math.min(было, ПОТОЛОК)} при потолке ${ПОТОЛОК}`);
  });
});
