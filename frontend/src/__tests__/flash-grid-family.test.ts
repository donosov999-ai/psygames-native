...
describe('семейство «сетка со вспышкой»', () => {
  ...
  it('🔴 каждая отвечает звуком на действие игрока', () => {
    /**
     * ⚠️ Проверяем СМЫСЛ, а не имя функции. Первая редакция искала `snd*(` — и
     * покраснела, когда звук стали брать из `hapticSuccess()` (он звучит сам,
     * см. `juice/haptics.ts`), то есть на правке, которая УБРАЛА двойной «динь».
     * Годится любой источник ответа: свой звук или хаптик-обёртка.
     */
    for (const f of FAMILY) {
      const answers = /snd[A-Za-z]*\(|haptic(Success|Error)\(/.test(G(f));
      expect(`${f}: ${answers}`).toBe(`${f}: true`);
    }
  });
