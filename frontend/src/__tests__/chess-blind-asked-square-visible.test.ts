...
describe('«Доска в уме»: спрашиваемая клетка и фигура на ней', () => {
  ...
  it('🔴 при ВЕРНОМ ответе клетка зеленеет, а не молчит', async () => {
    ...
    /**
     * 🔴 ПОКАЗАННАЯ РАССТАНОВКА — НЕ ИТОГОВАЯ, И С 12.09.2026 ЭТО ВЕРНО УЖЕ НА ПЕРВОМ
     * УРОВНЕ. Фишки ходят вслепую, и спрашивают про позицию ПОСЛЕ ходов. Первая
     * редакция пробы отвечала по показанной расстановке — то есть заведомо неверно,
     * как только на первой ступени появился ход, — и краснела на исправном коде
     * («знаем, что стояло на c6: false»: клетка была пуста, а ход привёл туда фигуру).
     *
     * Ход прослеживается по фишкам: они одинаковы по типу, но занятость клетки видна,
     * а цвет фишки хранит сторону. Клетка, которая освободилась, — «откуда»; которая
     * занялась — «куда». На первом уровне ход ОДИН, поэтому разность множеств точна.
     */
    const занятые = () => new Set([...клетки(r)].filter(([, к]) => к.findAll((n: any) => {
      const st = плоский(n.props?.style);
      const c = String(st.backgroundColor ?? '').toLowerCase();
      return c === '#cbd5e1' || c === '#475569';
    }).length > 0).map(([имя]) => имя));

    let былиЗаняты = new Set<string>();
    for (let шаг = 0; шаг < 40 && !былиЗаняты.size; шаг += 1) { await осесть(1, 200); былиЗаняты = занятые(); }
    expect(`фишки на доске появились: ${былиЗаняты.size > 0}`).toBe('фишки на доске появились: true');

    expect(await доОпроса(r)).toBe(true);
    const сталиЗаняты = занятые();
    const откуда = [...былиЗаняты].filter((к) => !сталиЗаняты.has(к));
    const куда = [...сталиЗаняты].filter((к) => !былиЗаняты.has(к));
    // Итоговая расстановка = показанная плюс прослеженные ходы.
    const итог = new Map(расстановка);
    if (откуда.length === 1 && куда.length === 1) {
      const кто = расстановка.get(откуда[0]!);
      итог.delete(откуда[0]!);
      if (кто) итог.set(куда[0]!, кто);
    }

    const имя = спрошеннаяКлетка(r)!;
    const нужная = итог.get(имя);
    expect(`знаем, что стоит на «${имя}» ПОСЛЕ ходов (прослежено ${откуда.length}→${куда.length}): ${!!нужная}`)
      .toBe(`знаем, что стоит на «${имя}» ПОСЛЕ ходов (прослежено ${откуда.length}→${куда.length}): true`);
