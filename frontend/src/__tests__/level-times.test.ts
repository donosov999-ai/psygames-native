...
describe('где время показывать нельзя', () => {
   ...
   * «дольше = лучше» у охвата, восстановление и успокоение.
   *
   * ⚠️ СУДОКУ ОТСЮДА УШЛА 20.08.2026, И НЕ ПОТОМУ, ЧТО «ПЕРЕДУМАЛИ». Обе доски стояли
   * здесь ровно из-за одной беды: самурай писал партии в корзину `sudoku` со своим
   * номером уровня, и «лучшее время уровня 5» у классической было бы отчасти про поле
   * 21×21. Корзины разведены — причина исчезла, и обе игры переехали в белый список
   * ниже. Проба на разведение стоит отдельно, в самом низу файла.
   */
...
describe('белый список сходится с экранами', () => {
  ...
  /**
   * 🔴 РАЗБОР ИДЁТ ПО КОДУ БЕЗ КОММЕНТАРИЕВ, И ЭТО НЕ ПРЕДОСТОРОЖНОСТЬ, А ПОЙМАННАЯ
   * ОШИБКА. В `sudoku.tsx` объяснение к `GAME_ID` цитирует соседний гейт словами
   * «ищет в экране ЛИТЕРАЛ `const GAME_ID = '…'`». Разбор ниже брал ПОСЛЕДНЕЕ
   * присваивание, то есть вычитывал имя игры из цитаты и получал многоточие. Пока
   * судоку не было в белом списке, эта поломка ничего не красила: имя, которого не
   * ищут, может разбираться как угодно. Стоило внести судоку — и гейт покраснел на
   * исправном экране. Тот же приём, что в развилке (`sudoku-hub.test.ts`).
   */
  ...
  const SCREENS = FILES.map((f: string) => {
    const src = stripComments(readFileSync(join(GAMES_DIR, f), 'utf8') as string);
    const c = consts(src);
    return { file: f, maps: mapIds(src, c), buckets: sessionIds(src, c) };
  });

  it('срез комментариев работает — иначе разбор читает имя игры из объяснения', () => {
    expect(stripComments("/* цитата: const GAME_ID = '…' */\nconst GAME_ID = 'sudoku';\n"))
      .not.toContain('…');
    expect(consts(stripComments("/* const GAME_ID = '…' */\nconst GAME_ID = 'sudoku';\n")).GAME_ID)
      .toBe('sudoku');
    // …и на живом экране, а не только на синтетике: у судоку такая цитата в шапке есть.
    expect(SCREENS.find((s: any) => s.file === 'sudoku.tsx')?.maps).toContain('sudoku');
  });
...
describe('карта показывает время', () => {
  ...
  /**
   * ⚠️ ТАЙМАУТ ЗАДАН ЯВНО, И ЭТО НЕ ПОДГОНКА. Тест рисует НАСТОЯЩЕЕ дерево карты
   * уровней; стандартные пять секунд jest рассчитаны на чистую логику, а не на
   * рендер. Он укладывался в них впритык и начал падать, когда карта обросла
   * подписями ступеней и временем — то есть краснел не на поломке, а на
   * собственном росте. Ложная краснота хуже отсутствия проверки: набор, который
   * иногда красный без причины, перестают читать.
   */
