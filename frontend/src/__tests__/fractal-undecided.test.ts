...
describe('экран', () => {
  ...
  /**
   * ⚠️ Проверяем СВЯЗЬ «доказуемый конфликт → ошибка», а не дословную запись ветки.
   * 02.09.2026 в ту же ветку добавилось объяснение красной цифры (первая ошибка
   * партии говорится словами), и однострочная регулярка покраснела на правке,
  ...
  it('🔴 ошибка засчитывается только за доказуемый конфликт', () => {
    ...
    expect(SRC).toMatch(/else if \(provable\) \{[\s\S]{0,400}?setErrors\(\(e\) => e \+ 1\)/);
    // И ветка «не ошибка» по-прежнему НЕ трогает счётчик.
    expect(SRC).toMatch(/else \{[\s\S]{0,200}?setUndecided\(true\)/);
  });
