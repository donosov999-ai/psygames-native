...
describe('единая геометрия раздела «Поиск»', () => {
  ...
  it('🔴 по окну ответ не угадать сверх неизбежного — если все возможные ответы помещаются, окно их накрывает', () => {
    // Замер 17.09.2026 на VER 1: игрок, запомнивший для каждого окна самый частый ответ, угадывал на L1–3 и L31+
    // 45 % при потолке 33 % (возможных ответов три), на L5–8 — 38 % при 25. Окно строилось вокруг n и прижималось
    // к краям — крайние числа окна почти не бывали ответом. Перебор точный: все n и все сдвиги, без случайности.
    const выдают: string[] = [];
    for (let L = 1; L <= QUICK_COUNT_LEVELS; L++) {
      const p = levelParams(L) as { minN: number; maxN: number };
      const возможных = p.maxN - p.minN + 1;
      if (возможных > ANSWER_MAX) continue;       // L20–24: семь ответов в окно из шести не накрыть никаким сдвигом
      const поОкну = new Map<string, Map<number, number>>();
      let всего = 0;
      for (let n = p.minN; n <= p.maxN; n++) {
        for (let сдвиг = 0; сдвиг < ANSWER_MAX; сдвиг++) {
          const ключ = answerWindow(p, n, сдвиг).join(',');
          const счёт = поОкну.get(ключ) ?? new Map<number, number>();
          счёт.set(n, (счёт.get(n) ?? 0) + 1);
          поОкну.set(ключ, счёт);
          всего += 1;
        }
      }
      let угадал = 0;
      for (const счёт of поОкну.values()) угадал += Math.max(...счёт.values());
      const доля = угадал / всего, потолок = 1 / возможных;
      if (доля > потолок + 0.005) выдают.push(`L${L} [${p.minN}–${p.maxN}]: по окну ${(100 * доля).toFixed(0)} % при потолке ${(100 * потолок).toFixed(0)}`);
    }
    expect(выдают.slice(0, 5)).toEqual([]);
  });
