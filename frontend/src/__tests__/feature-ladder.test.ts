...
describe('вторая лестница: замки по уровню игрока', () => {
  it('есть что проверять и порядок не сломан', () => {
    expect(FEATURE_LADDER.length).toBeGreaterThanOrEqual(3);
  ...
  /**
   * 🔴 ЗАМОК ДЕРЖИТ НОВУЮ СПОСОБНОСТЬ, А НЕ ЧУЖИЕ ДАННЫЕ (правка 03.09.2026).
   *
   * Прежний порог «не меньше пяти ступеней» толкал ровно к тому, что потом пришлось
   * убирать: чтобы набрать длину, в лестницу дописали три существующих экрана —
   * личную статистику, календарь серии и вечерний комплекс перед сном. Замок на них
   * не дарит способность, а ОТНИМАЕТ уже написанное: свои замеры, свою серию и своё
   * успокоение перед сном человек не должен «зарабатывать».
   *
   * Поэтому число ступеней больше не самоцель, а три ключа занесены в чёрный список
   * с причиной — чтобы их не вернули ради длины.
   */
  it('🔴 убранные ступени не возвращаются: они отнимали, а не дарили', () => {
    const нельзя: Record<string, string> = {
      eveningMode: 'вечерний комплекс перед сном — успокоение, а не награда (freshGames.ts:118)',
      records: 'app/statistics.tsx — личные замеры игрока; прятать измерение в измеряющем приложении нельзя',
      streakMap: 'app/streak-calendar.tsx — серия важна в первую неделю, а не на 25-м уровне',
    };
    const вернулись = FEATURE_LADDER
      .filter((l) => нельзя[l.key])
      .map((l) => `${l.key}: ${нельзя[l.key]}`);
    expect(вернулись).toEqual([]);
  });
