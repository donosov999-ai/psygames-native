...
export function RotationShape({shape,size,axis='x',degrees=0,ghost,unit}:{shape:Shape;size:number;axis?:Axis;degrees?:number;ghost?:Shape;unit?:number}){
  ...
  return <Svg testID="rotation-shape" width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
    ...
    {shapeSurface(shape,size,axis,degrees,ghost,unit).map(f=>{
