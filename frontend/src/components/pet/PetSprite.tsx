...
/**
 * 🔴 ТАБЛИЦА ЗАДАЁТ ТАКТ НА КАДР, А КАДРОВ СТАЛО СЕМЬ ВМЕСТО ЧЕТЫРЁХ.
 *
 * Числа ниже писались, когда на состояние приходилось ЧЕТЫРЕ кадра. У кота их
 * теперь семь — и каждая анимация поехала в 1,75 раза медленнее задуманного, при
 * этом никто число не менял. Замер 10.09.2026 по всем состояниям кота:
 *   покой   420 мс × 7 = 2,94 с на цикл — это 2,4 кадра в секунду;
 *   сон     620 мс × 7 = 4,34 с;
 *   гордость 300 мс × 7 = 2,1 с.
 * Два с лишним кадра в секунду глаз читает как рывки, а не как неспешность. Это и
 * есть половина жалобы «дёргается»: таймер исправен, кадров просто слишком мало
 * в секунду.
 *
 * Лечим сохранением ЦИКЛА, а не такта: `ЦИКЛ_МС` = прежний такт × 4, то есть ровно
 * та длительность, которую автор и задумывал. Такт считается делением на настоящее
 * число кадров, поэтому добавление фаз впредь делает движение ПЛАВНЕЕ, а не
 * медленнее.
 *
 * ⚠️ ХОДЬБА — ИСКЛЮЧЕНИЕ, И ЭТО НЕ ЛЕНЬ. Её перегенерировали 09.09 (`97e8012f`)
 * уже под шесть фаз и подбирали темп под новый цикл; 140 мс там измеренная
 * величина, а не наследие четырёх кадров. Пересчёт сломал бы согласование шага со
 * скоростью проезда гуляки.
 */
const ТАКТ_ИЗМЕРЕН: Partial<Record<PetState, true>> = { walk: true };
...
/**
 * 🔴 КАДРЫ-ВСТУПЛЕНИЕ: ИХ ПОКАЗЫВАЮТ ОДИН РАЗ, А НЕ КРУТЯТ В ЦИКЛЕ.
 *
 * Замер 10.09.2026 по кадрам `cat/sleep`: первые два кадра — кот СИДИТ с открытыми
 * глазами, остальные пять — лежит и спит. Это переход «лёг спать», а не цикл. Код
 * же крутил все семь по кругу, и спящий кот каждые 4,2 секунды СКАЧКОМ садился.
 *
 * 📍 Числа: разрыв силуэта на стыке последнего кадра с первым 29,5 % при среднем
 * шаге 19,6 %; опора гуляла на 128 px из 512, рост — на 32,9 %. Если крутить только
 * лежачие, стык падает до 8,2 %, а разброс роста между ними и вовсе 3,4 % — то есть
 * лежачие кадры всегда были согласованы, всю кривизну давала подмешанная посадка.
 *
 * ⚠️ ВЫРАВНИВАНИЕМ ЭТО НЕ ЛЕЧИТСЯ, И Я ПРОВЕРИЛ. Прогон `sprite_align --height 0.61`
 * по всем семи даёт красивые числа (33,8 % → 1,0 %), но подтягивает лежащего кота к
 * росту сидящего — у лежачей позы «рост» это толщина, и спящий кот раздувается до
 * размеров стоящего. Инструмент честно выровнял то, что выравнивать не следовало.
 *
 * Поэтому чиним показом, а не картинками: кадры остаются все (они настоящие), но
 * цикл начинается с первого кадра ПОЗЫ. Куда деть вступление — вопрос отдельный:
 * сейчас оно просто не участвует.
 */
export const ЦИКЛ_С_КАДРА: Partial<Record<PetState, number>> = {
  sleep: 2,
};
...
export function petCycleMs(skin: PetSkin, state: PetState): number {
  /**
   * ⚠️ Считаем ТЕМ ЖЕ правилом, что и сам показ, иначе гуляка вернётся в покой не на
   * стыке цикла, а посреди движения — а обрыв на середине и выглядит как рывок
   * (`WalkingPet.tsx` зовёт эту величину именно ради стыка).
   * Прежняя редакция брала `FRAME_MS[state]` по ЗАПРОШЕННОМУ состоянию, а длину — по
   * разрешённому: у робота `dance` это давало четыре кадра в темпе прыжка.
   */
  const разрешено = petResolveState(skin, state);
  const набор = SKINS[skin][разрешено] ?? SKINS[skin].idle;
  const срез = ЦИКЛ_С_КАДРА[разрешено] ?? 0;
  const кадров = срез > 0 && набор.length - срез >= 2 ? набор.length - срез : набор.length;
  const такт = ТАКТ_ИЗМЕРЕН[разрешено]
    ? FRAME_MS[разрешено]
    : Math.max(60, Math.round((FRAME_MS[разрешено] * 4) / Math.max(1, кадров)));
  return кадров * такт;
}
...
export function petHeadCenter(skin: PetSkin, state: PetState): { x: number; y: number } {
  ...
  let x = 0, глаза = 0, макушка = 0, сМакушкой = 0;
  for (const f of list) {
    x += f.eyes.x;
    глаза += f.eyes.y;
    if (f.head_top && !(f.off ?? []).includes('head_top')) { макушка += f.head_top.y; сМакушкой += 1; }
  }
  /**
   * 🔴 ПО ВЕРТИКАЛИ ОКНО СТАВИТСЯ МЕЖДУ МАКУШКОЙ И ГЛАЗАМИ, А НЕ ПО ГЛАЗАМ.
   *
   * Замер 10.09.2026 по всем состояниям трёх обликов: с окном по глазам голова
   * влезала целиком только у КОТА (34 состояния из 34). У робота и созвездия
   * макушка оказывалась ВЫШЕ окна почти всегда — 11 состояний из 12: робот
   * `idle` — глаза на 72 % кадра, макушка на 46 %, окно 47…97, и свод черепа
   * срезан. Причина не в якорях, а в форме: у круглого облика глаза сидят низко
   * относительно головы, и половина кадра, отсчитанная от глаз, до макушки не
   * достаёт.
   *
   * Середина отрезка «макушка — глаза» держит в окне и то, и другое: у кота она
   * почти совпадает с прежней точкой (глаза 44 %, макушка 27 % → 35 %), у робота
   * поднимает окно ровно настолько, чтобы свод черепа вошёл.
   *
   * ⚠️ Кадры с пометкой `off` на макушке не участвуют: там правило поиска
   * неприменимо (поза перевёрнута или разрезана лапой), и среднее бы уехало.
   */
  const yГлаза = глаза / n;
  const y = сМакушкой ? (yГлаза + макушка / сМакушкой) / 2 : yГлаза;
  return { x: x / n, y };
}
...
export default function PetSprite({ state, size = 56, skin = 'cat', accessory = null, look = null, subject = false }: {
  ...
  const всеКадры = видЗаботы ? [видЗаботы] : (SKINS[skin][petResolveState(skin, state)] ?? SKINS[skin].idle);
  /**
   * Отрезаем кадры-вступление (см. `ЦИКЛ_С_КАДРА`). Срез применяется только когда
   * кадров после него остаётся не меньше двух — иначе облик без своих кадров
   * (у робота и созвездия состояний меньше) остался бы с пустым циклом.
   */
  const сКадра = видЗаботы ? 0 : (ЦИКЛ_С_КАДРА[petResolveState(skin, state)] ?? 0);
  const frames = сКадра > 0 && всеКадры.length - сКадра >= 2 ? всеКадры.slice(сКадра) : всеКадры;
  ...
  /**
   * ⚠️ Такт берём по РАЗРЕШЁННОМУ состоянию, а не по запрошенному. Кадры уже берутся
   * по разрешённому (у робота `dance` отдаёт кадры `jump`), и рассогласование давало
   * четыре чужих кадра в чужом темпе.
   */
  const состояниеКадров = petResolveState(skin, state);
  const тактМс = кан
    ? Math.round(1000 / Math.max(1, кан.fps))
