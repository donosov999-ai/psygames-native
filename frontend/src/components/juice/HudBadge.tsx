...
const styles = StyleSheet.create({
  ...
  /**
   * 🔴 `alignSelf: 'center'` — ЧТОБЫ ТЕНЬ НЕ ОБВОДИЛА ПУСТОТУ.
   *
   * Отчёт Дениса 03.09.2026 по маджонгу: «верхний тулбар выглядит будто две
   * кнопки». На кадре под каждой цветной пилюлей — белый прямоугольник с тенью,
   * ростом почти с саму пилюлю: получается два ряда, из которых второй пустой.
   *
   * Причина не в тени, а в РАСТЯЖЕНИИ. У ряда счётчиков в игре не задан
   * `alignItems`, а по умолчанию флексбокс ставит `stretch`: все пилюли строки
   * дотягиваются до самой высокой. Высокая берётся из цели нажатия — пилюля с
   * пояснением оборачивается в `tapTarget` ростом 48 (норма Material), сама
   * пилюля при этом 29. Соседи растягиваются до 48, содержимое остаётся 29, и
   * тень обводит 19 точек пустоты под каждым числом.
   *
   * ⚠️ Чиним ЗДЕСЬ, а не в ряду конкретной игры: счётчик рисуют семь экранов
   * своими рядами (маджонг, парные картинки, матрица памяти, оболочка игр…), и
   * правка в одном ряду оставила бы остальные шесть. `alignSelf` сильнее чужого
   * `alignItems`, поэтому пилюля больше не растягивается ни в каком ряду.
   *
   * ⚠️ И ТЕНЬ ПЕРЕЕХАЛА С ОБЁРТКИ НА ПИЛЮЛЮ. `alignSelf` вылечил пять счётчиков
   * из шести, а шестой остался с белой коробкой: у пилюли С ПОЯСНЕНИЕМ внутри
   * лежит прозрачная цель нажатия ростом 48, и тень обёртки обводила именно её.
   * Тень обязана повторять ВИДИМУЮ фигуру, а не область касания — поэтому она
   * теперь на самой пилюле, а обёртка только держит ряд и сжатие.
   *
   * Замер: обёртка 74×48 при пилюле 74×29 (WebKit и Chromium одинаково) → после
   * правки тень по 29 у всех шести.
   */
  shadow: { alignSelf: 'center', flexShrink: 1, minWidth: 0, borderRadius: 16 },
  /**
  ...
  pill: {
    ...
    // Тень живёт здесь, а не на обёртке: см. разбор у `shadow`.
    shadowColor: '#000', shadowOpacity: 0.25, shadowRadius: 4, shadowOffset: { width: 0, height: 3 }, elevation: 4,
  },
