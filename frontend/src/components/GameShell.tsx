        ...
        <View
          ...
          style={[
            styles.toolbar,
            {
              ...
              paddingBottom: Math.max(insets.bottom, 10),
              /**
               * 🔴 ОТСТУП ПОД ПЛАВАЮЩУЮ КНОПКУ — СИММЕТРИЧНЫЙ, ИНАЧЕ ОТВЕТ СЪЕЗЖАЕТ.
               *
               * Отчёт 02.09.2026 («поворот фигур»): «картинка съехала вправо».
               * Замер браузером: стояло paddingLeft 66 и paddingRight 16 — резерв
               * под кнопку отзыва в левом нижнем углу. Содержимое внутри
               * центрируется, но центрируется в СМЕЩЁННОЙ коробке: разница
               * отступов 50 px сдвигает всё вправо ровно на 25. Эталон стоит по
               * центру экрана, варианты — на 25 правее, и глаз ловит это сразу,
               * хотя за край ничего не вылезает.
               *
               * Резерв нужен (кнопка перекрывала бы крайний вариант), поэтому он
               * теперь с ОБЕИХ сторон и зеркалить его не нужно. Цена — доступная
               * ширина меньше, и игры, считающие размеры от неё, обязаны считать
               * от ширины экрана, а не от зашитых чисел (см. mental-rotation).
               */
              paddingLeft: FAB_GUTTER,
