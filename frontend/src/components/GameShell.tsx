...
/**
 * Подпись пункта ухода приводится к его поведению: `leave` уходит через `onBack`
 * экрана, то есть на ШАГ НАЗАД. Если экран назвал его «На главную», а настоящая
 * «На главную» добавляется рядом, две одинаковые подписи с разным поведением —
 * хуже, чем одна неверная. Переподписывается только этот случай.
 */
function domesticate(
  ...
  if (домойЕсть) return свои;
  const домой = t('goHome');
...
export default function GameShell({
  ...
  const действияПаузы = React.useMemo<PauseAction[]>(() => {
    ...
    if (свои) {
      /**
       * 🔴 ДВА ВЫХОДА НУЖНЫ И ТЕМ ЭКРАНАМ, У КОТОРЫХ МЕНЮ СВОЁ.
       *
       * 📍 Денис 10.09.2026: «не хватает кнопки выйти из упражнения и выйти в главное
       * меню, т.е. надо оба варианта: один на шаг назад выходит, а второй на десктоп
       * главной». Я сделал это только в ветке ниже — там, где меню собирает каркас.
       * Замер по коду: своё меню у ВОСЕМНАДЦАТИ экранов, `leave` есть у всех
       * восемнадцати, `toHome` — НИ У ОДНОГО. То есть просьба выполнена у шестидесяти
       * трёх экранов и не выполнена у восемнадцати.
       *
       * 🔴 И ХУЖЕ ТОГО — ЕДИНСТВЕННЫЙ ВЫХОД ТАМ ПОДПИСАН НЕВЕРНО. Двадцать два пункта
       * `leave` из двадцати трёх носят подпись `goHome` («На главную»), а `leave`
       * уходит через `onBack` экрана — то есть на ШАГ НАЗАД, в развилку раздела, а не
       * на главную. Ровно та же болезнь, что была у стрелки в шапке: обещает одно,
       * делает другое.
       *
       * Поэтому здесь две правки, и вторая без первой бессмысленна:
       *   · дописать «На главную» настоящую (`toHome` → `router.replace('/')`);
       *   · переподписать чужой `leave` на «Выйти из игры», если он звался «На
       *     главную», иначе в меню окажутся два пункта с одинаковой подписью и
       *     разным поведением.
       * Подпись правится ТОЛЬКО в этом случае: экран, назвавший выход по-своему
       * («Настроить игру» у одного), остаётся как есть — он не врёт.
       */
      const домойЕсть = свои.some((a) => a.toHome);
      const домой: PauseAction[] = домойЕсть
        ? []
        : [{ id: 'home', label: t('goHome'), icon: 'home', toHome: true }];
      const свои2 = domesticate(свои, домойЕсть, t);

      /**
       * 🔴 ВЫХОД ОБЯЗАН ОСТАВАТЬСЯ ПОСЛЕДНИМ. Замер 10.09.2026 на маджонге: добавка
       * вставала в хвост, и получалось «…· На главную · Отменить · Перемешать» —
       * кнопка ухода посреди списка, а игровые действия после неё. Собранное
       * встаёт ПЕРЕД первым пунктом с `leave`, а если такого нет — в конец.
       */
      const выход = свои2.findIndex((a) => a.leave);
      if (выход < 0) return [...свои2, ...добавка, ...общие, ...домой];
      return [...свои2.slice(0, выход), ...добавка, ...общие, ...свои2.slice(выход), ...домой];
    }
    return [
  ...
  return (
    <SafeAreaView edges={['top', 'left', 'right']} style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      {/**
        ...
        */}
      {/**
        * 🔴 ПОЛОСА ПОКАЗАТЕЛЕЙ НЕ РИСУЕТСЯ, КОГДА ПОКАЗЫВАТЬ НЕЧЕГО.
        *
        * Замер раздела «Зарядки и серии» 23.09.2026 (экспорт main 165f5e6d, WebKit
        * 390×844, `/games/pause`): полоса 390×61, плашка внутри схлопнулась до 18×8 —
        * белый огрызок без текста, а между заголовком и содержимым 79 px пустоты,
        * это 9,4 % экрана телефона. Забота об этом случае в каркасе БЫЛА
        * (`statsPlateBare` сжимает плашку), но внешний контейнер держал
        * `minHeight: ПОЛОСА_ПОКАЗАТЕЛЕЙ` и место съедалось всё равно.
        *
        * Видно это у «Паузы» потому, что остальные экраны входят в каркас уже в
        * партии, где показатели есть. У «Паузы» своего экрана настройки нет —
        * планировщик живёт во встроенной странице, и каркас показывается там, где
        * партии ещё не началось.
        *
        * ⚠️ При заданном `frame` полосу НЕ прячем: там высота полосы посчитана и
        * входит в раскладку поля — спрятав её, мы сдвинули бы доску, которую
        * экран уже разместил по этому числу.
        */}
      {(hud?.length || mods?.length || stats || frame) ? (
          ...
          */}
          ...
          {действияПаузы.length > 0 ? (
            <View style={styles.pauseMenu}>
              {действияПаузы.map((a, место) => (
                <TouchableOpacity
                  /**
                   * 🔴 КЛЮЧ — МЕСТО ПЛЮС `id`, А НЕ ОДИН `id`.
                   *
                   * ЗАМЕР 16.09.2026, живой стенд, судоку на 58-м уровне: консоль
                   * ругалась «Encountered two children with the same key, `home`»
                   * (React: поведение не поддерживается, дети могут дублироваться
                   * ИЛИ ПРОПАДАТЬ). Причина — сборка набора: игра отдаёт свой пункт
                   * ухода `{ id: 'home', leave: true }`, а каркас ниже добавляет
                   * СВОЙ `{ id: 'home', toHome: true }`. Оба нужны и оба остаются —
                   * `domesticate` даже переименовывает первый в «Выйти из
                   * упражнения», — но ключ у них выходил один.
                   *
                   * ⚠️ ЧИНИМ В КАРКАСЕ, А НЕ В ДВАДЦАТИ ЭКРАНАХ. `id: 'home'` стоит
                   * у 20 игр (греп по app/games/*.tsx); переименовывать их по одной
                   * значило бы двадцать правок в чужих файлах ради одной и той же
                   * беды. Ключ — забота того, кто рисует список.
                   */
                  key={`${место}:${a.id}`}
                  ...
                  onPress={() => {
                    ...
                    /**
                     * 🔴 МЕНЮ ЗАКРЫВАЕМ САМИ, А НЕ ЖДЁМ ОБЩЕГО СЧЁТЧИКА.
                     *
                     * ПОВОД — отчёт «Релакс» 15.09.2026, v2.54.11, судоку, уровень 60,
                     * партия шла 12:39: «Кнопка продолжить игру не работает, не могу
                     * вернуться в игру». Кадр показывает исправно нарисованное меню —
                     * значит ломалось не рисование, а закрытие.
                     *
                     * ПОЧЕМУ ЛОМАЛОСЬ. Меню видно, пока `paused`, а `paused` — отражение
                     * ОБЩЕГО счётчика пауз (подписка `onGameHold`). Снятия своей задержки
                     * мало: держателей в приложении четыре — FeedbackWidget,
                     * GameHelpOverlay и две здесь. Держит хоть один — счётчик не ноль,
                     * `paused` не гаснет, меню остаётся, и кнопка выглядит мёртвой.
                     * У неё это повторяемо: «Сообщить о проблеме» открывается ИЗ ЭТОГО ЖЕ
                     * меню и берёт свою задержку.
                     *
                     * ⚠️ ЧУЖУЮ ЗАДЕРЖКУ НЕ ТРОГАЕМ — она не наша, и часы по ней стоят
                     * законно. Закрываем только СВОЁ окно: решение о показе меню
                     * принимает каркас, значит и снимать его должен он.
                     */
                    setPaused(false);
 ...
 */
/**
 * 🔴 ЗАПРЕТ ЖЕСТА ПРОКРУТКИ ДЛЯ ОДНОГО УЗЛА — ДЛЯ ИГР, РИСУЮЩИХ ПАЛЬЦЕМ.
 *
 * ЗАЧЕМ ОТДЕЛЬНЫЙ ЭКСПОРТ, А НЕ КОПИЯ В КАЖДОЙ ИГРЕ. У каркаса такой запрет уже
 * есть (`безПрокрутки` выше) и он вешается на ПОЛЕ каркаса. Но игра, которая
 * рисует внутри СВОЕГО `ScrollView`, каркасную защиту обходит: её скроллер стоит
 * между пальцем и защищённым узлом, и `touch-action` предка его не останавливает.
 * Замер 12.09.2026: так устроены девять экранов, защищён был ровно один.
 * Девять копий двух строк разъехались бы молча — поэтому одна строка здесь.
 *
 * 🔴 ВЕШАТЬ ТОЛЬКО НА САМУ ДОСКУ, НЕ НА КОНТЕЙНЕР СО ВСЕЙ СТРАНИЦЕЙ.
 * `touch-action: none` запрещает панорамирование для касаний, НАЧАВШИХСЯ на этом
 * узле. Повесив его на доску, мы забираем у браузера право толковать рисование
 * как прокрутку — и НЕ отнимаем прокрутку у остального экрана. Повесив его на
 * внешний контейнер, мы сделаем недостижимым всё, что не влезло в окно: на
 * маленьком экране это нижние клетки той же доски, то есть хуже исходной беды.
 *
 * `overscrollBehavior: 'contain'`, а не `'none'`: цепочку прокрутки к родителю
 * рвём (чтобы палец на доске не тянул страницу), но собственную резинку узла не
 * трогаем — её тут всё равно нет, узел не прокручивается.
 */
export const БЕЗ_ЖЕСТА_ПРОКРУТКИ = Platform.OS === 'web'
  ? ({ touchAction: 'none', overscrollBehavior: 'contain' } as unknown as Record<string, string>)
  : null;
