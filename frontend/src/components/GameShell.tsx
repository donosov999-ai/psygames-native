...
const styles = StyleSheet.create({
  ...
  /**
   * 🔴 РЯД — ИМЕННО РЯД, И ЭТО НЕ КОСМЕТИКА.
   *
   * 📍 Замер 09.09.2026 живьём (сборка, 375×812 и 1200 px — одинаково): полоса
   * `game-header-actions` имела высоту 6 точек при кнопке внутри в 48. Кнопка
   * вылезала из своей коробки на 42 точки, поле накрывало её сверху, и нажатие
   * до неё НЕ ДОХОДИЛО: `document.elementFromPoint` в центре кнопки возвращал
   * чужой элемент. Playwright не мог кликнуть по ней десятью попытками подряд.
   *
   * ПРИЧИНА — ПУТАНИЦА ОСЕЙ. `GameAuxBar` несёт `flexGrow: 1, flexBasis: 0`
   * (GameAuxAction.tsx:216) — это лечило вылет по ШИРИНЕ и верно ровно тогда,
   * когда родитель — ряд. Здесь родителем была колонка (умолчание RN), и та же
   * пара стала управлять ВЫСОТОЙ: основа 0, а расти не от чего — высота
   * родителя сама считается по содержимому. Полоса схлопывалась в отступ.
   *
   * ⚠️ Отсюда же тянулись отчёты «подсказки не работают» (`26af9227` 05.09,
   * `19eaaa3a` 08.09): кнопка была нарисована и объявлена скринридеру, но
   * физически не нажималась. Дефект общий для каркаса — раздел «Поиск» завёл на
   * него задачу `96a8ee4f` 07.09 и насчитал 17 экранов из 18.
   */
  headerActions: {
    flexDirection: 'row',
    alignItems: 'center',
