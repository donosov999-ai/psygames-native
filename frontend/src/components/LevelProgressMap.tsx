      ...
      >
        <View style={{ width: totalW, height: H }}>
          ...
          <Image
            ...
            accessible={false}
          />
          {/* ⚠️ SVG ПОДНЯТ НАД ПОДЛОЖКОЙ ЯВНО. На вебе позиционированный элемент
              красится выше статичного независимо от порядка в разметке, и подложка
              оказывается ПОВЕРХ карты. Здесь это до сих пор сходило с рук только
              потому, что у неё непрозрачность 0,16 — карта просто вымыта на
              шестнадцать процентов. Подними кто-нибудь непрозрачность, и карта
              исчезнет. В тортах тот же узор с плотной картинкой съел игру целиком
              (отчёт NZT-48 «А где тортики ?»). */}
          <Svg width={totalW} height={H} style={{ position: 'relative', zIndex: 1 }}>
            <Defs>
