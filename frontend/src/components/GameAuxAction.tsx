...
export interface GameAuxActionProps {
   ...
   *
   * 📍 ЗАЧЕМ ОН НУЖЕН. `GameShell.auxInHud` кладёт служебное в полосу счётчиков,
   * и в его же шапке записано ограничение: «если действий больше одного —
   * включать НЕЛЬЗЯ: кнопка уедет на вторую строку». Замер 11.09.2026 показал,
   * что ограничение уже нарушено — у «Анаграмм» режим «Все слова» несёт ДВЕ
   * кнопки, и подписанные пилюли не влезают:
   *     390×844  «Перемешать» 146px, правый край 387 при границе полосы 380
   *     360×844  та же кнопка 241…387 — за правым краем ЭКРАНА на 27 точек
   *     свободно 254, нужно 283 (131 + 6 + 146)
   * Два значка по 48 (48 + 6 + 48 = 102) влезают с запасом. Компактный вид и
   * есть то, что снимает ограничение.
   *
