...
const CARD_MS = 1200;
/**
 * 🔴 ДЛИТЕЛЬНОСТЬ ВРЕЗКИ. Репорт Дениса 26.08.2026: «дорожку сделай чуть дольше по
 * времени, а то сейчас пара секунд». Было ровно 2000 мс — он назвал число точно.
 * Стало 3500: шаг питомца идёт `ms * 0.7` = 2,45 с, и на рисунок остаётся время,
 * а не мелькание.
 *
 * ⚠️ ЭКСПОРТИРУЕТСЯ НАРОЧНО. Проверка `proofreading-series` держала СВОЮ копию
 * этого числа с подписью «ровно как в экране» — и копия разошлась: в экране было
 * 2000, в проверке 2500. Зеркало, которое молча расходится, хуже отсутствия
 * зеркала: оно выглядит сверенным. Теперь источник один.
 */
export const INTERLUDE_MS = 3500;
