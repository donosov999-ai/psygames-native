...
import { estimateStepSec } from './gameDuration';
...
/**
 * 🔴 БЮДЖЕТ НАБИРАЕТСЯ ЗАМЕРЕННЫМ ВРЕМЕНЕМ, А НЕ ОБЪЯВЛЕННЫМ.
 *
 * 📍 Находка координатора 09.09.2026: 08.09 в проект приехал снимок медиан по
 * живой базе (`gameDuration.ts`, коммит 7ec6202d) — повод был «просишь пять
 * минут, получаешь 2:45», и замер оказался хуже жалобы. Но на замер перевели
 * только `warmup.ts`; здесь, на общем сборщике ТРЁХ карточек (шахматы, слова,
 * языки), по-прежнему складывалось объявленное `т.секунд`.
 *
 * ЧИСЛА ПО СЛОВАМ, замер 09.09 (объявлено → замер + переход 12 с):
 *   анаграммы   90 → 200 с  (занижено в 2,22 раза)
 *   филворды   120 →  60 с  (завышено вдвое)
 *   беглость    60 →  72 с
 *   цикл       270 → 332 с  → обещание «10 минут» давало около 12,3
 *
 * ⚠️ У РАЗНЫХ КАРТОЧЕК ОШИБКА В РАЗНЫЕ СТОРОНЫ, и это меняет смысл починки. У
 * шахмат и языков объявленные числа ЗАВЫШЕНЫ (vocab_srs 60 → 24, semantic_sort
 * 70 → 32), поэтому план обрывался рано и зарядка выходила КОРОЧЕ обещанной. У
 * слов главный и самый частый шаг занижен вдвое с лишним, поэтому она выходила
 * ДЛИННЕЕ. После правки у слов из плана уходит шаг-другой — это не регресс, а
 * снятое завышение: обещание становится честным в обе стороны.
 *
 * Поле `секунд` у темы остаётся ЗАПАСОМ: `estimateStepSec` берёт его, когда по
 * игре замера ещё нет (мало живых партий — так сейчас у `cloze` и
 * `lexical_decision`). Убирать его нельзя, пока такие игры есть.
 */
export function темаШаги(темы: readonly ТемаЗарядки[], minutes: WarmupMinutes): PlaylistStep[] {
  ...
  for (let i = 0; занято < бюджет; i++) {
    ...
    // Цена шага для ЧЕЛОВЕКА, а не для таблицы: медиана живых партий + переход.
...
export function собратьТемуЗарядки(
  ...
  // Обещание человеку («зарядка на N минут») считается тем же замером, что и
  // набор шагов, — иначе карточка и план говорили бы разные числа.
