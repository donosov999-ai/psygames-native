 ...
 *
 * ⚠️ РАУНДЫ БЕЗ МЕТКИ ВЛАДЕЛЬЦА — ЧУЖИЕ ДЛЯ ВСЕХ, и это решение Дениса 20.08.2026.
 *
 * Раньше они показывались каждому профилю: рассуждение было «выбросить значит стереть
 * человеку его же месяцы тренировок». Рассуждение оказалось про никого. Метка владельца
 * пишется с 20.06.2026, а в Google Play приложение уехало в августе — значит партий без
 * метки нет ни у одного человека за пределами наших собственных устройств. Потерю этих
 * строк мы уже приняли.
 *
 * Поэтому здесь нет догадки о прошлом. Партия принадлежит профилю, если так написано в
 * ней самой; «ничья» партия не принадлежит никому. Догадка вида «до июня устройство было
 * Денисово» правдой не является: на семейном устройстве играли двое, и вердикт «хуже
 * прошлого раза» сравнивал бы взрослого с семилеткой и врал бы обоим.
 */
export function belongsToProfile(s: HistorySession, profileId: string): boolean {
  return s.profile_id === profileId;
}
...
/**
 * ДОРОГА СЛОЖНОСТИ, если игра ходит по нескольким лестницам сразу.
 *
 * ⚠️ ТРЕТЬЯ ЛОВУШКА КЛЮЧА, И ОНА ТОГО ЖЕ РОДА, ЧТО 5×5 ПРОТИВ 6×6. Судоку с
 * 20.08.2026 ведёт три отдельные лестницы — «полегче», «обычная», «пожёстче»
 * (services/sudoku-roads). Уровень 12 на лёгкой и уровень 12 на тяжёлой — это разные
 * задачи: у них разная полоса техник, разное число пустых клеток и разный лимит
 * подсказок. Без дороги в ключе история объявила бы «хуже на 300 очков» человеку,
 * который просто перешёл на тяжёлую дорогу, и «лучше» тому, кто ушёл на лёгкую.
 *
 * ⚠️ ДОРОГА ПО УМОЛЧАНИЮ СХЛОПЫВАЕТСЯ В ПУСТО — НАРОЧНО. Обычная дорога это ровно те
 * же параметры, в которые человек играл ДО появления дорог, то есть та же задача.
 * Партии тех лет поля `road` не несут вовсе; если бы новая запись давала `normal`, а
 * старая — пусто, в день обновления вся история судоку превратилась бы в «новую
 * сложность» и человек потерял бы сравнение с прошлым разом на ровном месте.
 *
 * Имя дороги по умолчанию тут повторено СТРОКОЙ, чтобы этот файл не тянул за собой
 * решатель судоку (его через recommend.ts вытянул бы главный экран). Совпадение с
 * `roadTaskPart` из sudoku-roads доказывается прогоном, а не обещанием.
 */
export const DEFAULT_ROAD = 'normal';

export function entryRoad(s: HistorySession): string {
  const r = s.details?.road;
  return typeof r === 'string' && r !== '' && r !== DEFAULT_ROAD ? r : '';
}
