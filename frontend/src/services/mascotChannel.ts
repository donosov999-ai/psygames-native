 ...
 */
/**
 * 🔴 КАНАЛ НЕ ИМЕЕТ ПРАВА ПОДМЕНЯТЬ КАДРЫ, КОТОРЫЕ НОВЕЕ ЕГО САМОГО.
 *
 * 📍 ПОВОД, замер 10.09.2026 по снимку Дениса: «у меня старый дырявый питомец…
 * вроде у Вали на этом же экране новый». Так и было, и вот механизм.
 *
 * Канал ПЕРЕБИВАЕТ вшитые кадры (`PetSprite`: `const кан = изКанала(...)`, и если
 * пак есть — рисуется он). Отдаёт он пять состояний из тридцати пяти — `walk`,
 * `idle`, `wave`, `jump`, `sleep`, — поэтому экран получался смесью: покой из
 * канала, остальное из сборки.
 *
 * Живой замер канала 10.09.2026:
 *   `channels/stable.json` → 200, поле `updated` = **2026-09-05**
 *   пак `synapse-cat` → версия 1.0.5, путь packs/synapse-cat/1.0.5/
 * А кот в приложении перерисован ПОЗЖЕ: 09.09 (`97e8012f`, новый цикл ходьбы,
 * листы Синапс-2). То есть канал раздавал заведомо устаревший рисунок и молча
 * затирал им свежий.
 *
 * ⚠️ И ЭТО НЕ РАЗОВО: пак ложится в `AsyncStorage`, поэтому устройство, один раз
 * успевшее его забрать, показывает старого кота и дальше — «он долго держится
 * уже». У кого загрузка не прошла, видит вшитого нового. Отсюда и расхождение
 * между двумя телефонами на одном и том же экране.
 *
 * 📌 Тот же канал уже давал отчёт `6734447a` (04.09.2026, iPhone, v2.37.48):
 * «кот дырявый» — тогда чинили втягивание листа в свой origin, а не первенство.
 *
 * ПРАВИЛО: пак берётся, только если канал объявляет себя НЕ СТАРШЕ вшитых кадров.
 * Дата ниже — день последней перерисовки облика в репозитории; поднимать её тем же
 * коммитом, что и новые кадры, иначе канал снова начнёт затирать свежее старым.
 */
const ВШИТЫЕ_КАДРЫ_ОТ = '2026-09-09';
...
export async function loadMascotChannel(): Promise<void> {
  ...
  try {
    ...
    /**
     * Канал старше вшитых кадров — не берём НИЧЕГО и выбрасываем то, что уже
     * подняли из кэша: устройство, успевшее забрать старый пак, обязано вернуться
     * на свежие вшитые кадры само, без переустановки.
     */
    const датаКанала = typeof man?.updated === 'string' ? man.updated : '';
    if (датаКанала && датаКанала < ВШИТЫЕ_КАДРЫ_ОТ) {
      if (Object.keys(готовые).length) {
        for (const pack of Object.values(готовые)) отпустить(pack);
        готовые = {};
        слушатели.forEach((f) => f());
      }
      try { await AsyncStorage.removeItem(CACHE_KEY); } catch { /* кэш — удобство */ }
      return;
    }
