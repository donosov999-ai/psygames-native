 ...
 *
 * ЗАМЕР 11.09.2026, подтверждён 16.09: открытие пресета abyss вызывало 2548
 * материализаций — ВСЮ партию целиком. Рекурсия шла так: отрисовка корня зовёт
 * `deepValueAt` на каждую кормимую клетку → `deepNodeDone(ребёнок)` →
 * `deepNodeProgress` → `nodeAt(ребёнок)` и вопрос к ЕГО детям, и так до дна.
 * Это сводило на нет весь смысл файла (см. шапку: «породить их разом нельзя ни по
 * времени, ни по памяти») — человек с телефона получал замирание на открытии.
 *
 * ПОЧЕМУ ОТСЕЧКА ТОЧНАЯ, А НЕ ПРИБЛИЖЁННАЯ. Порог открытия всегда ≥ 1
 * (`unlockCells = Math.max(1, …)`), а прогресс складывается ровно из двух слагаемых:
 * своих верно закрытых клеток (их даёт `played[p]`) и дорешанных детей (а те по той
 * же причине требуют наигранного у себя). Значит прогресс > 0 НЕВОЗМОЖЕН, если во
 * всём поддереве не тронуто ни одной клетки. Поведение не меняется — меняется цена.
 */
function тронутоПоддерево(played: DeepPlayed, p: DeepPath): boolean {
  if (played[p]) return true;
  // Ключ наигранного — путь узла; потомок начинается с пути родителя и косой черты.
  const префикс = p === '' ? '' : `${p}/`;
  for (const ключ of Object.keys(played)) {
    if (ключ === p) return true;
    if (префикс === '' || ключ.startsWith(префикс)) return true;
  }
  return false;
}
...
export function deepNodeDone(nodeAt: NodeAt, played: DeepPlayed, p: DeepPath): boolean {
  ...
  if (!тронутоПоддерево(played, p)) return false;
