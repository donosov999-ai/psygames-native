...
/** На сколько кнопка поднята над безопасной зоной в своём месте по умолчанию. */
export const FAB_BOTTOM = 92;

/**
 * СКОЛЬКО МЕСТА ВНИЗУ ЭКРАНА ЗАНЯТО И НЕ ГОДИТСЯ ПОД ТЕКСТ.
 *
 * 🔴 ЗАЧЕМ ЧИСЛО ЗДЕСЬ, А НЕ В КАЖДОМ ЭКРАНЕ. В левом нижнем углу висит кнопка
 * отзыва, по центру внизу ходит питомец. Экран, чей текст доходит до низа,
 * оказывается под ними — и это не «некрасиво», а нечитаемо.
 *
 * Снимки 21.08.2026: в магазине кнопка легла на строку «Очки копятся за игры,
 * стрики и ачивки» (замер: кнопка 672–720 по вертикали, строка 698–716 — прямое
 * пересечение); в лигах — на заголовок «Заработанные рамки» и на плашку лиги,
 * а питомец накрыл нижнюю строку. Оба экрана отступали числом, подобранным на
 * глаз, и оба промахнулись.
 *
 * ⚠️ ЧИСЛО ВЫВЕДЕНО, А НЕ НАЗНАЧЕНО: `FAB_BOTTOM` (на сколько кнопка поднята)
 * + `FAB_SIZE` (её высота) + зазор. Подвинут кнопку — отступ уедет следом сам.
 */
