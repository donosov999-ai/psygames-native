 ...
 */
/**
 * 🔴 ГЛУБИНЫ ПЕРЕСЧИТАНЫ 07.09.2026 ПО ЗАМЕРУ НУЖНОСТИ КРАЯ, а не по ступени техники.
 *
 * Ступень у башен плоская (4 на всех верхних ступенях — потолок варианта), и по ней
 * лестница выглядела мёртвой. Настоящая ось нашлась другая и работающая: ДОЛЯ ДОСОК,
 * КОТОРЫЕ БЕРУТСЯ БЕЗ КРАЕВЫХ ПОДСКАЗОК ВООБЩЕ. Замер по 40 досок на глубину:
 *
 *   пусто 12 → без края решается 39/40   пусто 18 → 25/40
 *   пусто 13 → 37/40                     пусто 19 → 14/40
 *   пусто 14 → 35/40                     пусто 20 → 14/40
 *   пусто 15 → 32/40                     пусто 22 → 6/40
 *   пусто 16 → 27/40                     пусто 24 → 5/40
 *
 * То есть на прежних первых ступенях (12–14) вариант НЕ ЗАСЛУЖИВАЛ СВОЕГО ИМЕНИ:
 * почти любая доска решалась как обычная судоку 6×6, край был украшением. Это же
 * записано и в шапке гейта `sudoku-towers` («на мелкой копке подсказки декоративны»),
 * но гейт мерит одну точку (BLANKS = 18), а лестница гоняла 12…20 — пять ступеней
 * ниже его рабочей точки.
 *
 * ⚠️ ВВЕРХ РАСТИ ТОЖЕ НЕ БЕСКОНЕЧНО, и предел здесь не «потолок сложности», а
 * РЕШАЕМОСТЬ: глубже 20 доска всё чаще не берётся логикой вовсе (с краем 22/40 при
 * 20, но 10…17/40 при 22–24). Доска, которую нельзя взять логикой, — не трудный
 * уровень, а сломанная задача. Поэтому верх поставлен на 22: край там нужен
 * шести доскам из сорока, и это ещё решаемо.
 */
export const TOWERS_LADDER: readonly SideStep[] = [
  { blanks: 13, band: { min: 1, max: 2 } },
  { blanks: 15, band: { min: 1, max: 4 } },
  { blanks: 16, band: { min: 4, max: 4 } },
  { blanks: 17, band: { min: 4, max: 4 } },
  { blanks: 18, band: { min: 4, max: 4 } },
  { blanks: 19, band: { min: 4, max: 4 } },
  { blanks: 20, band: { min: 4, max: 4 } },
  { blanks: 22, band: { min: 4, max: 4 } },
];
