...
/**
 * 🔴 ПОРОГИ ЗАМКОВ ИЗ ФАЙЛА НАСТРОЕК — чтобы баланс крутился без сборки.
 *
 * Решение Дениса 13.09.2026: «уровни, логика уровней, много копались». Замер
 * показал, что правок в этом файле мало (2 за 60 дней) — но это СЛЕДСТВИЕ цены:
 * попробовать «а если подсказку с четвёртого уровня» стоило сборки и часа, поэтому
 * и не пробовали. Файл снимает эту цену.
 *
 * Заводские пороги остаются здесь и действуют всегда, когда файла нет.
 */
let порогиИзФайла: Record<string, number> | null = null;

export function установитьЗамкиИзФайла(x: Record<string, number> | null): void {
  порогиИзФайла = x;
}

/** Порог замка с учётом файла. `null` — такого приёма в лестнице нет. */
export function порогЗамка(key: string): number | null {
  const свой = порогиИзФайла?.[key];
  if (typeof свой === 'number') return свой;
  return FEATURE_LADDER.find((l) => l.key === key)?.level ?? null;
}
