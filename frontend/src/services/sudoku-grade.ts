/* psygames-sudoku-grade · VER 1 · 07.09.2026 */
/**
...
/**
 * ДОКОПАТЬ ДОСКУ ДО ПОЛА ПОЛОСЫ — ЗЕРКАЛО `easeToCeiling`, КОТОРОГО НЕ БЫЛО.
 *
 * 🔴 ЧЕГО НЕ ХВАТАЛО. У потолка страж есть с 26.08: доску, которую логикой не
 * взять, `easeToCeiling` возвращает в полосу, возвращая подсказки. У ПОЛА стража
 * не было НИ ОДНОГО. В коде это даже сказано прямым текстом: «Пол полосы,
 * наоборот, уступаем времени: слишком лёгкая доска играется, просто хуже».
 *
 * ЗАМЕР 27.08.2026, по 6 досок на уровень, боевой бюджет 1100 мс:
 *     L20 hyper    цель 3..3  выпало [3,3,3,1,3,3]   ← ступень ОДИН на двадцатом уровне
 *     L45 thermo   цель 5..6  выпало [4,3,4,4,3,4]   ← 6 из 6 НИЖЕ ПОЛА
 *     L53 jigsaw   цель 5..6  выпало [4,3,3,5,6,5]   ← 3 из 6
 *     всего: 10 досок из 48 ниже собственного пола = 21%
 * То есть каждая пятая партия легче, чем обещает её уровень, а на L45 — КАЖДАЯ.
 *
 * ⚠️ И ЭТО НЕ «НИЗКИЙ ПОТОЛОК ВАРИАНТА». Замер с ВТРОЕ увеличенным бюджетом (по 8
 * досок): thermo достаёт до 6, jigsaw достаёт до 6 — оба ВЫШЕ записанного потолка 5.
 * Значит мешает не предел варианта, а РАЗБРОС: у jigsaw L53 выпадает что угодно от
 * 2 до 6. Лишние заходы этого не лечат — они бросают кости заново.
 *
 * ━━━ ПРИЁМ ВЗЯТ У ТЭТХЭМА ━━━
 * `mines.c:1931-1968` и `untangle.c:750-775`: когда доска вышла не такой, её НЕ
 * ВЫБРАСЫВАЮТ, а ВОЗМУЩАЮТ — правят то, что уже есть, пока не станет годной.
 * `untangle` прямо перемешивает точки, ПОКА НЕ ПОЯВИТСЯ пересечение, то есть держит
 * ровно нижнюю границу. Здесь то же самое: доска ниже пола — выкалываем ещё одну
 * подсказку, а не собираем новую доску с нуля.
 *
 * ⚠️ ЕДИНСТВЕННОСТЬ СОХРАНЯЕТСЯ ПО ПОСТРОЕНИЮ, а не проверяется дорогим перебором:
 * выкалывание принимается ТОЛЬКО если доска по-прежнему берётся логикой
 * (`grade.solved`), а у добираемой логикой доски каждый шаг вынужден — второму
 * решению взяться неоткуда. Тот же довод, на котором стоит гейт единственности.
 *
 * ━━━ ЧТО ЭТОТ ПРИЁМ ДАЁТ И ЧЕГО НЕ ДАЁТ — ЗАМЕР МЕХАНИЗМА, А НЕ ВЫБОРКИ ━━━
 * Считали не «стало ли лучше в среднем» (шум выборки на этих разбросах съедает
 * любой вывод), а прямо: сколько выкалываний ПРИНЯТО и сколько досок ПОДНЯТО.
 *     L20 hyper:   досок ниже пола 2 · поднято 2 · принято 34 выкалывания · +3 ступени
 *     L45 thermo:  досок ниже пола 5 · поднято 0 · принято 0
 *     L53 jigsaw:  досок ниже пола 4 · поднято 0 · принято 0
 * 🔴 ТАМ, ГДЕ ЕСТЬ ЗАПАС, ПРИЁМ РАБОТАЕТ: hyper поднят в обоих случаях из двух.
 * 🔴 ТАМ, ГДЕ ЗАПАСА НЕТ, ОН НЕ ДЕЛАЕТ НИЧЕГО — и это не отказ приёма, а ДИАГНОЗ:
 * на thermo и jigsaw в верхней полосе доска уже выкопана до `blanksCap`, и НИ ОДНО
 * выкалывание не проходит: каждое либо ломает логическую решаемость, либо понижает
 * ступень. Значит объявленный там пол недостижим выкапыванием в принципе, и чинить
 * надо не генератор, а объявленную полосу — ровно как это уже сделано у `unequal`,
 * где невозможность записана замером прямо в коде.
 *
 * ⚠️ ПОТОЛОК УВАЖАЕМ: выкалывание, поднявшее ступень выше `max`, откатывается.
 * Иначе, чиня пол, мы бы ломали потолок — а это дефект дороже: доска, которую
 * нельзя взять логикой, оставляет человека в тупике, где он не знает, что упёрся не он.
 */
export function digToFloor(
  ...
  for (const idx of filled) {
    ...
    puzzle[r][c] = 0;
    const next = { ...gen, puzzle };
    const пробная = gradeOf(next, N, BR, BC, variant);
    /**
     * 🔴 ТРИ УСЛОВИЯ ОТКАТА, И ТРЕТЬЕ Я СНАЧАЛА ЗАБЫЛ — ЗАМЕР ЭТО ПОЙМАЛ.
     * Первая редакция откатывала только потерю решаемости и пробитый потолок, а
     * выкалывание, ПОНИЗИВШЕЕ ступень, принимала — и цикл шёл дальше уже с
     * испорченной доски. Итог был обратен задуманному: доля досок ниже пола
     * выросла с 21% до 29%, а L20 hyper поехал с [3,3,3,1,3,3] на [3,2,1,3,2,1].
     * Убрать подсказку — не то же самое, что усложнить: пропажа клетки может
     * закрыть дорогой путь рассуждения и оставить дешёвый.
     * Поэтому принимаем ТОЛЬКО НЕ ПОНИЖАЮЩЕЕ выкалывание.
     */
...
export function generateLogical(
  ...
  if (LOGIC_VARIANTS.includes(variant)) {
    ...
    if (best) {
      /**
       * ⚠️ ПОЛ ПРОВЕРЯЕМ И ЗДЕСЬ. `digByLogic` откатывает выкалывания выше потолка,
       * но НИЖЕ пола не поднимает ничего: «лучшая из пяти» бывает лучшей и при
  ...
  /**
   * ⚠️ ПОРЯДОК ИМЕННО ТАКОЙ: сперва потолок, потом пол.
   * `easeToCeiling` гарантирует решаемость логикой — это жёсткое требование, без
   * него партия нечестна. `digToFloor` затем поднимает слишком лёгкую доску, и он
   * сам откатывает любое выкалывание, пробившее потолок, поэтому гарантию не рушит.
