 ...
 * Дорогой перебор countSolutions нужен только там, где логики не хватило: это варианты,
 * которые решатель тянет слабо (несоседние, термометры, стрелки, сэндвич).
 *
...
describe('гейт: сложность растёт по уровням, а не стоит', () => {
  ...
  // Берём фазы, которые решатель понимает уверенно (у остальных генерация уходит на
  // прежний путь, и их сложность мы пока не контролируем — см. LOGIC_VARIANTS).
  const PROBES = [8, 12, 19, 28, 33, 37];
