...
import type { CellIndex, FillwordsPuzzle, PlantedWord, ПорядокСдачи } from './types';
import {
  FILLWORDS_MAX_WORD,
  FILLWORDS_MIN_WORD,
  isFillwordsLocale,
...
export interface FillwordsLevelCfg {
  ...
  timeLimitSec: number;
  /**
   * Сколько подсказок даётся на уровень — пятая ось (цена ошибки).
   *
   * ⚠️ Раньше это была константа `FILLWORDS_HINTS = 3` в экране, одна на все
   * уровни. Константа на экране осью быть не может: уровень о ней не знает, и
   * лестница ею не растёт.
   */
  hints: number;
  /**
   * Порядок сдачи слов — шестая ось (строгость порядка).
   *
   * ⚠️ ЧЕСТНА ТОЛЬКО ПРИ ПОКАЗАННОМ СПИСКЕ. Без списка человек не знает, какое
   * слово «следующее», и требование превращается в угадайку. Экран передаёт
   * строгий порядок в партию ТОЛЬКО когда список виден, иначе оставляет
   * `свободно`. Поэтому здесь это ПРЕДЛОЖЕНИЕ уровня, а не приказ.
   */
  порядок: ПорядокСдачи;
}
...
/**
 * ⚠️ ЗДЕСЬ БЫЛ ОТДЕЛЬНЫЙ ПОТОЛОК ФОРМЫ ДЛЯ РЕЖИМА СО СПИСКОМ СЛОВ — снят
 * 06.09.2026. Замер сохранён, чтобы ограничение не завели заново.
 *
 * Список сначала рисовался НАД полем и забирал ВЫСОТУ: на экране 320×568 он
 * съедал около 126 точек из 358, полю оставалось 232 — это десять строк при
 * клетке 22, а шестнадцать уезжали за экран МОЛЧА (клетка упирается в свой
 * минимум и дальше не ужимается, поэтому переполнение ничем не сигналит).
 * Лечением было ограничить форму десятью строками — лечение симптома.
 *
 * Замер нашёл лучшее: поставить список СБОКУ. Тогда он забирает ширину, которой
 * у поля запас: 90 точек оставляют 206 — девять столбцов, а строк снова
 * шестнадцать. Клеток выходит 144 против 90, поле в полтора раза больше при том
 * же списке. Ограничение по высоте перестало быть нужным.
 */
...
/** Уровень, с которого включается ось времени: объёмные оси исчерпаны на 94-м. */
const СКОРОСТЬ_С = 95;
/** Нижняя граница секунд на клетку. При 144 клетках это 144 с против 259. */
const ПОЛ_СЕК = 1.0;
const ШАГ_СЕК = 0.05;
const СТУПЕНЕЙ_СЕК = 16;
/** Уровень, с которого убавляются подсказки: 95 + 16 ступеней по три уровня. */
const ПОДСКАЗКИ_С = СКОРОСТЬ_С + СТУПЕНЕЙ_СЕК * 3;
/** Уровень, с которого включается строгий порядок: 143 + 3 ступени по 20 уровней. */
const ПОРЯДОК_С = ПОДСКАЗКИ_С + 60;
/** Сколько уровней держится прямой порядок, прежде чем включится обратный. */
const ШАГ_ПОРЯДКА = 40;
...
export function fillwordsLevel(level: number): FillwordsLevelCfg {
  ...
  /*
    🔴 ОСЬ 2 «СКОРОСТЬ» — ВКЛЮЧАЕТСЯ ПОСЛЕ ТОГО, КАК КОНЧИЛИСЬ ОБЪЁМНЫЕ.

    📍 Правило Дениса 06.09.2026: «никаких потолков нет нигде, всегда можно
    растить сложность, вопрос подхода». До него здесь стоял вывод «лестница
    упёрлась на 94-м уровне, дальше без новых слов не растёт» — он был получен
    перебором ОДНОЙ оси (объём) и оказался не потолком, а недоискан­ной осью.

    Замер, который это показал: длина слова даёт ×5,63 к пространству поиска на
    букву, ширина поля 9→13 — всего ×1,10, запрет диагоналей ×0,01 (то есть
    облегчение, а не ступень). Объёмные оси действительно исчерпаны на 94-м. Но
    скорость не объёмная: поле то же, а времени меньше.

    Пол 1,0 с/клетку выбран так: при 144 клетках это 144 секунды против нынешних
    259. Шаг 0,05 — шестнадцать различимых ступеней; по одной на три уровня,
    чтобы лестница оставалась лестницей, а не обрывом.
  */
  const базоваяСек = Math.max(1.8, 3 - (n - 1) * 0.06);
  const послеПола = Math.max(0, n - СКОРОСТЬ_С + 1);
  const снято = Math.min(СТУПЕНЕЙ_СЕК, Math.floor(послеПола / 3)) * ШАГ_СЕК;
  const perCellSec = Math.max(ПОЛ_СЕК, Number((базоваяСек - снято).toFixed(2)));

  /*
    🔴 ОСЬ 10 «ЦЕНА ОШИБКИ» — ПОСЛЕДНЕЙ, когда исчерпана и скорость.

    Подсказка стоит звезды (промах в счёте) и потому уже влияет на исход; здесь
    она становится ступенью: 3 → 0. Шаг редкий — раз в двадцать уровней, — потому
    что каждая отнятая подсказка меняет партию сильнее, чем полсекунды на клетку.
  */
  const послеСкорости = Math.max(0, n - ПОДСКАЗКИ_С + 1);
  const hints = Math.max(0, 3 - Math.floor(послеСкорости / 20));

  /*
    🔴 ОСЬ 6 «СТРОГОСТЬ ПОРЯДКА» — ПОСЛЕДНЕЙ, когда исчерпаны объём, скорость и
    цена ошибки. Свободно → по списку → обратный.

    Обратный порядок труднее прямого не «на глаз»: список читается сверху вниз, и
    держать в голове хвост, отсчитывая от конца, — это уже удержание, а не только
    поиск. Тот же приём, что в пробах на объём памяти, где обратное
    воспроизведение всегда идёт ступенью выше прямого.
  */
  const послеПодсказок = Math.max(0, n - ПОРЯДОК_С + 1);
  const порядок: ПорядокСдачи = послеПодсказок <= 0 ? 'свободно'
    : послеПодсказок <= ШАГ_ПОРЯДКА ? 'поСписку' : 'обратный';

  return { rows, cols, maxWordLen, minWordLen, hints, порядок,
    timeLimitSec: Math.round(rows * cols * perCellSec) };
}

/**
 * 🔴 СКОЛЬКО ШИРИНЫ ОСТАЁТСЯ ПОЛЮ. Вынесено из экрана сюда, чтобы это можно было
 * ПРОВЕРИТЬ. Пока расчёт жил в вёрстке, проба считала ту же формулу у себя — и
 * мутация «экран не вычитает ширину списка» проходила мимо неё незамеченной:
 * проба сверяла модель сама с собой.
 *
 * Список искомых слов стоит СБОКУ и забирает ширину, которой у поля запас: на
 * 320×568 его 90 точек оставляют 206, то есть девять столбцов при клетке 22, а
 * строк остаётся шестнадцать. Над полем он забирал бы высоту, и полю доставалось
 * бы десять строк — клеток 90 вместо 144.
 */
export function ширинаПодПоле(ширинаЭкрана: number, списокСбоку: boolean): number {
  const списка = списокСбоку ? Math.min(120, Math.round(ширинаЭкрана * 0.3)) : 0;
  return Math.min(ширинаЭкрана - 24, 760) - списка;
}
...
export function generateFillwords(request: FillwordsRequest): FillwordsPuzzle {
  ...
  /*
    ⚠️ ЗДЕСЬ СТОЯЛ ЗАСЛОН «ПОЛ ДЛИНЫ ПО ЯЗЫКУ» — СНЯТ, ПОТОМУ ЧТО НИЧЕГО НЕ ДЕРЖАЛ.

    У французского и итальянского трёхбуквенных слов нет вовсе, и я добавил сюда
    `Math.max(полДлиныЯзыка(locale), …)`, чтобы генератор не просил того, чего в
    словаре не бывает. Мутация показала, что заслон лишний: убрал его — все семь
    проб остались зелёными, поля по-прежнему собираются и ни одно слово не короче
    четырёх букв. Причина в том, что отступление пола ниже уже делает ровно это:
    оно берёт длины, КОТОРЫЕ У ЯЗЫКА ЕСТЬ, а несуществующие пропускает само.

    Оставлять заслон, который не краснеет ни от какой мутации, значит держать
    код, про который нельзя сказать, работает ли он. Настоящая правка — в
    `полДлиныЯзыка` и `isFillwordsLocale`: они РЕШАЮТ, что язык пригоден.
  */
