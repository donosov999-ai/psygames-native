...
/**
 * 🔴 СО СПИСКОМ СЛОВ ПОЛЕ НИЖЕ, И ЭТО НЕ ПРИДИРКА, А ЗАМЕР.
 *
 * Список искомых слов рисуется НАД полем и занимает место. На экране 320×568 под
 * сетку остаётся 358 точек; двадцать слов по три в строке — это семь строк, около
 * 126 точек. Полю достаётся 232, а шестнадцать строк по 22 точки требуют 352 —
 * поле вылезло бы за экран, причём молча: клетка упирается в свой минимум 22 и
 * дальше не ужимается.
 *
 * 232 / 22 = 10,5, поэтому со списком потолок — десять строк. Это честная плата:
 * список показан, значит места под поле меньше.
 */
const FILLWORDS_MAX_ROWS_СО_СПИСКОМ = 10;
 ...
 */
export function fillwordsLevel(level: number, сСписком = false): FillwordsLevelCfg {
  const n = Math.max(1, Math.floor(level) || 1);
  const cols = Math.min(9, 5 + Math.floor((n - 1) / 5));
  const rows = Math.min(сСписком ? FILLWORDS_MAX_ROWS_СО_СПИСКОМ : FILLWORDS_MAX_ROWS,
                        5 + Math.floor((n - 1) / 3));
