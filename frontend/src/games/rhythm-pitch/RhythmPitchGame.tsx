...
function RhythmPitchSessionView({
  ...
  const begin = () => {
     ...
     *
     * Отчёт Дениса 79e2ff09 (05.09.2026, tauri-ios): «звуки так и не появились» —
     * при том, что партия шла и счётчик тапов работал. Контекст Web Audio
     * создавался лениво, уже внутри `playRound`, то есть за несколькими `await`
     * от касания. WebKit такой контекст считает созданным НЕ по жесту и оставляет
     * немым навсегда; Chrome — нет, поэтому в браузере звук был.
     *
     * `initialize()` до первого `await` создаёт контекст и проигрывает пустой
     * буфер — обе операции синхронные и попадают в задачу жеста. Ошибку глушим:
     * если звука нет по-настоящему, это скажет `engine.available` строкой ниже,
     * а вываливать окно «нет звука» на нажатие «Начать» незачем.
     */
    void engine?.initialize().catch(() => {});
