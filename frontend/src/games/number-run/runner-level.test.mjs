...
const tasks={blitz:(L,rnd)=>generateSprintProblem(L,rnd),exact:(L,rnd)=>makePuzzle(levelParams(L),rnd)};
...
test('chapters: L1–3 no stations, L4–6 blitz only, L7–9 add exact, L10+ and bosses mix six',()=>{
 ...
 for(const L of [9,10,15,24])assert.equal(level(L,3).rows.filter(r=>r.station).length,6);
...
test('blitz arch: one correct answer among three distinct options; correct lane adds, each wrong lane subtracts and counts a mistake',()=>{
 ...
 assert.ok(rows>=12);
 const probe=level(30,1).rows.find(r=>r.kind==='answer');assert.ok(/=/.test(probe.prompt));
});
