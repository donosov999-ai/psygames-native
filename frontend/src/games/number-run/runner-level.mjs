...
// 1–3 дорога VER 4 · 4–6 + блиц-арки (math-sprint) · 7–9 + ворота «ровно N» (number-bonds) · 10+ смесь.
...
export const LEVEL_VERSION='number-run-level/1',LEVEL_SLOTS=15,PASS_WALLS=5,BOSS_SHARE=.6;
...
// страж и уровни 10+ — шесть вперемешку из всех введённых.
export function stationPlan(level,boss){
 const known=CHAPTERS.filter(c=>level>=c.from).map(c=>c.station);
 if(!known.length)return [];
 if(boss||level>=10)return Array.from({length:6},(_,i)=>known[(i+level)%known.length]);
...
export function makeLevel(level,seed,tasks,{boss=false}={}){
 ...
 const rng=random((seed^Math.imul(level,0x9e3779b1))>>>0),track=createTrack(rng),k=level+1,stations=stationPlan(level,boss);
 ...
 if(!path)throw Error(`No reachable level: ${level}/${seed}`);
 if([-1,0,1].some(lane=>stationaryWins(course,lane)))throw Error(`Stationary level winner: ${level}/${seed}`);
