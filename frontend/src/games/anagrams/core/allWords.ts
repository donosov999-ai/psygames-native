 ...
 * минуту, на восемнадцать — это уже сеанс. Поэтому сортируем по нему, а при
 * равенстве по базе: без второго ключа порядок зависел бы от сортировки в
 * движке, и «уровень N» перестал бы повторяться.
 */
...
function лестница(locale: string): AllWordsPack[] {
  ...
  const из = [...набор].sort((a, b) =>
    (a.words.length - b.words.length) || (a.base < b.base ? -1 : a.base > b.base ? 1 : 0));
