...
export function AllWordsGame({ pack, seed, size, theme, now, onComplete, onProgress, maxListHeight, locale, rtl, подписьНайденного, labels }: AllWordsProps) {
  ...
  const сдать = React.useCallback((слово: string) => {
    ...
    /*
      ⚠️ `locale` В ЗАВИСИМОСТЯХ — НЕ ФОРМАЛЬНОСТЬ. Он уходит в `сдатьСлово` и
      решает, какой словарь спрашивать про БОНУСНОЕ слово. Без него колбэк держал
      язык, каким тот был на первом кадре: сменил человек язык внутри сессии — и
      копилка продолжала сверяться с прежним словарём, то есть засчитывала и не
      засчитывала слова чужого языка. Линтер называл это «missing dependency», и
      это был единственный из его упрёков здесь, за которым стояла поломка.
    */
  }, [pack, onComplete, now, locale]);
...
const стили = StyleSheet.create({
  ...
  /*
    ⚠️ `flexWrap` ЕСТЬ, А ГРАНИЦЫ ШИРИНЫ НЕТ — И ПЕРЕНОС НЕ РАБОТАЕТ. Ряду не от
    чего отсчитывать «кончилось место», поэтому он едет за край. Замер 07.09.2026
    на 375×812, уровень из восьми слов: сетка вылезала на 48 точек слева и на 75
    справа. На коротких уровнях помещалась — оттого дефект и не бросался в глаза.
  */
  список: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 8, paddingBottom: 2, maxWidth: '100%' },
  ...
  счёт: { fontSize: 13 },
  /*
    🔴 ПЕРЕНОС И ГРАНИЦА ШИРИНЫ — ОБЯЗАТЕЛЬНЫ ОБА, ИНАЧЕ РЯД НЕ ПЕРЕНОСИТСЯ.

    Было `{ flexDirection: 'row', gap: 10 }`. Четыре кнопки по `minWidth: 110`
    дают минимум 4×110 + 3×10 = 470 точек, а с текстом выходит 519 — при экране
    375. Замер живьём 07.09.2026 на 375×812: полоса вылезала на 72 точки СЛЕВА и
    на 72 СПРАВА, «Сбросить» и «Проверить» обрезаны краем, до «Проверить» нельзя
    дотянуться пальцем.

    ⚠️ `flexWrap` без границы ширины НЕ переносит — строка просто едет за край.
    Поэтому здесь и перенос, и `maxWidth: '100%'`: четыре кнопки ложатся в два
    ряда по две (2×110 + 10 = 230 ≤ 375), а на просторном экране остаются в один.
  */
...
/**
 * Стили наружу — ДЛЯ ПРОБЫ, которая складывает размеры, а не читает исходник.
 * Проверка «в файле есть flexWrap» зеленела бы и на сломанной вёрстке: он там был
 * и до починки, а ряд всё равно ехал за край, потому что не было границы ширины.
 */
export const стилиДляПробы = стили;
