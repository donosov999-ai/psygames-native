...
export const SESSION_MEASURE: Record<AttentionMode, { field: string; norm: string }> = {
  ...
  'choice-rt':        { field: 'hick_slope_ms_per_bit', norm: '🟢 КЛИНИЧЕСКИЙ ПОКАЗАТЕЛЬ ПОЯВИЛСЯ 16.09.2026 — до этого его не было вовсе, и здесь стояло «mean_rt, показателя НЕТ». Наклон Хика: RT ≈ a + b·log2(n), клиническое значение у b — мс на бит, скорость перебора вариантов (Hick 1952, Hyman 1953). ⚠️ НОРМЫ В БАТАРЕЕ НЕТ. ⚠️ Поле бывает null, и это достоинство: наклон требует минимум ДВУХ разных n, и пустое место честнее выдуманного нуля — ноль означал бы «перебор бесплатен». Рядом лежат hick_distinct_n, hick_trials и hick_by_n (среднее RT по каждому n), чтобы наклон можно было перепроверить. ⚠️ УСЛОВИЕ ДЕЙСТВИТЕЛЬНОСТИ: число вариантов меняется БЛОКАМИ ВНУТРИ партии при ОДНОМ окне ответа, а крестовина рисует все четыре позиции всегда (неактивные пустыми) — иначе в наклон влезет закон Фиттса и он станет смесью двух законов' },
