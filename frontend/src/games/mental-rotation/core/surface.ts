 ...
 */
export function shapeSurface(shape:Shape,size:number,axis:Axis='x',degrees=0,ghost:Shape=[]){
  ...
  for(const cube of shape)for(let f=0;f<FACES.length;f++){
    ...
    faces.push({id:`${cube.join(',')}:${f}`,depth,ghost:isGhost,fill:`rgb(${rgb.join(',')})`,points:points3.map(([x,y,z])=>[(x-z)*Math.sqrt(3)/2*scale+size/2,(-y+(x+z)/2)*scale+size/2])});
  }
