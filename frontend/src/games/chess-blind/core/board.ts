/* psygames-chess-blind-board · VER 3 · 27.08.2026 */
/**
...
/**
 * КЛЕТКА СЕТКИ ЭКРАНА → НАШ ИНДЕКС. Обратный ход того же перевода.
 *
 * ⚠️ ТЕЛО У НЕГО ТО ЖЕ САМОЕ, И ЭТО НЕ ОПЕЧАТКА: переворот по вертикали сам себе
 * обратен — `screenIndex(screenIndex(i)) === i` для всех 64 клеток, потому что
 * `7 − (7 − r) = r`. Отдельное ИМЯ нужно не ради тела, а ради читателя: в разборе
 * серии сетка обходится по строкам, и `coreIndex(row * 8 + col)` говорит, что
 * происходит, а `screenIndex(row * 8 + col)` в том же месте читается наоборот.
 *
 * 🔴 ЗАЧЕМ ВООБЩЕ ФУНКЦИЯ, ЕСЛИ ФОРМУЛА В ОДНУ СТРОКУ. Потому что до 27.08.2026
 * она стояла ТРЕТЬЕЙ копией: `(BOARD_SIDE - 1 - row) * BOARD_SIDE + col` был
 * выписан прямо в разборе доски серии, рядом со своим двойником в разборе доски
 * партии. Три копии одной формулы — три места, где её правят по одному, и шапка
 * этого файла ровно об этом: перевод координат живёт в ОДНОМ месте.
 */
export function coreIndex(square: number): number {
  return screenIndex(square);
}
