...
/**
 * Сколько фигур должны стоять в единственном экземпляре, чтобы квиз «розыск»
 * собрал все свои вопросы. Три — потому что вопросов в партии три
 * (`levelParams().questions`), а спросить «где стоит ♜» про одну из двух ладей
 * нельзя: верных ответов два.
 *
 * ⚠️ ДВЕ УНИКАЛЬНЫЕ ЕСТЬ ВСЕГДА — это короли. Поэтому без явного требования
 * недобор не падает и не ругается: `buildQuestions` режет список по
 * `Math.min(questions, uniques.length)`, и партия молча выходит на два вопроса
 * вместо трёх. Замер 27.08.2026: в полосе цели 10 таких позиций 27 из 149.
 */
export const LOCATE_MIN_UNIQUE = 3;
