...
/**
 * ДО КАКОГО УРОВНЯ ПРЕДМЕТ В ЛЕНТЕ ПОДПИСАН ИМЕНЕМ — 3.
 *
 * 🔴 ЗАЧЕМ. Два отчёта NZT-48 (31.08 и 02.09.2026) «нихуя не понятно ни по смыслу
 * игры» упирались в одно: на фазе размещения предмет нарисован фигурой и цветом,
 * а имя лежало только в `accessibilityLabel`. Человек связывал с местом «оранжевый
 * ромб», а на опросе его спрашивали «Оранжевая лампа?» — вход и выход стояли на
 * разных носителях, и приём не работал.
 *
 * 🔴 ПОЧЕМУ НЕ ВСЕГДА. Подписи убрал предыдущий заход не по недосмотру: плитку
 * ужали до 76×76 без подписи, чтобы лента и сцена мест влезли в один экран
 * (жалоба «бегаешь между двумя экранами», задача 9421ebcb). Возврат подписи
 * НИЖЕ плитки вернул бы и беготню. Поэтому подпись вложена ВНУТРЬ плитки
 * (фигура 34 → 26), высота ленты не меняется, а на четвёртом уровне подпись
 * снимается: приём к этому моменту пройден трижды, предметов становится 8 и
 * больше, и место в плитке нужнее подписи.
 *
 * Решение Дениса 06.09.2026: «подписи показывать только первые 2-3 уровня».
 */
export const MEMORY_PALACE_LABELLED_LEVELS = 3;

/** Подписывать ли предметы в ленте на этом уровне. */
export function palaceShowsItemNames(level: number): boolean {
  return Math.floor(level) <= MEMORY_PALACE_LABELLED_LEVELS;
}
