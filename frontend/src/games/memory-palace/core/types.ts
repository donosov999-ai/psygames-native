...
/**
 * ДО КАКОГО УРОВНЯ МАРШРУТ ПОСТОЯНЕН — 5.
 *
 * 🔴 ЗАЧЕМ ГРАНИЦА, А НЕ «ВСЕГДА ПЕРЕМЕШИВАТЬ». В приёме loci маршрут — это
 * ОПОРА, которую человек знает наизусть (свой дом, дорога на работу); на неё и
 * вешаются образы. Поэтому первые пять уровней маршрут неизменен: человек
 * осваивает сам приём на знакомой дороге, и трудность растёт только числом
 * мест.
 *
 * 🔴 ЗАЧЕМ ВООБЩЕ ПЕРЕМЕШИВАТЬ. Фаза «Маршрут» объявлена шагом 1 из 3
 * («Маршрут: запомните порядок мест»), но запоминать было нечего: порядок один
 * и тот же на всех пятнадцати уровнях, и экран сам писал «Порядок постоянный».
 * Правила обещали работу, которой нет.
 *
 * ⚠️ ОДНОГО ПЕРЕМЕШИВАНИЯ МАЛО, И ЭТО ЗАМЕР, А НЕ МНЕНИЕ. Номер места рисуется
 * в плитке (`MemoryPalaceGame`, ромб с `locus.order`) на ВСЕХ фазах, включая
 * опрос. Пока номер виден в момент вопроса, порядок читается с экрана, и
 * перемешивание не добавляет ни единицы нагрузки — лестница росла бы только на
 * бумаге. Поэтому с шестого уровня номер скрывается везде, кроме самой фазы
 * маршрута: там его показывают, чтобы было что запоминать.
 *
 * Решение Дениса 06.09.2026, вариант C.
 */
export const MEMORY_PALACE_FIXED_ROUTE_LEVELS = 5;

/** Перемешивается ли маршрут на этом уровне (и скрыт ли номер вне фазы маршрута). */
export function memoryPalaceRouteIsShuffled(level: number): boolean {
  return Math.floor(level) > MEMORY_PALACE_FIXED_ROUTE_LEVELS;
}
