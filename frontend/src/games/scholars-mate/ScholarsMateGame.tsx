...
export default function ScholarsMateGame({
  ...
  /**
   * 🔴 СРОК ПОКАЗА ВЕРДИКТА — НА ИГРОВЫХ ЧАСАХ, А НЕ НА НАСТЕННЫХ.
   *
   * Было `setTimeout(…, 550 | 1400)`. Пауза (меню паузы, окно отзыва, системный
   * диалог) останавливает `now`, но не останавливала обычный таймер: ответил,
   * нажал паузу в те же полсекунды — и позиция сменилась за спиной у меню.
   * Родня дефекта, найденного 09.09.2026 в «Доске в уме», где так же уезжала фаза.
   *
   * ⚠️ Тик 100 мс, а не новый таймер на каждый кадр: `now` замирает сам, и условие
   * просто не наступает, пока держат паузу. Шаг мельче окна вердикта (550 мс) и
   * мельче тика секундомера (200 мс) — на глаз переход не меняется.
   */
  React.useEffect(() => {
    if (!вердикт) return;
    const срок = now() + (вердикт.ok ? 550 : 1400);
    const t = setInterval(() => { if (now() >= срок) дальшеRef.current(); }, 100);
    return () => clearInterval(t);
  }, [вердикт]);   // eslint-disable-line react-hooks/exhaustive-deps

  /**
   * 🔴 ДОСКА ПОКАЗАНА СО СТОРОНЫ ТОГО, КТО ХОДИТ.
   *
   * 📍 ОТЧЁТ ДЕНИСА 05.09.2026 (`8eab6183`), просил не в первый раз: «надо
   * переворачивать доску, если я за чёрных играю… ты пытаешься ходить за белых,
   * теряешь время».
   *
   * ⚠️ ЗДЕСЬ СТОЯЛА ОРИЕНТАЦИЯ ПО ПЕРВОЙ ПОЗИЦИИ НАБОРА, и она держалась весь
   * подход — чтобы доска не переворачивалась посреди упражнения на узнавание
   * картинки. Довод верный, но лечил он уже вылеченное: колода лестницы
   * ОДНОРОДНА по цвету, и замер 06.09 это подтвердил — 50 колод, 460 позиций,
   * ни одной чужой стороны. То есть внутри подхода переворачивать было нечего,
   * а замок при этом ломал единственное место, где цвет всё-таки меняется.
   *
   * 📍 ЭТО МЕСТО — РЕЖИМ ПОТОКА, где колода склеивается из нескольких наборов с
   * разными семенами. Замер 06.09: 2803 позиции, **1414 (50,4%) показывались со
   * стороны соперника**, испорчены 15 колод из 15. Десять минут подряд — ровно
   * тот режим, в котором Денис и сидит, поэтому жалоба и не уходила.
   *
   * Ориентация по ЗАДАЧЕ, а не по текущему `fen`: внутри позиции в несколько
   * ходов сторона чередуется, и доска дёргалась бы на каждом полуходе.
   * Однородные колоды (лестница, узор, жертва) от этого не меняются вовсе —
   * там сторона одна на весь подход, и переворотов остаётся ноль.
   */
  const снизуБелые = React.useMemo(
    () => (задача ? sideToMove(задача) === 'w' : true),
    [задача],
  );
