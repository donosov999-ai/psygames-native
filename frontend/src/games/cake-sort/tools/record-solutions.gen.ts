/* psygames-cake-sort-record-solutions · VER 2 · 23.09.2026 */
/**
 * ⚠️ ОКРУЖЕНИЕ NODE ЗАДАЁТСЯ КЛЮЧОМ ЗАПУСКА, А НЕ ДОКБЛОКОМ. Докблок
 * `@jest-environment` читается только ПЕРВЫМ комментарием файла, а первой
 * строкой обязан стоять штамп редакции (§10). Два «первым» не уживаются,
 * поэтому окружение переехало в саму команду ниже — ярлык-исключение в
 * гейте не заводился: правило одно на все 147 ядер.
 *
 ...
 *   npx jest --env=node --testMatch '**\/tools/record-solutions.gen.ts' --testTimeout 3600000
 *
