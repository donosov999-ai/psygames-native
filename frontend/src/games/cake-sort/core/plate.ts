/* psygames-cake-sort-plate · VER 3 · 09.09.2026 */
/**
...
/**
 * 🔴 ХОД ЛЮБЫМ КУСКОМ, А НЕ ТОЛЬКО ВЕРХНИМ — РЕШЕНИЕ ДЕНИСА 16.09.2026.
 *
 * Дословно: «мы тапаем по пицце или тортику, он увеличивается до большого круга,
 * в нём мы выбираем нужный кусок».
 *
 * 📍 ЧТО ЭТО ЧИНИТ. `moveTop` берёт `src[src.length - 1]` — последний положенный.
 * Тарелка внутри стопка, а НАРИСОВАНА КРУГОМ: какой кусок «верхний», игрок не
 * видит ниоткуда. Отсюда отчёт c72e57cb «Не перетаскивается ничего» — человек
 * жал по нужному куску, а игра брала другой.
 *
 * 🔴 ГЛАВНОЕ СЛЕДСТВИЕ, И ОНО В НАШУ ПОЛЬЗУ: если брать можно любой кусок,
 * ПОРЯДОК внутри тарелки перестаёт значить что-либо — тарелка становится
 * МУЛЬТИМНОЖЕСТВОМ. Значит и состояний у решателя меньше: доски, отличавшиеся
 * только перестановкой кусков, теперь одно состояние.
 *
 * ⚠️ Кусок задаётся ВИДОМ, а не местом в списке. Два куска одного вида
 * неразличимы, и «взять третий по счёту» ничего бы не означало.
 */
export function moveType(board: Board, from: number, type: number, to: number): Board | null {
  if (from === to) return null;
  const src = board.plates[from] ?? [];
  const где = src.indexOf(type);
  if (где < 0) return null;
  if (!canPlace(board, to, type)) return null;
  const plates = board.plates.map((p) => [...p]);
  (plates[from] as number[]).splice(где, 1);
  (plates[to] as number[]).push(type);
  return collapse({ ...board, plates }).board;
}

/** Какие виды лежат в тарелке — по одному разу, по возрастанию. */
export function typesIn(board: Board, i: number): number[] {
  return [...new Set(board.plates[i] ?? [])].sort((a, b) => a - b);
}
