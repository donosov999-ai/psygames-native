...
function ключ(b: Board): string {
  /*
   * 🔴 ВМЕСТИМОСТЬ ВХОДИТ В КЛЮЧ. Тарелка на шесть и тарелка на восемь с ОДНИМ
   * И ТЕМ ЖЕ содержимым — разные состояния: в одну влезет ещё два сектора, в
   * другую ничего. Не включи её — решатель объявит виденным то, чего не видел,
   * и выбросит настоящие решения. Ровно эта ловушка стоила дня у переливалки:
   * там ключ собирался по содержимому сосудов без высоты и без камней.
...
export function lowerBound(b: Board): number {
  ...
  /*
   * 🔴 ДЕЛИМ НА САМЫЙ БОЛЬШОЙ КРУГ НА ДОСКЕ, А НЕ НА СВОЙ У КАЖДОЙ ТАРЕЛКИ.
   *
   * Оценка обязана оставаться ДОПУСТИМОЙ — не больше настоящего минимума, иначе
   * A* перестанет находить оптимум, а звёзды начнут врать в пользу игрока.
   * Самый большой круг даёт наименьшее число тарелок, в которые вид МОГ бы
   * уложиться, значит наименьшее `h`: занизить оценку безопасно, завысить —
   * нет. Пока все круги одинаковы, число побайтово прежнее.
   */
  const круг = Math.max(
    CIRCLE,
    ...b.plates.map((_, i) => capOf(b, i)),
    ...b.queue.map((_, i) => queueCapOf(b, i)),
  );
  let h = 0;
  for (const [t, k] of тарелок) {
    h += Math.max(0, k - Math.ceil((всего.get(t) ?? 0) / круг));
  }
  return h;
}
