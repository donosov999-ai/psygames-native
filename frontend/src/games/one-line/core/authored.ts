/* psygames-one-line-authored · VER 3 · 17.09.2026 */
/**
...
/**
 * 🔴 ТРЕНИРОВОЧНАЯ ФИГУРА — СВОЯ, А НЕ ПЕРВЫЙ УРОВЕНЬ.
 *
 * 📍 Отчёты тестировщика 12.09.2026 (c96bfdd3, 76d9a90e, 2.54.5, iPhone): «Почему стоит?
 * Не переходит дальше». Тренировка бралась генератором с номером 1, а с 22.08 первый номер —
 * рисованный «воздушный змей». То есть тренировка и первый уровень были ОДНОЙ фигурой: человек
 * решал змея, нажимал «Начать партию» — и видел того же змея с нулём рёбер. Смена экрана
 * читалась как «ничего не произошло». Замер 17.09 по ядру: путь решения обеих фигур
 * совпадал ход в ход (3-1-5-4-2-5-3-4).
 *
 * «БАНТ» — два треугольника с общей вершиной. Выбран не за простоту, а за правило, которому
 * тренировка учит: «в вершину вернуться можно, в ребро нельзя». Без повторного прохода через
 * середину его не нарисовать, так что правило проверяется делом за пять секунд. Замкнутая
 * фигура (у всех вершин чётная степень) — начать можно откуда угодно, подсвечен один из стартов.
 * Совпадать с рисованными уровнями фигура не должна — это стережёт проба one-line-intro-once.
 */
export const ONE_LINE_TRAINING_FIGURE: AuthoredLevel = {
  shape: 'bow-tie',
  vertices: [v(0, 0.5, 0.5), v(1, 0.16, 0.22), v(2, 0.16, 0.78), v(3, 0.84, 0.22), v(4, 0.84, 0.78)],
  edges: [e(0, 1, 2, 'single'), e(1, 2, 0, 'single'), e(2, 0, 3, 'single'), e(3, 3, 4, 'single'), e(4, 4, 0, 'single'), e(5, 0, 1, 'single')],
  solution: ['v1', 'v2', 'v0', 'v3', 'v4', 'v0', 'v1'],
};
