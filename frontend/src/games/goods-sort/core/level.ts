...
export const GS_RULES: LevelRule[] = [
   ...
   */
  /**
   * Ниша ровно на один товар. Без инлайн-текстов: поля `ru`/`en` у `LevelRule`
   * устарели (знают два языка из двенадцати), тексты лежат в словаре —
   * `lr_goods_sort_singlecap_*`.
   */
  { key: 'singlecap', fromLevel: 28 },
  ...
  { key: 'collapse', fromLevel: 30 },  // = COLLAPSE_FROM (опущен 56 → 30, решение Дениса 25.09.2026)
];
...
/**
 * 🔴 ОПУЩЕН С 56 ДО 30 РЕШЕНИЕМ ДЕНИСА 25.09.2026 — МЕХАНИКА БЫЛА СПРЯТАНА.
 *
 * Повод — отзыв тестировщика (8339d04f, 06.09): он описал ровно схлопывание
 * полок и предложил его «сделать», не подозревая, что оно уже построено и
 * доказано. Замер объяснил почему: при пороге 56 схлопывание попадало на ТРИ
 * уровня из шестидесяти (56, 58, 59), то есть на 5 % лестницы и после 55
 * ступеней игры. Готовая, проверяемая механика была недоступна почти никому.
 *
 * 📍 ЦЕНА ОПУЩЕНИЯ ПОМЕРЕНА, А НЕ ПРИКИНУТА (24.09.2026, пересборка лестницы
 * экспортёром `tools/export-levels.gen.ts` с проверкой поля `proven` у каждого
 * уровня со схлопыванием):
 *   порог 56 — 3 уровня из 60, недоказанных нет;
 *   порог 40 — 14 уровней, недоказанных нет;
 *   порог 30 — 20 уровней, недоказанных нет;   ← взято
 *   порог 20 — 30 уровней, недоказанных нет.
 * Решаемость не страдает ни при одном пороге: `dealCollapse` доказывает расклад
 * вместе с очередью. Вопрос был только про кривую сложности, и это продуктовое
 * решение, а не техническое.
 *
 * ⚠️ ПОЧЕМУ 30, А НЕ 20. Строгие уровни схлопывание пропускают, поэтому первая
 * схлопывающаяся ступень — 31-я, и дальше идёт ритм «две со схлопыванием, одна
 * строгая». Треть лестницы с механикой — она заметна, но остаётся особенной;
 * на половине (порог 20) перестала бы ею быть.
 *
 * ⚠️ ЗАДНИЕ РЯДЫ (`BACK_FROM = 52`) НАМЕРЕННО ОСТАВЛЕНЫ НА МЕСТЕ. Порядок
 * механик теперь «схлопывание с 31, задние ряды с 52», и правило «по одному
 * новому вопросу за раз» соблюдено: между ними двадцать ступеней.
 */
export const COLLAPSE_FROM = 30;
 ...
 *
 * 🔴 ШАГ РАСТЯНУТ С 3 ДО 12 ВМЕСТЕ С ПЕРЕНОСОМ ПОРОГА (25.09.2026). Формула
 * считает от `COLLAPSE_FROM`, и опущение порога 56 → 30 раздуло бы очередь на
 * ПОЗДНИХ уровнях, которых решение не касалось:
 *   было (порог 56, шаг 3):   L56 → 2 · L58 → 2 · L60 → 3
 *   стало бы (порог 30, шаг 3): L40 → 5 · L50 → 6 · L60 → 6 — потолок с полтинника
 *   взято (порог 30, шаг 12):  L31 → 2 · L40 → 2 · L50 → 3 · L60 → 4
 * То есть механика приходит раньше, но поздние доски остаются близки к прежним,
 * а не подменяются максимальной очередью. Рост при этом живой: 2 → 3 → 4 на
 * тридцати ступенях вместо двух ступенек на пяти.
 */
export function queueSize(L: number): number {
  if (!collapseLevel(L)) return 0;
  return Math.min(6, 2 + Math.floor((L - COLLAPSE_FROM) / 12));
}
 ...
 */
/**
 * 🔴 ПОРОГ 28, А НЕ 26 — РЕШЕНИЕ ДЕНИСА 07.09.2026.
 *
 * Механика работала с 26-го, но МОЛЧА: окно правил на 26-м занято «примёрзшим
 * рядом», а двух правил на одном уровне гейт не допускает — иначе человек
 * получает два новых объяснения разом и не запоминает ни одного. Ниша на один
 * товар при этом требует объяснения больше прочих: она выглядит как обычная, а
 * ведёт себя как замок, и понять это можно было только положив товар и увидев,
 * что место кончилось.
 *
 * ⚠️ Сливать два правила в одно окно отвергнуто: примёрзший ряд про «сюда
 * нельзя, пока не разморозишь», а ниша на один — про «думай на ход вперёд».
 * В одном окне они стирают друг друга.
 *
 * Свободные уровни рядом были 24, 27 и 28; взят 28 — два уровня разницы кривую
 * сложности не ломают, а дают отдышаться после примёрзшего ряда.
 */
export const SINGLE_CAP_FROM = 28;
...
/**
 * 🔴 ЁМКОСТИ ЖИВОЙ ПАРТИИ: СНАЧАЛА ЗАПОМНЕННЫЕ, ПОТОМ ФОРМУЛА — И НИКОГДА
 * МЕНЬШЕ ТОГО, ЧТО В НИШЕ УЖЕ ЛЕЖИТ.
 *
 * 📍 БОЕВОЙ СЛУЧАЙ ДЕНИСА 09.09.2026, 28-й уровень: «я оттуда вытащил один
 * товар, их было два изначально» — и обратно второй уже не лез. Разбор: раздача
 * обычного уровня ёмкостей не отдавала, в записи партии их не было, и на
 * возврате экран считал их СЕГОДНЯШНЕЙ формулой. Партию роздали ДО того, как
 * появилась ниша на один товар (`SINGLE_CAP_FROM`), полка честно получила два
 * товара по прежним правилам — и наутро та же полка стала полкой на один.
 *
 * Доска оказалась в состоянии, которого игра сама создать не может: вынуть
 * можно, положить назад нельзя, и на вид это ничем не объяснено. Класс дефекта —
 * «правило поменяли, а сохранённое состояние осталось по старому»; он повторится
 * при ЛЮБОЙ правке `capsFor`, поэтому лечится не осторожностью, а этой функцией.
 *
 * ⚠️ ПОДНИМАЕМ ЁМКОСТЬ ДО СОДЕРЖИМОГО, А НЕ ВЫБРАСЫВАЕМ ЛИШНИЙ ТОВАР. Доска
 * человека — это его партия; терять из неё товар игра права не имеет.
 */
export function capsForParty(L: number, cells: readonly unknown[][], saved?: number[] | null): number[] {
  const база = saved && saved.length === cells.length ? saved : capsFor(L, cells.length);
  return база.map((c, i) => Math.max(c, cells[i]?.length ?? 0));
}
...
export const THRESHOLD_RULES: Record<string, string | null> = {
  ...
  SINGLE_CAP_FROM: 'singlecap',  // ниша ровно на один товар — порог сдвинут на L28 и правило заведено 07.09.2026
  HIDDEN_FROM: 'hidden',
  JOKER_FROM: 'joker',
  MONO_FROM: 'mono',
  MOVING_FROM: 'moving',
...
export interface GsLayout {
  ...
  /**
   * Ширина САМОЙ НИШИ такой вместимости. Полка на один товар обязана быть
   * узкой — см. `nicheW` в `gsLayout`.
   */
  nicheW: (cap: number) => number;
...
export function gsLayout(
   ...
   */
  /**
   * 🔴 ПОТОЛОК ТОВАРА — РАЗМЕР В КАНОНИЧЕСКОЙ НИШЕ НА ТРИ, А НЕ «сколько влезет».
   *
   * 📍 ЗАМЕР 09.09.2026 (планшет 750, уровень 28): без потолка `itemBox(1)`
   * отдавал 115–148 точек против 69 у ниши на три — товар в полке на один
   * РАЗДУВАЛСЯ вчетверо по площади просто потому, что рядом никого нет. На
   * телефоне этого не видно: там всё упирается в высоту ряда (`fitsInRow`), и
   * дефект живёт только на широких экранах.
   *
   * Товар — предмет, а не заливка: банка не становится больше оттого, что полка
   * короче. И ровно из-за этого раздувания полку на один товар нельзя было
   * нарисовать узкой — её «ряд» занимал всю ячейку.
   *
   * ⚠️ ПОТОЛОК СЧИТАЕТСЯ ОТ `CAP` (три), А НЕ ОТ `capWide`. `capWide` — самая
   * жадная ниша ДОСКИ, и на уровне с нишей на четыре она даёт товар мельче
   * канонического; возьми её потолком — и вернулась бы та самая беда, ради
   * которой заведён `itemBox`: одна ниша на четыре ужимала товар во всём шкафу
   * (замер 03.09.2026, 44 → 33 по стороне).
   */
  const потолокТовара = Math.max(18, Math.min(
    148,
    Math.floor((cellW - CELL_GAP * (CAP - 1)) / (1 + (CAP - 1) * (1 - OVERLAP))),
    Math.max(floorItem, fitsInRow),
  ));
  ...
  const itemBox = (cap: number): { w: number; h: number } => {
    ...
    const w = Math.max(18, Math.min(148, поШирине, Math.max(floorItem, fitsInRow), потолокТовара));
    return { w, h: Math.round(w / ITEM_ASPECT) };
  };

  /**
   * 🔴 ПОЛКА НА ОДИН ТОВАР ОБЯЗАНА БЫТЬ УЗКОЙ.
   *
   * Денис 09.09.2026, дословно: «полка узкая должна быть под один товар, а не
   * широкая». До этой правки ниша любой вместимости рисовалась во всю ячейку, и
   * полка на один товар отличалась от полки на три ровно ничем — узнать её
   * вместимость можно было единственным способом: попробовать положить второй
   * товар и увидеть, что он вернулся. Ровно та угадайка, из-за которой в этом
   * же экране рисуются насечки ёмкости, звезда джокера и край второго ряда.
   *
   * ⚠️ ЯЧЕЙКА ОСТАЁТСЯ ШИРИНОЙ `cellW`, УЗКОЙ СТАНОВИТСЯ ТОЛЬКО НИША. Ряд ниш
   * центрирован, попадание пальца считает `nicheAtPoint` по СЕТКЕ, а товар
   * внутри ниши — `itemAtX` от ширины ЯЧЕЙКИ. Сожми саму ячейку — и разъедутся
   * оба: колонки перестанут совпадать между рядами, а палец начнёт брать
   * соседний товар. Поэтому узкая ниша ЦЕНТРИРУЕТСЯ в ячейке прежней ширины:
   * ряд товаров остаётся на той же оси, вся арифметика жеста не меняется, а
   * лишнее место занимает доска шкафа — как настоящая маленькая ячейка в мебели.
   *
   * Ширина выводится, а не назначается: у полной ниши по бокам от ряда товаров
   * остаётся `cellW − rowW`; узкая получает СТОЛЬКО ЖЕ полей вокруг своего ряда.
   */
  const nicheW = (cap: number): number => {
    const c = Math.max(1, Math.round(cap));
    if (c >= capWide) return cellW;
    const б = itemBox(c);
    const рядC = б.w + (c - 1) * (б.w * (1 - OVERLAP) + CELL_GAP);
    return Math.max(б.w + 6, Math.min(cellW, Math.round(рядC + (cellW - rowW))));
  };
  ...
  return { boardW, cellW, itemSize, itemH, nicheH, shelfH, overlap: OVERLAP, rowW, itemBox, nicheW, boardH, scrolls };
}
...
/**
 * 🔴 УРОВЕНЬ ВЗЯТ — ЭТО ЦЕЛЬ ПЛЮС ПУСТАЯ ОЧЕРЕДЬ (отчёт `be449226`, 08.09.2026).
 *
 * `goalMet` смотрит ТОЛЬКО на доску, и до 08.09 экран объявлял уровень взятым
 * по ней одной. С 56-го уровня полки закрываются и уходят, а на их место сверху
 * приходят отложенные — то есть пустая доска там означает не конец, а «текущие
 * полки разобраны, следующие ещё в очереди». Отсюда жалоба «перешёл на след
 * уровень, не закончив»: уровень засчитывался в середине.
 *
 * ⚠️ Ядро это знало и говорило прямо — в `isCleared` (board.ts) очередь и задние
 * ряды учтены с самого начала, там же записано «пустая доска при непустой
 * очереди — не победа, а промежуточное состояние». Знание было, а экран
 * пользовался другой функцией. Теперь функция одна, и зовут её оба.
 *
 * 📍 ЗАМЕР 08.09.2026: очередь есть на трёх уровнях из шестидесяти — L56 (цель
 * «убрать всё»), L58 («убрать названные»), L59 («убрать всё»). Очередь важна и
 * для `pick`: названный товар может лежать в отложенной полке, и убрать его с
 * доски — ещё не значит убрать с уровня.
 */
export function levelWon(
  state: {
    cells: readonly (readonly number[])[];
    queue?: readonly unknown[] | null;
    back?: readonly (readonly number[])[] | null;
  },
  goal: Goal,
): boolean {
  if (!goalMet(state.cells as number[][], goal)) return false;
  if ((state.queue?.length ?? 0) > 0) return false;
  return (state.back ?? []).every((b) => b.length === 0);
}
 ...
 */
/**
 * 🔴 КАЖДЫЙ ВИД ЛЕЖИТ ЦЕЛЫМИ ТРОЙКАМИ — ИНВАРИАНТ РАЗДАЧИ, А НЕ ПОЖЕЛАНИЕ.
 *
 * Тройка в игре — три ОДИНАКОВЫХ товара (`tripleIn`, исключений нет даже у
 * джокера). Значит вид, которого на раскладе не кратно трём, не исчезнет
 * никогда, а цель «разобрать всё» станет недостижимой: уровень нельзя выиграть.
 *
 * ⚠️ СЧИТАЕМ ВЕСЬ РАСКЛАД, А НЕ ДОСКУ. С L52 часть товара ждёт в очереди полок,
 * с L64 — в задних рядах. Посчитать одни ниши значит объявить битой каждую
 * доску со схлопыванием и пропустить настоящие поломки.
 *
 * 📍 ЗАЧЕМ ЭТО РЯДОМ С `provenUnsolvable`, У КОТОРОГО ЕСТЬ ПЕРЕБОР. Перебор
 * зовётся ТОЛЬКО на строгих уровнях (он дорогой), а замер 24.09.2026 показал
 * семь невыигрываемых уровней из 60, и все семь — МЯГКИЕ. Здесь же арифметика:
 * один проход по товарам, работает на любом уровне и ловит все семь.
 */
export function целымиТройками(
  cells: readonly (readonly number[])[],
  queue?: readonly Shelf[],
  back?: readonly (readonly number[])[],
): boolean {
  const счёт = new Map<number, number>();
  const добавить = (товары: readonly number[]) => {
    for (const t of товары) счёт.set(t, (счёт.get(t) ?? 0) + 1);
  };
  for (const c of cells) добавить(c);
  for (const s of queue ?? []) добавить(s.cell);
  for (const b of back ?? []) добавить(b);
  // ⚠️ Кратность считается ТРОЙКЕ (`CORE_TRIPLE`), а не ёмкости ниши по
  // умолчанию: сейчас оба равны трём, и подмена ничего не меняла бы — но
  // ёмкость уже бывает 2 и 4, и на первой же правке `CAP` инвариант стал бы
  // проверять не то, что написано в его названии.
  for (const n of счёт.values()) if (n % CORE_TRIPLE !== 0) return false;
  return true;
}

/**
 * 🔴 РАСКЛАД НЕ ПРИЕЗЖАЕТ С ГОТОВОЙ ТРОЙКОЙ — И ЭТО ОДНО ТРЕБОВАНИЕ НА ДВЕ ВЕТКИ.
 *
 * 📍 ЗАМЕР 25.09.2026, НАСТОЯЩИЙ ДЕФЕКТ, А НЕ ПОСЛЕДСТВИЕ ПРАВКИ. Требование
 * жило только в `dealBoard` (`dealtWrong`), а `dealCollapse` о нём не знал
 * вовсе. Пока схлопывание начиналось с L56, гейт `goods-sort-hint-legal` смотрел
 * уровни 3…41 и НИ РАЗУ не попадал в ветку схлопывания — то есть на уровнях
 * L56+ доски с готовой тройкой приезжали всё это время незамеченными. Опущение
 * порога 56 → 30 вывело их на L38 и L41: `[12,5,12,12]`, `[9,12,9,9]`,
 * `[11,11,11,9]` — тройка схлопывается первым касанием, очко и место достаются
 * ни за что.
 *
 * ⚠️ СЧИТАЕМ ВЕСЬ РАСКЛАД. Полка из очереди приедет и схлопнется сразу; задний
 * ряд выйдет вперёд и схлопнется сразу. Проверить одни ниши значит закрыть
 * ровно ту треть, что видна.
 */
function приехалРешённым(
  cells: readonly (readonly number[])[],
  queue?: readonly Shelf[],
  back?: readonly (readonly number[])[],
): boolean {
  if (cells.some((c) => tripleIn(c) !== null)) return true;
  if ((queue ?? []).some((s) => tripleIn(s.cell) !== null)) return true;
  return (back ?? []).some((b) => tripleIn(b) !== null);
}
...
export function dealBoard(L: number, pool: number[], narrow = false): {
  ...
  const dealtWrong = (b: number[][]): boolean =>
    // Дверь одна с веткой схлопывания: `приехалРешённым` (25.09.2026).
    приехалРешённым(b)
    // ⚠️ Кратность трём проверяется НА ВСЕХ уровнях, а не только на строгих:
    // перебор дорогой и зовётся под `strictPlacement`, а все семь найденных
    // невыигрываемых досок были мягкими и проходили мимо него молча.
    || !целымиТройками(b)
    || (strictPlacement(L) && provenUnsolvable(b, caps));
 ...
 */
/**
 * СТУПЕНИ ДЛИНЫ ОЧЕРЕДИ: сколько полок пробуем отложить, от заказанного вниз.
 *
 * 🔴 ОТДЕЛЬНОЙ ФУНКЦИЕЙ — ЧТОБЫ УТВЕРЖДЕНИЕ О ЛЕСТНИЦЕ БЫЛО ПРОВЕРЯЕМЫМ.
 * Пока лестница жила строчкой внутри `dealCollapse`, единственный способ
 * показать, что она работает, был статистическим: раздать много раз и увидеть
 * укороченную очередь. А раздача случайна (`generate` перемешивает пул без
 * семени), и такая проба то краснела, то зеленела от запуска к запуску —
 * поймано 07.09.2026, когда кэш раздач сделал случайность видимой.
 *
 * Здесь же утверждение точное: последовательность начинается с заказанной
 * длины, строго убывает и не опускается ниже единицы. Пустая очередь остаётся
 * последним средством и живёт не тут, а в запасной ветке `dealCollapse`.
 */
export function queueLadder(L: number): number[] {
  const ступени: number[] = [];
  for (let q = queueSize(L); q >= 1; q -= 2) ступени.push(q);
  return ступени.length ? ступени : [0];
}
...
export function dealCollapse(L: number, pool: number[], narrow = false, attempts = 24): {
  ...
  /*
   * 🔴 ОЧЕРЕДЬ УКОРАЧИВАЕТСЯ СТУПЕНЬКАМИ, А НЕ ПРОПАДАЕТ ЦЕЛИКОМ.
   *
   * Замер 07.09.2026 на САМОМ УЗКОМ живом наборе — «Еда», 6 видов товара, — по
   * 10 раскладов на ступень, бюджет решателя 20 000:
   *   L68  очередь 6 → доказано 2/10 · очередь 4 → 8/10 · очередь 2 → 9/10
   *   L130 очередь 6 → доказано 1/10 · очередь 4 → 6/10 · очередь 2 → 8/10
   * Решаемость съедает ИМЕННО длина очереди: при той же очереди задние ряды
   * стоят вдвое дешевле (L68 q4: b4 → 8/10 против b0 → 10/10).
   *
   * ⚠️ Прежний код после 24 неудач отдавал раздачу БЕЗ ОЧЕРЕДИ ВООБЩЕ, и окно
   * правил обещало «полка закрылась — сверху приходит новая» уровню, который
   * приходил без единой полки сверху. Обрыв в ноль заменён лестницей 6 → 4 → 2.
   *
   * 🔴 ГДЕ ИМЕННО ЭТО СЛУЧАЛОСЬ — важнее самого числа. По всем пяти наборам,
   * L1..200, до починки: «Еда» (6 видов) — 8 уровней из 100 без очереди,
   * попыток p95 24; наборы на 8, 9, 12 и 34 вида — ноль без очереди, попыток
   * p95 2–3. После: без очереди 0 везде, у «Еды» 16 уровней с укороченной
   * очередью, p95 9. Первый мой замер шёл на пуле из 6 видов, а вывод я записал
   * без этой оговорки — на широком наборе он был бы просто неверен.
   *
   * ⚠️ Ещё одна ловушка, на которую я тут наступил: `solveStrict` при
   * исчерпании бюджета отвечает «не решил», и это НЕ «нерешаемо». Сначала я
   * решил, что лестницу режет бюджет, — прогон с бюджетом 3 000 000 показал
   * обратное: 25 досок из 40 нерешаемы по-настоящему. Чинить надо раздачу.
   */
  const ступени = queueLadder(L);
  const наСтупень = Math.max(4, Math.floor(attempts / ступени.length));

  let всегоПопыток = 0;
  for (const ступень of ступени) {
    for (let k = 1; k <= наСтупень; k += 1) {
      всегоПопыток += 1;
      const попытка = собрать(всегоПопыток, ступень);
      if (попытка.proven) return { cfg, ...попытка, tries: всегоПопыток };
    }
  }
  /*
   ...
   */
  // ⚠️ Заслон и здесь: запасной путь отдаёт доску БЕЗ очереди, но с теми же
  // правилами. Раньше он не проверял ни готовую тройку, ни кратность.
  let запасCells = generate(pool, cfg.types, cfg.spares + cfg.obst.blocked + cfg.obst.locked, cfg.slots, всеCaps);
  for (let k = 0; k < 8 && (приехалРешённым(запасCells) || !целымиТройками(запасCells)); k += 1) {
    запасCells = generate(pool, cfg.types, cfg.spares + cfg.obst.blocked + cfg.obst.locked, cfg.slots, всеCaps);
  }
  return {
  ...
  function собрать(seed: number, сколькоВОчередь: number) {
    ...
    /*
     * ⚠️ ПЕРЕБОР ЗДЕСЬ, А НЕ ПОСЛЕ СБОРКИ. Готовая тройка приходит ИЗ `generate`,
     * а очередь и задние ряды лишь переносят содержимое ниш целиком — значит
     * отсечь её достаточно у источника, и тогда ни в очереди, ни за спиной её
     * тоже не будет. Так проверка не тратит попытки решателя: `coreSolvable` на
     * бракованный расклад даже не зовётся.
     *
     * Кратность тройкам проверяем тем же заходом: `proven` её ловит как
     * следствие, но только когда решатель успел уложиться в бюджет.
     */
    let все = generate(pool, cfg.types, cfg.spares + подПрепятствия, cfg.slots, всеCaps);
    for (let k = 0; k < 8 && (приехалРешённым(все) || !целымиТройками(все)); k += 1) {
      все = generate(pool, cfg.types, cfg.spares + подПрепятствия, cfg.slots, всеCaps);
    }
    /*
     ...
     */
    /*
     * 🔴 В ОЧЕРЕДЬ УХОДЯТ ПОЛКИ С ТОВАРОМ, А НЕ ПУСТЫЕ МЕСТА.
     *
     * 📍 Замер 07.09.2026, после того как механику стало видно на экране: из 524
     * полок во всех очередях L1..200 **480 были ПУСТЫМИ (92%)**, товаров на
     * полку в среднем 0,08. То есть обещанное правилом «сверху приходит новая
     * полка, и всё, что в ней лежит, посчитано в задаче» на деле означало
     * «приезжает пустое место»: ни планировать, ни ждать было нечего.
     *
     * Причина — порядок отбора: первыми шли ниши «с большой ёмкостью ИЛИ
     * ПУСТЫЕ», а пустых на доске много (их специально держат под манёвр и
     * препятствия). Отбор выгребал именно их.
     *
     * Теперь порядок обратный: сперва полки С ТОВАРОМ (среди них — с большой
     * ёмкостью, их изъятие меньше стесняет доску), пустые — только если полок с
     * товаром не хватило. Решаемость от этого страдает, и правильно: за неё
     * отвечает лестница `queueLadder`, которая укоротит очередь, а не подсунет
     * пустышку.
     *
     * Замер после правки, L1..200, по наборам:
     *   видов  8, 12, 34 — пустых 0%, товаров на полку 1,08…1,49, очередь
     *                      укорочена на 2–4 уровнях из 100
     *   видов  6 («Еда») — пустых 5%, 0,95 товара на полку, укорочена на 89:
     *                      узкому набору очередь из шести и правда не по силам
     *
     * ⚠️ ПРОБОВАЛ ОТДАВАТЬ ПОЛКИ ПОПЛОТНЕЕ (сортировкой по числу товаров) —
     * отклонено замером: товаров на полку 1,48 → 1,67, но укорочение на широком
     * наборе 2 → 12 уровней из 100. Длина очереди — это ОСЬ ТРУДНОСТИ, а
     * содержимое полки — приправа; менять первое на второе невыгодно.
     */
    const сТоваром = все.map((_, i) => i).filter((i) => все[i]!.length > 0);
    const просторные = сТоваром.filter((i) => (всеCaps[i] ?? CAP) > CORE_TRIPLE);
    const обычные = сТоваром.filter((i) => (всеCaps[i] ?? CAP) <= CORE_TRIPLE);
    const пустые = все.map((_, i) => i).filter((i) => все[i]!.length === 0);
    const кандидаты = [...просторные, ...обычные, ...пустые];
...
export function generate(pool: number[], types: number, spares: number, slots: number, caps?: number[]): number[][] {
  ...
  do {
     ...
     */
    /**
     * 🔴 ПОДМНОЖЕСТВО НИШ ВЫБИРАЕТСЯ ПО МЕСТУ, А НЕ ТОЛЬКО ПО СЧЁТУ — ИНАЧЕ
     * ТОВАР МОЛЧА ПРОПАДАЛ, И УРОВЕНЬ СТАНОВИЛСЯ НЕВЫИГРЫВАЕМЫМ.
     *
     * Раскладываем не по всем нишам, а по `used` из них: остальные обязаны
     * остаться пустыми (запас плюс места под препятствия). Но `used` — это
     * ЧИСЛО ниш, а вместимость у них разная (1, 2, 3 и 4). Если в выпавшее
     * подмножество набралось много мелких, места на `types × CAP` товаров в нём
     * не хватает — и товар, обошедший полный круг и не нашедший щели, ТИХО
     * исчезал: у цикла ниже нет ветки «не положили».
     *
     * 📍 ЧЕМ ЭТО КОНЧАЛОСЬ (замер 24.09.2026 по выгруженной лестнице). Тройка в
     * игре — три ОДИНАКОВЫХ товара, значит вид, потерявший один экземпляр, не
     * исчезнет уже никогда, а цель «разобрать всё» станет недостижимой. Таких
     * уровней было СЕМЬ из 60 на узкой лестнице и ВОСЕМЬ из 60 на широкой, а
     * при снятой защите ломается каждая пятая ступень. Проверку решаемости эти
     * доски проходили молча: `provenUnsolvable` зовётся только на строгих
     * уровнях, а все битые — мягкие. Общей вместимости при этом хватало с
     * избытком (42 места на 30 товаров) — не хватало её именно в подмножестве.
     *
     * 🔴 ПОЭТОМУ МЕНЯЕМ СОСТАВ, А НЕ РАЗМЕР. Первая редакция чинила это иначе —
     * добирала ниши сверх `used`, пока не наберётся места. Замер прогоном:
     * `goods-sort-free-niches` и `blockers-and-spares` покраснели на L38, L39,
     * L54, L60 — свободных ниш осталось по одной вместо гарантированных двух.
     * Свободная ниша в этой игре главный рабочий ресурс, и разменивать её на
     * починку нельзя. Здесь вместо этого САМЫЕ МЕЛКИЕ выбранные ниши меняются
     * на САМЫЕ ПРОСТОРНЫЕ оставшиеся: число занятых ниш прежнее, запас цел,
     * места хватает. Пустыми чаще становятся мелкие ниши — это плата, и она
     * несравнимо дешевле доски, которую нельзя разобрать.
     */
    const мест = (ниши: readonly number[]) => ниши.reduce((n, c) => n + (caps?.[c] ?? CAP), 0);
    const нужноМест = chosen.length * CAP;
    const всеНиши = shuffle(Array.from({ length: slots }, (_, i) => i));
    let order = всеНиши.slice(0, used);
    if (мест(order) < нужноМест) {
      const вне = всеНиши.slice(used).sort((a, b) => (caps?.[b] ?? CAP) - (caps?.[a] ?? CAP));
      order.sort((a, b) => (caps?.[a] ?? CAP) - (caps?.[b] ?? CAP));
      for (let k = 0; k < вне.length && k < order.length && мест(order) < нужноМест; k += 1) {
        const крупная = вне[k] as number;
        const мелкая = order[k] as number;
        if ((caps?.[крупная] ?? CAP) <= (caps?.[мелкая] ?? CAP)) break;
        order[k] = крупная;
      }
      // Порядок заполнения обязан снова стать случайным: после сортировки по
      // вместимости товар ложился бы сперва в самые тесные ниши.
      order = shuffle(order);
    }
    const ниш = order.length;
    let ci = 0;
    for (const it of sh) {
      for (let tries = 0; tries < ниш; tries++) {
        const c = order[ci % ниш] as number; ci++;
...
/**
 * 🔴 КАРТА ОСЕДАНИЯ: какая ниша на сколько рядов проехала за ход.
 *
 * ЗАЧЕМ ОТДЕЛЬНОЙ ФУНКЦИЕЙ, А НЕ СТРОКАМИ В ЭКРАНЕ. Расчёт жил внутри
 * компонента, и проверить его можно было только через монтирование экрана и
 * подбор такого хода, который закроет полку. Это дорого, хрупко и потому не
 * проверялось: половина случая — приход НОВОЙ полки сверху — так и оставалась
 * без движения.
 *
 * 📍 ЗАМЕР 11.09.2026 по коду ядра: закрытая полка уходит (`ниши.splice`), а
 * сверху встаёт новая (`ниши.unshift`) и получает СВЕЖИЙ номер (`н.id = следующий++`).
 * Поэтому прежний расчёт её пропускал: он искал номер в списке «до», а там
 * такого номера никогда не было. Столбец осаживался красиво, а верхняя ниша
 * возникала из ниоткуда одним кадром.
 *
 * Считаем по НОМЕРАМ, а не по местам: место меняется под ногами. Два случая:
 *   · номер был и переехал ВНИЗ по своему столбцу — проехал столько рядов,
 *     сколько мест своего столбца между «было» и «стало»;
 *   · номера в списке «до» нет вовсе — полка пришла из очереди из-за верхнего
 *     края шкафа, это один ряд сверху.
 *
 * ⚠️ Перескок между столбцами — НЕ оседание: это перетасовка, её делают
 * подвижные ниши, и у них своё движение.
 */
export function картаОседания(
  доМест: readonly number[], послеМест: readonly number[], столбцы: readonly number[],
): Record<number, number> {
  const карта: Record<number, number> = {};
  if (!доМест.length || доМест.length !== послеМест.length) return карта;
  послеМест.forEach((номер, место) => {
    const было = доМест.indexOf(номер);
    if (было < 0) {
      /*
       * ⚠️ ЗДЕСЬ СТОЯЛА ОГОВОРКА «только если в столбце и правда что-то ушло»,
       * и МУТАЦИЯ «снять оговорку» ПЕРЕЖИЛА гейт. Разбор показал, что случая,
       * который она ловит, не существует: длина столбца постоянна (ядро держит
       * её намеренно — `ниши.unshift` на каждый `splice`), номера уникальны,
       * значит новый номер в столбце возможен ТОЛЬКО взамен ушедшего. Оговорка
       * была защитой от невозможного и осталась бы непроверяемой строкой.
       *
       * Проверка на разную длину списков (выше) остаётся: она защищает от
       * сравнения двух РАЗНЫХ досок, и это случай достижимый.
       */
      карта[место] = 1;
      return;
    }
    if (было === место) return;
    if (столбцы[было] !== столбцы[место]) return;
