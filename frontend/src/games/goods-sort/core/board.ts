...
export function collapseTriples(board: Board): Board {
  ...
  while (again) {
    ...
    for (const c of порядокСтолбцов) {
      ...
      for (let k = 0; k < ниши.length; k += 1) {
         ...
         * новая — из очереди, а если очередь пуста, то пустое место.
         *
         * ⚠️ Прежняя редакция при пустой очереди просто вырезала нишу, и
         * столбец становился короче. Экран рисует поле по номеру места
         * (`row * cols + col`), поэтому укоротившийся столбец сдвинул бы всё
         * ниже него — на первом же схлопывании доска перетасовалась бы целиком.
         * Держим число мест неизменным: тогда «полка закрылась» выглядит ровно
  ...
  for (const c of порядокСтолбцов) {
    for (const н of столбцы.get(c) as Ниша[]) if (н.id === -1) н.id = следующий++;
  }

  /*
   * 🔴 ОБРАТНО РАСКЛАДЫВАЕМ ПО РЯДАМ — В ТОТ ЖЕ ПОРЯДОК МЕСТ, ЧТО ПРИШЁЛ.
   *
   * ⚠️ Здесь стояла обратная договорённость: складывать столбец за столбцом, с
   * оправданием «экран читает место ниши из `col`, а не из номера в массиве».
   * Экран этого НЕ делает: он рисует поле как `row * cols + col`, а ёмкости
   * берёт по номеру места (`capsForBoard`). Замер 07.09.2026, L58: первый же
   * ход менял `col` с [0,1,2,3,0,1,2,3…] на [0,0,0,0,0,0,1,1…] — то есть доска
   * пересобиралась из «по рядам» в «по столбцам». На экране это перетасовка
   * всего поля на первом ходу, а ёмкости разъезжаются с содержимым.
   *
   * Правило: `col` — это ГЕОМЕТРИЯ МЕСТА, она не меняется никогда. Меняется
   * только то, ЧТО в этом месте лежит. Порядок мест сохраняем побуквенно:
   * k-я сверху ниша столбца c ложится в k-е сверху место столбца c.
   */
  const место = new Map<number, number>();
  const плоско: Ниша[] = [];
  for (let s = 0; s < col.length; s += 1) {
    const c = col[s] as number;
    const k = место.get(c) ?? 0;
    место.set(c, k + 1);
    const н = (столбцы.get(c) as Ниша[])[k];
    /*
     * Столбец обязан остаться той же длины. Если это не так — молчать нельзя:
     * дальше пошла бы доска, где ниша стоит не на своём месте, и разошлись бы
     * решатель с экраном.
     */
    if (!н) throw new Error(`столбец ${c} укоротился: мест ${col.filter((x) => x === c).length}, ниш ${(столбцы.get(c) as Ниша[]).length}`);
    плоско.push(н);
  }
  const новСтолбцы = col;
