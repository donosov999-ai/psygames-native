 ...
 *
 * Каждое движение пальца зовёт `setHover`, а это смена состояния экрана: React
 * перестраивает ВСЕ ниши со стопками картинок. Геометрия (замер выше) стоит
 * доли микросекунды, значит если жест и лагает, то здесь.
 *
...
it('цена перерисовки доски на смену состояния', async () => {
  ...
  console.log('\nПЕРЕРИСОВКА ДОСКИ (то, что делает КАЖДОЕ движение пальца через setHover):\n' + строки.join('\n'));
