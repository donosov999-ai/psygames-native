...
export interface DotsPuzzle {
   ...
   *
   * Третий вид особой клетки после обычной и стены, и он не сводится ни к
   * одной из них: стена не пускает НИКОГО и потому просто вырезает поле, а
   * ворота пускают ровно одного — то есть не уменьшают задачу, а ПРИКАЛЫВАЮТ
   * маршрут одной пары к точке на поле. Для вывода это подарок и груз
   * одновременно: цепочка от ворот идёт в обе стороны, но остальные пары
   * обязаны эту клетку обойти.
   *
   * ⚠️ ХРАНИТСЯ РЯДОМ СО СТЕНАМИ И ПО ТОЙ ЖЕ ПРИЧИНЕ: величина, ставшая
   * данными, обязана быть прочитана ВСЕМИ потребителями (генератор, оба
   * решателя, проверка, ход игрока, доска). Со стенами я прошёл мимо трёх из
   * шести и починил только после замера — здесь список известен заранее.
   */
  gates?: { cell: Cell; pairId: string }[];
  difficulty: number;
  /**
...
export interface DotsSession {
  ...
  /**
   * ПАРЫ, ОТКРЫТЫЕ ПОШТУЧНО. Дешёвая подсказка против дорогой: показ решения
   * открывает доску ЦЕЛИКОМ, и после него разгадывать нечего. Здесь открывается
   * ровно один путь — застрявший видит, где ошибся, а остальная доска остаётся
   * задачей.
   *
   * ⚠️ ОБРАТНО НЕ УБИРАЕТСЯ, как и `solutionShown`: увиденное увидено. Иначе
   * «открыл → закрыл → уровень чистый» было бы дырой размером во всю игру.
   */
  revealedPairIds: readonly string[];
  history: DotsPaths[];
