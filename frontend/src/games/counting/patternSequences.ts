/* psygames-counting-pattern-sequences · VER 2 · 17.09.2026 */
/**
...
export function makeOptions(answer: number, count = 4, rng: Rnd = Math.random): number[] {
  ...
  while (opts.size < count) {
    ...
    // Было ещё `candidate > -1000`: при ответе ниже −1176 оно отбрасывает ВСЕ кандидаты, и цикл не кончается
    // (смена знака L21 и смесь дают такие ответы). При ответе ≥ 0 условие всегда выполнялось — L1–14 не изменились.
