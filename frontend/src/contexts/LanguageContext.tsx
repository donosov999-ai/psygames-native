...
const translations: Translations = {
  ...
  flexibilityGroupDesc: { ru: 'Переключаться между правилами, не залипая', en: 'Switch between rules without sticking' },
  flexibilityGroupFootnote: { ru: 'Цена переключения меряется одинаково во всех трёх: сколько теряешь, когда правило сменилось.', en: 'All three measure the same switch cost: what you lose when the rule changes.' },
  ...
  flexibilityGroupIntroDesc: {
    ru: 'Три пробы на цену переключения. «Следопыт» гоняет между двумя рядами — числами и буквами; «Переключение задач» меняет правило по сигналу; Висконсинский тест меняет правило МОЛЧА, и заметить это надо самому.\n\nМеряется у всех троих одно: сколько вы теряете в тот момент, когда правило сменилось. Эта потеря и есть цена переключения — она растёт от усталости и от того, насколько вы «залипли» в предыдущем правиле.',
    en: 'Three paradigms measuring switch cost. Trail Making alternates between numbers and letters; the Switching Task changes the rule on a cue; the Wisconsin test changes it SILENTLY, and noticing is on you.\n\nAll three measure the same thing: what you lose at the moment the rule changes. That loss is the switch cost — it grows with fatigue and with how stuck you are in the previous rule.',
  },
  riskGroupIntroDesc: {
