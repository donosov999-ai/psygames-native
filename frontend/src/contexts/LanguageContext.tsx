...
const translations: Translations = {
  ...
  fractalSolutionAskChild: { ru: "Показать ответ этой нижней сетки? Уровень за партию не засчитается, но и не понизится.", en: "Show the answer to this lower grid? The level won't count for this game, but it won't drop either." },
  fractalSolutionAskAll: { ru: "Показать ответ всей судоку? Партия закончится разбором: уровень не засчитается, но и не понизится.", en: "Show the answer to the whole sudoku? The game ends as a review: the level won't count, but it won't drop either." },
  fractalSolutionUsed: { ru: "Решение показано — уровень за эту партию не засчитается.", en: "Solution shown — the level won't count for this game." },
  ...
  /**
   * «Сокобан»: бочку только толкают. Попав в угол двух стен, она не сдвинется уже
   * никогда — и человек об этом узнаёт, только упёршись. Отчёт 59b99133 (17.09.2026):
   * «если я загнал фигуру так, что не могу вытолкнуть бочку, — партия проиграна или
   * есть способ вытолкать её по диагонали?». По диагонали не ходят: только отмена.
   */
  puzzleBarrelStuck: { ru: 'Бочка застряла в углу. Отмени ход или начни заново', en: 'The barrel is stuck in a corner. Undo the move or start over' },
  ...
  teachSlantFull: { ru: 'Узел «{n}» уже набрал своё число диагоналей. Остальные клетки вокруг него получают диагональ мимо этого узла.', en: 'The “{n}” corner already has its number of diagonals. Every other cell around it gets a diagonal that misses this corner.' },
  teachSlantNeed: { ru: 'Узлу «{n}» нужно ещё {k}, и пустых клеток вокруг него тоже {k}. Значит, во всех них диагональ идёт в этот узел.', en: 'The “{n}” corner still needs {k}, and exactly {k} empty cells surround it. So each of them gets a diagonal into this corner.' },
  teachSlantLoop: { ru: 'Диагональ «{a}» в этой клетке замкнула бы выделенную цепочку в петлю, а петель не бывает. Значит, здесь «{b}».', en: 'A “{a}” diagonal in this cell would close the highlighted chain into a loop, and loops are not allowed. So this cell is “{b}”.' },
  teachSlantTryClue: { ru: 'Проверка: если поставить здесь «{a}», дальше по правилам узел «{n}» не сможет набрать своё число. Значит, здесь «{b}».', en: 'Check: put “{a}” here and, following the rules, the “{n}” corner can no longer reach its number. So this cell is “{b}”.' },
  teachSlantTryLoop: { ru: 'Проверка: если поставить здесь «{a}», дальше по правилам диагонали замкнутся в петлю. Значит, здесь «{b}».', en: 'Check: put “{a}” here and, following the rules, the diagonals close into a loop. So this cell is “{b}”.' },
  teachGuessFirst: { ru: 'Первая попытка — {code}: цвета парами (номера — как в палитре сверху вниз). Ответ сразу скажет, сколько из этих цветов есть в коде и стоят ли они на своих местах.', en: 'First guess — {code}: colours in pairs (numbers follow the palette from top to bottom). The answer at once tells how many of these colours are in the code and whether they sit in the right places.' },
  teachGuessNext: { ru: 'Ответ на попытку {n}: ● {b} — цвет на своём месте, ○ {w} — цвет есть, но в другом месте. Кодов, согласных со всеми ответами: {k}. Ставим {code}: после любого ответа их останется не больше {m}.', en: 'Answer to guess {n}: ● {b} — a colour in its right place, ○ {w} — a colour that is in the code but elsewhere. Codes that fit every answer: {k}. We play {code}: whatever the answer, at most {m} will remain.' },
  teachGuessOnly: { ru: 'Ответ на попытку {n}: ● {b}, ○ {w}. Со всеми ответами согласен только один код — {code}. Ставим его.', en: 'Answer to guess {n}: ● {b}, ○ {w}. Only one code fits every answer — {code}. We play it.' },
  teachBoxFirst: { ru: 'Шары спрятаны — узнать о них можно только лучом. Стреляем отсюда: на этом входе возможные раскладки отвечают по-разному, значит выстрел скажет больше всего.', en: 'The balls are hidden — only a ray can tell you about them. We fire from here: the possible layouts answer differently at this entry, so this shot tells the most.' },
  teachBoxNext: { ru: 'Прошлый луч {answer}. Раскладок, согласных со всеми выстрелами: {k}. Стреляем отсюда: здесь они дают {r} разных ответов — выстрел их разделит.', en: 'The last ray {answer}. Layouts that fit every shot: {k}. We fire from here: they give {r} different answers at this entry — the shot will split them.' },
  teachBoxNextMany: { ru: 'Прошлый луч {answer}. Согласных раскладок пока не меньше {k}. Стреляем отсюда: здесь они расходятся сильнее всего — {r} разных ответов.', en: 'The last ray {answer}. At least {k} layouts still fit. We fire from here: they disagree most at this entry — {r} different answers.' },
  teachBoxPlace: { ru: 'Прошлый луч {answer}. Теперь все раскладки, согласные с выстрелами, отвечают одинаково на любой луч — отмечаем шары ({b}) и проверяем.', en: 'The last ray {answer}. Now every layout that fits the shots answers the same for any ray — we mark the balls ({b}) and check.' },
  teachBoxHit: { ru: 'поглощён — шар стоит прямо на его пути', en: 'was absorbed — a ball sits right in its path' },
  teachBoxReflect: { ru: 'вернулся к входу — шар по диагонали у самого входа, шары с обеих сторон пути или повороты привели луч обратно', en: 'came back to its entry — a ball sits diagonally next to the entry, balls sit on both sides of the path, or its turns led it back' },
  teachBoxThrough: { ru: 'прошёл насквозь, не повернув, — на его прямой и по бокам шаров нет', en: 'went straight through without turning — no balls on its line or beside it' },
  teachBoxTurn: { ru: 'вышел в другом месте — каждый поворот значит шар по диагонали впереди', en: 'came out somewhere else — every turn means a ball diagonally ahead' },
  ...
  puzzlesMinesIntroDesc: { ru: 'Под закрытыми клетками спрятаны мины — открой все клетки без мин. Число в открытой клетке — сколько мин в восьми соседних клетках, включая угловые. Пустая открытая клетка значит «рядом мин нет»: её соседи откроются сами. Пример: у открытой 1 осталась закрытой только одна соседняя клетка — мина в ней. Нажатие открывает клетку. Кнопка «Флажок» или долгое нажатие ставит флажок «здесь мина» — такую клетку случайно не откроешь. Нажатие по числу, вокруг которого флажков уже столько же, открывает остальных соседей. Доска решается без угадывания; флажки для победы не обязательны.', en: 'Mines hide under the closed cells — open every cell that has no mine. A number in an open cell says how many mines are among its eight neighbours, corners included. An open cell with no number means “no mines around”, and its neighbours open by themselves. Example: an open 1 with only one closed neighbour left — the mine is there. Tap to open a cell. The “Flag” button or a long press puts a flag for “mine here”, and a flagged cell can\'t be opened by accident. Tapping a number that already has that many flags around it opens the rest of its neighbours. The board is always solvable without guessing; flags are not required to win.' },
  puzzlesMosaic: { ru: 'Мозаика', en: 'Mosaic' },
  puzzlesMosaicDesc: { ru: 'Число — сколько чёрных в квадрате 3×3 вокруг клетки, считая её. Тап — чёрная, «Пусто» — белая; засчитается, когда решены все', en: 'A number counts the black cells in the 3×3 square around it, itself included. Tap for black, “Empty” for white; it counts once all are decided' },
  puzzlesMosaicIntroDesc: { ru: 'Каждую клетку надо сделать чёрной или белой. Число — сколько чёрных клеток в квадрате 3×3 с центром в этой клетке, считая её саму. У края квадрат меньше: в углу это 4 клетки, у стороны — 6. Пример: 9 — все девять чёрные, 0 — все белые, 4 в углу — все четыре чёрные. Нажатие по клетке красит её в чёрный, следующие нажатия — в белый и обратно в пусто. Кнопка «Пусто» или долгое нажатие начинает с белого. Решено, только когда вокруг каждого числа решены все клетки: белые тоже надо отметить.', en: 'Every cell must end up black or white. A number says how many black cells are in the 3×3 square centred on that cell, the cell itself included. At the edge the square is smaller: 4 cells in a corner, 6 along a side. Example: 9 means all nine are black, 0 means all white, a 4 in a corner means all four are black. Tap a cell to make it black; further taps make it white and then empty again. The “Empty” button or a long press starts with white. It is solved only when every cell around every number is decided — whites must be marked too.' },
  ...
  puzzlesRangeIntroDesc: { ru: 'Некоторые клетки надо закрасить чёрным. Число — сколько белых клеток видно из этой клетки по прямой вверх, вниз, влево и вправо до чёрной клетки или края поля, считая её саму. Клетка с числом всегда белая. Чёрные клетки не касаются друг друга стороной, а все белые клетки связаны в одну область. Пример: 2 в углу — кроме себя видна ровно одна клетка, значит, с одной стороны сразу чёрная, а с другой — одна белая и за ней чёрная. Нажатие красит клетку в чёрный; кнопка «Точка» или долгое нажатие ставит точку «точно белая». Точки для решения не обязательны.', en: 'Paint some cells black. A number says how many white cells can be seen from that cell in straight lines up, down, left and right until a black cell or the board edge, the cell itself included. A numbered cell is always white. Black cells never touch side by side, and all white cells join up into one area. Example: a 2 in a corner sees itself and exactly one more cell — so on one side the next cell is black, and on the other side there is one white and then a black. Tap to paint a cell black; the “Dot” button or a long press puts a dot for “surely white”. Dots are not required to solve.' },
  ...
  puzzlesRectanglesIntroDesc: { ru: 'Поле надо разрезать на прямоугольники. В каждом прямоугольнике ровно одно число, и оно равно его площади — числу клеток; квадрат тоже прямоугольник. Пример: 6 может быть полоской 1×6 или прямоугольником 2×3, стоя или лёжа, — выбирай тот, что не заденет другие числа. Проведи пальцем от угла до угла — появится рамка; одиночное нажатие берёт одну клетку. С кнопкой «Стереть» протяжка стирает линии внутри, не трогая рамку. Верный прямоугольник закрашивается; решено, когда закрашено всё поле.', en: 'Cut the board into rectangles. Each rectangle contains exactly one number, and that number is its area — how many cells it covers; a square counts as a rectangle. Example: a 6 can be a 1×6 strip or a 2×3 block, standing or lying — pick the one that doesn\'t cover another number. Drag from corner to corner to draw a frame; a single tap takes one cell. With the “Erase” button, a drag wipes the lines inside without touching the frame. A correct rectangle gets shaded; the puzzle is solved when the whole board is shaded.' },
  ...
  puzzlesNetDesc: { ru: 'Нажимай кусок — он поворачивается. Включи «Повернуть обратно» — повернётся в другую сторону. Собери всю сеть', en: 'Tap a piece to turn it. Switch on “Turn back” to turn it the other way. Join the whole network' },
  ...
  puzzlesFloodDesc: { ru: 'Залить всё поле одним цветом за отведённое число ходов. Тапни клетку того цвета, в который красить.', en: 'Flood the whole board with a single colour inside the move limit. Tap a square of the colour you want.' },
  puzzlesSameGame: { ru: 'Снос групп', en: 'Same Game' },
  puzzlesSameGameDesc: { ru: 'Снимать группы одного цвета: чем крупнее группа, тем больше очков. Тапни группу, потом ещё раз — снимется.', en: 'Clear groups of one colour: the bigger the group, the bigger the score. Tap a group, then tap again to clear it.' },
  ...
  puzzlesMagnetsDesc: { ru: 'Сверху и слева — сколько «+» в ряду, снизу и справа — «−». Тап — магнит; «Пусто» — пустое домино, его тоже ставят', en: 'Edge numbers: top and left count the “+” in that line, bottom and right the “−”. Tap for a magnet; “Empty” lays blank dominoes — they count too' },
  puzzlesMagnetsIntroDesc: { ru: 'Поле разбито на домино — видны только их контуры. Каждое домино — либо магнит с полюсами «+» и «−», либо пустое. Числа вокруг поля считают полюса в своём ряду: сверху и слева — сколько «+», снизу и справа — сколько «−»; где числа нет, ряд свободен. Одинаковые полюса не касаются стороной, пустые половинки могут стоять где угодно. Пример: у строки слева 2, справа 1 — в ней два «+», один «−», а остальные клетки пустые. Нажатие по домино кладёт магнит «+» на эту половинку, второе нажатие переворачивает его, третье убирает. Кнопка «Пусто» под полем или долгое нажатие ставит пустое домино. Решено, только когда решено каждое домино: пустые тоже надо поставить.', en: 'The board is split into dominoes — you only see their outlines. Each domino is either a magnet with a “+” end and a “−” end, or blank. The numbers around the board count poles in their line: top and left say how many “+”, bottom and right how many “−”; a line with no number is free. Equal poles never touch side by side; blank halves can sit anywhere. Example: a row with 2 on the left and 1 on the right holds two “+” and one “−”, and all its other cells are blank. Tap a domino to lay a magnet with “+” on that half; tap again to flip it, a third time to clear it. The “Empty” button below the board, or a long press, lays a blank domino. It is solved only when every domino is decided — blank ones must be placed too.' },
  puzzlesPearlDesc: { ru: 'Замкнутая петля: белый кружок требует прямой, чёрный — поворота. Тапни между клетками — проведёшь черту.', en: 'A closed loop: white circles need a straight, black ones a turn. Tap between cells to draw a line.' },
  ...
  puzzlesMapDesc: { ru: 'Соседние области — разного цвета. Цвет тянут пальцем от закрашенной области к пустой; «Карандаш» — пометка-догадка', en: 'Colour the regions so that neighbours differ. Drag from a coloured region to an empty one to carry the colour; the second action leaves a pencil mark' },
  puzzlesMapIntroDesc: { ru: 'Каждую область карты надо закрасить одним из четырёх цветов так, чтобы соседи с общей границей были разного цвета; касание только углом соседством не считается. Уже закрашенные области менять нельзя — с них и берут цвет. Проведи пальцем от закрашенной области к пустой — цвет перенесётся; протяжка от пустой области стирает цвет. С кнопкой «Карандаш» цвет ложится штриховкой — это заметка «может быть этот цвет», в одной области можно оставить несколько. Пример: у области три соседа уже трёх разных цветов — ей остаётся только четвёртый.', en: 'Colour every region of the map with one of four colours so that neighbours sharing a border get different colours; touching only at a corner doesn\'t count. Regions coloured from the start can\'t be changed — that\'s where you pick colours up. Drag from a coloured region into an empty one to carry the colour over; dragging from an empty region clears a colour. With the “Pencil” button the colour goes on as hatching — a note meaning “might be this colour”, and one region can hold several. Example: a region whose three neighbours already use three different colours can only take the fourth.' },
  puzzlesSignpostDesc: { ru: 'Соединить числа по порядку, каждый шаг — вдоль стрелки. Тяни от клетки к следующей.', en: 'Link the numbers in order, each step along an arrow. Drag from one cell to the next.' },
  ...
  puzzlesGalaxiesIntroDesc: { ru: 'Поле надо разрезать стенками на области. В каждой области ровно одна точка, и область симметрична вокруг неё: поверни её на пол-оборота вокруг точки — совпадёт сама с собой. Точка стоит в центре клетки, на стенке между клетками или в углу. Каждая клетка входит в какую-то область, лишних стенок внутри области нет. Пример: вокруг точки на стенке между двумя клетками годится область ровно из этих двух клеток. Верная область сразу перекрашивается: с белой точкой — в белый, с чёрной — в тёмный. Серая клетка ещё не в верной области, и пока серые есть, головоломка не решена. Нажатие по линии между клетками ставит или убирает стенку. Кнопка «Пометка» рисует стрелки-заметки для себя, для решения они не нужны.', en: 'Cut the board with walls into regions. Each region holds exactly one dot and is symmetric around it: turn it half a turn about the dot and it lands on itself. A dot sits in the middle of a cell, on the wall between two cells, or on a corner. Every cell belongs to some region, and there are no extra walls inside a region. Example: around a dot on the wall between two cells, a region of just those two cells works. A correct region changes colour at once — white for a white dot, dark for a black dot. A grey cell is not yet in a correct region, and while any grey remains the puzzle is not solved. Tap the line between cells to place or remove a wall. The “Mark” button draws reminder arrows for yourself; you don\'t need them to solve.' },
  ...
  puzzlesLightUpIntroDesc: { ru: 'Надо осветить все белые клетки. Фонарь светит на свою клетку и по прямой во все четыре стороны до чёрной клетки. Два фонаря не должны освещать друг друга. Число на чёрной клетке — сколько фонарей стоит вплотную к ней сверху, снизу, слева и справа; рядом с чёрной клеткой без числа фонарей может быть сколько угодно. Пример: 0 — рядом ни одного фонаря, 4 — фонари со всех четырёх сторон. Нажатие ставит или убирает фонарь; кнопка «Не ставить» или долгое нажатие ставит метку «здесь фонаря нет». Освещённое желтеет, ошибки краснеют: решено, когда всё жёлтое и красного нет.', en: 'Light up every white cell. A lamp lights its own cell and shines in straight lines in all four directions until a black cell. No two lamps may shine on each other. A number on a black cell says how many lamps sit right next to it — above, below, left and right; a black cell without a number can have any number of lamps beside it. Example: 0 means no lamp beside it, 4 means lamps on all four sides. Tap to place or remove a lamp; the “No lamp” button or a long press marks “no lamp here”. Lit cells turn yellow and mistakes turn red: it is solved when everything is yellow and nothing is red.' },
  puzzlesLoopyDesc: { ru: 'Собрать одну замкнутую петлю по числам в клетках. Тапни между двумя точками — проведёшь черту.', en: 'Build one closed loop guided by the numbers. Tap between two dots to draw a line.' },
  /**
  ...
  spatialNetslide: { ru: 'Сеть со сдвигом', en: 'Sliding Network' },
  ...
  waterSortEmptyVessel:   { ru: "Пустая пробирка", en: "Empty tube" },
  ballSortIntroDesc: { ru: "Нажми трубку, чтобы поднять верхние шарики одного цвета, и вторую — чтобы перекатить их туда. Катить можно только на свой цвет или в пустую трубку, и только пока есть место. Уровень взят, когда каждая трубка пуста или полна одним цветом. Заранее считай, куда денется цвет, который сейчас мешает: свободных трубок всего две.", en: "Tap a tube to lift its top run of one colour, tap another to roll them across. You may only roll onto the same colour or into an empty tube, and only while there is room. The level is done when every tube is empty or full of one colour. Think ahead about where the colour in your way will go: there are only two free tubes." },
  ballSortLvlParams: { ru: "{c} цветов · по {h} шариков · {e} свободные трубки", en: "{c} colours · {h} balls each · {e} free tubes" },
  ballSortHint: { ru: "Нажми трубку, потом вторую — верхние шарики перекатятся", en: "Tap one tube, then another — the top balls roll across" },
  ballSortEmptyVessel: { ru: "Пустая трубка", en: "Empty tube" },
  nutSortIntroDesc: { ru: "Нажми стержень, чтобы снять верхние гайки одного цвета, и второй — чтобы накрутить их туда. Накручивать можно только на свой цвет или на пустой стержень, и только пока есть место. Уровень взят, когда каждый стержень пуст или занят одним цветом. Заранее считай, куда денется цвет, который сейчас мешает: свободных стержней всего два.", en: "Tap a bolt to lift its top run of one colour, tap another to screw them on. You may only add onto the same colour or onto an empty bolt, and only while there is room. The level is done when every bolt is empty or holds one colour. Think ahead about where the colour in your way will go: there are only two free bolts." },
  nutSortLvlParams: { ru: "{c} цветов · по {h} гаек · {e} свободные стержни", en: "{c} colours · {h} nuts each · {e} free bolts" },
  nutSortHint: { ru: "Нажми стержень, потом второй — верхние гайки перекрутятся", en: "Tap one bolt, then another — the top nuts screw across" },
  nutSortEmptyVessel: { ru: "Пустой стержень", en: "Empty bolt" },
  sortRefuseFull: { ru: "Там уже полно", en: "No room there" },
  sortRefuseColour: { ru: "Сверху другой цвет — сюда нельзя", en: "A different colour is on top — not here" },
  sortRefusePointless: { ru: "Так ничего не изменится", en: "That would change nothing" },
  sortRefuseLocked: { ru: "Этот ещё закрыт", en: "This one is still locked" },
  waterSortBenefitPlan: { ru: 'Считать наперёд: ход, освобождающий место сейчас, может закрыть выход потом', en: 'Planning ahead: a move that frees a slot now can block the way out later' },
  ...
  // Подписи вкладок нижнего тулбара. Короткие нарочно: пять штук делят ширину
  // телефона, и длинное слово там обрежется многоточием у всех сразу.
  // ⚠️ ТРЁХ ИЗ ПЯТИ ЗДЕСЬ НЕТ — и это не пропуск. «Главная», «Прогресс» и
  // «Зарядка» уже живут ключами `home`, `progress` и `warmupPickerTitle`; свои
  // копии поймал гейт dictionary-duplicates в тот же день, и он прав: одно
  ...
  spatialLabSaveOldUnreadable: { ru: 'Старое сохранение не прочитано и не перезаписано. Эта партия без сохранения.', en: 'Your old save could not be read and was left untouched. This game will not be saved.' },
  spatialLabSaveUnavailable: { ru: 'Хранилище недоступно. Эта партия без сохранения.', en: 'Storage is unavailable. This game will not be saved.' },
  spatialLabSaveFailed: { ru: 'Не удалось сохранить последние ходы.', en: 'Could not save your latest moves.' },
  spatialLabRestoring: { ru: 'Восстанавливаю локальную партию…', en: 'Restoring your game…' },
  spatialLabRedo: { ru: 'Вернуть', en: 'Redo' },
  spatialLabReplay: { ru: 'Повтор', en: 'Replay' },
  spatialLabNew: { ru: 'Новая', en: 'New' },
  spatialLabTurnLeft: { ru: 'Повернуть влево', en: 'Turn left' },
  spatialLabTurnRight: { ru: 'Повернуть вправо', en: 'Turn right' },
  spatialLabRestartConfirm: { ru: 'Начать заново? Текущие ходы будут потеряны.', en: 'Start over? Your current moves will be lost.' },
  spatialLabStay: { ru: 'Остаться', en: 'Keep playing' },
  spatialLabEasier: { ru: 'Проще', en: 'Easier' },
  spatialLabHarder: { ru: 'Сложнее', en: 'Harder' },
  ...
  spatialLabLocked: { ru: 'Закреплена', en: 'Locked in place' },
  spatialLabTurning: { ru: 'Поворот…', en: 'Turning…' },
  spatialLabReplayProgress: { ru: 'Повтор: {i}/{n}', en: 'Replay: {i}/{n}' },
  spatialLabSolved: { ru: 'Готово! Задача решена.', en: 'Done! Puzzle solved.' },
  spatialLabOpenEnds: { ru: 'Открытых концов: {n}', en: 'Open ends: {n}' },
  spatialLabBlockPos: { ru: 'Блок: {r}, {c}', en: 'Block: {r}, {c}' },
  spatialLabWarmupNote: { ru: 'Шаг зарядки · отдельная партия, личное сохранение не меняется', en: 'Workout step · a separate game, your own save stays as it is' },
  spatialLabLocalSave: { ru: 'Партия и прогресс сохраняются только на этом устройстве', en: 'Your game and progress are saved on this device only' },
  spatialLabTwiddleL1: { ru: 'Один поворот: блок уже выделен, направление подсказано.', en: 'One turn: the block is already highlighted and the direction is shown.' },
  spatialLabTwiddleL2: { ru: 'Один поворот: блок и направление найди сам.', en: 'One turn: find the block and the direction yourself.' },
  spatialLabTwiddleL3: { ru: 'Два поворота одного и того же блока — разворот на 180°.', en: 'Two turns of the same block — a 180° turn.' },
  spatialLabTwiddleL4: { ru: 'Два поворота разных блоков, которые перекрываются.', en: 'Two turns of different blocks that overlap.' },
  ...
  spatialLabNetL3: { ru: 'Неверно повёрнуты две соседние трубы, соединённые друг с другом.', en: 'Two neighbouring pipes that join each other are turned the wrong way.' },
  spatialLabNetL4: { ru: 'Неверно повёрнуты Т-развилка и три трубы рядом с ней.', en: 'A T-junction and the three pipes next to it are turned the wrong way.' },
  spatialLabNetL5: { ru: 'Сбиты все трубы, которые можно повернуть, — собери сеть целиком.', en: 'Every pipe that can turn is off — rebuild the whole network.' },
  spatialLabNetPatch: { ru: 'Поле {w}×{w}. Неверно повёрнутых труб: {a}, все одним связным участком; развилок среди них — не меньше {j}.', en: '{w}×{w} board. Wrongly turned pipes: {a}, all in one connected patch; junctions among them: at least {j}.' },
  spatialLabNetLoops: { ru: 'Замкнутых колец в сети: {c}.', en: 'Closed loops in the network: {c}.' },
  spatialLabNetNoColour: { ru: 'Трубы, до которых дошла вода, не подсвечиваются.', en: 'Pipes the water has reached are not shown in colour.' },
  spatialLabSixteenGoal: { ru: 'Нажми клетку — выделятся её строка и столбец. Стрелки внизу сдвигают их по кругу. Расставь числа по строкам от 1 до {n}.', en: 'Tap a cell to select its row and column. The arrows below slide them round. Put the numbers in order, row by row, from 1 to {n}.' },
  spatialLabNetslideGoal: { ru: 'Нажми клетку — выделятся её строка и столбец. Стрелки внизу сдвигают их по кругу. Соедини все трубы с источником, без открытых концов.', en: 'Tap a cell to select its row and column. The arrows below slide them round. Connect every pipe to the source, with no open ends.' },
  spatialLabShiftRowLeft: { ru: 'Сдвинуть строку влево', en: 'Slide row left' },
  spatialLabShiftRowRight: { ru: 'Сдвинуть строку вправо', en: 'Slide row right' },
  spatialLabShiftColUp: { ru: 'Сдвинуть столбец вверх', en: 'Slide column up' },
  spatialLabShiftColDown: { ru: 'Сдвинуть столбец вниз', en: 'Slide column down' },
  spatialLabLinePos: { ru: 'Строка {r}, столбец {c}', en: 'Row {r}, column {c}' },
  spatialLabGuideRowLeft: { ru: 'Выделенная строка: сдвинь влево ←.', en: 'Highlighted row: slide it left ←.' },
  spatialLabGuideRowRight: { ru: 'Выделенная строка: сдвинь вправо →.', en: 'Highlighted row: slide it right →.' },
  spatialLabGuideColUp: { ru: 'Выделенный столбец: сдвинь вверх ↑.', en: 'Highlighted column: slide it up ↑.' },
  spatialLabGuideColDown: { ru: 'Выделенный столбец: сдвинь вниз ↓.', en: 'Highlighted column: slide it down ↓.' },
  spatialLabShiftL1: { ru: 'Один сдвиг: линия выделена, направление подсказано.', en: 'One slide: the line is highlighted and the direction is shown.' },
  spatialLabNetslideShifts: { ru: 'Поле {w}×{w}. Сдвигов, которыми перемешана сеть: {s}.', en: '{w}×{w} board. Slides used to scramble the network: {s}.' },
  ...
  puzzlesNetIntroDesc: { ru: 'Цель — соединить все трубы с источником так, чтобы не осталось открытых концов. Нажимай кусок — он поворачивается на четверть. Нужно в другую сторону — включи «Повернуть обратно» под полем: пока кнопка горит, нажатие поворачивает кусок обратно.', en: 'Goal: connect every pipe to the source with no open ends. Tap a piece to turn it a quarter. Need the other way? Switch on “Turn back” below the board: while it is lit, a tap turns the piece back.' },
  ...
  wordsWarmupTitle: { ru: 'Словесная зарядка', en: 'Word warm-up' },
  wordsWarmupDesc: { ru: 'Анаграммы, филворды и беглость речи подряд, каждое со своего уровня', en: 'Anagrams, word search and verbal fluency in turn, each from its own level' },
  warmupPlanCount: { ru: 'Подходов: {n}', en: 'Rounds: {n}' },
  chessWarmupTitle: { ru: 'Шахматная зарядка', en: 'Chess warm-up' },
  chessWarmupDesc: { ru: 'Оба упражнения подряд, каждое со своего уровня. Уровни зарядка не двигает', en: 'Both exercises in turn, each from its own level. The warm-up does not move your levels' },
  ...
  sortingGroup: { ru: 'Сортировка', en: 'Sorting' },
  sortingGroupDesc: { ru: 'Разложить по ёмкостям, где место кончается', en: 'Fill containers where space runs out' },
  sortingGroupFootnote: { ru: 'Обе пробы про один дефицит: не в каком порядке класть, а КУДА — свободных мест меньше, чем предметов.', en: 'Both are about one scarcity: not what order to place things in, but WHERE — there are fewer free slots than items.' },
  sortingGroupIntroDesc: { ru: 'Две пробы про один дефицит: не в каком ПОРЯДКЕ класть, а КУДА. В «Сортировке товаров» ниша вмещает от одного до четырёх предметов, и три одинаковых исчезают; в «Переливалке» слой можно лить только на свой цвет и только пока в колбе есть место.\n\nЭтим они отличаются от «Башен», куда просятся по названию. Там ход ограничен ПОРЯДКОМ — больший диск не ложится на меньший, — а места хватает всегда. Здесь наоборот: правило простое, а свободных мест меньше, чем предметов, и почти каждый законный ход сужает будущее. Выигрывает не тот, кто быстро находит, куда влезет, а тот, кто держит в голове, что останется после хода.', en: 'Both are about one scarcity: not what ORDER to place things in, but WHERE. In Goods Sorting a niche holds one to four items and three alike vanish; in Water Sort a layer pours only onto its own colour and only while the tube has room.\n\nThat is what separates them from Towers, where the name suggests they belong. There a move is limited by ORDER — a larger disc never sits on a smaller one — and room never runs out. Here it is the reverse: the rule is simple, but free slots are fewer than items, and almost every legal move narrows the future. The winner is not whoever finds a slot fastest, but whoever holds in mind what will be left after the move.' },
  anagramCrossword: { ru: 'Кроссворд', en: 'Crossword' },
  anagramCrosswordHint: { ru: 'Слова из этих букв вписываются в сетку и пересекаются: найденная буква открывает соседнее слово', en: 'Words from these letters go into a grid and cross: a found letter opens the word beside it' },
  ...
  numberRunIntroDesc: {
    ru: 'Ты ведёшь одно число по дороге и на ходу решаешь, что подобрать. Синее число прибавит написанное, красное — вычтет. Берёт середина твоего числа, а не края: чтобы взять из ряда несколько чисел, проведи своё число вбок прямо через них. Змейку синих веди пальцем, красные рядом объезжай. Столб делит дорогу надвое: сторону выбери до него — по сумме её стопки, через столб не перебраться. Из стен во всю дорогу проезжаешь одну, и её действие применится к твоему числу: бери меньший минус или сравни «×2» с прибавкой. Зелёная площадка перебрасывает через разрыв — мимо неё падение — и уносит над большим красным. Двенадцать этапов идут подряд, без остановок и без сброса числа; на финише число пробивает стены, сколько хватит. В режиме уровней на дороге встречаются задачи хаба «Счёт»: пример над тремя арками — въезжай в арку с верным ответом; янтарные числа — собери из них ровно число на табло. Каждый третий уровень охраняет страж: доберись до финиша с числом не меньше его. Ряд чисел на табло — въезжай в арку с его продолжением; числовая прямая поперёк дороги — проезжай там, где лежит значение выражения. Знаки над дорогой запомни по порядку: через участок выбери их в том же порядке.',
    en: 'You steer one number down the road and decide on the move what to pick up. A blue number adds what is written on it, a red one subtracts. Your number takes what its middle runs over, not what its edges brush: to take several numbers from one row, slide your number sideways right through them. Follow a snake of blues with your finger and swerve around the reds beside it. A pole splits the road in two: pick your side before it by the sum of that stack — you cannot cross the pole. Of a row of walls across the road you drive through one, and its action applies to your number: take the smaller minus, or compare ×2 with the plus. The green pad throws you across a gap — miss it and you fall — and carries you over a big red. Twelve stages run back to back, with no stops and no reset of your number; at the finish your number smashes through as many walls as it can. In level mode the road brings Counting hub tasks: a problem over three arches — drive into the arch with the right answer; amber numbers — collect exactly the number on the sign. Every third level is held by a guardian: reach the finish with at least its number. A number sequence on the sign — drive into the arch with its next term; a number line across the road — pass where the value of the expression lies. Remember the symbols above the road in order: a stretch later, pick them in the same order.',
  },
  ...
  numberRunLevelsHint: { ru: 'Уровень — 90 секунд. Задачи хаба «Счёт» входят по одной: с 4-го уровня — примеры на арках, с 7-го — «собери ровно N», с 10-го — ряды чисел, с 13-го — шкала, с 16-го — память на знаки. Каждый третий уровень — страж.', en: 'A level lasts 90 seconds. Counting hub tasks join one at a time: problems over arches from level 4, “collect exactly N” from 7, number sequences from 10, the number line from 13, symbol memory from 16. Every third level has a guardian.' },
  numberRunMarathonHint: { ru: 'Свободно — полный забег без уровней: 12 этапов подряд, 8:24.', en: 'Free: the full run without levels — 12 stages back to back, 8:24.' },
  numberRunExact: { ru: 'Собери ровно {n}', en: 'Collect exactly {n}' },
  numberRunGuard: { ru: 'страж', en: 'guardian' },
  numberRunMemorize: { ru: 'Запомни знаки по порядку', en: 'Remember the symbols in order' },
  numberRunRecall: { ru: 'Выбери знаки в том же порядке', en: 'Pick the symbols in the same order' },
  ...
  anagramSquare: { ru: 'Слово-квадрат', en: 'Word square' },
  anagramClassicHint: { ru: 'Собери загаданное слово из перемешанных букв', en: 'Unscramble the hidden word' },
  anagramSquareHint: { ru: 'Четыре слова по краям поля из одного банка букв. Углы у соседних слов общие', en: 'Four words around the board from one letter bank. Neighbours share their corners' },
  ...
  fwShowWordsHint: { ru: 'Показывать слова рядом с полем — их надо найти, а не придумать', en: 'Show the words beside the grid — find them instead of guessing them' },
  ...
  anagramBonus: { ru: 'бонусных', en: 'bonus' },
  anagramSquareLvlParams: { ru: '4 слова по краям · 5 букв каждое · без лимита времени', en: 'Four words around the board · five letters each · no time limit' },
  anagramSquarePass: { ru: 'Проход уровня: собраны все четыре слова, промахов не больше двух', en: 'Level passed: all four words found with two misses or fewer' },
  anagramSquareEmpty: { ru: 'Для этого языка раскладок пока нет', en: 'No puzzles for this language yet' },
   ...
   * замер 07.09.2026: на кривых блоках игрок просидел 3 ч 18 мин, ни разу не
   * взяв подсказку, — правило варианта он видел, приёма не знал.
   */
  sudokuHowLabel: { ru: 'Чем берётся эта доска', en: 'How this board cracks' },
  sudokuHowNakedSingle: { ru: 'Голый одиночка. Возьми пустую клетку и вычеркни всё, что уже стоит в её строке, столбце и блоке. Если осталась одна цифра — она туда и идёт.', en: 'Naked single. Take an empty cell and cross out every digit already in its row, column and block. One digit left — that is the answer.' },
  sudokuHowHiddenSingle: { ru: 'Скрытый одиночка. Возьми одну цифру и одну область — строку, столбец или блок. Если в области ей осталось единственное свободное место, она встаёт туда, даже если в ту клетку подходили и другие цифры.', en: 'Hidden single. Take one digit and one region — row, column or block. If only one cell there can still hold it, that is where it goes, even if other digits fit that cell too.' },
  sudokuHowLocked: { ru: 'Связанные кандидаты. Если внутри блока цифра помещается только в клетки одной строки, из остальной части этой строки её можно вычеркнуть. И наоборот: если в строке цифра держится в одном блоке, из остального блока она уходит.', en: 'Locked candidates. If inside a block a digit only fits cells of one row, erase it from the rest of that row — and the other way round.' },
  sudokuHowNakedPair: { ru: 'Голая пара. Две клетки одной области, и в каждой одни и те же две цифры. Значит эти две цифры заняты ими — из остальных клеток области обе вычёркиваем.', en: 'Naked pair. Two cells of one region hold the same two candidates. Those digits belong to them — erase both from every other cell of the region.' },
  sudokuHowHiddenPair: { ru: 'Скрытая пара. Двум цифрам в области осталось всего два общих места. Эти два места принадлежат им — всё лишнее из обеих клеток убираем.', en: 'Hidden pair. Two digits share exactly two possible cells in a region. Those cells belong to them — erase everything else from both.' },
  sudokuHowXWing: { ru: 'X-wing. Одна цифра в двух строках стоит ровно на одних и тех же двух столбцах. Тогда из этих столбцов во всех остальных строках её можно убрать.', en: 'X-wing. One digit in two rows sits in exactly the same two columns. Erase it from those columns in every other row.' },
  sudokuHowUnmeasured: { ru: 'Приём этой доски наш оценщик не называет — она выше его лестницы. Верный вход тот же, что и на простых: возьми одну цифру и пройди с ней все девять блоков. На 114 досках из 115 этого хватает, чтобы найти первый ход.', en: 'Our grader does not name a technique here — the board sits above its ladder. The way in is the same as on easy boards: take one digit and walk it through all nine blocks. On 114 boards out of 115 that finds the first move.' },
  sudokuScanLabel: { ru: 'Как смотреть на этом варианте', en: 'How to scan this variant' },
  sudokuScanJigsaw: { ru: 'Считай не клетки блока, а сколько строк и столбцов он пересекает. Блок, зажатый в два столбца, но растянутый на пять строк, закрывается чужими строками: посмотри, в скольких из них цифра уже стоит. Часто закрыты четыре из пяти — и место остаётся одно.', en: 'Count the rows and columns a block spans, not its cells. A block squeezed into two columns but stretched over five rows is killed by rows: check how many of them already hold the digit. Often four of five are taken — and one cell is left.' },
  sudokuScanDiagonal: { ru: 'Две диагонали — такие же области, как строка и блок, но про них забывают. Пройди цифру по ним отдельно: клеток там девять, а пересечений с блоками много, поэтому места кончаются быстрее всего.', en: 'The two diagonals are regions just like rows and blocks, and they are the ones people forget. Walk a digit along them separately: they run out of room fastest.' },
  sudokuScanHyper: { ru: 'Четыре доп. зоны пересекают обычные блоки. Клетка, попавшая сразу в блок и в зону, видит больше всех остальных — кандидатов в ней меньше всего, с неё и начинай.', en: 'The four extra zones overlap the ordinary blocks. A cell inside both sees more than any other, so it has the fewest candidates — start there.' },
  sudokuScanAntiknight: { ru: 'У каждой поставленной цифры вычеркни её же на всех восьми клетках буквой Г. Это убирает кандидатов там, куда строка, столбец и блок не достают, — обычно именно там и прячется ход.', en: 'For every placed digit, erase it from all eight knight-move cells. That clears candidates where row, column and block cannot reach — usually exactly where the next move hides.' },
  sudokuScanAntiking: { ru: 'Одинаковые цифры не касаются даже углами. У каждой поставленной цифры вычеркни её во всех восьми соседних клетках — вокруг плотных участков доски это сразу режет половину вариантов.', en: 'Equal digits may not touch even at corners. Erase each placed digit from all eight neighbouring cells — around dense areas that halves the options at once.' },
  sudokuScanNonconsec: { ru: 'Соседние по стороне клетки не отличаются на единицу. Поставил 5 — у соседей сверху, снизу, слева и справа нет ни 4, ни 6. Крайние цифры 1 и 9 удобнее всего: у них запрет односторонний.', en: 'Side-adjacent cells never differ by one. Place a 5 and its four neighbours lose both 4 and 6. Start from 1 and 9 — their restriction is one-sided.' },
  sudokuScanEvenodd: { ru: 'Метка режет половину кандидатов ещё до всякого счёта: в чётной клетке возможны только 2, 4, 6, 8, в нечётной — 1, 3, 5, 7, 9. Сначала расставь эти множества по помеченным клеткам, потом смотри строки.', en: 'A mark halves the candidates before you count anything: an even cell allows only 2, 4, 6, 8; an odd one 1, 3, 5, 7, 9. Fill those sets in first, then scan rows.' },
  sudokuScanKropki: { ru: 'Начинай с чёрных точек: пар, где одна цифра вдвое больше другой, всего четыре (1–2, 2–4, 3–6, 4–8). Цепочка из двух чёрных точек почти всегда задаёт клетки однозначно.', en: 'Start with black dots: only four pairs double each other (1–2, 2–4, 3–6, 4–8). A chain of two black dots almost always pins the cells down.' },
  sudokuScanThermo: { ru: 'Длина термометра сама режет диапазон: на пять клеток в колбе не бывает больше 5, а в конце — меньше 5. Сначала подпиши границы у самых длинных термометров, короткие подождут.', en: 'A thermometer’s length limits it by itself: on five cells the bulb cannot exceed 5 and the tip cannot go below 5. Bound the longest thermometers first.' },
  sudokuScanSandwich: { ru: 'Смотри на крайние подсказки. Ноль означает, что 1 и 9 стоят вплотную; большое число — что они разнесены к краям ряда. Именно нули и максимумы ставят 1 и 9 почти без перебора.', en: 'Look at the extreme clues. A zero means 1 and 9 are adjacent; a large number means they sit apart near the ends. Zeros and maxima place 1 and 9 almost without search.' },
  sudokuScanArrow: { ru: 'Длинная стрелка не может начинаться с маленькой суммы: три клетки дают минимум 1+2+3=6. И наоборот, кружок с маленькой цифрой обрубает длину стрелки — начинай с самых длинных и самых коротких.', en: 'A long arrow cannot start from a small sum: three cells give at least 1+2+3=6. A small circle caps the arrow’s length — start from the longest and the shortest.' },
  sudokuScanCages: { ru: 'Начинай с самых маленьких и самых больших сумм: у них мало разложений. Сумма 4 на две клетки — это только 1 и 3, сумма 17 на две — только 8 и 9. Такие рамки ставят цифры почти сразу.', en: 'Start from the smallest and largest cages: they have few decompositions. A 4 across two cells is only 1 and 3; a 17 across two is only 8 and 9.' },
  sudokuScanTowers: { ru: 'Крайние числа читаются с концов: единица у края означает, что там стоит самая высокая башня, а максимум — что башни растут строго по порядку. Эти два случая заполняют целый ряд без перебора.', en: 'Read the edge clues from their extremes: a 1 means the tallest tower stands at that edge, the maximum means the towers rise strictly in order. Both fill a whole line outright.' },
  sudokuScanUnequal: { ru: 'Иди по цепочке знаков целиком, а не по одному. Если четыре клетки связаны как a<b<c<d, то в a не бывает больше 6, а в d — меньше 4. Чем длиннее цепь, тем сильнее она режет концы.', en: 'Follow the whole chain of signs, not one sign at a time. If four cells run a<b<c<d, then a is at most 6 and d at least 4. The longer the chain, the harder it clamps the ends.' },
  sudokuLevelHasLabel: { ru: 'На этом уровне', en: 'On this level' },
  sudokuLevelHas: { ru: 'Подсказок: {h} · ошибок можно: {e}', en: 'Hints: {h} · mistakes allowed: {e}' },
  sudokuLevelHasNoLimit: { ru: "без лимита", en: "no limit" },
  ...
  patternRuleLinear: { ru: 'Каждое число — предыдущее × {a}, потом {b}', en: 'Each term is the previous × {a}, then {b}' },
  patternClassTwoOps: { ru: 'Два действия по очереди', en: 'Two operations in turn' },
  patternRuleTwoOps: { ru: 'Действия чередуются: {a}, потом {b}, снова {a}…', en: 'Operations alternate: {a}, then {b}, then {a} again…' },
  patternClassInterMixed: { ru: 'Два разных ряда через один', en: 'Two different interleaved series' },
  patternRuleInterMixed: { ru: 'Позиции 1,3,5…: каждый раз {a}; позиции 2,4…: {b}. Нужна следующая нечётная', en: 'Odd positions {a} each time, even {b}' },
  patternClassSignFlip: { ru: 'Знак меняется каждый шаг', en: 'The sign flips every step' },
  patternClassMixed: { ru: 'Любой из трудных рядов — числа растут с уровнем', en: 'Any hard series — numbers grow with the level' },
  ...
  lr_stroop_switch_title: { ru: 'Правило может смениться', en: 'The rule can switch' },
  lr_stroop_switch_rule: { ru: 'Иногда посреди партии задание меняется: вместо цвета чернил нужно ответить по значению слова — или наоборот. Какое правило действует сейчас, написано строкой под словом. Читай её перед каждым ответом: чем выше уровень, тем чаще смена.', en: 'Sometimes the task changes mid-game: instead of the INK colour you answer by the word MEANING, or the other way round. The line under the word tells you which rule applies right now. Read it before every answer: the higher the level, the more often it switches.' },
  lr_stroop_switch_example: { ru: 'Пример: слово «КРАСНЫЙ» написано синими чернилами. Если строка говорит про цвет ЧЕРНИЛ — жми синюю плашку, если про ЗНАЧЕНИЕ слова — красную.', en: 'Example: the word "RED" is written in blue ink. If the line mentions INK colour, tap the blue swatch; if it mentions word MEANING, tap the red one.' },
  ...
  lr_goods_sort_singlecap_title: { ru: 'Ниша на один товар', en: 'A niche for one good' },
  lr_goods_sort_singlecap_rule: { ru: 'Под некоторыми нишами одна насечка — в такую влезает РОВНО ОДИН товар. Положил, и место кончилось: снять оттуда можно только этот же товар, никого к нему не подставишь. Это не маленькая полка, а замок: думай на ход вперёд, что туда класть.', en: 'Some niches have a single notch — exactly ONE good fits there. Put something in and the space is gone: only that same good can come back out, and nothing can join it. This is not a small shelf but a lock: think a move ahead about what goes in.' },
  lr_goods_sort_singlecap_example: { ru: 'Пример: под нишей одна насечка. Поставил туда колу — ниша занята до тех пор, пока ты колу не заберёшь.', en: 'Example: one notch under a niche. Put the cola there and the niche is taken until you take the cola back.' },
  ...
  ballSort: { ru: 'Сортировка шариков', en: 'Ball sort' },
  ballSortDesc: { ru: 'Разложить шарики по трубкам: один цвет — одна трубка', en: 'Sort balls into tubes: one colour per tube' },
  nutSort: { ru: 'Сортировка гаек', en: 'Nut sort' },
  nutSortDesc: { ru: 'Разложить гайки по стержням: один цвет — один стержень', en: 'Sort nuts onto bolts: one colour per bolt' },
  lr_water_sort_hidden_title: { ru: 'Скрытый слой', en: 'Hidden layer' },
  lr_water_sort_hidden_rule: { ru: 'Один слой в пробирке закрыт знаком вопроса — цвет там есть, но неизвестен. Он открывается сам, когда над ним ничего не останется. Минимума ходов у такого уровня нет: разведка стоит ходов, и счёт ими не меряется. Лей, узнавай и перестраивай план — отмена бесплатна.', en: 'One layer in a tube is covered by a question mark — there is a colour there, but you cannot see it. It opens by itself once nothing remains above it. Such a level has no move minimum: scouting costs moves, and the count is not judged by them. Pour, find out, replan — undo is free.' },
  lr_water_sort_hidden_example: { ru: 'Пример: под синим стоит «?». Перелил синий — открылось: там был жёлтый.', en: 'Example: a “?” sits under the blue. Pour the blue away and it opens: yellow was hiding there.' },
  /* Лимит ходов переливалки (L14+, = ХОДЫ_С). Ключи мои новые. */
  lr_water_sort_movelimit_title: { ru: 'Ходы теперь считаются', en: 'Moves are counted now' },
  lr_water_sort_movelimit_rule: { ru: 'С этого уровня у партии есть предел ходов — он показан сверху. Считается не «сколько получится», а с запасом: примерно четверть ходов сверх безошибочной игры остаётся на передумки. Отмена ход НЕ возвращает: передумал — значит потратил. Под скрытым слоем предела нет, там разведка стоит ходов и мерить их нечем.', en: 'From this level the round has a move limit — it is shown at the top. It is not «as many as it takes»: it leaves roughly a quarter of the moves above flawless play for changing your mind. Undo does NOT give a move back: you changed your mind, you spent it. On hidden-layer levels there is no limit — scouting costs moves and there is nothing to measure them against.' },
  lr_water_sort_movelimit_example: { ru: 'Пример: предел 40, потрачено 38 — два хода на то, чтобы доразобрать. Не хватило — уровень начинается заново, доска та же.', en: 'Example: the limit is 40 and you have spent 38 — two moves left to finish. Not enough means the level restarts with the same board.' },
  lr_water_sort_short_title: { ru: 'Низкий сосуд', en: 'Short vessel' },
  lr_water_sort_short_rule: { ru: 'Один сосуд ниже остальных — порций в него влезает меньше. Он полноправный: собранный в нём цвет засчитывается, как и везде. Но цвет из пяти порций в сосуд на четыре целиком не убрать — его придётся вести в другой или разделить и собрать потом.', en: 'One vessel is lower than the rest — it holds fewer portions. It still counts as a full home: a colour completed there is finished like anywhere else. But a colour of five portions will not fit into a vessel of four — take it elsewhere, or split it and gather it later.' },
  lr_water_sort_short_example: { ru: 'Пример: сосуд на три порции. Красного пять — значит домом ему этот сосуд не станет.', en: 'Example: a vessel holding three. Red has five portions, so this vessel cannot be its home.' },
  ...
  lr_nut_sort_movelimit_title: { ru: "Предел ходов у гаек", en: "A move limit for the nuts" },
  lr_nut_sort_movelimit_rule: { ru: "С этого уровня у партии есть предел ходов — он показан сверху. Он с запасом: примерно четверть ходов сверх безошибочной игры остаётся на передумки. Отмена ход НЕ возвращает: передумал — значит потратил. На уровнях со скрытыми гайками предела нет — там разведка стоит ходов.", en: "From this level the round has a move limit, shown at the top. It has slack: roughly a quarter of the moves above flawless play is left for changing your mind. Undo does NOT give a move back: change your mind and you have spent it. Levels with hidden nuts have no limit — scouting costs moves there." },
  lr_nut_sort_movelimit_example: { ru: "Пример: предел 40, потрачено 38 — два хода, чтобы докрутить. Не хватило — уровень начинается заново, стержни те же.", en: "Example: the limit is 40 and you have used 38 — two moves left to finish screwing. Not enough means the level restarts with the same bolts." },
  lr_nut_sort_hidden_title: { ru: "Скрытая гайка", en: "A hidden nut" },
  lr_nut_sort_hidden_rule: { ru: "Одна гайка на стержне закрыта знаком вопроса — цвет у неё есть, но неизвестен. Она открывается сама, когда над ней ничего не останется. Минимума ходов у такого уровня нет: разведка стоит ходов. Перекручивай, узнавай и перестраивай план — отмена бесплатна.", en: "One nut on a bolt is covered by a question mark — it has a colour, but you cannot see it. It opens by itself once nothing is left above it. Such a level has no move minimum: scouting costs moves. Screw across, find out and replan — undo is free." },
  lr_nut_sort_hidden_example: { ru: "Пример: под синей гайкой «?». Перекрутил синюю — открылось: там была жёлтая.", en: "Example: a “?” sits under the blue nut. Screw the blue one away and it opens: a yellow nut was hiding there." },
  lr_nut_sort_short_title: { ru: "Короткий стержень", en: "A short bolt" },
  lr_nut_sort_short_rule: { ru: "Один стержень короче остальных — гаек на него помещается меньше. Он полноправный: собранный на нём цвет засчитывается, как и везде. Но цвет из пяти гаек на стержень на четыре целиком не накрутить — его придётся вести на другой или разделить и собрать потом.", en: "One bolt is shorter than the rest — it holds fewer nuts. It still counts as a full home: a colour completed there is finished like anywhere else. But a colour of five nuts will not fit on a bolt for four — take it elsewhere, or split it and gather it later." },
  lr_nut_sort_short_example: { ru: "Пример: стержень на три гайки. Красных пять — значит домом для красных этот стержень не станет.", en: "Example: a bolt that holds three nuts. There are five red ones, so this bolt cannot be their home." },
  lr_nut_sort_stones_title: { ru: "Втулка внизу", en: "A sleeve at the bottom" },
  lr_nut_sort_stones_rule: { ru: "На одном стержне внизу сидит втулка — снять её нельзя. Накручивать на этот стержень можно, и в этом весь его смысл: это временное хранилище. Но домом цвета он не станет никогда, и уровень сходится только тогда, когда он снова пуст.", en: "One bolt has a sleeve stuck at its base — it cannot be removed. You may screw nuts onto it, and that is its whole purpose: temporary storage. But it will never become a colour’s home, and the level only comes together once it is empty again." },
  lr_nut_sort_stones_example: { ru: "Пример: перекрутил туда синие, чтобы добраться до жёлтых. Прежде чем закончить, синие придётся снять обратно.", en: "Example: you park the blue nuts there to reach the yellow ones below. Before you finish, those blue nuts have to come back off." },
  lr_nut_sort_sealed_title: { ru: "Запертый стержень", en: "A locked bolt" },
  lr_nut_sort_sealed_rule: { ru: "Один стержень закрыт замком: гайки на него не накрутить и не снять, пока не соберёшь нужное число цветов. Замок снимается сам — считаются собранные ЦВЕТА, а не ходы. До этого придётся обходиться остальными стержнями.", en: "One bolt is locked: nuts can neither go on nor come off until you have completed enough colours. The lock lifts by itself — it counts finished COLOURS, not moves. Until then you have to manage with the other bolts." },
  lr_nut_sort_sealed_example: { ru: "Пример: замок снимется, когда будет собран первый цвет. Значит первый цвет надо докрутить без этого стержня.", en: "Example: the lock lifts once the first colour is done. So the first colour must be finished without this bolt." },
  lr_ball_sort_movelimit_title: { ru: "Предел ходов у шариков", en: "A move limit for the balls" },
  lr_ball_sort_movelimit_rule: { ru: "С этого уровня у партии есть предел ходов — он показан сверху. Он с запасом: примерно четверть ходов сверх безошибочной игры остаётся на передумки. Отмена ход НЕ возвращает: передумал — значит потратил. На уровнях со скрытыми шариками предела нет — там разведка стоит ходов.", en: "From this level the round has a move limit, shown at the top. It has slack: roughly a quarter of the moves above flawless play is left for changing your mind. Undo does NOT give a move back: change your mind and you have spent it. Levels with hidden balls have no limit — scouting costs moves there." },
  lr_ball_sort_movelimit_example: { ru: "Пример: предел 40, потрачено 38 — два хода, чтобы докатить. Не хватило — уровень начинается заново, трубки те же.", en: "Example: the limit is 40 and you have used 38 — two moves left to roll the rest home. Not enough means the level restarts with the same tubes." },
  lr_ball_sort_hidden_title: { ru: "Скрытый шарик", en: "A hidden ball" },
  lr_ball_sort_hidden_rule: { ru: "Один шарик в трубке закрыт знаком вопроса — цвет у него есть, но неизвестен. Он открывается сам, когда над ним ничего не останется. Минимума ходов у такого уровня нет: разведка стоит ходов. Перекатывай, узнавай и перестраивай план — отмена бесплатна.", en: "One ball in a tube is covered by a question mark — it has a colour, but you cannot see it. It opens by itself once nothing is left above it. Such a level has no move minimum: scouting costs moves. Roll, find out and replan — undo is free." },
  lr_ball_sort_hidden_example: { ru: "Пример: под синим шариком «?». Перекатил синий — открылось: там был жёлтый.", en: "Example: a “?” sits under the blue ball. Roll the blue one away and it opens: a yellow ball was hiding there." },
  lr_ball_sort_short_title: { ru: "Низкая трубка", en: "A short tube" },
  lr_ball_sort_short_rule: { ru: "Одна трубка ниже остальных — шариков в неё помещается меньше. Она полноправная: собранный в ней цвет засчитывается, как и везде. Но цвет из пяти шариков в трубку на четыре целиком не убрать — его придётся вести в другую или разделить и собрать потом.", en: "One tube is shorter than the rest — it holds fewer balls. It still counts as a full home: a colour completed there is finished like anywhere else. But a colour of five balls will not fit into a tube for four — take it elsewhere, or split it and gather it later." },
  lr_ball_sort_short_example: { ru: "Пример: трубка на три шарика. Красных пять — значит домом для красных эта трубка не станет.", en: "Example: a tube that holds three balls. There are five red ones, so this tube cannot be their home." },
  lr_ball_sort_stones_title: { ru: "Камни на дне трубки", en: "Stones in the tube" },
  lr_ball_sort_stones_rule: { ru: "В одной трубке дно занято камнями — сдвинуть их нельзя. Закатывать в неё шарики можно, и в этом весь её смысл: это временное хранилище. Но домом цвета она не станет никогда, и уровень сходится только тогда, когда она снова пуста.", en: "One tube has stones on its floor — they cannot be moved. You may roll balls into it, and that is its whole purpose: temporary storage. But it will never become a colour’s home, and the level only comes together once it is empty again." },
  lr_ball_sort_stones_example: { ru: "Пример: закатил туда синие, чтобы добраться до жёлтых. Прежде чем закончить, синие придётся выкатить обратно.", en: "Example: you park the blue balls there to reach the yellow ones below. Before you finish, those blue balls have to roll back out." },
  lr_ball_sort_sealed_title: { ru: "Запертая трубка", en: "A locked tube" },
  lr_ball_sort_sealed_rule: { ru: "Одна трубка закрыта замком: шарики в неё не закатить и не выкатить, пока не соберёшь нужное число цветов. Замок снимается сам — считаются собранные ЦВЕТА, а не ходы. До этого придётся обходиться остальными трубками.", en: "One tube is locked: balls can neither roll in nor roll out until you have completed enough colours. The lock lifts by itself — it counts finished COLOURS, not moves. Until then you have to manage with the other tubes." },
  lr_ball_sort_sealed_example: { ru: "Пример: замок снимется, когда будет собран первый цвет. Значит первый цвет надо докатить без этой трубки.", en: "Example: the lock lifts once the first colour is done. So the first colour must be finished without this tube." },
  ...
  lr_goods_sort_joker_title: { ru: 'Ниша-джокер', en: 'The wildcard niche' },
  lr_goods_sort_joker_rule: { ru: 'Одна ниша помечена звездой: в неё можно класть любой товар, даже когда на уровне действует строгая укладка. Места она не добавляет — вмещает столько же, сколько остальные; она снимает не тесноту, а запрет. Держи её пустой, пока не упрёшься: это единственный ход в сторону.', en: 'One niche is marked with a star: any good may go into it, even while strict stacking is in force. It adds no room — it holds as much as the others; what it lifts is the ban, not the crowding. Keep it empty until you are stuck: it is your only sideways move.' },
  lr_goods_sort_joker_example: { ru: 'Пример: ★ пустая, наверху лежит кола, а колы больше нет нигде. Убери её в джокер — освободишь нишу под тройку кефира.', en: 'Example: ★ is empty, a cola sits on top and no other cola is left. Park it in the wildcard and you free the niche for a triple of kefir.' },
  lr_goods_sort_moving_title: { ru: 'Ниши меняются местами', en: 'The niches swap places' },
  ...
  /* Витрина выше экрана (L46+, = SCROLL_FROM). Ключи мои, чужих строк не трогаю. */
  lr_goods_sort_showcase_title: { ru: 'Витрина выше экрана', en: 'The shelf runs taller than the screen' },
  lr_goods_sort_showcase_rule: { ru: 'С этого уровня шкаф не влезает в экран целиком — его можно прокручивать вверх и вниз. Товар при этом НЕ мельчает: вместо того чтобы ужиматься, витрина растёт вниз. Справа появляется полоска-карта: точка на нишу, светлые — свободные, а рамка показывает, какая часть витрины сейчас перед глазами.', en: 'From this level the cabinet no longer fits on one screen — you can scroll it up and down. The goods do NOT get smaller: instead of shrinking, the shelf grows downwards. A map strip appears on the right: one dot per niche, light dots are free, and the frame shows which part of the shelf you are looking at.' },
  lr_goods_sort_showcase_example: { ru: 'Пример: свободной ниши не видно — взгляни на карту справа: светлая точка ниже, значит прокрути витрину вниз.', en: 'Example: you cannot see a free niche — check the map on the right: a light dot sits lower, so scroll the shelf down.' },
  /* Схлопывание полки (L56+, = COLLAPSE_FROM). Ключи мои новые. */
  lr_goods_sort_collapse_title: { ru: 'Собранная полка уезжает', en: 'A finished shelf leaves' },
  lr_goods_sort_collapse_rule: { ru: 'Сложил тройку в нише на три — полка закрывается и уходит со шкафа. Всё, что было над ней в этом столбце, оседает вниз, а сверху приходит новая полка из очереди. Очередь конечная: сколько полок в ней, столько и придёт, и всё, что в них лежит, посчитано в задаче с самого начала. Ниша на четыре не закрывается — там тройка складывается рядом с четвёртым товаром, и он никуда не денется.', en: 'Complete a triple in a three-slot niche and the shelf closes and leaves the cabinet. Everything above it in that column settles down, and a new shelf arrives from the queue at the top. The queue is finite: as many shelves as it holds will arrive, and everything on them was counted into the task from the start. A four-slot niche does not close — there the triple forms next to a fourth item, and that item is not going anywhere.' },
  lr_goods_sort_collapse_example: { ru: 'Пример: наметил отнести колу в нишу над собранной — полка уехала, и то, что было выше, встало на её место. Товар не пропал, поменялся адрес.', en: 'Example: you lined up a cola for the niche above the finished one — the shelf left and what stood higher took its place. Nothing vanished; the address changed.' },
  /* Задние ряды (L52+, = BACK_FROM). Ключи мои новые. */
  ...
  lr_cake_sort_queue_example: { ru: 'Пример: в очереди 2. Собрал круг — тарелка ушла, и на её место сразу встала первая из очереди.', en: 'Example: the queue shows 2. You complete a circle, the plate leaves, and the first one from the queue takes its place at once.' },
  lr_pizza_sort_queue_title: { ru: "Доски в очереди", en: "Boards in the queue" },
  lr_pizza_sort_queue_rule: { ru: "Часть досок ждёт своей очереди и выходит на стол сама — как только освободится место, то есть когда ты соберёшь целую пиццу. Очередь конечна и видна в шапке: это не бесконечный поток, а отложенная часть того же уровня. Значит место надо освобождать вовремя, а не копить смесь по всем доскам.", en: "Some boards wait their turn and come out by themselves — as soon as a spot frees up, that is, when you complete a whole pizza. The queue is finite and shown in the header: it is not an endless stream but a delayed part of this very level. So free up space in good time instead of piling a mix onto every board." },
  lr_pizza_sort_queue_example: { ru: "Пример: в очереди 2. Собрал пиццу — доска ушла, и на её место сразу встала первая из очереди.", en: "Example: the queue shows 2. You complete a pizza, the board leaves, and the first one from the queue takes its place at once." },
  lr_goods_sort_mono_title: { ru: 'Уровень в одном цвете', en: 'A level in one colour' },
  lr_goods_sort_mono_rule: { ru: 'Здесь весь товар в одной цветовой семье, и цвет больше не подсказывает. Обычно пару находишь боковым зрением — по жёлтому пятну; сейчас так не выйдет, придётся смотреть на форму: силуэт, крышку, пропорции. Правила не меняются, меняется то, чем ты различаешь.', en: 'Here every good sits in one colour family, and colour stops helping. Usually you spot a pair with the corner of your eye — by a yellow patch; not now. You will have to look at shape: the outline, the cap, the proportions. The rules are the same; what changes is what you tell them apart by.' },
  lr_goods_sort_mono_example: { ru: 'Пример: банан, батон и мишка — все тёплые и похожи пятном. Различаются формой: длинный, широкий, с ушами.', en: 'Example: a banana, a loaf and a teddy — all warm, all alike as a blob. They differ in shape: long, wide, with ears.' },
  ...
  lr_math_sprint_mult_rule: { ru: 'Полоса умножения: от таблицы к двузначным множителям.', en: 'Multiplication band: from times tables to two-digit factors.' },
  lr_math_sprint_mult_example: { ru: 'Пример: 7 × 6 = 42.', en: 'Example: 7 × 6 = 42.' },
  lr_math_sprint_div_title: { ru: 'Деление', en: 'Division' },
  lr_math_sprint_div_rule: { ru: 'Полоса деления (÷) — всегда нацело, без остатка.', en: 'Division band (÷) — always exact, no remainder.' },
  lr_math_sprint_div_example: { ru: 'Пример: 42 ÷ 6 = 7.', en: 'Example: 42 ÷ 6 = 7.' },
  lr_math_sprint_chain_title: { ru: 'Цепочки', en: 'Chains' },
  lr_math_sprint_chain_rule: { ru: 'Два действия подряд: сначала умножь, потом прибавь или вычти.', en: 'Two steps in one: multiply first, then add or subtract.' },
  lr_math_sprint_chain_example: { ru: 'Пример: 6 × 4 + 13 = 37.', en: 'Example: 6 × 4 + 13 = 37.' },
  lr_math_sprint_square_title: { ru: 'Квадраты', en: 'Squares' },
  lr_math_sprint_square_rule: { ru: 'Число во второй степени: умножь его само на себя.', en: 'A number squared: multiply it by itself.' },
  lr_math_sprint_square_example: { ru: 'Пример: 13² = 169.', en: 'Example: 13² = 169.' },
  lr_math_sprint_root_title: { ru: 'Корни', en: 'Roots' },
  lr_math_sprint_root_rule: { ru: 'Квадратный корень — ответ всегда целый: какое число в квадрате даёт это?', en: 'Square roots — the answer is always whole: which number squared gives this?' },
  lr_math_sprint_root_example: { ru: 'Пример: √144 = 12.', en: 'Example: √144 = 12.' },
  lr_math_sprint_equation_title: { ru: 'Уравнения', en: 'Equations' },
  lr_math_sprint_equation_rule: { ru: 'Найди x: посчитай в голове и введи его. Дальше уравнения будут встречаться вперемешку с другими темами.', en: 'Solve for x: work it out in your head and type it. From here on, equations mix with other topics.' },
  lr_math_sprint_equation_example: { ru: 'Пример: 3x + 7 = 25 → x = 6.', en: 'Example: 3x + 7 = 25 → x = 6.' },
  ...
  lr_mental_rotation_cubes_title: { ru: 'Фигура растёт', en: 'The shape grows' },
  lr_mental_rotation_cubes_rule: { ru: 'С 6-го уровня в фигуре пять кубиков, и каждые пять уровней добавляется ещё один — до тринадцати на 46-м. Узор поворота внутри пятёрки идёт по кругу: одна ось, две четверти, две оси, четыре варианта, подделки с переставленным кубиком.', en: 'From level 6 the shape has five cubes, and one more is added every five levels — up to thirteen at level 46. Within each five the rotation pattern cycles: one axis, two quarter-turns, two axes, four options, foils with one cube moved.' },
  lr_mental_rotation_cubes_example: { ru: 'Пример: на 6-м уровне та же задача, что на 1-м, но фигура из пяти кубиков — держать в голове надо на один блок больше.', en: 'Example: level 6 is the same task as level 1, but the shape has five cubes — one more block to hold in mind.' },
