...
const translations: Translations = {
  ...
  puzzlesUnrulyIntroDesc: { ru: 'Доска заполняется чёрными и белыми клетками по трём правилам: в каждой строке и в каждом столбце чёрных ровно столько же, сколько белых; трёх одинаковых подряд не бывает ни по горизонтали, ни по вертикали; двух одинаковых строк или столбцов тоже. Начинают с расставленных клеток — они не меняются. Короткое нажатие перебирает пусто → чёрное → белое, долгое перебирает в обратную сторону. Общее для всех головоломок Тэтхэма: у доски ровно одно решение, и до него можно дойти рассуждением — угадывать не нужно. Короткое нажатие делает ход, ДОЛГОЕ — второе действие. Отменить ход, попросить подсказку или начать заново — в меню паузы, кнопка ⏸ слева вверху.', en: 'Fill the grid with black and white cells under three rules: every row and every column holds as many black cells as white; no three of a kind ever sit in a row, across or down; and no two rows or columns are identical. The cells you start with are fixed. A short tap cycles empty → black → white, a long press cycles the other way. Shared by every Tatham puzzle: each board has exactly one solution and reasoning always gets you there — guessing is never required. A short tap makes a move, a LONG press is the second action. Undo, ask for a hint or start over from the pause menu — the ⏸ button at the top left.' },
  puzzlesSlantIntroDesc: { ru: 'В каждой клетке надо провести одну диагональ — либо «/», либо «\». Числа стоят в узлах сетки и говорят, сколько концов диагоналей сходится ИМЕННО в этом узле: 0 — ни одной, 4 — все четыре. И главное ограничение: линии не должны образовать ни одной замкнутой петли, даже большой. Короткое нажатие по клетке меняет наклон, долгое — меняет в обратную сторону. Общее для всех головоломок Тэтхэма: у доски ровно одно решение, и до него можно дойти рассуждением — угадывать не нужно. Короткое нажатие делает ход, ДОЛГОЕ — второе действие. Отменить ход, попросить подсказку или начать заново — в меню паузы, кнопка ⏸ слева вверху.', en: 'Draw exactly one diagonal in every cell — either “/” or “\”. The numbers sit on grid corners and say how many diagonal ends meet AT THAT corner: 0 means none, 4 means all four. The key restriction: the lines must never form a closed loop, however large. A short tap flips the slash, a long press flips it the other way. Shared by every Tatham puzzle: each board has exactly one solution and reasoning always gets you there — guessing is never required. A short tap makes a move, a LONG press is the second action. Undo, ask for a hint or start over from the pause menu — the ⏸ button at the top left.' },
  puzzlesBlackBoxIntroDesc: { ru: 'В закрытом ящике спрятаны шары, и найти их надо, не заглядывая внутрь. Нажатие на стрелку по краю пускает луч: он летит прямо, но рядом с шаром отклоняется, а попав в шар — гаснет. По тому, из какой точки луч вышел, вернулся назад или не вышел вовсе, положение шаров вычисляется однозначно. Нажатие по клетке внутри ящика ставит догадку «здесь шар», долгое — помечает клетку как пустую, чтобы не путаться. Когда все шары расставлены, ящик открывается и показывает, где они были. Общее для всех головоломок Тэтхэма: у доски ровно одно решение, и до него можно дойти рассуждением — угадывать не нужно. Короткое нажатие делает ход, ДОЛГОЕ — второе действие. Отменить ход, попросить подсказку или начать заново — в меню паузы, кнопка ⏸ слева вверху.', en: 'Balls are hidden inside a closed box and you must find them without looking in. Tapping an arrow on the edge fires a beam: it travels straight, deflects when it passes beside a ball, and is absorbed when it hits one. From where the beam comes out, returns, or fails to appear at all, the positions follow with certainty. Tapping a cell inside places a guess “a ball is here”, a long press marks the cell as empty so you don’t lose track. Place every ball and the box opens to show where they really were. Shared by every Tatham puzzle: each board has exactly one solution and reasoning always gets you there — guessing is never required. A short tap makes a move, a LONG press is the second action. Undo, ask for a hint or start over from the pause menu — the ⏸ button at the top left.' },
  puzzlesGuessIntroDesc: { ru: 'Загадан ряд цветов, и его надо угадать за несколько попыток. Собираешь свой ряд и подтверждаешь; в ответ выставляются метки: сколько цветов стоят на СВОИХ местах и сколько угаданы, но стоят не там. Какой метке какой цвет соответствует — не говорят, поэтому каждая попытка это вывод из прошлых, а не перебор. Цвет в ячейке выбирается нажатием, долгое нажатие снимает его. Ряд подтверждается кнопкой под полем. Общее для всех головоломок Тэтхэма: у доски ровно одно решение, и до него можно дойти рассуждением — угадывать не нужно. Короткое нажатие делает ход, ДОЛГОЕ — второе действие. Отменить ход, попросить подсказку или начать заново — в меню паузы, кнопка ⏸ слева вверху.', en: 'A row of colours is hidden and you have a limited number of tries to name it. Build your row and submit; the answer comes back as markers: how many colours sit in the RIGHT place, and how many are in the code but placed elsewhere. Which marker belongs to which peg is never revealed, so every try is a deduction from the previous ones rather than a sweep. Tap a slot to set a colour, long press to clear it, then confirm the row with the button below the board. Shared by every Tatham puzzle: each board has exactly one solution and reasoning always gets you there — guessing is never required. A short tap makes a move, a LONG press is the second action. Undo, ask for a hint or start over from the pause menu — the ⏸ button at the top left.' },
  /**
