...
const translations: Translations = {
  ...
  chessBlindIntroDesc: {
    ru: 'ДВА РЕЖИМА. Серия из трёх заданий на ОДНОЙ позиции из реальной партии: сперва сравнить два поля по цвету, потом — дойдёт ли конь с одного на другое за N ходов, потом — стояла ли на поле названная фигура. Разница во времени между заданиями показывает, чего вам стоит применить правило хода и чего — удержать позицию в голове. Доски во время вопросов нет: она живёт в голове, иначе это не замер. Второй режим — прежний: позицию показывают на несколько секунд, потом фигуры превращаются в одинаковые фишки. ⚠️ Там расстановка случайная, и это меряет зрительно-пространственную память, а не шахматный расчёт: на случайной расстановке преимущество опытного игрока как раз исчезает.',
    en: 'TWO MODES. A series of three tasks on ONE position from a real game: first compare two squares by colour, then tell whether a knight gets from one to the other in N moves, then whether a named piece stood on a square. The time difference between tasks shows what it costs you to apply the move rule, and what it costs to hold the position in your head. There is no board on screen during the questions: it lives in your head, otherwise this is not a measurement. The second mode is the old one: a position is shown for a few seconds, then the pieces turn into identical tokens. ⚠️ There the placement is random, and that measures visuospatial memory, not chess calculation: on a random placement the expert advantage is precisely what disappears.',
  },
  ...
  chessBlindConfigDesc: { ru: 'Запомни позицию — фигуры замаскируются одинаковыми фишками. Держи в голове, какая фишка что, даже когда они ходят.', en: 'Memorize the position — the pieces get masked as identical tokens. Keep track of what each token is, even as they move.' },
