...
const translations: Translations = {
  ...
  spatialLabIntroDesc: { ru: 'Две головоломки на одном экране. «Сеть труб»: нажми трубу, чтобы выбрать её, и поворачивай кнопками «Влево» и «Вправо» под полем, пока вода не дойдёт до каждого конца. «Поворот чисел»: нажми клетку — выберется блок 2×2 с этой клеткой; те же кнопки вращают его целиком, расставь числа по порядку. Уровень личный, партия сохраняется на устройстве.', en: 'Two puzzles on one screen. “Pipe Network”: tap a pipe to select it and turn it with the “Left” and “Right” buttons below the board until water reaches every end. “Number Twist”: tap a cell to select a 2×2 block with that cell; the same buttons turn the whole block — put the numbers in order. Your own level; the game is saved on the device.' },
