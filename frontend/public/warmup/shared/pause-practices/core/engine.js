 ...
 */
import { PAUSE_I18N } from './i18nSteps.js';
/**
 ...
 */
const l = (ru, en) => ({ ru, en, ...(PAUSE_I18N[ru] ?? {}) });
const s = (id, ruTitle, enTitle, ruCue, enCue, durationMs, attention, channel) => {
    const guideChannel = channel === 'breath'
        ? 'scale'
        : channel === 'tension'
            ? 'tension'
            : channel === 'eyes'
...
const p = (id, ruTitle, enTitle, ruDescription, enDescription, steps, options = {}) => ({
    id,
    title: l(ruTitle, enTitle),
...
export function getPracticeSet(setId) {
    const found = PRACTICE_CATALOG.find((item) => item.id === setId);
    if (!found)
        throw new Error(`Unknown practice set: ${setId}`);
    return found;
}
export function getPracticeProgram(setId, programId) {
    ...
    if (!program)
        throw new Error(`Unknown program ${resolvedId} for ${setId}`);
    return program;
}
export function getPracticeStatus(setId, programId) {
    const set = getPracticeSet(setId);
    return getPracticeProgram(setId, programId).status ?? set.status;
}
export function getDefaultPracticeSets() {
    return PRACTICE_CATALOG.filter((set) => set.defaultEnabled);
}
function issue(code, ru, en, extras = {}) {
...
export function getRequiredWarnings(selections) {
    const ids = [];
...
export function getRequiredPriorExperience(selections) {
    const ids = [];
    for (const selection of selections) {
        const resolved = resolveSelection(selection);
...
function validateResolved(request, resolved) {
    const issues = [];
    ...
    for (const programRef of getRequiredPriorExperience(resolved.map((item) => item.selection))) {
        if (!confirmedPriorExperience.has(programRef)) {
            issues.push(issue('PRIOR_EXPERIENCE_REQUIRED', 'Этот режим не обучает технике. Подтвердите, что вы уже умеете выполнять выбранную практику.', 'This mode does not teach the technique. Confirm that you already know how to perform the selected practice.', { programRef }));
        }
    }
    if (request.mode === 'parallel') {
        const parallelProblems = [];
        for (const item of resolved) {
            const parallelProblem = parallelSelectionIssue(item, request);
            if (parallelProblem)
                parallelProblems.push(parallelProblem);
        }
...
function groupForCharge(resolved, request) {
    const parallel = [];
    const blocks = [];
    for (const item of resolved) {
...
function localizeStep(item, step, locale, lane, startMs, endMs) {
...
function buildLane(item, locale, lane, startMs, endMs) {
    const timeline = [];
    let cursor = startMs;
    let stepIndex = 0;
    while (cursor < endMs) {
...
function buildPelvicLaneOnBreath(item, locale, lane, breathingLane) {
...
function buildLaneOnBreathClock(item, locale, lane, breathingLane) {
...
function staggerSecondaryPeaks(primary, secondary) {
...
export function createPracticePlan(request) {
...
export function tickPracticeSession(session, nowMs) {
    if (session.phase !== 'running')
        return session;
    const elapsedMs = runningElapsed(session, nowMs);
    if (elapsedMs >= session.plan.durationMs)
...
export function restartPracticeSession(session) {
    if (session.phase === 'disposed')
        throw new Error('Disposed sessions cannot restart');
    return createPracticeSession(session.plan);
}
export function disposePracticeSession(session) {
    return { ...session, phase: 'disposed', runningSinceMs: null, result: null };
}
export function elapsedPracticeTime(session, nowMs) {
    if (session.phase === 'running') {
        if (nowMs === undefined)
            throw new Error('nowMs is required for a running session');
        return runningElapsed(session, nowMs);
    }
    return session.elapsedMs;
}
function toneForStep(step, guideMode) {
    if (guideMode === 'visual' || step.channel !== 'scale')
        return null;
...
export function getActiveFrame(plan, elapsedMs) {
    if (!safeNonNegativeInteger(elapsedMs))
...
export function getVisualGuideFrame(cue, mode) {
    let expansion = 0.5;
    if (cue.stepId === 'hold-in' || cue.stepId === 'hold')
        expansion = 1;
    else if (cue.stepId === 'hold-out')
        expansion = 0;
    else if (cue.stepId === 'inhale-one')
        expansion = cue.progress * 2 / 3;
    else if (cue.stepId === 'inhale-two')
        expansion = 2 / 3 + cue.progress / 3;
    else if (cue.stepId.includes('inhale') || cue.stepId.endsWith('-in'))
        expansion = cue.progress;
    else if (cue.stepId.includes('exhale') || cue.stepId.endsWith('-out'))
        expansion = 1 - cue.progress;
    return {
        mode,
 ...
 */
export function skipToNextStepBoundary(session, nowMs) {
    ...
    for (const step of session.plan.timeline) {
        if (step.endMs > elapsed && step.endMs < target)
            target = step.endMs;
    }
 ...
 */
export function extendCurrentStep(session, nowMs, extraMs) {
    if (extraMs <= 0 || (session.phase !== 'running' && session.phase !== 'paused'))
        return session;
    const elapsed = elapsedPracticeTime(session, session.phase === 'running' ? nowMs : undefined);
    const shiftPoint = (v) => (v === null ? null : (v > elapsed ? v + extraMs : v));
    ...
    const plan = { ...session.plan, durationMs: session.plan.durationMs + extraMs, timeline, blocks };
    return session.phase === 'running'
 ...
 */
export function segmentPosition(plan, elapsedMs) {
...
export function getSessionFrame(session, nowMs) {
    return getActiveFrame(session.plan, elapsedPracticeTime(session, nowMs));
}
export function assertNoAttentionPeakCollisions(plan) {
