export const IMAGE_EFFECT_CATALOG = [
...
function parameter(parameters, key, fallback, minimum, maximum) {
    const raw = parameters?.[key];
    const value = typeof raw === 'number' && Number.isFinite(raw) ? raw : fallback;
    return Math.max(minimum, Math.min(maximum, value));
}
function booleanParameter(parameters, key, fallback) {
    const raw = parameters?.[key];
...
function convolve(surface, kernel, divisor = 1, offset = 0, grayscale = false) {
    const size = Math.sqrt(kernel.length);
    if (!Number.isSafeInteger(size) || size % 2 === 0)
        throw new Error('Kernel must be an odd square');
...
function boxBlur(surface, radius) {
    const source = new Uint8ClampedArray(surface.data);
...
function applyOilPaint(surface, radius, levels) {
    const source = new Uint8ClampedArray(surface.data);
        ...
        for (let x = 0; x < surface.width; x += 1) {
            ...
            for (let offsetY = -boundedRadius; offsetY <= boundedRadius; offsetY += 1) {
                for (let offsetX = -boundedRadius; offsetX <= boundedRadius; offsetX += 1) {
                    ...
                    const sampleY = reflectedIndex(y + offsetY, surface.height);
                    const index = (sampleY * surface.width + sampleX) * 4;
                    const intensity = Math.min(boundedLevels - 1, Math.floor(((source[index] + source[index + 1] + source[index + 2]) / 3) * boundedLevels / 256));
            ...
            for (let level = 1; level < boundedLevels; level += 1) {
                if (counts[level] > counts[dominant])
                    dominant = level;
            }
...
export function applyImageEffect(input, request) {
    ...
    switch (request.id) {
        ...
        default: {
            const exhaustive = request.id;
            throw new Error(`Unknown image effect: ${exhaustive}`);
        }
    }
    return surface;
}
function reflectedIndex(value, length) {
    if (length === 1)
        return 0;
...
export function createMirroredSurface(input, targetWidth, targetHeight) {
    assertSurface(input);
...
export function hashPixelSurface(surface) {
    assertSurface(surface);
    let hash = 0x811c9dc5;
    hash ^= surface.width;
...
export function buildImagePhaseRecipe(frame, options = {}) {
