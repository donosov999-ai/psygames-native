...
const невошли = итог.filter((x) => !x.поле && !x.ошибка);

console.log(`\nокно ${Ш}×${В}; экранов ${экраны.length}, померено ${измерено.length}, не вошли ${невошли.length}, ошибок ${итог.filter((x) => x.ошибка).length}`);
const строка = (x) => `    ${x.экран.padEnd(22)} поле ${String(x.поле.t).padStart(3)}…${String(x.поле.b).padStart(3)} (${x.поле.h})  вверх ${String(x.вверх).padStart(3)}  вниз ${String(x.вниз).padStart(3)}${x.доска ? `  доска ${x.доска} (за полем ↑${x.доскаВверх} ↓${x.доскаВниз})` : ''}${x.прокручиваемое ? '  (поле прокручивается)' : ''}`;
