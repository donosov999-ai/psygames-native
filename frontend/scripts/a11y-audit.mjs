...
for (const d of DIRS) {
  for (const f of walk(join(ROOT, d))) {
    /**
     * 🔴 КОММЕНТАРИИ ГАСИМ ВО ВСЁМ ФАЙЛЕ, А НЕ В ПРОЧИТАННОМ КУСКЕ.
     *
     * Конец открывающего тега ищется по первому `>`, и знак `>` внутри
     * комментария среди свойств обрывает чтение раньше времени: всё, что после
     * него, — включая `accessibilityLabel` — перестаёт находиться.
     *
     * Поймано 03.09.2026 на себе: комментарий к кнопке содержал фразу
     * «`<Pressable>` без подписи», и гейт доложил ровно её же про элемент, у
     * которого подпись стояла строкой ниже. Ложная тревога такого рода дороже
     * пропуска: она посылает чинить то, что не сломано.
     *
     * ⚠️ Гасим ПРОБЕЛАМИ, сохраняя переводы строк: номера строк в отчёте должны
     * остаться настоящими, иначе исправлять придётся по догадке.
     */
    const гасить = (t) => t.replace(/\/\*[\s\S]*?\*\/|\/\/[^\n]*/g,
      (m) => m.replace(/[^\n]/g, ' '));
    const s = гасить(readFileSync(f, 'utf8'));
