...
async function main() {
  ...
  for (const route of routes) {
    ...
    // У развилки поля нет: меряем экран и жест по нему, входа не ждём.
    const наезд = await page.evaluate(наездНаЗаголовок);
    if (ХАБЫ.has(route)) {
      итог.push({ route, развилка: true, наЭкране, наезд, уехало: await протащить(page) });
      continue;
    }
    const подпись = await войти(page);
    if (!подпись) { итог.push({ route, наЭкране, провал: 'кнопка входа не найдена' }); continue; }
