...
/**
 * 🔴 РОВНЫЙ СРЕЗ ПО РЕЙТИНГУ, А НЕ ВЕСЬ ПУЛ.
 *
 * 📍 ЗАМЕР 05.09.2026: полный набор (63 638 позиций) весит 8,2 МБ на диске и
 * занимает 23 МБ В КУЧЕ после разбора — а разбирается он при первом же импорте
 * модуля, то есть на старте. На телефоне это дорого, и платится оно за то,
 * чего человек не увидит: за подход берётся 8–10 позиций.
 *
 * ⚠️ РЕЖЕМ КОЛИЧЕСТВО, А НЕ ДИАПАЗОН. Срез идёт КАЖДОЙ k-й по возрастанию
 * рейтинга: полоса лестницы [min, max] остаётся заполненной на всём протяжении,
 * и медиана поданного не сдвигается. Взять «первые 6000» значило бы отрезать
 * весь верх лестницы.
 *
 * Найденные 16 427 узорных никуда не делись — они в
 * `chess-scholars-mate/lichess_scholar_pattern_m1.json`, и поднять потолок
 * можно одной цифрой.
 */
const В_СБОРКУ_УЗОРА = 6000;
function ровныйСрез(список, сколько) {
...
const итог = {
  ...
  threat: угроза,
  fromGames: [
    ...ровныйСрез([...узорЗаписи].sort((a, b) => a.r - b.r), В_СБОРКУ_УЗОРА),
    ...поМотивам,
  ],
  sacrifice: сЖертвой,
  named: поИменам,
