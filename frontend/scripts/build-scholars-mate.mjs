...
/**
 * 🔴 СЕМЬ УЗОРОВ ВМЕСТО ОДНОГО — ЗАМЕЧАНИЕ ДЕНИСА 05.09.2026 ПОСЛЕ ТРЁХ
 * ПРОЙДЕННЫХ УРОВНЕЙ: «по сути одна комбинация, слон и ферзь». Лестница меняла
 * секунды и обстановку, а узор был один на сорок ступеней.
...
/**
 * Позиции из партий по СЕМИ узорам. Поле `m` называет узор: по нему лестница
 * пускает мотив на нужной ступени, а экран показывает его имя человеку.
 *
 * ⚠️ `scholar` отсюда НЕ берём — он уже пришёл полным пулом выше (16 427
 * позиций против четырёх тысяч в этой выборке).
 */
const поМотивам = [];
for (const [имяУзора, список] of Object.entries(мотивы)) {
  if (имяУзора === 'scholar') continue;
  for (const x of список) {
    const запись = записьИзLichess(x);
    if (!запись) continue;
    запись.m = имяУзора;
    поМотивам.push(запись);
  }
}
