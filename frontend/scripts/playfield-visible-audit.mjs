 ...
 * 🔴 СОСТОЯНИЕ: ЗАГОТОВКА. ПОРОГ НЕ ПОСТАВЛЕН, В CI ВКЛЮЧАТЬ РАНО.
 * Замер 09.09.2026 по всем 97 играм (403×873) показал, что ОБЕ задуманные оси
 * слепы, и записан здесь, чтобы следующий заход не начал с того же тупика:
 *
 *   · `перелив` = 0 у 97 игр из 97. Вертикального переполнения в приложении не
 *     бывает в принципе: react-native-web прибивает высоту к экрану, и лишнее не
 *     прокручивается, а ОБРЕЗАЕТСЯ. Ось мёртвая.
 *   · `доляШапки` у сломанного spatial-lab вышла 0.01 — ЛУЧШАЯ из 97, а красным
 *     стало бы судоку (0.94). Порог по этому замеру пропустил бы ровно ту беду,
 *     ради которой гейт писался, и покрасил бы здоровую игру.
 *   · «вошли в партию 80 из 97» — само по себе неправда: у n-back, digit-span и
 *     sudoku «за экраном» оказались подписи ЭКРАНА НАСТРОЙКИ. Клик по кнопке со
 *     словом «Start» не доказывает, что партия началась. Вход уже решён
 *     координатором в smoke-games.mjs (80b89066: 84 → 95 из 96) и в slot-audit.mjs
 *     — брать оттуда, третий велосипед не нужен.
 *
 * 🔴 ЗАТО ДЕФЕКТ СНЯТ ЖИВЬЁМ И ЕГО ПОДПИСЬ ИЗВЕСТНА. Сборка на теге v2.52.5
 * (⚠️ у неё baseUrl=/psygames-web — в корне все ассеты 404 и экран пустой; класть
 * dist в подпапку psygames-web), экран /games/spatial-lab, 403×873, после нажатия
 * «Начать уровни» на экране партии стоит:
 *     «Number Twist 0 #42 Level 1/50 Beginner Beginner Beginner … Easy Easy Easy …»
 * то есть ЛЕСТНИЦА УРОВНЕЙ С ПРЕДЫДУЩЕГО ЭКРАНА осталась примонтированной внутри
 * партии. Это и есть «лишние половины экрана заняты» из отчётов cc1a7535/652e6eee.
 *
 * ЧТО МЕРИТЬ ДАЛЬШЕ (следующему заходу): не геометрию вообще, а «блок прошлого
 * экрана жив во время партии» — присутствие узла выбора уровня/сложности при
 * идущей партии. Геометрия слишком груба: она одинакова у здорового и больного.
 *
 * Запуск:
