...
try {
   ...
   *
   * 🔴 ОТВЕТОВ ДВА ВИДА, И ПРОВЕРЯТЬ НАДО ОТСУТСТВИЕ ОБОИХ.
   * `feedbackDialog.ts:59-64` кладёт автору пузырь разработчика в двух случаях:
   * из `dev_reply` (ответ словами) и из пары `fixed_in_version`+`fix_note`
   * (ответ-починка, «сделано в версии X»). Человеку достаточно любого.
   *
   * Замер 14.09.2026 по 331 починенному отчёту:
   * · `fix_note is null` (прежняя формула) → 14, и у ВСЕХ четырнадцати есть `dev_reply`;
   * · `dev_reply is null` → 291, и у ВСЕХ 291 есть `fix_note`;
   * · нет НИ ОДНОГО из двух → 0.
   * То есть обе односторонние формулы дают только ложную тревогу. 14.09 на этом
   * потеряли заход: датчик переключили с `fix_note` на `dev_reply`, число прыгнуло
   * с 14 на 291, и это прочли как найденную дыру — хотя людям ответили всем.
   */
  const f = await q('app_feedback?select=id&is_robot=eq.false&fixed_in_version=not.is.null&fix_note=is.null&dev_reply=is.null&limit=1');
