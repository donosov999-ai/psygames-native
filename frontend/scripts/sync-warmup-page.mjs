 ...
 *
 * Папка — параметром (по умолчанию боевая), чтобы пробу можно было прогнать на
 * подделке: 23.09 полифил потерялся не от опечатки в строке, а от того, ГДЕ
 * стояла вставка. Такое видно только прогоном, чтением исходника — нет.
 */
export function вшитьВстраивание(папка = цель) {
  const п = path.join(папка, 'index.html');
   ...
   *
   * 📍 23.09.2026: эта вставка ЛЕЖАЛА ВНУТРИ предыдущего `if` — по скобкам, а не по
   * смыслу. Пока привозной index.html не упоминал embed.js, ветка открывалась и
   * полифил попадал внутрь. Пришла сборка, где ссылки на embed.* стоят уже сверху
   * (отступ в шесть пробелов, не наш), ветка закрылась — и полифил молча перестал
   * вставляться (коммит 39368dd4, потерян вместе с запасной высотой в embed.css).
   * Вставка обязана быть СОСЕДОМ, а не вложением: она ни от чего здесь не зависит.
   */
  ...
  /*
   * Запасная высота живёт в СВОЁМ embed.css, и потерять её так же легко: файл
   * возвращается из `сохранённое`, а значит зависит от того, что лежало в дереве
   * ДО прогона. Прогон в дереве со старой копией вернёт старую копию — ровно это
   * и случилось 23.09. Поэтому проверяем не только вставки в index.html.
   */
  const своиСтили = path.join(папка, 'embed.css');
  const высотаЕсть = existsSync(своиСтили)
    && /@supports not \(height: 100dvh\)/.test(readFileSync(своиСтили, 'utf8'));
  if (!высотаЕсть) console.error('🔴 embed.css без запасной высоты @supports not (height: 100dvh) — WebView Chrome 90 оставит экран пустым');
  return html.includes('embed.css') && html.includes('embed.js')
    && html.includes('psygames-embed-polyfill') && высотаЕсть;
}
...
// Прогон — только при прямом вызове: проба импортирует этот файл ради одной функции,
// и перенос при импорте полез бы в лабораторию и стёр бы public/warmup.
const запущенНапрямую = process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url);
if (запущенНапрямую) {
