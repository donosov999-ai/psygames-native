...
const HUB_REASONS = {
  ...
  '/games/sorting-hub': 'развилка: две пробы на раскладывание по ёмкостям (goods-sort / water-sort), каждая проверяется своим маршрутом',
  '/games/spatial-hub': 'развилка «Ментальная ротация»: меню трёх проб на вращение в уме — кнопки входа в партию нет по устройству, поле не проверяется',
};
...
const FIELD_DEBT = {
  ...
  '/games/scholars-mate':  { max: 64,  why: 'ФИЗИКА: та же доска 8×8, что у «Доски в уме» — 44pt на 390pt ширины. Восемь колонок по 48 требуют 388pt плюс поля, то есть экран шире 420 — таких телефонов в отчётах нет. 44 — минимум Apple HIG (у Android 48), и клетки идут ВПЛОТНУЮ, без промахов между ними' },
...
/**
 * Всплывающие окна «Понятно» закрываем — они перекрывают экран.
 *
 * 🔴 БЕЗ УЧЁТА РЕГИСТРА И ВСЕ ПОДРЯД, А НЕ ПЕРВОЕ ТОЧНОЕ. Карточка правила уровня
 * (`LevelRules.tsx`) пишет кнопку заглавными: ключ `ctaGotIt` — «ПОНЯТНО» / «GOT IT».
 * Пока карточка открывалась уже в партии, это никому не мешало. 16.09.2026 раздел
 * «Внимание» перенёс её на экран настройки (99c3969c), и у PRL она встречает человека
 * на первом уровне прямо поверх «Начать». Точное «Понятно» её не узнавало, клик по
 * «Начать» упирался в карточку, и CI на main (8461775f) краснел двумя гейтами сразу:
 * «/games/prl: нажал „Начать“, но экран не сменился». Поверх карточки может стоять
 * ещё и подсказка первого входа — закрываем, пока есть что закрывать.
 */
async function dismissCoach(page) {
  for (let i = 0; i < 3; i += 1) {
    // Только видимое: закрытое окно, оставшееся в разметке, не должно съедать секунды на каждом экране.
    const b = page.getByText(/^(понятно|got it)$/i).filter({ visible: true }).first();
...
const HEADER_DEBT = {
  /*
   * ПУСТО С 16.09.2026, И ЭТО ПОГАШЕННЫЙ ДОЛГ, А НЕ ЗАБЫТЫЙ СПИСОК.
   *
   * Здесь стояла единственная запись — `'/games/sudoku': 1`: на 360 px подпись
   * «Отменить» в шапке судоку обрезалась на 17 px, потому что обёртка-колонка
   * мерилась по содержимому и душила ряд кнопок (задача f4fea6b5). Починил
   * psygames-sudoku-claude-mac в своём файле: блоку подсказки дана своя ширина
   * (`hintBlock: { … width: '100%' }`, коммит 6af0c771).
   *
   * 📍 Запись сняла не сверка по сроку, а сам аудит: на прогоне main `21e8f2ab`
   * джоба «Веб-гейты (tap-header)» покраснела с «ИСКЛЮЧЕНИЕ ПРОТУХЛО: /games/sudoku:
   * мелких больше нет — убери исключение». Этот гейт стоит в `needs` у TestFlight и
   * у Google Play, то есть протухшая запись одна держала выпуск 2.54.12.
   * ⚠️ Хвост был записан мной в канал в тот же день («снимаю сам после прогона
   * аудита») и не снят до следующего выпуска — починка чужими руками закрыла
   * дефект, а отметку о нём оставила висеть.
   */
};
