...
const READ_SLOTS = () => {
   ...
   * уход ЗА ПРАВЫЙ КРАЙ не виден ни по высоте, ни по размеру — кнопка остаётся
   * 48×48 и «нарисованной», просто её половины нет на экране.
   */
  ...
  if (hud) {
    ...
    полеПолосы = {
      left: Math.round(r.left + parseFloat(cs.paddingLeft || '0')),
      right: Math.round(r.right - parseFloat(cs.paddingRight || '0')),
    };
  }
  return {
...
async function main() {
   ...
   * границе полосы 380. Ни один из трёх проходов выше этого не видел: кнопка
