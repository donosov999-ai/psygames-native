...
/**
 * 🔴 ОСЬ 3 — ЗАДЕРЖКА. Введена 07.09.2026: N упёрся в человека, а не в код.
 *
 * ЗАМЕР ДО. Прогон этой функции на L1…L60: на L13 `N` упирается в 6, и уровни
 * L14…L60 становятся побайтово одинаковыми — сорок семь клонов. Подтверждено
 * ИГРОЙ: на живом билде L13, L14 и L16 показывали одно и то же «repeats 6 back»
 * (`span-chat/probe/shots/n-back-L*.png`), а L12 честно давал «5 back».
 *
 * ПОЧЕМУ НЕ БОЛЬШЕ N. Семибэк не берёт никто; выше шести проба перестаёт различать
 * людей — все проваливают одинаково, а это не измерение, а отсев.
 *
 * ⚠️ КАК ЗАДЕРЖКА ВЫГЛЯДИТ ИМЕННО ЗДЕСЬ. В остальных четырёх играх это пауза между
 * показом и вводом. В n-back ввода-«потом» нет вовсе: человек отвечает по ходу
 * потока. Удержание здесь — интервал МЕЖДУ стимулами: чем он длиннее, тем дольше
 * держать цепочку из N элементов, не давая ей рассыпаться. Поэтому осью служит
 * растущий `gapMs`, а не отдельная пауза. ⚠️ В полосе L6…L8 `gapMs` наоборот
 * СОКРАЩАЕТСЯ — там ось 2 (скорость), и её не трогаем: две оси в одной полосе
 * дали бы обрыв вместо ступени.
 *
 * ⚠️ ПОТОЛКА У ЗАДЕРЖКИ НЕТ сознательно (правило раздела: потолков нет нигде).
 * Полосу можно будет закончить, когда начнутся оси 5 и 9 — приманки N±1 и
 * переменный N внутри партии, задача 103cd98d.
 */
...
const styles = StyleSheet.create({
  ...
  statsRow: { flexDirection: 'row', gap: 24, flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center', maxWidth: '100%' },
  ...
  readoutRow: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 18, maxWidth: '100%' },
