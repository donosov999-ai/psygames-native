...
export const MAHJONG_RULES: LevelRule[] = [
  {
...
export default function MahjongGame() {
  ...
  /**
   * 🔴 ПЯТЬ ПРОВАЛОВ ПОДРЯД, А НЕ ТРИ, — РЕШЕНИЕ ДЕНИСА 11.09.2026.
   * Общее правило приложения — три, но здесь цена проигрыша другая: доска встаёт
   * насмерть у 60 % раздач на 28-м уровне и у 53 % на 40-м (замер 06.09.2026,
   * 30 партий на уровень). Три подряд на верхних ступенях — обычное невезение;
   * пять подряд — уже стена, и с неё пора спускать.
   */
  const lvl = usePersistentLevel('mahjong', 1, 5);   // персист достигнутого уровня между сессиями
  ...
  const restartLevel = () => {
     ...
     *
     * 🔴 С 11.09.2026 ЛЕСТНИЦА ПОНИЖАЕТСЯ — решение Дениса по задаче 8543237a.
     * До этого `lvl.fail()` здесь не звался, и лестница была ОДНОСТОРОННЕЙ:
     * дошедший до 28-го уровня оставался на нём навсегда, проигрывая больше
     * половины раздач, и спуститься мог только выбрав уровень руками.
     * Понижение с гистерезисом в ПЯТЬ провалов подряд (а не в три, как у
     * остальных игр) — порог задан третьим доводом `usePersistentLevel`.
     * Успех обнуляет счётчик: одна собранная доска стирает серию.
     */
    ...
    /**
     * ⚠️ ШАГ ЗАРЯДКИ УРОВЕНЬ НЕ ДВИГАЕТ — ни вверх, ни вниз. Правило общее для
     * приложения: партия из плейлиста подобрана помощником, и понижать за неё
     * значит наказывать человека за чужой выбор. Поймано гейтом
     * `warmup-level-drift` («круг таких экранов не растёт») сразу же, как только
     * я добавил понижение без оговорки.
     */
    if (!isPreset) lvl.fail();
