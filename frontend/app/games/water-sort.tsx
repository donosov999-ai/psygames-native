...
const ПОЛЯ_СТЕКЛА: Record<string, { слева: number; справа: number }> = {
  ...
  nuts: { слева: 0.182, справа: 0.818 },   // стекла нет, но столб гаек считается так же
};
...
export function SortGameScreen({ gameId, skin, titleKey }: SortScreenProps) {
  ...
  const рисоватьПробирку = (трубка: readonly number[], i: number) => {
    ...
    const левo = ш * поля.слева;
    const ширинаЖ = ш * (поля.справа - поля.слева);
            ...
            return (
              <Animated.View
                ...
                style={[
                  styles.порция,
                  /*
                   * Поворот на пол-оборота и осадка вниз: гайка «докручивается»
                   * на место. Пол-оборота, а не полный: шестигранник при 360°
                   * выглядит неподвижным — грани совпадают сами с собой.
                   */
                  крутится ? {
                    transform: [
                      { rotate: винт.interpolate({ inputRange: [0, 1], outputRange: ['-180deg', '0deg'] }) },
                      { translateY: винт.interpolate({ inputRange: [0, 1], outputRange: [-6, 0] }) },
                    ],
                  } : null,
                  {
                    /*
                     * ⚠️ ЗАЗОР ЕСТЬ ТОЛЬКО У ШАРОВ. Круги без зазора слипаются в
                     * столбик и читаются как та же вода. Гайки наоборот: они
                     * НАКРУЧЕНЫ на болт и стоят вплотную, зазор между ними
                     * читался бы как «висят в воздухе». Отчёт `75ed86f3`
                     * дословно: «гайки должны быть плотнее».
                     */
                    height: гайка ? ширинаЖ / АСПЕКТ_ГАЙКИ : высотаПорции - (круглый ? 3 : 0),
                    marginBottom: круглый ? 3 : 0,
                    borderRadius: радиус,
                ...
                */}
                {шар ? (
                  <Image
                     ...
                     */
                    style={{ width: ширинаЖ, height: гайка ? ширинаЖ / АСПЕКТ_ГАЙКИ : высотаПорции - (круглый ? 3 : 0) }}
                    resizeMode="contain"
                  />
                ) : null}
