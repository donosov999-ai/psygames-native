...
export function levelParams(level: number): { delay: number; numSquares: number } {
   ...
   * и на L14, и на L15. Значит пятнадцатый уровень не отличался от четырнадцатого
   * ничем: человек «брал» ступень, за которой не стояло ни одного нового условия.
   * Замер 06.09.2026 по L1..L15: одна пара уровней из четырнадцати переходов —
   * полный дубль.
   *
   * Шаг 110 вместо 120 доводит ту же лестницу до того же пола ровно на L15:
   * задержки L1..L15 = 1990…450, все пятнадцать РАЗНЫЕ. Верх не смягчён —
   * на L15 по-прежнему 450 мс и 5 квадратов, как было. Цена — середина стала
   * просторнее не более чем на 130 мс (L13: 670 против 540).
   * Сторожит `attention-ladder-per-mode.test.ts` (пробы «нет уровней-дублей»).
   */
  const delay = Math.max(450, 2100 - level * 110);          // L1=1990мс → L15=450мс, все разные
