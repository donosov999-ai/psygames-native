        ...
        <View style={styles.сцена}>
          ...
          */}
          ...
          {ЦИФРОВЫЕ.has(имяРежима) ? (
            <View style={styles.цифры}>
              {/* ⚠️ ПАРАМЕТРЫ — ИЗ `ступени`, ТОЙ ЖЕ ЛЕСТНИЦЫ, ПО КОТОРОЙ ОТКРЫТА ДОСКА (`лестницаДвижка`).
                  Замер 16.09.2026 на main a34a2dd7: ряд читал АВТОРСКУЮ лестницу, доска — нашу.
                  У «Судоку Тэтхэма» на первой ступени доска 9×9 (наш `3x3db`), а клавиш
                  четыре (авторский `2x2`): цифры 5–9 вводить нечем при подписи «Цифры 1–9». */}
              {Array.from({ length: клавишДоски(имяРежима, лестницаДвижка(имяРежима, движок?.ступени ?? [])[ступень]?.параметры ?? '') }, (_, k) => k + 1).map((ц) => (
                <Pressable
                  key={ц}
                  accessibilityRole="button"
                  accessibilityLabel={String(ц)}
