...
/**
 * ⚠️ Тип по МИНИМУМУ, который читают ниже: у наших ступеней нет `индекс`, он и не нужен —
 * доска открывается по индексу ДВИЖКА и строке параметров.
 * ⚠️ И объявление ВНЕ компонента — не для красоты: внутри оно попадало в зависимости
 * `useCallback` и добавляло замечание линту, а ratchet запрещает растить долг.
 */
type Ступенька = { имя: string; параметры: string };
function лестницаДвижка(имя: string, свои: readonly Ступенька[]): readonly Ступенька[] {
  return (СВОЯ_ЛЕСТНИЦА[имя]?.length ? СВОЯ_ЛЕСТНИЦА[имя] : свои) ?? [];
}
