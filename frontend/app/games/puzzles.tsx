/* psygames-game-puzzles · VER 13 · 17.09.2026 */
/**
...
import { открыть, указатель, стрелка, ходЗаЖест, клавиша, стеретьВвод, выбрать, поДиагонали, отменить, решить, сверитьСРешением, нажать, протянуть, type Итог, type Партия, type Жест, type Сторона, type Диагональ } from '@/src/games/tatham-bridge/play';
import { ЦВЕТ_ЛИНИИ, лишниеЛинии, type Рамка } from '@/src/games/tatham-bridge/whereError';
import { УЧИТЕЛЯ, type КарточкаУрока as КарточкаУчителя } from '@/src/games/tatham-bridge/teach';
import { ПЛАН_ШАГАМИ, ИМЯ_ВТОРОГО, ВЫБОР, ВЫБОР_ВТОРОЙ, ВОСЕМЬ_НАПРАВЛЕНИЙ, КЛЮЧ_ИМЕНИ, КЛЮЧ_ОПИСАНИЯ, ПО_УМОЛЧАНИЮ, СТРЕЛОЧНЫЕ, лестницаДвижка, ВТОРОЕ_ДЕЙСТВИЕ, ВВОД, ТОЛЬКО_ПРОТЯЖКА, ЦИФРОВЫЕ, клавишДоски, ЗНАКИ_ЦИФР, ПОДСВЕТКА_ЧИСЛА, клавишПодсветки, ГНЁЗД_ПОДСВЕТКИ, ПРИТЯЖЕНИЕ_К_УЗЛУ } from '@/src/games/tatham-bridge/names';
import { ширинаРовныхРядов, ШИРИНА_КЛАВИШИ, ЗАЗОР_КЛАВИШ } from '@/src/games/tatham-bridge/keyRows';
import { РИСУНКИ_КЛАВИШ } from '@/src/games/tatham-bridge/MonsterGlyph';
 ...
 */
/**
 * У КАКИХ РЕЖИМОВ ЕСТЬ РАЗБОР — решает реестр учителей (`tatham-bridge/teach`), а не этот экран:
 * раздел вписывает учителя своего режима в `sections/<раздел>.teach.ts`, экран его находит сам.
 * Денис 17.09.2026, проверив пилот: «обучение зашло — раскатывай везде по всем играм».
 */
const СТУПЕНЕЙ_С_РАЗБОРОМ = 3;
/** Что показывает карточка разбора: шаг учителя или итог. */
type КарточкаРазбора = { вид: 'шаг'; карточка: КарточкаУчителя } | { вид: 'готово' };
/** Цвета рамок: основание вывода, куда ставим, ошибка. */
const ЦВЕТ_РАМКИ: Record<КарточкаУчителя['рамки'][number]['вид'], string> = {
  опора: '#f59e0b', ставим: '#10b981', ошибка: '#ef4444',
};
/** Рамка лишней линии «Где ошибка?» — тот же красный, что у строки под доской (`ошибкаСтрока`). */
const ЦВЕТ_ЛИШНЕЙ_ЛИНИИ = '#dc2626';
...
export default function PuzzlesScreen() {
  ...
  /**
   * «Где ошибка?» (`whereError.ts`): рамки лишних линий и ДОСКА, к которой они относятся. Любой ход даёт
   * новую доску — и рамки гаснут сами, без отдельного сброса в каждом обработчике.
   */
  const [сверка, setСверка] = useState<{ партия: Партия; рамки: Рамка[] } | null>(null);
  /** Сколько раз за партию спросили «Где ошибка?» — в подробности сессии, ступень не сдаёт. */
  const сверок = useRef(0);
  const сверяю = useRef(false);
  /**
  ...
  /**
   * Ширина ряда под доской — ИЗМЕРЕННАЯ, а не посчитанная из ширины окна.
   * Замер 18.09.2026 (экспорт, WebKit): на 360×640 в ряд вставало 5 клавиш, хотя
   * `окно − 2·ПОЛЕ_СЦЕНЫ` обещало 6. Между окном и рядом лежат поля каркаса, и
   * считать их по памяти — значит снова промахнуться. Узел `подДоской` растянут
   * (`alignSelf: 'stretch'`), поэтому его ширина и есть место, доступное ряду.
   */
  const [ширинаПодДоской, setШиринаПодДоской] = useState(0);
  ...
  */
  ...
  /**
   * Ровные ряды клавиш цифр: 10 клавиш при шести влезающих — это 5+5, а не 6+4.
   * Клавиш столько, сколько сторона доски, плюс «Стереть». У режима с «Готово» (`ВВОД`)
   * ряд шире на 24 точки — там ничего не навязываем, иначе перенос считался бы по чужой мерке.
   */
  const клавишЦифр = клавишДоски(имяРежима, ступени[ступень]?.параметры ?? '');
  const ширинаРядаЦифр = ВВОД.has(имяРежима) ? undefined
    : ширинаРовныхРядов(клавишЦифр + 1, ширинаПодДоской);   // +1 — «Стереть»
  ...
  /**
   * 🔴 «ГДЕ ОШИБКА?» НЕ СДАЁТ СТУПЕНЬ — в отличие от «Показать решение» (задача 9022b6ff).
   * Отзывы Дениса 16–17.09.2026: «решил — не засчитано, а почему, не видно». Лишние линии обводятся
   * красным; недостающие не показываются — это был бы ответ, а куда дорисовывать, движок говорит сам
   * заливкой сошедшихся областей. Механика и замер — `tatham-bridge/whereError.ts`.
   */
  const гдеОшибка = useCallback(async () => {
    if (сверяю.current || ЦВЕТ_ЛИНИИ[имяРежима] === undefined) return;
    сверяю.current = true;
    try {
      const с = await сверитьСРешением();
      if (!с) return;                      // решатель отказал — молчим, доска не тронута
      setПартия(с.партия);
      if (!с.вернулась) return;            // доска не вернулась к позиции игрока — сверке не верим
      const рамки = лишниеЛинии(имяРежима, с.партия.примитивы, с.решение.примитивы);
      if (!рамки) return;
      сверок.current += 1;
      setСверка({ партия: с.партия, рамки });
    } finally {
      сверяю.current = false;
    }
  }, [имяРежима]);

  /** Учитель режима — из реестра разделов; разбор только на первых ступенях («обучить в начале»). */
  const учитель = УЧИТЕЛЯ[имяРежима];
  const разборДоступен = !!учитель && ступень < СТУПЕНЕЙ_С_РАЗБОРОМ && (!учитель.нуженРешатель || !!движок?.решаем);

  /**
   * Начать разбор с ТЕКУЩЕЙ позиции человека, а не с пустой доски: он застрял именно здесь.
   * Решение, если учителю оно нужно, берём у движка: решить → запомнить → отменить. Карточки
   * строит учитель режима. Партия с разбором не засчитывается — тот же признак, что у
   * «Показать решение».
   */
  const начатьУрок = useCallback(async () => {
    if (!партия || !учитель || урокЗанят) return;
    setУрокЗанят(true);
    try {
      let решение: Партия | null = null;
      if (учитель.нуженРешатель) {
        решение = await решить();
        if (!решение) return;
        setПартия(await отменить());
      }
      const карточки = учитель.урок({ партия, решение, параметры: ступени[ступень]?.параметры ?? '' });
      if (!карточки) return;
      setСдался(true);
      setУрок({
        карточки: [...карточки.map((карточка) => ({ вид: 'шаг' as const, карточка })), { вид: 'готово' }],
        индекс: 0, нажатий: [],
      });
    } finally {
      setУрокЗанят(false);
    }
  }, [партия, учитель, урокЗанят, ступени, ступень]);

  /** «Дальше»: сделать ход текущей карточки нажатиями по доске и открыть следующую. */
  const урокДальше = useCallback(async () => {
    if (!урок || !партия || урокЗанят) return;
    const текущая = урок.карточки[урок.индекс];
    if (!текущая || текущая.вид === 'готово') return;
    setУрокЗанят(true);
    try {
      let последняя = партия;
      for (const н of текущая.карточка.нажатия) {
        последняя = (н.до ? await протянуть(н.x, н.y, н.до.x, н.до.y, н.правой) : await нажать(н.x, н.y, н.правой)).партия;
      }
      setПартия(последняя);
      // «Назад» отменяет ходы движка, а не нажатия: у «Угадай код» пять нажатий — один ход.
      const ходов = текущая.карточка.ходов ?? текущая.карточка.нажатия.length;
      setУрок({ ...урок, индекс: урок.индекс + 1, нажатий: [...урок.нажатий, ходов] });
    } finally {
      setУрокЗанят(false);
    }
  }, [урок, партия, урокЗанят]);
  ...
  const подсветкаУрока = карточкаУрока?.вид === 'шаг'
    ? карточкаУрока.карточка.рамки.map((р) => ({ x: р.x, y: р.y, ш: р.ш, в: р.в, цвет: ЦВЕТ_РАМКИ[р.вид] }))
    : undefined;
  const текстУрока = !карточкаУрока ? ''
    : карточкаУрока.вид === 'шаг' ? карточкаУрока.карточка.текст(t)
    : t('teachDone');
  /**
  ...
  const длительностьУрока = карточкаУрока?.вид === 'шаг'
    && урок!.карточки.slice(0, урок!.индекс).some((к) => к.вид === 'шаг' && к.карточка.приём === карточкаУрока.карточка.приём)
   ...
   * значками в одном ряду под полем, «нахуя раздул в два ряда»).
   * Замер на экспорте 17.09.2026, «Галактики», WebKit: с «Где ошибка?» и подписанным переключателем «Пометка»
   * ряд переносился на вторую строку на 360×640, 360×780 и 375×667; доска 290 → 270 и 317 → 297, кнопка
   * отзыва накрывала ряд на 673 px² вместо 224 (360×640) и на 448 вместо 149 (375×667). Подпись уходит в
   * `accessibilityLabel`. Условие — по РЕЖИМУ, а не по ходу партии: после победы кнопка сверки прячется, но
   * переключатель не разворачивается обратно — иначе ряд сменил бы высоту и доска прыгнула бы на последнем ходе.
   * У режимов с крестовиной («Мосты») лампочка и лупа стоят в её пустых клетках, в ряду три команды — подпись остаётся.
   */
  const рядЗначками = ЦВЕТ_ЛИНИИ[имяРежима] !== undefined && ВТОРОЕ_ДЕЙСТВИЕ.has(имяРежима) && !лампочкаУКрестовины;
  /**
  ...
  useEffect(() => {
    ...
    void saveSession({
      ...
      mode: имяРежима,
      details: { level: lvl.level, mode: имяРежима, moves: ходов, solver_used: сдался, error_checks: сверок.current },
  ...
  return (
    <GameShell
      ...
      onBack={() => router.back()}
      // «Заново» уже стоит значком в своём ряду под доской (`рядКоманд`) — второго из каркаса не нужно.
      auxRestart={false}
      ...
      overlay={фаза === 'cleared' ? (
        <LevelCleared
          ...
          passed={прошёл}
          /**
           * 🔴 ПРОИГРЫШ ОБЯЗАН НАЗВАТЬ ПРИЧИНУ (отчёт 91785f91, 17.09.2026).
           * «Заливка», ступень 1: человек доиграл ЧЕТЫРЕ партии подряд за четыре
           * минуты, каждый раз увидел «Уровень 1 — почти! Ещё раз» и написал «не
           * переходит на следующий уровень». Он не укладывался в бюджет ходов —
           * но узнать это было неоткуда: счётчик «9 / 8 ходов» при `FAILED!`
           * гаснет (признак тупика проверяется раньше счётчиков), а карточка
           * говорила только «почти».
           * ⚠️ Числа берём у ДВИЖКА (`партия.провал`), а не считаем заново:
           * своя копия бюджета разошлась бы с ним молча.
           */
          reasonLine={партия?.провал
            ? t('puzzleOutOfMoves')
                .replace('{m}', String(партия.провал.ходов))
                .replace('{n}', String(партия.провал.лимит))
            : undefined}
    ...
    >
      ...
      {фаза === 'config' && !движок ? (
        ...
        <View style={styles.сцена}>
          ...
          */}
          ...
          >
            <PuzzleCanvas
              ...
              подсветка={сверкаНаДоске?.рамки.map((р) => ({ ...р, цвет: ЦВЕТ_ЛИШНЕЙ_ЛИНИИ }))}
              // Узел «Распутать» мельче пальца — касание притягивается к нему (см. ПРИТЯЖЕНИЕ_К_УЗЛУ).
              примагничивание={ПРИТЯЖЕНИЕ_К_УЗЛУ[имяРежима] ?? 0}
          ...
          <View
            ...
            onLayout={(e) => {
              const h = Math.round(e.nativeEvent.layout.height); setВысотаПодДоской((п) => (п === h ? п : h));
              const w = Math.round(e.nativeEvent.layout.width); setШиринаПодДоской((п) => (п === w ? п : w));
            }}
          >
          {/*
          ...
          */}
          {/*
            Тот же слот говорит итог «Где ошибка?» у режимов без своей подсветки (`ЦВЕТ_ЛИНИИ`): сколько лишних
            линий обведено — красным, «лишних нет» — серым, это не ошибка. Слот так же постоянный.
          */}
          {ЦВЕТ_ОШИБКИ[имяРежима] || ЦВЕТ_ЛИНИИ[имяРежима] !== undefined ? (
            <Text
              testID="puzzle-error-hint" numberOfLines={1}
              style={[styles.ошибкаСтрока, сверкаНаДоске && сверкаНаДоске.рамки.length === 0 ? { color: colors.textSecondary } : null]}
            >
              {сверкаНаДоске
                ? (сверкаНаДоске.рамки.length
                  ? t('puzzleWrongLines').replace('{n}', String(сверкаНаДоске.рамки.length))
                  : t('puzzleNoWrongLines'))
          ...
          */}
          ...
          {ВТОРОЕ_ДЕЙСТВИЕ.has(имяРежима) ? (
            <Pressable
              testID="puzzle-second-action"
              accessibilityRole="button"
              accessibilityState={{ selected: второе }}
              accessibilityLabel={t(ИМЯ_ВТОРОГО[имяРежима] ?? 'puzzleSecondAction')}
              onPress={() => setВторое((v) => !v)}
              style={[styles.второе, рядЗначками ? styles.второеЗначком : null, {
            ...
            {/* «Где ошибка?» — рядом с лампочкой: обе кнопки про решение, но эта ступень не сдаёт. У режимов с
                крестовиной обе стоят в её пустых клетках, см. ниже. */}
            {гдеОшибкаДоступна && !лампочкаУКрестовины ? (
              <GameAuxAction
                compact icon="search-outline" tint="#d97706" label={t('puzzleWhereError')}
                onPress={() => { void гдеОшибка(); }}
              />
            ) : null}
          ...
          */}
          ...
          {ЦИФРОВЫЕ.has(имяРежима) ? (
            <View style={[styles.цифры, ширинаРядаЦифр ? { maxWidth: ширинаРядаЦифр } : null]}>
              ...
              {Array.from({ length: клавишЦифр }, (_, k) => k + 1).map((ц) => {
                ...
                const Рисунок = знак?.рисунок ? РИСУНКИ_КЛАВИШ[знак.рисунок] : null;
                return (
                  <Pressable
                    key={ц}
                    accessibilityRole="button"
                  ...
                  >
                    {/* Где у знака есть свой рисунок («Нежить» — фигуры движка), клавиша рисует его,
                        а не эмодзи: на доске счётчик показывает то же самое существо. */}
                    {Рисунок ? <Рисунок номер={ц} размер={30} /> : (
          ...
          */}
          {СТРЕЛОЧНЫЕ.has(имяРежима) && !ВОСЕМЬ_НАПРАВЛЕНИЙ.has(имяРежима) ? (
            ...
            <View style={styles.крестовинаСРешением} testID="arrow-pad-with-solution">
              ...
              {/*
                «Где ошибка?» у режимов с крестовиной — в пустой клетке СЛЕВА от «↑», зеркально лампочке.
                Замер 17.09.2026, «Мосты», экспорт WebKit 360×640: четвёртой в ряду под полем лупа расширяла ряд, и
                кнопка отзыва накрывала «Отменить» на 996 px². В клетке крестовины высоты и ширины ряда она не меняет.
              */}
              {гдеОшибкаДоступна ? (
                <View style={styles.сверкаУСтрелки}>
                  <GameAuxAction
                    compact icon="search-outline" tint="#d97706" label={t('puzzleWhereError')}
                    onPress={() => { void гдеОшибка(); }}
                  />
...
const styles = StyleSheet.create({
  ...
  сверкаУСтрелки: { position: 'absolute', top: 0, left: 0 },
  ...
  // Переключатель значком — в ряду из пяти команд (`рядЗначками`): тот же квадрат 48, что у значков каркаса.
  второеЗначком: { paddingHorizontal: 0, width: 48, justifyContent: 'center' },
  ...
  цифры: { flexDirection: 'row', gap: ЗАЗОР_КЛАВИШ, flexWrap: 'wrap', justifyContent: 'center', marginTop: 12, maxWidth: 420 },
  подсветкаБлок: { alignItems: 'center', marginTop: 12 },
  подсветкаПодпись: { fontSize: 13, fontWeight: '700', textAlign: 'center' },
  цифра: { width: ШИРИНА_КЛАВИШИ, height: 50, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
