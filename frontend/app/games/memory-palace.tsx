 ...
 */
/*
 * ⚠️ ИМЯ ЛАТИНИЦЕЙ, И ЭТО НЕ ПРИДИРКА СТИЛЯ. Правило хуков React опознаёт компонент по
 * ЗАГЛАВНОЙ ЛАТИНСКОЙ букве в имени. Кириллическая «Р» ему заглавной не считается, и
 * `React.useMemo` внутри читается как «хук вне компонента» — это React #310 у человека на
 * экране, а гейт держит по таким нулевую терпимость. 23.09.2026 из-за этого имени CI был
 * красным у всех девяти чатов.
 */
function LessonScene({ seed, level, карточка, сторона, locale, colors }: {
  seed: string;
  level: number;
