 ...
 *   different variants of the go/no-go paradigm. Psychophysiology, doi:10.1111/psyp.12871.
 *   Преобладающую реакцию надёжно вызывают только БЫСТРЫЕ задания с РЕДКИМИ no-go.
 *   В медленных задачах с равными долями связанная с торможением активность на 75 % ниже.
 *   По литературному обзору той же статьи ~40 % работ ставят доли 50/50, а ~20 % — интервал
 *   между стимулами больше 4 с. У нас no-go 25 %, пауза 280–600 мс, окно 550–1100 мс — то есть
 *   редкие no-go и темп в разы быстрее 4 с. ⚠️ Точные пороги «быстрого» и «редкого» из
 *   полного текста не сверены: статья закрыта, опора — только её аннотация.
 * · Zhang N. et al. (2024). Go/No-Go Ratios Modulate Inhibition-Related Brain Activity.
 *   Brain Sciences 14(5):414, doi:10.3390/brainsci14050414 — соотношение go/no-go меняет
 *   само торможение (NoGo-P3), поэтому долю нельзя делать ручкой уровня.
 * Отступление от канона: нет. В распространённых реализациях no-go 20 % (80/20) или 25 % (75/25);
 * взята верхняя из двух ради числа запретных проб на короткой партии (24 пробы на L1 — 6 no-go
 * против 4,8 при 20 %).
 */
    ...
    return (
      ...
      <ScrollView style={styles.configScroll} contentContainerStyle={styles.configContainer} showsVerticalScrollIndicator={false}>
        <LinearGradient colors={GRADIENT as [string, string]} start={{x:0,y:0}} end={{x:1,y:1}} style={styles.configCard}>
          <Ionicons name="pause-circle" size={48} color={ON_GRAD.color} />
