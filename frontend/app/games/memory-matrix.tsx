...
export const MEMORYMATRIX_RULES: LevelRule[] = [
  ...
  /**
   * 🔴 ДВЕ СЕРИИ БЫЛИ МОЛЧАЛИВОЙ МЕХАНИКОЙ С САМОГО ЗАВЕДЕНИЯ. Долг записан гейтом
   * `level-step-explained` 06.09.2026: с L11 показываются ДВА набора разного цвета
   * вместо одного, а правила не было — только grid6 (L4) и fast (L8).
   */
  { key: 'two_series', fromLevel: 11 },   // lr_memory_matrix_two_series_*
  /**
   * Ось 5 — ложные вспышки. Объявляется вместе с механикой, а не после: ровно на
   * этом разделе уже обожглись, выпустив ось 9 «Объёма цифр» без правила.
   */
  { key: 'decoys', fromLevel: MM_VOLUME_TOP + 1 },   // lr_memory_matrix_decoys_*
];
...
export default function MemoryMatrixGame() {
   ...
   */
  const levelRules = useLevelRules('memory_matrix', lvl.level, MEMORYMATRIX_RULES, phase === 'config');
  ...
  const renderConfig = () => (
    <View style={{ flex: 1 }}>
      ...
      <LevelProgressMap bestLevel={lvl.best} gameId="memory_matrix" currentLevel={lvl.level} onPickLevel={lvl.pick} colors={colors} language={language} />
        ...
        >
          <Text style={[styles.hintText, { color: colors.textSecondary }]}>
            {holding
              ? t('memorize')
              /**
               * 🔴 ПОДСКАЗКА ПРО КРЕСТ — НА ПОЛЕ, А НЕ ТОЛЬКО В КАРТОЧКЕ ПРАВИЛ.
               * Карточку читают один раз, а крест виден каждый показ. Снимок витрины
               * 07.09.2026 показал, почему это нужно: ложные того же красного, что и
               * вторая серия, различает только форма, а подпись рядом говорила
               * «запоминай красные» — рядом с перечёркнутыми красными это ребус.
