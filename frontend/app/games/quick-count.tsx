/* psygames-game-quick-count · VER 2 · 17.09.2026 */
 ...
 *
 * 🔴 VER 2 (17.09.2026): СДВИГА ОТ ОТВЕТА МАЛО — ОКНО САМО ВЫДАВАЛО ОТВЕТ. Окно строилось вокруг `n` и
 * прижималось к краям, поэтому крайние числа окна почти не бывали ответом. Замер точным перебором всех `n`
 * и сдвигов: игрок, запомнивший для каждого окна самый частый ответ, угадывал на L1–3 и L31+ 45 % при
 * потолке 33 % (возможных ответов три), на L5–8 — 38 % при 25, на L16 и L25 — 31 % при 17.
 * Теперь, если все возможные ответы уровня (`minN…maxN`) помещаются в окно, окно их накрывает, а сдвиг
 * выбирает лишь, с какого числа оно начнётся. Окно от ответа не зависит, и угадать можно ровно потолок.
 * На L20–24 возможных ответов семь, а окно из шести не накрывает их никаким сдвигом: там 29 % при потолке 14,
 * и лучше без седьмой кнопки не выйти (замер равномерного выбора среди всех окон с ответом — те же 29 %).
 *
...
export function answerWindow(p: { minN: number; maxN: number }, n: number, сдвиг: number): number[] {
  ...
  const от = Math.max(первый, p.maxN - ANSWER_MAX + 1);
  const до = Math.min(p.minN, последний - ANSWER_MAX + 1);
  if (от <= до) {
    const начало = от + (место % (до - от + 1));
    return Array.from({ length: ANSWER_MAX }, (_, i) => начало + i);
  }
  let lo = n - место;
