...
export default function HanoiGame() {
  ...
  const renderPlaying = () => (
    <GameShell
       ...
       * низ свободен — но объявления не было, и ряд стоял НАД полем, опуская его.
       *
       * 📍 Замер чата «Поиск» 11.09.2026 и перемер на 2.53.8: у семи экранов с этим
       * рядом верх поля 173 вместо 119, центр поля 509/516 вместо 482. Содержимое
       * при этом стоит ровно по центру СВОЕГО поля — виновато не оно, а высота поля.
       */
      bottom="actions"
      onBack={() => goBackOrHome()}
