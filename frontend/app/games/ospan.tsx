...
// Было зашито '#FFF' — контраст 1.12 (норма AA 4.5), стало 4.52.
// Сплошным цветом этот градиент AA не берёт ни при каком цвете текста — GradientSurface
// кладёт поверх вуаль #dbfff8 @0.05 цветом самого градиента. Подробности — в шапке сервиса.
...
export default function OSpanGame() {
   ...
   */
  const levelRules = useLevelRules('ospan', lvl.level, OSPAN_RULES, phase === 'config');
            ...
            phase === 'eq' ? (
              <View style={styles.choiceRow}>
                <TouchableOpacity
                  ...
                  <Text style={[styles.choiceText, { color: textOn('#22c55e') }]} numberOfLines={1}>{t('correct')}</Text>
                </TouchableOpacity>
                <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  modeButton: { minWidth: 48, minHeight: 48, justifyContent: 'center', paddingVertical: 10, paddingHorizontal: 18, borderRadius: 16 },
  ...
  choiceRow: { flexDirection: 'row', gap: 16, flexWrap: 'wrap', justifyContent: 'center', maxWidth: '100%' },
