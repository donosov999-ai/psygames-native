 ...
 */
const PROFILE_GOOD_SET: Record<string, string> = {
  kids: 'toys',
  free: 'pets',
  chess: 'mix',
  odv999: 'mix',
 ...
 */
const лимитГрупп = (slots: number): number => (slots >= 7 ? 1 + Math.floor(slots / 16) : 0);
const лимитПар = (slots: number): number => 1 + Math.floor(slots / 12);
 ...
 */
function порядокНиш(L: number, slots: number): number[] {
  const нод = (a: number, b: number): number => (b === 0 ? a : нод(b, a % b));
  let шаг = 5;
...
const GOAL_RULE_KEY: Record<GoalKind, string | null> = {
