 ...
 */
import {
  CAP, CAP_MAX, CAP_MIN, CAP_ONE, CLEAR_SCORE, EMPTY_HIDDEN_STATS, GOODS_BENEFITS, GOOD_ONBOARD_H,
  GOOD_ONBOARD_W, GOOD_SETS, GOOD_SETS_KEYS, GOOD_SET_POOL_SIZE, GRADIENT, GS_GAME_ID,
  GS_RESUME_DEBOUNCE_MS, GS_RESUME_V, GS_RULES, HIDDEN_FROM, HINTS_PER_LEVEL, JOKER_FROM,
  MIXED_CAP_FROM, MONO_FROM, MOVE_SHIFT_EVERY, MOVING_FROM, PAIR_HINT_UNTIL, REF_PER_TYPE,
  SET_COLS, SHAPES, SHUFFLES_PER_LEVEL, SINGLE_CAP_FROM, THUMBS_PER_CARD, TYPES_ON_BOARD_MAX,
  WARM_FAMILY, WIDEST_POOL, capsFor, capsForBoard, clampGoalToLevel, clampGoalToRule, findHint,
...
/**
 * 🔴 СТОПКА ТОВАРОВ НИШИ — ОТДЕЛЬНЫМ ЗАПОМИНАЮЩИМ КОМПОНЕНТОМ, И ЭТО ЗАМЕР, А НЕ
 * ВКУС.
 *
 * Замер 07.09.2026 (`tools/drag-cost.bench.ts`): одна перерисовка доски стоит
 * медиану 5,5–9,4 мс и p95 13–24 мс при бюджете кадра 16,7 мс, а под нишами
 * лежит 62–67 % всех узлов дерева. При перетаскивании доска перерисовывается на
 * КАЖДОМ пересечении границы ниши — девять раз за полсекундную протяжку через
 * витрину. Отсюда рывки: девять кадров из тридцати не укладываются в бюджет.
 *
 * ⚠️ ПОЧЕМУ ИМЕННО СТОПКА, А НЕ ВСЯ НИША. Подсветка «сюда ляжет» (`aimed`) —
 * единственное, что меняется при движении пальца, и она живёт на РАМКЕ ниши.
 * Стопка товаров от неё не зависит вовсе. Значит рамку можно перерисовывать
 * сколько угодно, а дорогую стопку — только когда изменилась она сама.
 *
 * ⚠️ ВСЕ ПРОПЫ ОБЯЗАНЫ БЫТЬ УСТОЙЧИВЫМИ, ИНАЧЕ ЗАПОМИНАНИЕ НЕ РАБОТАЕТ ВОВСЕ.
 * Поэтому накрытость приходит МАСКОЙ (число), а не множеством, подпись
 * собирается здесь из имени товара и слова «полка», а обработчик тапа приходит
 * стабильной ссылкой через ref. Передай сюда `covered` множеством или свежую
 * стрелку — сравнение пропов будет промахиваться каждый раз, и вынос окажется
 * чистым убытком: тот же рендер плюс лишний уровень.
 */
interface СтопкаПропы {
  niche: number;
  cell: number[];
  iS: number;
  iH: number;
  /** Индексы «выбран / подсказан / в руке»; −1 — никакой. */
  selIdx: number;
  hintIdx: number;
  handIdx: number;
  /** Верхний товар прилетает копией — настоящий на время полёта скрыт. */
  arrivingLast: boolean;
  /** Накрытые места битами: бит `s` — товар с индексом `s`. */
  coveredMask: number;
  hiddenHere: boolean;
  overlap: number;
  ru: boolean;
  shelfWord: string;
  onTap: (niche: number, idx: number) => void;
}

const СтопкаНиши = React.memo(function СтопкаНиши(p: СтопкаПропы) {
  return (
        <View pointerEvents="box-none" style={styles.cellRow}>
          ...
          {p.cell.map((tp, s) => {
            ...
            const inHand = p.handIdx === s;
            /** Товар в полёте: настоящий скрыт, пока летит копия — иначе на экране две штуки. */
            const накрыт = (p.coveredMask & (1 << s)) !== 0;
            const arriving = p.arrivingLast && s === p.cell.length - 1;
            return (
              <TouchableOpacity key={s} activeOpacity={0.7} onPress={() => p.onTap(p.niche, s)}
                accessibilityRole="button"
                accessibilityLabel={`${накрыт ? '?' : goodName(tp, p.ru)}, ${p.shelfWord} ${p.niche + 1}`}
                accessibilityState={{ selected }}
                style={[styles.itemSlot, {
                  width: p.iS, height: p.iH,
                  /**
                   ...
                   */
                  marginLeft: s === 0 ? 0 : -Math.round(p.iS * p.overlap),
                ...
                {накрыт ? (
                  p.hiddenHere ? (
                     ...
                     */
                    <UnknownGood width={p.iS} height={p.iH - 2} />
                  ) : (
                  /**
                  ...
                  <Image {...a11yDecor} source={GOOD_SPRITES[tp % GOOD_SPRITES.length]}
                    style={{ width: p.iS, height: p.iH - 2, tintColor: 'rgba(35,20,8,0.82)' }}
                    resizeMode="contain" />
                  )
                ) : (
                  <GoodIcon type={tp} width={p.iS} height={p.iH - 2} />
                )}
              </TouchableOpacity>
            );
          })}
...
export default function GoodsSortGame() {
  ...
  /**
   * 🔴 СТОЛБЕЦ ОСЕДАЕТ, А НЕ ПЕРЕСКАКИВАЕТ.
   *
   * Полка закрылась — всё, что стояло выше в этом столбце, сползает на её
   * место. Без движения это выглядит подменой: товары просто оказываются в
   * других нишах, и человеку не за что зацепиться взглядом, чтобы понять, ЧТО
   * произошло. Разобрать это по статичной картинке нельзя — а правило уровня
   * обещает именно «оседает вниз».
   *
   * Как считаем: у полок есть НОМЕРА, и они едут вместе с содержимым. Значит
   * достаточно сравнить «какой номер на каком месте» до и после хода —
   * пересчитывать ничего не надо, отчёт ядра уже всё принёс.
   *
   * ⚠️ Только по своему столбцу: перескок между столбцами означал бы не
  ...
  const оседание = (доМест: number[], послеМест: number[], столбцы: number[]) => {
     ...
     * концу движения.
     *
     * 📍 Сначала обёртку снимал сам `Animated` (`start(() => setОсевшие({}))`), и
     * состояние жило ровно 230 мс. Для глаза разницы нет, а для замера есть:
     * проба, которая смотрит «есть ли сейчас оседание», начинала зависеть от
     * того, чем ещё занят процесс. Соло зелено, вдвоём с соседней — красное, и
     * оба раза честно. Теперь состояние держится до следующего хода: на вид то
     * же самое (сдвиг в покое равен нулю), а гонки нет.
     */
    setОсевшие(карта);
    if (!Object.keys(карта).length) return;
    осадка.setValue(1);
    Animated.timing(осадка, { toValue: 0, duration: 230, easing: Easing.out(Easing.quad), useNativeDriver: true })
      .start();
  };
...
/** Ряд, в котором лежит ниша с плотным индексом i (нумерация по существующим). */
  ...
  const moveItem = (fromCell: number, fromIdx: number, toCell: number) => {
    ...
    if (расширеннаяДоска) {
      // Движение запускаем ДО смены состояния: карта строится по старым местам.
      оседание(номераДо, номераПосле, столбецНиши);
...
const LAY = gsLayout(width, availH, gridDim.cols, gridDim.rows, capWideHere, hintH || undefined,
  ...
  /**
   * Накрытые места ниши одним числом: бит `s` — товар с индексом `s`.
   *
   * 🔴 ЧИСЛОМ, А НЕ МНОЖЕСТВОМ, РАДИ ЗАПОМИНАНИЯ СТОПКИ. `covered` — это `Set`,
   * и его ссылка меняется на каждом ходе; передай его вниз, и сравнение пропов
   * промахивалось бы всегда, а вынос стопки в `React.memo` оказался бы чистым
   * убытком. Число сравнивается по значению и не меняется, пока не изменилась
   * сама накрытость.
   *
   * ⚠️ Товаров в нише не больше `CAP_MAX`, поэтому битов заведомо хватает.
   */
  const маскаНакрытых = (i: number, длина: number): number => {
    let m = 0;
    for (let s = 0; s < длина; s += 1) if (covered.has(`${i}:${s}`)) m |= 1 << s;
    return m;
  };

  /**
   * Стабильная ссылка на обработчик тапа по товару.
   *
   * ⚠️ Без неё запоминание стопки не работает: `handleItemTap` объявлен в теле
   * экрана и получает новую ссылку на каждом рендере. Держим последнюю версию в
   * `ref`, а вниз отдаём обёртку, созданную один раз, — поведение то же,
   * замыкание всегда свежее.
   */
  const тапРеф = useRef(handleItemTap);
  /*
   * ⚠️ Свежая версия кладётся в ЭФФЕКТЕ, а не в теле рендера: запись в ref во
   * время рендера линт запрещает по делу — при повторном рендере (React их
   * повторяет) значение уехало бы до того, как кадр показан. Тап приходит от
   * человека, то есть заведомо после эффектов, и стареть ссылке негде.
   */
  useEffect(() => { тапРеф.current = handleItemTap; });
  const тапПоТовару = useCallback((niche: number, idx: number) => тапРеф.current(niche, idx), []);
  /**
   ...
   * проехала, и приезжает на место.
   *
   * ⚠️ Обёрткой `Animated.View`, а не строкой в стиле самой ниши: ниша —
   * `ImageBackground`, обычный компонент, и живое значение анимации до него не
   * доходит. Рядом это уже видно на вспышке: она нарисована `Animated.View`.
   */
  const осадкаСтиль = (i: number) => {
    const рядов = осевшие[i] ?? 0;
    if (!рядов) return null;
    return {
      transform: [{
        /*
         * Шаг ряда равен высоте ниши: у шкафа НЕТ зазоров по вертикали — ряды
         * разделяют сами ниши (решение «один шкаф, а не стопка полок»).
         */
        translateY: осадка.interpolate({ inputRange: [0, 1], outputRange: [0, -рядов * nicheH] }),
      }],
    };
  };
  ...
  const renderCell = (i: number) => {
    ...
    const ниша = (
      <ImageBackground key={i}
        ...
        */}
        {/*
          🔴 ЗА НИШЕЙ ЕСТЬ ВТОРОЙ РЯД — И ЭТО ВИДНО ДО ХОДА, А НЕ ПОСЛЕ.

          С L52 у части ниш за спиной стоит вторая полка: она выйдет вперёд,
          когда ниша опустеет. Без метки такая ниша неотличима от обычной, и
          человек узнаёт о втором ряде единственным способом — разобрав первый и
          получив неожиданное. Ровно та угадайка, из-за которой рядом рисуются
          насечки ёмкости и звезда джокера.

          ⚠️ РИСУЕМ ПОЗАДИ, А НЕ ПОВЕРХ. Значок поверх закрыл бы товар — та самая
          ошибка, что чинилась в сосудах шариков (замер: стекло закрывало 43%
          площади). Здесь глубина показана тем же способом, каким её показывает
          мебель: краем второй полки, выглядывающим из-за первой, и точками по
          числу товаров за спиной.
        */}
        {(задние[i]?.length ?? 0) > 0 && (
          <View pointerEvents="none" style={styles.backRow}>
            <View style={styles.backEdge} />
            <View style={styles.backDots}>
              {Array.from({ length: Math.min(4, задние[i]?.length ?? 0) }).map((_, k) => (
                <View key={k} style={styles.backDot} />
              ))}
            </View>
          </View>
        )}
        ...
        {/* Рисуем ТОЛЬКО реальные товары (по центру) — без пустых боксов; пустое место ячейки = куда класть.
            Стопка вынесена в запоминающий компонент: при перетаскивании доска
            перерисовывается на каждом пересечении ниши, а стопка от подсветки не
            зависит — см. шапку `СтопкаНиши`. */}
        <СтопкаНиши
          niche={i}
          cell={cell}
          iS={LAY.itemBox(capOf(i)).w}
          iH={LAY.itemBox(capOf(i)).h}
          selIdx={isSelCell ? (sel?.idx ?? -1) : -1}
          hintIdx={hint?.fromCell === i ? (hint?.fromIdx ?? -1) : -1}
          handIdx={drag?.cell === i ? drag.idx : -1}
          arrivingLast={fly?.toCell === i}
          coveredMask={маскаНакрытых(i, cell.length)}
          hiddenHere={hiddenHere}
          overlap={overlap}
          ru={ru}
          shelfWord={t('a11yShelf')}
          onTap={тапПоТовару}
        />
    ...
    /*
     * Без движения обёртку не заводим: лишний узел на каждой нише каждого кадра.
     *
     * ⚠️ `testID` здесь не для красоты. `translateY` в этом экране есть ещё у
     * мини-карты, у выбранного товара, у летящей копии и у слоя перетаскивания —
     * проба «есть узел со сдвигом» зеленела от них, и три мутации подряд
...
const styles = StyleSheet.create({
  ...
  /*
   * Метка второго ряда: край полки, выглядывающий сверху из-за первой, и точки
   * по числу товаров за спиной. Высота 5 px — столько же занимает тень ниши,
   * то есть ряд от метки не подрастает и товары не мельчают.
   */
  backRow: { position: 'absolute', top: 0, left: 0, right: 0, alignItems: 'center', zIndex: 0 },
  /*
   * 🔴 КРАЙ ВТОРОЙ ПОЛКИ — ДЕРЕВЯННЫЙ, ТОЛЬКО ПРИТЕНЁННЫЙ.
   *
   * ⚠️ Первая редакция красила его тёмным `rgba(60,38,12,0.55)` «чтобы читалось
   * как глубина» — и метка исчезла. Замер по снимку живой игры: интерьер ниши
   * RGB(42,23,12), а тёмная метка ложилась поверх в (52,31,12) — та же краска
   * на той же краске. Рама шкафа при этом RGB(234,207,173).
   *
   * Полка позади сделана из того же дерева, что и передняя, — значит и цвет у
   * неё тот же, приглушённый тенью: (200,170,130) при 0,55 даёт над интерьером
   * ниши примерно (129,104,77). Видно, и при этом читается как «в глубине», а
   * не как предмет на переднем плане.
   */
  backEdge: {
    width: '78%', height: 5,
    backgroundColor: 'rgba(200,170,130,0.55)',
    borderBottomLeftRadius: 4, borderBottomRightRadius: 4,
    borderBottomWidth: 1, borderBottomColor: 'rgba(30,16,6,0.45)',
  },
  backDots: { flexDirection: 'row', gap: 2, marginTop: 1 },
  backDot: { width: 3, height: 3, borderRadius: 2, backgroundColor: 'rgba(232,205,168,0.75)' },
