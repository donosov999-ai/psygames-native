...
export default function NavigatorScreen() {
  ...
  /**
   * Перезапуск партии, полученный от модуля: каркас кладёт его значком в ряд под полем.
   * Ссылка держится в состоянии, а не в ref, — значок обязан появиться на том же кадре.
   */
  const [заново, setЗаново] = React.useState<(() => void) | null>(null);
  const принятьЗаново = React.useCallback((restart: () => void) => setЗаново(() => restart), []);
  ...
  if (phase === 'playing') {
    return (
      ...
      <GameShell
        ...
        confirmExit={armed}
        /**
         * 🔴 ПОЛЕ ОТДАЁТ СВОЮ ВЫСОТУ ЧИСЛОМ (задача 2752f33f, отчёт e5bfc2f0 «игры ездят»).
         * Без `scrollableField` каркас высоту не измеряет и не отдаёт, и карта считалась от
         * высоты ОКНА минус константы по языкам — они устарели с первой же правкой каркаса
         * (обход координатора 17.09: +230 px переполнения на 360×640).
         */
        scrollableField
        /**
         * Служебное — одним рядом значков под полем (решение Дениса 17.09.2026). «Начать заново»
         * каркас берёт из пункта паузы с `id: 'restart'`; модуль отдаёт сюда свой перезапуск.
         */
        pauseActions={заново ? [{ id: 'restart', label: navStrings.restart, icon: 'refresh', onPress: заново }] : []}
      >
        <View style={styles.stage}>
          <NavigatorGame
            key={attempt}                 /* новый заход — чистое состояние модуля */
            onRestartReady={принятьЗаново}
            seed={seed}
            level={level}
            mode={roundMode}
            locale={locale}
