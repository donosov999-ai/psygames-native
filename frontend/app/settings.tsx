...
/**
 * 🔴 ОХРАНА ВЫНЕСЕНА В ОБЁРТКУ БЕЗ ХУКОВ (правка 07.09.2026).
 *
 * Было: `if (isWebDemo()) return <Redirect/>` ПЕРВОЙ строкой компонента, до
 * всех хуков. На деле порядок хуков не плавал — `isWebDemo()` это флаг времени
 * сборки, он одинаков во всех отрисовках, — но линтер этого знать не может и
 * помечал КАЖДЫЙ хук после возврата: тридцать ошибок в одном файле.
 *
 * Заглушить их подавлением значило бы поставить тридцать `eslint-disable` и
 * спрятать настоящее правило. Обёртка решает то же самое честно: у внешней
 * функции хуков НЕТ, поэтому её ранний возврат законен, а тело живёт отдельным
 * компонентом, где все хуки безусловны.
 *
 * ⚠️ Поведение не меняется: в веб-демо тело не отрисовывается вовсе, как и
 * прежде — обёртка возвращает перенаправление до создания тела.
 */
...
function SettingsScreenBody() {
  ...
  const загрузитьСоставИзТекста = async (текст: string) => {
    ...
    await сохранитьСостав({ профили: р.состав, наборы: р.наборы, хабы: р.хабы, замки: р.замки, коллекция: р.коллекция });
...
const styles = StyleSheet.create({
  ...
  langButton: { minHeight: 48, justifyContent: 'center',
