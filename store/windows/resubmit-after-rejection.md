...
- **Название** `PsyGames: Brain Training` → **`Brain Tools: Cognitive Training`**
  в английской витрине и **`Когнитивный тренажёр`** в русской.
  Имена РАЗНЫЕ намеренно: латиница в русской карточке читается как непереведённая,
  а «тренажёр» — слово из спортзала, взрослое и никак не игрушечное. Витрины у
  Microsoft раздельные по языкам, связывает их строка издателя.
...
1. **Зарезервировать ОБА новых имени.** Product management → Manage app names →
   добавить `Brain Tools: Cognitive Training` и `Когнитивный тренажёр`.
   Поле Product name в витрине выбирается из выпадающего списка зарезервированных
   имён и задаётся ОТДЕЛЬНО для каждого языка — значит русской витрине нужно своё
   имя, и пока оно не зарезервировано, выбрать его нельзя.
...
6. В КАЖДОЙ языковой витрине выбрать своё **Product name** из выпадающего списка:
   английская → `Brain Tools: Cognitive Training`, русская → `Когнитивный тренажёр`.
