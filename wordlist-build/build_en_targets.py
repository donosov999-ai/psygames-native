...
МАКС_РАСКЛАДОК = 3_000        # больше в приложение не нужно, а вес растёт
МИН_ДЛИНА = 4
...
МИН_ПОДСЛОВ = 8
# 🔴 ПОТОЛОК СЛОВ РАСТЁТ ПО ХОДУ УРОВНЕЙ, А НЕ ОДИН НА ВЕСЬ НАБОР.
#
# 📍 Замер 06.09.2026: жёсткий потолок 14 выбрасывал 58% уже собранного — 40 804
# слова из 70 774, при том что медиана доступного 23, а максимум 152. Но поднять
# потолок до 23 всем скопом значило бы сделать ПЕРВЫЙ уровень таким же тяжёлым,
# как тысячный: 58% — это не только потеря, это ещё и отбор.
#
# Поэтому число слов стало осью сложности. Уровень 1 берёт 8 слов, последний —
# 22, между ними линейно. Порядок баз уже идёт от частого к редкому, так что
# сложность растёт по двум осям сразу: слова редеют, а искать их надо больше.
